...
const epochTag = (e) => e ? (e.num != null ? 'E' + e.num : e.id) : '—';
// config-driven taxonomy (terminology, tracks, priorities, kinds, groups)
const CFG = () => S.config || {};
const TERM = (k, fb) => ((CFG().terms || {})[k] || fb);
const milestonesOf = (epochId) => (S.milestones || []).filter(m => m.epochId === epochId);
...
// ---- decisions (default) -----------------------------------------------
// groups come from the project's config; anything unknown lands in Other
const decisionGroups = () => [
  ...(CFG().decisionGroups || []).map(k => [k, k.charAt(0).toUpperCase() + k.slice(1), '']),
  ['other', 'Other', 'needs triage'],
];

function decisionGroupOf(d) {
  const id = d.group || 'other';
  return decisionGroups().find(([k]) => k === id) ? id : 'other';
}
...
function viewDecisions() {
  ...
  if (open.length) {
    decisionGroups().forEach(([key, name, blurb]) => {
...
function decisionCard(d) {
  ...
  const groupName = (decisionGroups().find(([k]) => k === decisionGroupOf(d)) || [null, 'Other'])[1];
...
function viewBoard() {
  ...
  for (const e of sorted) {
    if (e.status === 'arrived' || e.status === 'done') continue;
    ...
    const ms = milestonesOf(e.id);
    v.appendChild(trackSection('epoch:' + e.id, true,
      { num: `${TERM('epoch', 'Epoch')} ${e.num ?? e.id}`, name: e.name, status: e.status, statusClass: e.status === 'active' ? 'active' : '', count: `${doneCards.length}/${all.length} done · ${pct}%` },
      (body) => {
...
// milestones inside an epoch: goal + progress from linked cards
function milestoneStrip(ms) {
  const wrap = el(`<div class="mstrip"></div>`);
  const label = TERM('milestones', 'Milestones');
  wrap.appendChild(el(`<div class="mstrip__h">${esc(label)}</div>`));
  for (const m of ms) {
    const p = m.progress || { done: 0, total: 0 };
    const pct = p.total ? Math.round(p.done / p.total * 100) : 0;
    const met = m.status === 'met';
    const row = el(`<div class="mrow ${met ? 'mrow--met' : ''}" title="${esc(m.goal || '')}">
        <span class="mrow__dot">${met ? '✓' : '◇'}</span>
        <span class="mrow__t">${esc(m.title)}</span>
        <span class="mrow__goal">${esc(m.goal || '')}</span>
        <span class="mrow__bar"><i style="width:${met ? 100 : pct}%"></i></span>
        <span class="mrow__n">${met ? 'met' : `${p.done}/${p.total}`}</span>
        ${met ? '' : `<button class="btn btn--ghost btn--sm" data-met title="Mark this ${esc(TERM('milestone', 'milestone').toLowerCase())} met">Met</button>`}
      </div>`);
...
function viewIdeas() {
  const v = $('#view');
  const open = S.ideas.filter(b => b.status !== 'tagged');
  ...
  const fire = async () => { const i = $('input', add); const t = i.value.trim(); if (!t) return; i.value = ''; await api('idea/add', { text: t, by: 'owner' }); };
  ...
  for (const lv of levels) {
    ...
    open.filter(b => b.level === lv).forEach(b => {
      ...
      addBtn.addEventListener('click', async () => { const c = await api('idea/promote', { id: b.id, by: 'owner' }); if (c) showDetail(c.id); });
      delBtn.addEventListener('click', () => api('idea/delete', { id: b.id, by: 'owner' }));
      g.appendChild(row);
    });
    wrap.appendChild(g);
  }
  ...
  m.innerHTML = `<div class="modal__panel">
    ...
    <div class="modal__body">
      ...
      <div class="fields">
        ...
        <div class="fld"><div class="fld__k">${esc(TERM('epoch', 'Epoch'))}</div><select data-fld="epoch"><option value="" ${!c.epoch ? 'selected' : ''}>—</option>${S.epochs.map(e => `<option value="${e.id}" ${e.id === c.epoch ? 'selected' : ''}>${esc(epochTag(e))} ${esc(e.name)}</option>`).join('')}</select></div>
        <div class="fld"><div class="fld__k">${esc(TERM('milestone', 'Milestone'))}</div><select data-fld="milestoneId"><option value="" ${!c.milestoneId ? 'selected' : ''}>—</option>${(S.milestones || []).filter(m => !c.epoch || m.epochId === c.epoch).map(m => `<option value="${m.id}" ${m.id === c.milestoneId ? 'selected' : ''}>${esc(m.title)}</option>`).join('')}</select></div>
        ...
        <div class="fld"><div class="fld__k">Assignee</div><input class="fld__v" data-fld="assignee" value="${esc(c.assignee || '')}" placeholder="—"></div>
...
const commit = (id, k, v) => {
  let val = v;
  if ((k === 'plan' || k === 'milestoneId' || k === 'assignee' || k === 'epoch') && v === '') val = null;
...
const VIEWS = [
  ...
  { id: 'ideas', ico: '▤', name: 'Ideas', count: () => S.counts.ideas },
];
...
function renderChrome() {
  ...
  document.title = `Tower · ${S.meta.project || 'project board'}`;
  $('#project-sub').textContent = `${S.meta.project || 'project'} · board`;
  const cur = epochOf(S.meta.currentEpoch);
  $('#feed').innerHTML = `<b>${esc(TERM('epoch', 'Epoch'))} ${cur ? esc(String(cur.num ?? cur.id)) : '—'}</b> · <b>${S.cards.length}</b> cards · <b>${S.counts.agentReady}</b> agent-ready`;
...
function render() {
  ...
  try {
    ...
    console.error(err);
    $('#view').appendChild(el(`<div class="empty-view"><div class="empty-view__g">✕</div><div class="empty-view__t">Render error: ${esc(err?.message || err)}</div></div>`));
  }
...
// top-level await: the window load event waits for the first render
await load();
