...
impl Extension for JetExtension {
    ...
    fn get_dap_binary(
        ...
        let source = match &request {
            StartDebuggingRequestArgumentsRequest::Launch => value
                .get("program")
                .and_then(zed::serde_json::Value::as_str)
                .filter(|path| path.ends_with(".jet"))
                .map(str::to_owned)
                .ok_or_else(|| "Jet DAP launch needs a .jet source file".to_string())?,
            StartDebuggingRequestArgumentsRequest::Attach => {
                let map = value
                    .get("map")
                    .and_then(zed::serde_json::Value::as_str)
                    .ok_or_else(|| {
                        "Jet DAP attach needs the `.jetmap` sidecar in `map`".to_string()
                    })?;
                map_source(worktree, map)?
            }
        };
        let command = match user_provided_debug_adapter_path {
            Some(path) => path,
            None => find_jet_binary(worktree)?,
        };
...
fn map_source(worktree: &Worktree, map_path: &str) -> Result<String> {
    ...
    let relative = map.strip_prefix(&root).unwrap_or(map);
    ...
    let map_dir = relative.parent().unwrap_or_else(|| Path::new(""));
    Ok(Path::new(&root)
        .join(map_dir)
        .join(source)
        .to_string_lossy()
