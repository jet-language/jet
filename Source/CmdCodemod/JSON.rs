...
pub fn parse(input: &str) -> Result<Value, String> {
    ...
    p.ws();
    if p.at != p.bytes.len() {
        return Err(format!("unexpected data at byte {}", p.at));
    }
    Ok(value)
}
...
impl Parser<'_> {
    ...
    fn word(&mut self, word: &[u8]) -> Result<(), String> {
        if self.bytes.get(self.at..self.at + word.len()) == Some(word) {
            ...
            Err(format!("expected JSON value at byte {}", self.at))
        }
    }
    ...
    fn string(&mut self) -> Result<String, String> {
        ...
        while let Some(&b) = self.bytes.get(self.at) {
            ...
            match b {
                b'"' => return Ok(out),
                b'\\' => {
                    let esc = *self.bytes.get(self.at).ok_or("unfinished JSON escape")?;
                    self.at += 1;
                    match esc {
                        b'"' => out.push('"'),
                        b'\\' => out.push('\\'),
                        ...
                        b'u' => {
                            ...
                            self.at += 4;
                            let c = char::from_u32(n as u32).ok_or("bad unicode scalar")?;
                            out.push(c);
                        }
                ...
                _ => {
                    self.at -= 1;
                    let rest =
                        std::str::from_utf8(&self.bytes[self.at..]).map_err(|_| "invalid UTF-8")?;
    ...
    fn object(&mut self) -> Result<Value, String> {
        ...
        loop {
            ...
            self.at += 1;
            if out.insert(key.clone(), self.value()?).is_some() {
