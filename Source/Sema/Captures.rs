...
pub(crate) fn stmt_refs_name(stmt: &Stmt, name: &str) -> bool {
    match stmt {
        ...
        Stmt::Loop { body, .. } | Stmt::Unsafe { body, .. } | Stmt::Region { body, .. } | Stmt::Caps { body, .. } => body.iter().any(|s| stmt_refs_name(s, name)),
...
pub(crate) fn stmt_view_return_span(checker: &Checker<'_>, stmt: &Stmt) -> Option<Span> {
    match stmt {
        ...
        Stmt::While { body, .. } | Stmt::Loop { body, .. } | Stmt::Unsafe { body, .. } | Stmt::Region { body, .. } | Stmt::Caps { body, .. } => body
            .iter()
