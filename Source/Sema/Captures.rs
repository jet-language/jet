...
pub(crate) fn stmt_refs_name(stmt: &Stmt, name: &str) -> bool {
    match stmt {
        ...
        Stmt::Loop { body, .. } | Stmt::Unsafe { body, .. } | Stmt::Region { body, .. } | Stmt::Caps { body, .. } | Stmt::Transact { body, .. } => body.iter().any(|s| stmt_refs_name(s, name)),
...
pub(crate) fn stmt_view_return_span(checker: &Checker<'_>, stmt: &Stmt) -> Option<Span> {
    match stmt {
        ...
        Stmt::While { body, .. } | Stmt::Loop { body, .. } | Stmt::Unsafe { body, .. } | Stmt::Region { body, .. } | Stmt::Caps { body, .. } | Stmt::Transact { body, .. } => body
            .iter()
...
pub(crate) fn stmt_collect_captures(
    ...
    match stmt {
        ...
        Stmt::Loop { body, .. } | Stmt::Unsafe { body, .. } | Stmt::Region { body, .. } | Stmt::Caps { body, .. } | Stmt::Transact { body, .. } => {
            let mut body_bound = bound.clone();
            block_collect_captures(body, &mut body_bound, read, mut_cap);
        }
