...
pub const SYMBOLS: &[Symbol] = &[
    ...
    member!("Map", "insert", "Map.insert(key: K, value: V)", "Inserts or overwrites the value for key."),
    member!("Map", "remove", "Map.remove(key: K) -> V?", "Removes and returns the value for key, if present."),
    member!("Map", "contains_key", "Map.contains_key(key: K) -> Bool", "True when key has an entry."),
