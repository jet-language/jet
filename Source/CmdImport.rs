//! Semantic source migration and binder preparation framework (D-MIGRATE-SRC1=A).
//!
//! Narrow source conversion emits ordinary editable Jet. Binder-only families
//! emit preparation stubs and manual-migration omissions; unsupported source is
//! never discarded or represented by fake behavior.
...
pub(crate) fn run(raw: &[String], json: bool) -> i32 {
    ...
    if matches!(lang.as_str(), "c" | "cpp") {
        let foreign = foreign_language(&lang).expect("C/C++ importer language is mapped");
        let capability = jet::ForeignBridge::foreign_boundary_capability(foreign);
        return usage_error(
            &format!("source conversion for '{lang}' is unavailable"),
            &format!(
                "{}; use {lang}.<lib> for call-in-place binding and a checked #Extern overlay",
                capability.reason
            ),
            json,
        );
    ...
    if !matches!(
        ...
        return usage_error(
            ...
            "use jet import py|java|csharp|ts|js|go <dir> for narrow conversion; Ada/Pascal are binder-only/manual; C/C++ use their binder plus checked overlays",
            json,
        );
    }
...
fn foreign_language(language: &str) -> Option<jet::AST::ForeignLanguage> {
    Some(match language {
        ...
        "c" => jet::AST::ForeignLanguage::C,
        "cpp" => jet::AST::ForeignLanguage::Cpp,
...
fn translate_pascal_file(raw: &str, source: &Path, target: &Path) -> Translation {
    ...
    let mut output = binder_header("pascal");
...
fn translate_ada_file(raw: &str, source: &Path, target: &Path) -> Translation {
    ...
    let mut output = binder_header("ada");
    output.push_str(&format!("// Source: {}\n", source.display()));
...
fn binder_header(language: &str) -> String {
    format!(
        "// Prepared by jet import {language} (D-MIGRATE-SRC1). Binder-only/manual migration; semantic source conversion is unavailable.\n"
    )
}
...
fn apply_plan(mut plan: Plan, mode: Mode, json: bool) -> i32 {
    ...
    if json {
        ...
        if let Some(boundary) = plan.boundary.as_ref() {
            println!(
                "  migration: {} (mode: {}; source conversion: {}; source authority: {})",
                boundary.disposition.as_str(),
                migration_mode(boundary),
                migration_source_conversion(boundary),
                boundary.source_authority.as_str(),
            );
        }
...
fn migration_source_conversion(boundary: &jet::ForeignBridge::ForeignBoundaryContract) -> &'static str {
    if boundary.source_import {
        "narrow-checked-subset"
    } else {
        "unavailable"
    }
}

fn migration_mode(boundary: &jet::ForeignBridge::ForeignBoundaryContract) -> &'static str {
    if boundary.disposition == jet::ForeignBridge::ForeignBoundaryDisposition::BinderOnly {
        "manual"
    } else if boundary.source_import {
        "conversion"
    } else {
        "binder"
    }
...
fn migration_projection_json(plan: &Plan) -> String {
    let Some(boundary) = plan.boundary.as_ref() else {
        return "{\"disposition\":\"unavailable\",\"source_conversion\":\"unavailable\",\"mode\":\"manual\",\"source_authority\":\"foreign-until-accepted\"}".into();
    };
    format!(
        "{{\"disposition\":\"{}\",\"source_conversion\":\"{}\",\"mode\":\"{}\",\"source_authority\":\"{}\"}}",
        boundary.disposition.as_str(),
        migration_source_conversion(boundary),
        migration_mode(boundary),
        boundary.source_authority.as_str(),
...
fn migration_projection_value(plan: &Plan) -> StatusValue {
    let Some(boundary) = plan.boundary.as_ref() else {
        return StatusValue::object(
            StatusFields::new()
                .with("disposition", "unavailable")
                .with("source_conversion", "unavailable")
                .with("mode", "manual")
                .with("source_authority", "foreign-until-accepted"),
        );
    };
    StatusValue::object(
        StatusFields::new()
            .with("disposition", boundary.disposition.as_str())
            .with("source_conversion", migration_source_conversion(boundary))
            .with("mode", migration_mode(boundary))
            .with("source_authority", boundary.source_authority.as_str()),
    )
}
...
fn report(plan: &Plan) -> String {
    ...
    let migration = migration_projection_json(plan);
    let mut output = format!(
        "{{\"schema\":\"jet.source-import.v1\",\"law\":\"D-MIGRATE-SRC1\",\"language\":\"{}\",\"source\":\"{}\",\"target\":\"{}\",\"migration\":{migration},\"provenance\":{{\"source_root\":\"{}\",\"generated_root\":\"{}\",\"boundary\":{boundary}}},\"summary\":{{\"files\":{},\"translated_functions\":{},\"carried_tests\":{},\"omissions\":{}}},\"omissions\":[",
...
fn report_value(plan: &Plan) -> StatusValue {
    ...
    StatusValue::object(
        StatusFields::new()
            ...
            .with("migration", migration_projection_value(plan))
            .with(
                "provenance",
                StatusValue::object(
                    StatusFields::new()
