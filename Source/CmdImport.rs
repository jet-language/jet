...
pub(crate) fn run(raw: &[String], json: bool) -> i32 {
    ...
    if !matches!(lang.as_str(), "py" | "pascal") {
        return usage_error(
            &format!("source importer '{lang}' is not installed"),
            "use jet import py <dir> or jet import pascal <dir>; language importers ship as semantic translators",
            json,
        );
    }
...
fn build_plan(source: &Path, target: &Path, language: &str) -> Result<Plan, String> {
    ...
    if files.is_empty() {
        let extensions = if language == "pascal" {
            ".pas or .pp"
        } else {
            ".py"
        };
    ...
    for path in files {
        ...
        let translation = if language == "pascal" {
            translate_pascal_file(&raw, relative, &output)
        } else {
            translate_file(&raw, relative, &output)
        };
...
fn report(plan: &Plan) -> String {
    let mut output = format!(
        "{{\"schema\":\"jet.source-import.v1\",\"language\":\"{}\",\"source\":\"{}\",\"target\":\"{}\",\"summary\":{{\"files\":{},\"translated_functions\":{},\"carried_tests\":{},\"omissions\":{}}},\"omissions\":[",
    ...
    for (index, todo) in plan.todos.iter().enumerate() {
        ...
        let diagnostic = todo.diagnostic();
        output.push_str(&format!(
            "{{\"code\":\"{}\",\"what\":\"{}\",\"why\":\"{}\",\"fix\":\"{}\",\"source\":\"{}\",\"generated_target\":\"{}\",\"migration_status\":\"{}\"}}",
            diagnostic.code,
            json_escape(&diagnostic.what),
            json_escape(&diagnostic.why),
            json_escape(&diagnostic.fix),
