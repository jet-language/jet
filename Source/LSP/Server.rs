...
struct Server {
    ...
    /// Retain the opt-in timing sink for the whole LSP session. Reopening a
    /// predictable pathname for every request would make the sink replaceable.
    timing_log: Option<LspTimingSink>,
    shutdown: bool,
}
...
impl Server {
    ...
    fn lex(&self, doc: &Document) -> Arc<Vec<Token>> {
        match crate::Package::mask_inline_package_source(&doc.text) {
            Ok((masked, Some(_))) => Arc::new(crate::Lexer::lex(&masked).0),
            _ => self.queries.borrow_mut().lex_text(&doc.path, &doc.text),
        }
    }

    /// c121 Step 5: append one `{"method":…,"us":…}` JSON line to
    /// `jet-lsp-timing.json` in the cwd. JSON-lines suits an open-ended
    /// request stream; the file handle is retained for the session and all
    /// writes are best-effort, so a write failure never disturbs the server.
    fn record_lsp_latency(&mut self, method: &str, us: u128) {
        if !crate::PhaseTiming::enabled() {
            return;
        }
        let sink = self
            .timing_log
            .get_or_insert_with(LspTimingSink::open_for_session);
        let _ = sink.write_line(&format!("{{\"method\":\"{}\",\"us\":{}}}", method, us));
    }
}
...
fn write_log(msg: &str) -> io::Result<()> {
    ...
    write_log_line(&mut stderr, &format!("[jet-lsp] {msg}"))?;
    stderr.flush()
}

fn write_log_line<W: Write>(sink: &mut W, line: &str) -> io::Result<()> {
    writeln!(sink, "{line}")
}
...
impl LspTimingSink {
    fn open_for_session() -> Self {
        let path = std::env::current_dir()
            .map(|cwd| cwd.join("jet-lsp-timing.json"))
            .and_then(|path| open_lsp_timing_file(&path));
        match path {
    ...
    fn write_line(&mut self, line: &str) -> io::Result<()> {
        let result = match self {
            Self::File(file) => {
                if !safe_lsp_log_file(file).unwrap_or(false) {
                    Err(io::Error::new(
                        io::ErrorKind::PermissionDenied,
                        "LSP timing log is linked or not regular",
                    ))
                } else {
                    writeln!(file, "{line}")
                }
            }
            Self::Stderr => return write_stderr_line(line),
        };
        if result.is_err() {
            *self = Self::Stderr;
            return write_stderr_line(line);
        }
        result
    }
}
...
fn write_stderr_line(line: &str) -> io::Result<()> {
    let mut stderr = io::stderr().lock();
    write_log_line(&mut stderr, line)?;
    stderr.flush()
}

#[cfg(any(target_os = "linux", target_os = "android"))]
const LSP_O_CLOEXEC: i32 = 0o2000000;
#[cfg(any(target_os = "macos", target_os = "ios"))]
const LSP_O_CLOEXEC: i32 = 0x01000000;
#[cfg(any(target_os = "linux", target_os = "android"))]
const LSP_O_DIRECTORY: i32 = 0o200000;
#[cfg(any(target_os = "macos", target_os = "ios"))]
const LSP_O_DIRECTORY: i32 = 0x00100000;
#[cfg(any(target_os = "linux", target_os = "android"))]
const LSP_O_NOFOLLOW: i32 = 0o400000;
#[cfg(any(target_os = "macos", target_os = "ios"))]
const LSP_O_NOFOLLOW: i32 = 0x0100;
#[cfg(any(target_os = "linux", target_os = "android"))]
const LSP_O_CREAT: i32 = 0o100;
#[cfg(any(target_os = "macos", target_os = "ios"))]
const LSP_O_CREAT: i32 = 0x0200;
#[cfg(any(target_os = "linux", target_os = "android"))]
const LSP_O_APPEND: i32 = 0o2000;
#[cfg(any(target_os = "macos", target_os = "ios"))]
const LSP_O_APPEND: i32 = 0x0008;
...
fn open_lsp_timing_file_unix(path: &std::path::Path) -> io::Result<std::fs::File> {
    ...
    fn name(value: &OsStr) -> io::Result<CString> {
        ...
        if bytes.is_empty() || bytes == b"." || bytes == b".." || bytes.contains(&b'/') {
            return Err(io::Error::new(
                ...
                "LSP timing log path component is not safe",
            ));
        }
        CString::new(bytes).map_err(|_| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                "LSP timing log path component contains NUL",
            )
        })
    }

    fn open_directory(parent: &File, child: &OsStr) -> io::Result<File> {
        let child = name(child)?;
        let fd = unsafe {
            openat(
                parent.as_raw_fd(),
                child.as_ptr(),
                LSP_O_DIRECTORY | LSP_O_NOFOLLOW | LSP_O_CLOEXEC,
                0,
            )
        };
        if fd < 0 {
    ...
    let final_name = path.file_name().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "LSP timing log path has no regular file name",
        )
    })?;
    let parent = path
        .parent()
    ...
    let mut directory = OpenOptions::new()
        .read(true)
        .custom_flags(LSP_O_DIRECTORY | LSP_O_NOFOLLOW | LSP_O_CLOEXEC)
        .open(anchor)?;
    if !directory.metadata()?.is_dir() {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "LSP timing log anchor is not a directory",
        ));
    }
    for component in parent.components() {
        match component {
            Component::RootDir | Component::CurDir => {}
            Component::Normal(child) => {
                directory = open_directory(&directory, child)?;
            }
            Component::ParentDir | Component::Prefix(_) => {
                return Err(io::Error::new(
                    io::ErrorKind::PermissionDenied,
                    "LSP timing log parent contains unsupported path component",
                ));
            }
        }
    }
    ...
    let fd = unsafe {
        openat(
            directory.as_raw_fd(),
            final_name.as_ptr(),
            1 | LSP_O_APPEND | LSP_O_CREAT | LSP_O_NOFOLLOW | LSP_O_CLOEXEC,
            0o666,
        )
    };
    if fd < 0 {
...
fn open_lsp_timing_file_unix(_path: &std::path::Path) -> io::Result<std::fs::File> {
    Err(io::Error::new(
        io::ErrorKind::Unsupported,
        "this Unix target cannot enforce no-follow LSP timing log creation",
    ))
}

fn open_lsp_timing_file(path: &std::path::Path) -> io::Result<std::fs::File> {
    open_lsp_timing_file_inner(path, || {})
}

#[cfg(test)]
fn open_lsp_timing_file_with_hook<F>(
    path: &std::path::Path,
    before_open: F,
...
where
    F: FnOnce(),
{
    open_lsp_timing_file_inner(path, before_open)
}

fn open_lsp_timing_file_inner<F>(
    path: &std::path::Path,
    before_open: F,
) -> io::Result<std::fs::File>
where
    F: FnOnce(),
{
    ...
    if parent_metadata.file_type().is_symlink() || !parent_metadata.is_dir() {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "LSP timing log parent must be a real directory",
        ));
    }
    let real_parent = std::fs::canonicalize(parent)?;
    if real_parent.as_path() != parent {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "LSP timing log parent contains a symlink",
        ));
    }
    if let Ok(metadata) = std::fs::symlink_metadata(path) {
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "LSP timing log path must be a regular file",
            ));
        }
        if log_metadata_has_unsafe_link_count(&metadata) {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "LSP timing log path must not be a hard link",
            ));
        }
    }
    before_open();
    #[cfg(unix)]
    let file = open_lsp_timing_file_unix(path)?;
    #[cfg(windows)]
    return Err(io::Error::new(
        io::ErrorKind::Unsupported,
        "Windows LSP timing-file output requires descriptor-relative no-follow support",
    ));
...
fn safe_lsp_log_file(file: &std::fs::File) -> io::Result<bool> {
    let metadata = file.metadata()?;
    if !metadata.is_file() || log_metadata_has_unsafe_link_count(&metadata) {
        return Ok(false);
    }
    Ok(true)
}

fn log_metadata_has_unsafe_link_count(metadata: &std::fs::Metadata) -> bool {
    #[cfg(unix)]
    {
...
fn handle_request(
    ...
    if let Some(t0) = started {
        server.record_lsp_latency(method, t0.elapsed().as_micros());
    }
    out
}
...
mod project_part_tests {
    ...
    #[cfg(any(target_os = "linux", target_os = "android"))]
    #[test]
    fn lsp_nofollow_flag_uses_linux_family_value() {
        assert_eq!(LSP_O_NOFOLLOW, 0o400000);
    }

    #[cfg(any(target_os = "macos", target_os = "ios"))]
    #[test]
    fn lsp_nofollow_flag_uses_macos_family_value() {
        assert_eq!(LSP_O_NOFOLLOW, 0x0100);
    }
    ...
    fn lsp_log_writer_rejects_symlink_destination() {
        ...
        let root =
            std::env::temp_dir().join(format!("jet-lsp-log-symlink-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        ...
        assert!(open_lsp_timing_file(&log).is_err());
    ...
    #[cfg(unix)]
    #[test]
    fn lsp_log_writer_rejects_hardlink_destination() {
        let root =
            std::env::temp_dir().join(format!("jet-lsp-log-hardlink-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let outside = root.join("outside.log");
        let log = root.join("jet-lsp.log");
    ...
    #[cfg(unix)]
    #[test]
    fn lsp_log_writer_rejects_hostile_temp_root_ancestor() {
        use std::os::unix::fs::symlink;

        let root =
            std::env::temp_dir().join(format!("jet-lsp-log-ancestor-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let outside = root.join("outside");
        let hostile = root.join("tmp");
        std::fs::create_dir_all(&outside).unwrap();
        let victim = outside.join("victim.log");
        std::fs::write(&victim, "must survive\n").unwrap();
        symlink(&outside, &hostile).unwrap();

        assert!(open_lsp_timing_file(&hostile.join("jet-lsp.log")).is_err());
    ...
    #[test]
    fn lsp_log_writer_rejects_hostile_ancestor_swap_before_open() {
        use std::os::unix::fs::symlink;

        let root =
            std::env::temp_dir().join(format!("jet-lsp-log-ancestor-swap-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let outside = root.join("outside");
        let parent = root.join("logs");
        let held_parent = root.join("held-logs");
        std::fs::create_dir_all(&outside).unwrap();
        std::fs::create_dir_all(&parent).unwrap();
        let victim = outside.join("victim.log");
        std::fs::write(&victim, "must survive\n").unwrap();
        let log = parent.join("jet-lsp.log");

        open_lsp_timing_file_with_hook(&log, || {
            std::fs::rename(&parent, &held_parent).unwrap();
            symlink(&outside, &parent).unwrap();
        })
        .expect_err("ancestor replacement must be rejected");
        assert!(!outside.join("jet-lsp.log").exists());
        assert_eq!(std::fs::read_to_string(&victim).unwrap(), "must survive\n");
    ...
    #[test]
    fn lsp_timing_writer_emits_normal_line() {
        let root =
            std::env::temp_dir().join(format!("jet-lsp-log-normal-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let log = root.join("jet-lsp-timing.json");
        {
            let file = open_lsp_timing_file(&log).unwrap();
            let mut sink = LspTimingSink::File(file);
            sink.write_line(r#"{"method":"ping","us":7}"#).unwrap();
        }
        assert_eq!(
    ...
    #[test]
    fn lsp_timing_writer_rejects_unsupported_platform() {
        let path = std::env::temp_dir().join(format!(
            "jet-lsp-unsupported-{}.log",
            std::process::id()
        ));
        let error = open_lsp_timing_file(&path)
            .err()
    ...
    #[cfg(any(target_os = "linux", target_os = "android", target_os = "macos", target_os = "ios"))]
    #[test]
    fn lsp_timing_writer_retains_handle_after_path_swap() {
        let root =
            std::env::temp_dir().join(format!("jet-lsp-log-held-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let log = root.join("jet-lsp-timing.json");
        ...
        sink.write_line(r#"{"method":"swap","us":9}"#).unwrap();
        assert!(matches!(&sink, &LspTimingSink::Stderr));
        assert_eq!(std::fs::read_to_string(&victim).unwrap(), "must survive\n");
