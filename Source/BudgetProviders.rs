...
#[cfg(test)]
static FILE_READER_DELAY_MS: AtomicU64 = AtomicU64::new(0);
#[cfg(test)]
static ACTIVE_FILE_READERS: AtomicU64 = AtomicU64::new(0);
#[cfg(test)]
static ACTIVE_ISOLATED_WORKERS: AtomicU64 = AtomicU64::new(0);
#[cfg(test)]
static LAST_ISOLATED_GROUP: AtomicU64 = AtomicU64::new(0);
static COMPILE_WORKLOAD_SEQUENCE: AtomicU64 = AtomicU64::new(0);
#[cfg(test)]
struct ActiveFileReader;
#[cfg(test)]
impl ActiveFileReader {
    fn new() -> Self {
        ACTIVE_FILE_READERS.fetch_add(1, AtomicOrdering::SeqCst);
        Self
    }
}
#[cfg(test)]
impl Drop for ActiveFileReader {
    fn drop(&mut self) {
        ACTIVE_FILE_READERS.fetch_sub(1, AtomicOrdering::SeqCst);
    }
}
#[cfg(test)]
struct ActiveIsolatedWorker;
#[cfg(test)]
impl ActiveIsolatedWorker {
    fn new() -> Self {
        ACTIVE_ISOLATED_WORKERS.fetch_add(1, AtomicOrdering::SeqCst);
        Self
    }
}
#[cfg(test)]
impl Drop for ActiveIsolatedWorker {
    fn drop(&mut self) {
        ACTIVE_ISOLATED_WORKERS.fetch_sub(1, AtomicOrdering::SeqCst);
    }
}
...
impl ProviderRequest {
    pub fn validate(&self) -> Result<(), ProviderFailure> {
        if self.schema != "jet.provider-request" || self.version != 1 {
            return Err(ProviderFailure::malformed(
                "unsupported provider request schema/version",
            ));
        }
        for (name, value) in [
            ...
            if !is_hex64(value) {
                return Err(ProviderFailure::malformed(format!(
                    "{name} is not lowercase Hex64"
                )));
            }
        }
        if self.specs.is_empty() || self.specs.len() > MAX_SPECS {
            return Err(ProviderFailure::malformed(
                "provider request spec count is outside 1..=4096",
            ));
        }
        let mut previous: Option<(&str, &str)> = None;
        for spec in &self.specs {
            if !is_hex64(&spec.budget_hash) || spec.metric.is_empty() {
                return Err(ProviderFailure::malformed(
                    "provider request has an invalid budget hash or empty metric",
                ));
            }
            let key = (spec.metric.as_str(), spec.budget_hash.as_str());
            if previous.is_some_and(|prior| prior >= key) {
                return Err(ProviderFailure::malformed(
                    "provider request specs are not strictly ordered by metric then budget hash",
                ));
            }
            previous = Some(key);
        }
    ...
    pub fn bytes(&self) -> Result<Vec<u8>, ProviderFailure> {
        ...
        let specs = self
            ...
            .map(|spec| {
                CanonicalJson::object([
                    (
                        "budget_hash".into(),
                        CanonicalJson::String(spec.budget_hash.clone()),
                    ),
        ...
        Ok(CanonicalJson::object([
            (
                "context_hash".into(),
                CanonicalJson::String(self.context_hash.clone()),
            ),
            ("policy".into(), self.policy.clone()),
            (
                "provider_hash".into(),
                CanonicalJson::String(self.provider_hash.clone()),
            ),
            (
                "request_id".into(),
                CanonicalJson::String(self.request_id.clone()),
            ),
            ("schema".into(), CanonicalJson::String(self.schema.clone())),
            ("specs".into(), CanonicalJson::Array(specs)),
            (
                "version".into(),
                CanonicalJson::Integer(self.version.to_string()),
            ),
...
impl ProviderFailure {
    ...
    fn operation(class: FailureClass, reason: impl Into<String>) -> Self {
        Self {
            class,
            reason: reason.into(),
        }
...
type InProcessProvider =
    fn(&ProviderRequest, &ProviderCancellation) -> Result<Vec<ProviderEvent>, ProviderFailure>;
#[derive(Clone)]
enum Provider {
    InProcess(InProcessProvider),
    Subprocess(PathBuf),
    File(PathBuf),
}
...
impl ProviderRegistry {
    ...
    pub fn with_compiler_facts() -> Self {
        ...
        registry
            .register_in_process("CompilerFacts", compiler_facts_provider)
            .expect("fixed compiler provider identity");
        registry
            .register_in_process("CompilerProbe", compiler_latency_provider)
    ...
    pub fn with_builtins() -> Self {
        ...
        registry
            .register_in_process("BuildArtifact", build_artifact_provider)
    ...
    pub fn register_in_process(
        ...
        self.insert(identity.into(), Provider::InProcess(provider))
    }
    pub fn register_subprocess(
        ...
        if !executable.is_absolute() {
            return Err("provider executable must be an absolute compiler-resolved path".into());
        }
    ...
    pub fn register_file(
        ...
        if !response.is_absolute() {
            return Err("provider response must be an absolute compiler-resolved path".into());
        }
        self.insert(identity.into(), Provider::File(response))
    }
    fn insert(&mut self, identity: String, provider: Provider) -> Result<(), String> {
        if identity.is_empty() {
            return Err("provider identity is empty".into());
        }
        if self.providers.insert(identity.clone(), provider).is_some() {
            return Err(format!("duplicate provider identity `{identity}`"));
        }
        Ok(())
    }
    pub fn collect(
        ...
        timeout: Duration,
    ) -> Result<ProviderEvidence, ProviderFailure> {
        request.validate()?;
        let provider = self.providers.get(identity).ok_or_else(|| {
            ProviderFailure::operation(
                FailureClass::Unresolved,
                format!("provider `{identity}` is unresolved"),
            )
        })?;
        let events = match provider {
            Provider::InProcess(function) => run_in_process(*function, request, timeout, identity)?,
            Provider::Subprocess(path) => {
                decode_stream(&run_subprocess(path, &request.bytes()?, timeout)?, request)?
            }
...
fn build_artifact_provider(
    request: &ProviderRequest,
    _: &ProviderCancellation,
) -> Result<Vec<ProviderEvent>, ProviderFailure> {
    let CanonicalJson::Object(workload) = &request.workload else {
        return Err(ProviderFailure::malformed(
            "BuildArtifact workload is not an object",
        ));
    };
    let Some(CanonicalJson::String(path)) = workload.get("path") else {
        return Err(ProviderFailure::malformed(
            "BuildArtifact workload has no artifact path",
        ));
    };
    let path = Path::new(path);
    if !path.is_absolute() {
        return Err(ProviderFailure::malformed(
            "BuildArtifact path is not compiler-resolved absolute text",
        ));
    }
    let metadata = std::fs::symlink_metadata(path).map_err(|error| {
        ProviderFailure::operation(
            FailureClass::Unavailable,
            format!("built artifact is unavailable: {error}"),
        )
    })?;
    ...
    let value =
        Rational::parse(&metadata.len().to_string(), "1").map_err(ProviderFailure::malformed)?;
    let runtime = if request
        .specs
        .iter()
        .any(|spec| matches!(spec.metric.as_str(), "StartupTime" | "MemoryHighWater"))
    {
        #[cfg(target_os = "linux")]
        {
    ...
    for (index, spec) in request.specs.iter().enumerate() {
        match spec.metric.as_str() {
            ...
            "StartupTime" => {
                let runtime = runtime
                    .as_ref()
                    .expect("runtime measurements were requested");
            ...
            _ => {
                return Err(ProviderFailure::operation(
                    ...
                    format!("BuildArtifact does not support metric `{}`", spec.metric),
                ));
            }
        }
    }
...
#[cfg(target_os = "linux")]
fn measure_artifact_runtime(
    path: &Path,
    samples: usize,
) -> Result<ArtifactRuntime, ProviderFailure> {
    ...
    for _ in 0..samples {
        ...
        let mut child = command.spawn().map_err(|error| {
            ProviderFailure::operation(
                ...
                format!("cannot launch artifact for startup/RSS measurement: {error}"),
            )
        })?;
        let pid = child.id();
        let stdout = child.stdout.take().ok_or_else(|| {
            ProviderFailure::operation(
                FailureClass::Execution,
                "artifact stdout was unavailable for its readiness line",
            )
        })?;
        ...
        let ready_ns = loop {
            ...
            match ready_rx.try_recv() {
                ...
                Ok(Ok(_)) => {
                    ...
                    return Err(ProviderFailure::operation(
                        FailureClass::Unavailable,
                        "artifact exited without a readiness line",
                    ));
                }
                Ok(Err(error)) => {
                    let _ = reader.join();
                    return Err(ProviderFailure::operation(
                        FailureClass::Unavailable,
                        format!("artifact readiness read failed: {error}"),
                    ));
                }
                Err(mpsc::TryRecvError::Disconnected) => {
                    ...
                    return Err(ProviderFailure::operation(
                        FailureClass::Unavailable,
                        "artifact readiness reader exited unexpectedly",
                    ));
                }
            ...
            match wait4_artifact(pid, true)? {
                Some(value) => {
                    ...
                    match ready_rx.recv_timeout(Duration::from_millis(100)) {
                        ...
                        Err(_) => {
                            return Err(ProviderFailure::operation(
                                FailureClass::Unavailable,
                                "artifact exited before its readiness line",
                            ))
                        }
                    }
                }
                ...
                None => {
                    ...
                    return Err(ProviderFailure::operation(
                        ...
                        "artifact did not emit a readiness line within 2 seconds",
                    ));
                }
            }
        ...
        if high_water == 0 {
            return Err(ProviderFailure::operation(
                FailureClass::Unavailable,
                "artifact exited before VmHWM could be observed",
            ));
        }
        startup_ns
            .push(Rational::parse(&ready_ns.to_string(), "1").map_err(ProviderFailure::malformed)?);
        memory_bytes.push(
            Rational::parse(&high_water.to_string(), "1").map_err(ProviderFailure::malformed)?,
        );
    }
    Ok(ArtifactRuntime {
        startup_ns,
...
fn stop_artifact(pid: u32) -> Result<u64, ProviderFailure> {
    ...
    wait4_artifact(pid, false)?.ok_or_else(|| {
        ProviderFailure::operation(
            FailureClass::Execution,
            "artifact wait4 returned without reaping the process",
        )
    })
}
...
fn read_vm_hwm(pid: u32) -> Result<Option<u64>, ProviderFailure> {
    ...
    let kib = fields
        .next()
        .ok_or_else(|| ProviderFailure::malformed("artifact VmHWM has no value"))?
        .parse::<u64>()
        .map_err(|_| ProviderFailure::malformed("artifact VmHWM is not an integer"))?;
    if fields.next() != Some("kB") {
        return Err(ProviderFailure::malformed(
            "artifact VmHWM is not expressed in kB",
        ));
    }
    Ok(Some(kib.checked_mul(1024).ok_or_else(|| {
        ProviderFailure::malformed("artifact VmHWM overflowed bytes")
    })?))
}
...
pub fn collection_deadline(
    kind: &str,
    mode: Option<&str>,
    facts: Duration,
    native_build: Duration,
) -> Duration {
    match kind {
        ...
        "BuildArtifact" => {
            Duration::from_secs(ARTIFACT_READY_DEADLINE.as_secs() * ARTIFACT_RUNTIME_SAMPLES as u64)
                + facts
        }
        "CompilerProbe" => {
...
pub fn compiler_probe_workload(
    ...
    let root = project_root
        .canonicalize()
        .map_err(|error| format!("cannot resolve compile workload root: {error}"))?;
    let entry = entry
        .canonicalize()
        .map_err(|error| format!("cannot resolve compile workload entry: {error}"))?;
    if !entry.starts_with(&root) {
        return Err("compile workload entry escapes its project root".into());
    }
    ...
    CanonicalJson::object([
        (
            "entry".into(),
            CanonicalJson::String(entry.to_string_lossy().into_owned()),
        ),
        ...
        (
            "patch_sha256".into(),
            CanonicalJson::String(descriptor.patch_sha256),
        ),
        ("profile".into(), CanonicalJson::String(profile.into())),
        (
            "provider_version".into(),
            CanonicalJson::String(provider_version),
        ),
        (
            "project_root".into(),
            CanonicalJson::String(root.to_string_lossy().into_owned()),
        ),
        (
            "samples".into(),
            CanonicalJson::Integer(COMPILE_SAMPLES.to_string()),
        ),
        (
            "source_tree_sha256".into(),
            CanonicalJson::String(descriptor.source_tree_sha256),
        ),
        ("target".into(), CanonicalJson::String(target.into())),
        (
            "warmups".into(),
            CanonicalJson::Integer(COMPILE_WARMUPS.to_string()),
        ),
    ])
}
...
fn compile_descriptor(
    ...
    profile: &str,
    patch: Option<&str>,
) -> Result<CompileDescriptor, String> {
    if !matches!(mode, "Clean" | "NoChange" | "Edit") || target.is_empty() || profile.is_empty() {
        return Err(
            "compile workload has an unsupported mode, empty target, or empty profile".into(),
        );
    }
    if target != "cli" {
        return Err(format!(
            "compile workload target `{target}` is unsupported by the resident fixture compiler"
        ));
    }
    validate_compile_profile(root, profile)?;
    if mode == "Edit" && patch.is_none() {
        return Err("CompileProbe Edit workload has no patch path".into());
    }
    if mode != "Edit" && patch.is_some() {
        return Err("only an Edit workload may name a patch path".into());
    }
    let patch_sha256 = match patch {
        Some(path) if safe_relative_path(path) => {
            let path = root.join(path);
            let metadata = std::fs::symlink_metadata(&path)
                .map_err(|error| format!("compile workload patch is unavailable: {error}"))?;
            if metadata.file_type().is_symlink() || !metadata.is_file() {
                return Err("compile workload patch is not a regular file".into());
            }
            let bytes = std::fs::read(&path)
                .map_err(|error| format!("cannot read compile workload patch: {error}"))?;
            sha256_hex(&bytes)
        }
        Some(_) => return Err("compile workload patch path is not project-relative".into()),
        None => sha256_hex(&[]),
    };
    Ok(CompileDescriptor {
...
fn validate_compile_profile(root: &Path, profile: &str) -> Result<(), String> {
    ...
    let raw = std::fs::read_to_string(&manifest).map_err(|_| {
        format!(
            "compile workload profile `{profile}` is unsupported by the resident fixture compiler"
        )
    })?;
...
fn compiler_latency_provider(
    request: &ProviderRequest,
    _: &ProviderCancellation,
) -> Result<Vec<ProviderEvent>, ProviderFailure> {
    let CanonicalJson::Object(workload) = &request.workload else {
        return Err(ProviderFailure::malformed(
            "CompilerProbe workload is not an object",
        ));
    };
    ...
    if !root.is_absolute() || !entry.is_absolute() {
        return Err(ProviderFailure::malformed(
            "CompilerProbe paths are not compiler-resolved absolute text",
        ));
    }
    let patch = match workload.get("patch") {
        ...
        _ => {
            return Err(ProviderFailure::malformed(
                "CompilerProbe patch is not text or null",
            ))
        }
    };
    ...
    if warmups != COMPILE_WARMUPS || sample_count != COMPILE_SAMPLES as u128 {
        return Err(ProviderFailure::operation(
            FailureClass::Incompatible,
            "CompilerProbe workload does not use the pinned warmup/sample policy",
        ));
    }
    if target != "cli" {
    ...
    for spec in &request.specs {
        if !matches!(
            ...
            return Err(ProviderFailure::operation(
                FailureClass::Unsupported,
                format!("CompilerProbe does not support metric `{}`", spec.metric),
            ));
        }
    }
    let root = root.canonicalize().map_err(|error| {
        ProviderFailure::operation(
            FailureClass::Unavailable,
            format!("compile workload root is unavailable: {error}"),
        )
    })?;
    let entry = entry.canonicalize().map_err(|error| {
        ProviderFailure::operation(
            FailureClass::Unavailable,
            format!("compile workload entry is unavailable: {error}"),
        )
    })?;
    if !entry.starts_with(&root) {
        return Err(ProviderFailure::operation(
            FailureClass::Incompatible,
            "compile workload entry escapes its project root",
        ));
    }
    ...
    let expected_provider_version = compiler_probe_version(
        &root,
        mode.as_str(),
        target.as_str(),
        profile.as_str(),
    ...
    if provider_version != expected_provider_version {
        return Err(ProviderFailure::operation(
            FailureClass::Incompatible,
            "compile workload provider identity is stale or forged",
        ));
    }
    let source_tree_sha256 = workload_text(workload, "source_tree_sha256")?;
    let patch_sha256 = workload_text(workload, "patch_sha256")?;
    if source_tree_sha256.as_str() != descriptor.source_tree_sha256.as_str()
        || patch_sha256.as_str() != descriptor.patch_sha256.as_str()
    {
        return Err(ProviderFailure::operation(
            FailureClass::Incompatible,
            "compile workload source or patch identity is stale or forged",
        ));
    }
    ...
    let result = compile_latency_samples(
        &root,
        &entry,
        &scratch,
        mode.as_str(),
        patch,
        &descriptor.source_tree_sha256,
...
fn validate_compile_workload_identity(
    ...
    if fields.len() != KEYS.len() || KEYS.iter().any(|key| !fields.contains_key(*key)) {
        return Err(ProviderFailure::malformed(
            "CompilerProbe workload has an unexpected or missing identity field",
        ));
    }
    if request.request_id != stable_id(&request.workload) {
        return Err(ProviderFailure::operation(
            FailureClass::Incompatible,
            "compile workload request identity is stale or forged",
        ));
    }
    Ok(())
}
...
fn compile_latency_samples(
    ...
    let relative_entry = entry.strip_prefix(root).map_err(|_| {
        ProviderFailure::malformed("compile workload entry is outside its project root")
    })?;
    let workload_bytes = source_tree_bytes(root).map_err(ProviderFailure::malformed)?;
    let edit_bytes_data = patch
        .map(|path| std::fs::read(root.join(path)))
        .transpose()
        .map_err(|error| ProviderFailure::malformed(format!("cannot read edit bytes: {error}")))?;
    let edit_bytes = edit_bytes_data
        .as_ref()
        .map(|bytes| bytes.len() as u128)
        .unwrap_or(0);
    let edit_sha256 = edit_bytes_data
        .as_deref()
        .map(sha256_hex)
    ...
    if edit_sha256.as_str() != expected_patch_sha256 {
        return Err(ProviderFailure::operation(
            FailureClass::Incompatible,
            "compile workload patch changed while it was copied",
        ));
    }
    ...
    if mode == "Edit" {
        std::fs::create_dir_all(scratch).map_err(|error| {
            ProviderFailure::operation(
                FailureClass::Execution,
                format!("cannot create edit workload scratch directory: {error}"),
            )
        })?;
        copy_compile_project(root, &edit_trial).map_err(ProviderFailure::malformed)?;
        let copied_source_tree_sha256 =
            source_tree_digest(&edit_trial).map_err(ProviderFailure::malformed)?;
        if copied_source_tree_sha256.as_str() != expected_source_tree_sha256 {
            return Err(ProviderFailure::operation(
                FailureClass::Incompatible,
                "compile workload source changed while it was copied",
            ));
        }
        ...
        let copied_source_tree_sha256 =
            source_tree_digest(scratch).map_err(ProviderFailure::malformed)?;
        if copied_source_tree_sha256.as_str() != expected_source_tree_sha256 {
            return Err(ProviderFailure::operation(
                FailureClass::Incompatible,
                "compile workload source changed while it was copied",
    ...
    for _ in 0..warmups {
        if mode == "Edit" {
            let _ = compile_edit_trial(
                root,
                relative_entry,
                &edit_trial,
                &warm,
                edit_patch
                    .ok_or_else(|| ProviderFailure::malformed("Edit workload has no patch path"))?,
    ...
    for _ in 0..sample_count {
        ...
        sample_records.push(
            CanonicalJson::object([
                ...
                (
                    "core_digest".into(),
                    CanonicalJson::String(core_digest.clone()),
                ),
                (
                    "edit_bytes".into(),
                    CanonicalJson::Integer(edit_bytes.to_string()),
                ),
                (
                    "elapsed_ns".into(),
                    CanonicalJson::Integer(elapsed_ns.to_string()),
                ),
                ("host".into(), CanonicalJson::String(host.clone())),
                ("linker".into(), CanonicalJson::String(linker.clone())),
                (
                    "peak_rss_bytes".into(),
                    CanonicalJson::Integer(child.peak_rss_bytes.to_string()),
                ),
                ("phase_totals".into(), phase_totals_json(&phase_totals)?),
                ("profile".into(), CanonicalJson::String(profile.into())),
                (
                    "source_tree_sha256".into(),
                    CanonicalJson::String(expected_source_tree_sha256.into()),
                ),
                ("target".into(), CanonicalJson::String(target.into())),
                (
                    "workload_bytes".into(),
                    CanonicalJson::Integer(workload_bytes.to_string()),
                ),
            ])
            .map_err(ProviderFailure::malformed)?,
        );
    }
    let mean = sample_values
        .iter()
        .try_fold(Rational::zero(), |sum, value| sum.add(value))
        .map_err(ProviderFailure::malformed)?
        .div(&Rational::integer(sample_values.len() as i128))
        .map_err(ProviderFailure::malformed)?;
    let variance = sample_values
        .iter()
        .try_fold(Rational::zero(), |sum, value| {
    ...
    let metadata = CanonicalJson::object([
        ...
        (
            "peak_rss_bytes".into(),
            CanonicalJson::Integer(peak_rss_bytes.to_string()),
        ),
        ("phase_totals".into(), aggregate_phases),
        ("profile".into(), CanonicalJson::String(profile.into())),
        (
            "sample_records".into(),
            CanonicalJson::Array(sample_records),
        ),
        (
            "samples".into(),
            CanonicalJson::Integer(sample_count.to_string()),
        ),
        (
            "source_tree_sha256".into(),
            CanonicalJson::String(expected_source_tree_sha256.into()),
        ...
        (
            "warmups".into(),
            CanonicalJson::Integer(warmups.to_string()),
        ),
        (
            "workload_bytes".into(),
            CanonicalJson::Integer(workload_bytes.to_string()),
    ...
    .map_err(ProviderFailure::malformed)?;
    let metadata = String::from_utf8(metadata.bytes())
        .map_err(|_| ProviderFailure::malformed("compile metadata is not UTF-8"))?;
    let mut events = Vec::with_capacity(
        sample_count
            .saturating_mul(specs.len())
            .saturating_add(specs.len() + 1),
    );
    for (spec, request_spec) in specs.iter().enumerate() {
        for value in &sample_values {
            events.push(ProviderEvent::Sample {
                spec: spec as u32,
                metric: request_spec.metric.clone(),
                value: value.clone(),
            });
        }
        events.push(ProviderEvent::Metadata {
            spec: spec as u32,
            details: vec![("compile".into(), metadata.clone())],
        });
    }
    events.push(ProviderEvent::Complete {
        request_id: request_id.into(),
        samples: (sample_count * specs.len()) as u64,
    });
    Ok(events)
}
...
fn compile_edit_trial(
    ...
    let result = (|| {
        ...
        let copied_source_tree_sha256 =
            source_tree_digest(trial).map_err(ProviderFailure::malformed)?;
        if copied_source_tree_sha256.as_str() != expected_source_tree_sha256 {
            return Err(ProviderFailure::operation(
                FailureClass::Incompatible,
                "compile workload source changed while it was copied",
        ...
        let patch_bytes = std::fs::read(trial.join(patch)).map_err(|error| {
            ProviderFailure::operation(
                FailureClass::Incompatible,
                format!("compile workload patch changed while it was copied: {error}"),
            )
        })?;
...
fn workload_text(
    ...
    match fields.get(key) {
        ...
        _ => Err(ProviderFailure::malformed(format!(
            "CompilerProbe workload has no nonempty `{key}` text"
        ))),
    }
}

fn workload_unsigned(
    fields: &BTreeMap<String, CanonicalJson>,
    key: &str,
    ...
    let value = match fields.get(key) {
        ...
        _ => {
            return Err(ProviderFailure::malformed(format!(
                "CompilerProbe workload `{key}` is not a canonical unsigned integer"
            )))
        }
    };
...
fn source_files(root: &Path) -> Result<Vec<(String, PathBuf)>, String> {
    fn visit(root: &Path, dir: &Path, files: &mut Vec<(String, PathBuf)>) -> Result<(), String> {
        let mut entries = std::fs::read_dir(dir)
            .map_err(|error| format!("cannot read compile workload directory: {error}"))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|error| format!("cannot enumerate compile workload directory: {error}"))?;
        ...
        for entry in entries {
            ...
            if matches!(
                name.as_ref(),
                ".jet" | ".jet-compile-cache" | "build" | "target" | ".git"
            ) {
                continue;
            }
            let metadata = std::fs::symlink_metadata(&path)
                .map_err(|error| format!("cannot inspect compile workload path: {error}"))?;
            if metadata.file_type().is_symlink() {
                return Err(format!(
                    "compile workload contains a symlink: {}",
                    path.display()
                ));
            }
            if metadata.is_dir() {
                ...
                let relative = path
                    .strip_prefix(root)
                    .map_err(|_| "compile workload path escaped its root".to_string())?
                    .to_string_lossy()
                    .replace('\\', "/");
    ...
    if files.is_empty() {
        return Err("compile workload has no package or Jet source files".into());
    }
    Ok(files)
}
...
fn source_tree_digest(root: &Path) -> Result<String, String> {
    ...
    for (relative, path) in files {
        ...
        let bytes = std::fs::read(path)
            .map_err(|error| format!("cannot read compile workload source: {error}"))?;
...
fn copy_compile_project(source: &Path, destination: &Path) -> Result<(), String> {
    fn copy_dir(source: &Path, destination: &Path, total: &mut u64) -> Result<(), String> {
        std::fs::create_dir_all(destination).map_err(|error| {
            format!("cannot create compile workload scratch directory: {error}")
        })?;
        let mut entries = std::fs::read_dir(source)
            .map_err(|error| format!("cannot read compile workload project: {error}"))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|error| format!("cannot enumerate compile workload project: {error}"))?;
        entries.sort_by_key(|entry| entry.file_name());
        for entry in entries {
            let path = entry.path();
            let name = entry.file_name();
            ...
            let metadata = std::fs::symlink_metadata(&path).map_err(|error| {
                format!("cannot inspect compile workload project path: {error}")
            })?;
            if metadata.file_type().is_symlink() {
                return Err(format!(
                    "compile workload contains a symlink: {}",
            ...
            if metadata.is_dir() {
                ...
                *total = total
                    ...
                    .ok_or_else(|| "compile workload exceeds its byte limit".to_string())?;
                if *total > COMPILE_MAX_PROJECT_BYTES {
                    return Err("compile workload exceeds the 64 MiB project limit".into());
                }
                std::fs::copy(&path, &target)
                    .map_err(|error| format!("cannot copy compile workload input: {error}"))?;
            }
        }
        Ok(())
    }
...
fn copy_cache_tree(source: &Path, destination: &Path) -> Result<(), String> {
    std::fs::create_dir_all(destination)
        .map_err(|error| format!("cannot create compile workload cache directory: {error}"))?;
    let entries = std::fs::read_dir(source)
        .map_err(|error| format!("cannot read compile workload cache: {error}"))?
        .collect::<Result<Vec<_>, _>>()
        .map_err(|error| format!("cannot enumerate compile workload cache: {error}"))?;
    for entry in entries {
        let path = entry.path();
        let metadata = std::fs::symlink_metadata(&path)
            .map_err(|error| format!("cannot inspect compile workload cache path: {error}"))?;
        if metadata.file_type().is_symlink() {
            return Err(format!(
                "compile workload cache contains a symlink: {}",
                path.display()
            ));
        }
        let target = destination.join(entry.file_name());
        if metadata.is_dir() {
            ...
            std::fs::copy(&path, &target)
                .map_err(|error| format!("cannot copy compile workload cache artifact: {error}"))?;
        }
    }
    Ok(())
}
...
fn apply_unified_patch(root: &Path, patch: &str) -> Result<(), String> {
    if !safe_relative_path(patch) {
        return Err("compile workload patch path is not project-relative".into());
    }
    let patch_bytes = std::fs::read(root.join(patch))
        .map_err(|error| format!("cannot read compile workload patch: {error}"))?;
    let patch_text = std::str::from_utf8(&patch_bytes)
        .map_err(|_| "compile workload patch is not UTF-8".to_string())?;
    let lines = patch_text.lines().collect::<Vec<_>>();
    let old_header = lines
        .iter()
        .find(|line| line.starts_with("--- "))
        .ok_or("compile workload patch has no old-file header")?;
    let new_header = lines
        .iter()
        .find(|line| line.starts_with("+++ "))
        .ok_or("compile workload patch has no new-file header")?;
    let old_path = patch_header_path(old_header)?;
    let new_path = patch_header_path(new_header)?;
    if old_path != new_path {
        return Err("compile workload patch changes more than one file".into());
    }
    let hunk_index = lines
        .iter()
        .position(|line| line.starts_with("@@"))
        .ok_or("compile workload patch has no hunk")?;
    let mut header = lines[hunk_index].split_whitespace();
    if header.next() != Some("@@") {
        return Err("compile workload patch has an invalid hunk header".into());
    }
    let parse_range = |value: &str| -> Result<(usize, usize), String> {
        let value = value
            .get(1..)
            .ok_or("compile workload patch has an invalid hunk range")?;
        let (start, count) = value
            .split_once(',')
            .map_or((value, "1"), |(start, count)| (start, count));
        let start = start
            .parse::<usize>()
            .map_err(|_| "compile workload patch has an invalid hunk start".to_string())?;
        let count = count
            .parse::<usize>()
            .map_err(|_| "compile workload patch has an invalid hunk count".to_string())?;
        if start == 0 {
            return Err("compile workload patch has a zero hunk start".into());
        }
        Ok((start, count))
    };
    let (old_start, old_count) = parse_range(
        header
            .next()
            .ok_or("compile workload patch has no old hunk range")?,
    )?;
    let (_, new_count) = parse_range(
        header
            .next()
            .ok_or("compile workload patch has no new hunk range")?,
    )?;
    let target = root.join(&new_path);
    let metadata = std::fs::symlink_metadata(&target)
        .map_err(|error| format!("compile workload patch target is unavailable: {error}"))?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err("compile workload patch target is not a regular file".into());
    }
    let original = std::fs::read_to_string(&target)
        .map_err(|error| format!("cannot read compile workload patch target: {error}"))?;
    let trailing_newline = original.ends_with('\n');
    let current = original.lines().map(str::to_owned).collect::<Vec<_>>();
    let old_start = old_start - 1;
    if old_start > current.len() {
        return Err("compile workload patch hunk starts outside the source tree".into());
    }
    ...
    for line in &lines[hunk_index + 1..] {
        if line.starts_with("@@") {
            return Err("compile workload patch must contain exactly one hunk".into());
        }
        ...
        match kind {
            " " => {
                if current.get(cursor).map(String::as_str) != Some(body) {
                    return Err(
                        "compile workload patch context does not match the source tree".into(),
                    );
                }
            ...
            "-" => {
                if current.get(cursor).map(String::as_str) != Some(body) {
                    return Err(
                        "compile workload patch removal does not match the source tree".into(),
                    );
                }
                cursor += 1;
                seen_old += 1;
    ...
    if seen_old != old_count || seen_new != new_count {
        return Err("compile workload patch hunk counts do not match its body".into());
    }
    ...
    std::fs::write(target, rewritten)
        .map_err(|error| format!("cannot apply compile workload patch: {error}"))
}
...
fn patch_header_path(header: &str) -> Result<String, String> {
    ...
    if !safe_relative_path(value) {
        return Err("compile workload patch header has an unsafe path".into());
    }
    Ok(value.into())
}
...
fn run_compile_child(
    ...
    target: &str,
    profile: &str,
) -> Result<ChildMetrics, ProviderFailure> {
    if target != "cli" {
        return Err(ProviderFailure::operation(FailureClass::Unsupported, format!("compile workload target `{target}` is unsupported by the resident fixture compiler")));
    }
    let backend = CompileBackend::for_profile(profile);
    ...
    match profile {
        ...
        name => {
            command.arg(format!("--profile={name}"));
        }
    }
    #[cfg(unix)]
    {
        ...
        match child.try_wait() {
            Ok(Some(status)) if status.success() => {
                ...
                if peak_rss_bytes == 0 {
                    return Err(ProviderFailure::operation(
                        FailureClass::Unavailable,
                        "compile child peak RSS was not observable",
                    ));
                }
            ...
            Ok(None) => {
                ...
                return Err(ProviderFailure::operation(
                    FailureClass::Timeout,
                    "compile workload child exceeded its deadline",
                ));
            }
            Err(error) => {
                terminate_group(&mut child);
...
fn aggregate_compile_phases(records: &[CanonicalJson]) -> Result<CanonicalJson, ProviderFailure> {
    ...
    for record in records {
        let CanonicalJson::Object(record) = record else {
            return Err(ProviderFailure::malformed(
                "compile sample record is not an object",
            ));
        };
        let Some(CanonicalJson::Array(phases)) = record.get("phase_totals") else {
            return Err(ProviderFailure::malformed(
                "compile sample record has no phase totals",
            ));
        };
        for phase in phases {
            let CanonicalJson::Object(phase) = phase else {
                return Err(ProviderFailure::malformed(
                    "compile phase total is not an object",
                ));
            };
            let Some(CanonicalJson::String(name)) = phase.get("name") else {
                return Err(ProviderFailure::malformed(
                    "compile phase total has no name",
                ));
            };
            let Some(CanonicalJson::Integer(ns)) = phase.get("ns") else {
                return Err(ProviderFailure::malformed(
                    "compile phase total has no duration",
                ));
            };
            let ns = ns.parse::<u128>().map_err(|_| {
                ProviderFailure::malformed("compile phase total is not an unsigned integer")
            })?;
...
fn run_in_process(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let bytes = run_isolated_bytes(timeout, &format!("provider `{identity}`"), move || {
            ...
            .map_err(|_| {
                ProviderFailure::operation(
                    FailureClass::Panic,
                    "in-process provider failed unexpectedly",
                )
            })?
            .map(|events| encode_stream(&events))
        })?;
        decode_stream(&bytes, request)
    }
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (function, request, timeout, identity);
        Err(ProviderFailure::operation(
            FailureClass::Execution,
            "bounded in-process providers are enabled only on Linux",
        ))
    }
}
...
fn read_bounded(path: &Path, timeout: Duration) -> Result<Vec<u8>, ProviderFailure> {
    #[cfg(target_os = "linux")]
    {
        read_bounded_isolated(path, timeout)
    }
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (path, timeout);
        Err(ProviderFailure::operation(
            FailureClass::Execution,
            "isolated provider file reads are enabled only on Linux",
        ))
    }
}
...
fn read_bounded_isolated(path: &Path, timeout: Duration) -> Result<Vec<u8>, ProviderFailure> {
    ...
    let name = CString::new(path.as_os_str().as_bytes()).map_err(|_| {
        ProviderFailure::operation(
            FailureClass::Execution,
            "provider response path contains NUL",
        )
    })?;
    #[repr(C)]
    ...
    extern "C" {
        ...
        fn close(fd: i32) -> i32;
        fn statx(fd: i32, path: *const i8, flags: i32, mask: u32, stat: *mut Statx) -> i32;
        #[cfg(test)]
        fn usleep(micros: u32) -> i32;
    }
    #[cfg(test)]
    let _active = ActiveFileReader::new();
    run_isolated_bytes(
        timeout,
        &format!("provider response {}", path.display()),
        move || unsafe {
            #[cfg(test)]
            {
                let delay = FILE_READER_DELAY_MS.load(AtomicOrdering::Relaxed);
                if delay > 0 {
                    usleep((delay.min(u32::MAX as u64) * 1000) as u32);
                }
            }
            let fd = open(
                name.as_ptr(),
            ...
            if fd < 0 {
                return Err(ProviderFailure::operation(
                    FailureClass::Execution,
                    "cannot open provider response without following links",
                ));
            }
            let empty = b"\0";
            let mut info: Statx = std::mem::zeroed();
            if statx(
                fd,
                ...
                || info.mode & S_IFMT != S_IFREG
            {
                close(fd);
                return Err(ProviderFailure::operation(
                    FailureClass::Execution,
                    "provider response is not a regular file",
                ));
            }
            let mut bytes = Vec::new();
            let mut buffer = [0u8; 8192];
            loop {
                let count = read(fd, buffer.as_mut_ptr(), buffer.len());
                if count < 0 {
                    close(fd);
                    return Err(ProviderFailure::operation(
                        FailureClass::Execution,
                ...
                if bytes.len() > MAX_BYTES {
                    close(fd);
                    return Err(ProviderFailure::malformed("provider stream exceeds 16 MiB"));
                }
            }
            close(fd);
            Ok(bytes)
...
#[cfg(target_os = "linux")]
fn run_isolated_bytes<F>(
    timeout: Duration,
    label: &str,
    work: F,
) -> Result<Vec<u8>, ProviderFailure>
where
    F: FnOnce() -> Result<Vec<u8>, ProviderFailure>,
{
    const O_NONBLOCK: i32 = 0o4000;
    const F_SETFL: i32 = 4;
    const WNOHANG: i32 = 1;
    ...
    fn class_byte(class: FailureClass) -> u8 {
        match class {
            FailureClass::Unavailable => 0,
            FailureClass::Malformed => 1,
            FailureClass::Panic => 2,
            FailureClass::Timeout => 3,
            FailureClass::Execution => 4,
            FailureClass::Incompatible => 5,
            FailureClass::Unsupported => 6,
            FailureClass::Unresolved => 7,
        }
    }
    fn byte_class(value: u8) -> Option<FailureClass> {
        Some(match value {
            0 => FailureClass::Unavailable,
            1 => FailureClass::Malformed,
            2 => FailureClass::Panic,
            3 => FailureClass::Timeout,
            4 => FailureClass::Execution,
            5 => FailureClass::Incompatible,
            6 => FailureClass::Unsupported,
            7 => FailureClass::Unresolved,
            _ => return None,
        })
    }
    let supervisor = unsafe { getpid() };
    let mut pipes = [-1, -1];
    ...
    if pid == 0 {
        unsafe {
            if prctl(PR_SET_PDEATHSIG, SIGKILL as usize, 0, 0, 0) != 0 || getppid() != supervisor {
                _exit(1)
            }
            close(pipes[0]);
            setpgid(0, 0);
            ...
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(work))
                .map_err(|_| {
                    ProviderFailure::operation(FailureClass::Panic, "isolated worker panicked")
                })
            ...
            let frame = match result {
                ...
                Err(failure) => {
                    let mut frame = Vec::with_capacity(failure.reason.len() + 2);
                    frame.push(1);
                    frame.push(class_byte(failure.class));
                    frame.extend_from_slice(failure.reason.as_bytes());
                    frame
                }
            };
            let mut at = 0;
            while at < frame.len() {
                let sent = write(pipes[1], frame[at..].as_ptr(), frame.len() - at);
                if sent <= 0 {
                    break;
                }
                at += sent as usize
    ...
    #[cfg(test)]
    let _active = ActiveIsolatedWorker::new();
    unsafe {
        close(pipes[1]);
        setpgid(pid, pid);
    ...
    #[cfg(test)]
    LAST_ISOLATED_GROUP.store(pid as u64, AtomicOrdering::SeqCst);
    let deadline = Instant::now() + timeout;
    let mut frame = Vec::new();
    let mut worker_reaped = false;
    loop {
        let mut buffer = [0u8; 8192];
        let count = unsafe { read(pipes[0], buffer.as_mut_ptr(), buffer.len()) };
        if count > 0 {
            frame.extend_from_slice(&buffer[..count as usize]);
            continue;
        }
        if count == 0 {
            unsafe {
        ...
        if Instant::now() >= deadline {
            ...
            return Err(ProviderFailure::operation(
                FailureClass::Timeout,
                format!("{label} timed out and its isolated process group was terminated"),
            ));
        }
        std::thread::sleep(Duration::from_millis(1));
    }
    ...
    match frame.first().copied() {
        ...
        Some(1) => {
            let class = frame.get(1).copied().and_then(byte_class).ok_or_else(|| {
                ProviderFailure::operation(
                    FailureClass::Execution,
                    "isolated worker returned an invalid failure class",
                )
            })?;
            let reason = String::from_utf8(frame[2..].to_vec()).map_err(|_| {
                ProviderFailure::operation(
                    FailureClass::Execution,
                    "isolated worker returned non-UTF-8 failure text",
                )
            })?;
            Err(ProviderFailure::operation(class, reason))
        }
        _ => Err(ProviderFailure::operation(
            FailureClass::Execution,
            format!("{label} exited without a result"),
        )),
    }
}
...
fn run_subprocess(
    ...
    let mut child = command.spawn().map_err(|error| {
        ProviderFailure::operation(
            FailureClass::Execution,
            format!("cannot launch provider {}: {error}", path.display()),
        )
    })?;
    let mut stdin = child.stdin.take().ok_or_else(|| {
        ProviderFailure::operation(FailureClass::Execution, "provider stdin was unavailable")
    })?;
    ...
    let stdout = child.stdout.take().ok_or_else(|| {
        ProviderFailure::operation(FailureClass::Execution, "provider stdout was unavailable")
    })?;
    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                let bytes = rx
                    ...
                    .map_err(|_| {
                        ...
                        ProviderFailure::operation(
                            FailureClass::Timeout,
                            "provider stdout did not close before deadline",
                        )
                    })?
                    .map_err(|e| {
                        ProviderFailure::operation(
                            FailureClass::Execution,
                            format!("cannot read provider stdout: {e}"),
                        )
                    })?;
                writer_rx
                    ...
                    .map_err(|_| {
                        ...
                        ProviderFailure::operation(
                            FailureClass::Timeout,
                            "provider stdin did not close before deadline",
                        )
                    })?
                    .map_err(|e| {
                        ProviderFailure::operation(
                            FailureClass::Execution,
                            format!("cannot write provider stdin: {e}"),
                        )
                    })?;
                if bytes.len() > MAX_BYTES {
                    return Err(ProviderFailure::malformed("provider stream exceeds 16 MiB"));
            ...
            Ok(None) => {
                ...
                return Err(ProviderFailure::operation(
                    FailureClass::Timeout,
                    "provider timed out and was terminated",
                ));
            }
            Err(error) => {
                terminate_group(&mut child);
                return Err(ProviderFailure::operation(
                    FailureClass::Execution,
                    format!("cannot supervise provider: {error}"),
                ));
            }
        }
    }
...
pub fn encode_stream(events: &[ProviderEvent]) -> Vec<u8> {
    let mut out = MAGIC.to_vec();
    for event in events {
        match event {
            ProviderEvent::Sample {
                spec,
                metric,
                value,
            } => {
                out.push(1);
                put_u32(&mut out, *spec);
                put_text(&mut out, metric);
                put_text(&mut out, &value.num.to_string());
                put_text(&mut out, &value.den.to_string());
            }
            ProviderEvent::Unavailable {
                spec,
                reason,
                details,
            } => {
                ...
                put_u32(&mut out, details.len() as u32);
                for (k, v) in details {
                    put_text(&mut out, k);
                    put_text(&mut out, v);
                }
...
fn decode_stream(
    ...
    if r.take(MAGIC.len())? != MAGIC {
        return Err(ProviderFailure::malformed("provider stream has bad magic"));
    }
    let mut events = Vec::new();
    while r.at < bytes.len() {
        let tag = r.byte()?;
        let event = match tag {
            1 => ProviderEvent::Sample {
                spec: r.u32()?,
                metric: r.text()?,
                value: Rational::parse(&r.text()?, &r.text()?)
                    .map_err(ProviderFailure::malformed)?,
            },
            2 => {
                let spec = r.u32()?;
                ...
                if count > MAX_DETAIL_SCALARS {
                    return Err(ProviderFailure::malformed(
                        "provider unavailable detail exceeds 512 scalars",
                    ));
                }
                let mut details = Vec::with_capacity(count);
                for _ in 0..count {
                    details.push((r.text()?, r.text()?));
                }
                if detail_scalars(&reason, &details) > MAX_DETAIL_SCALARS {
                    return Err(ProviderFailure::malformed(
                        "provider unavailable detail exceeds 512 scalars",
                    ));
                }
            ...
            4 => {
                let spec = r.u32()?;
                let count = r.u32()? as usize;
                if count > MAX_METADATA_SCALARS {
                    return Err(ProviderFailure::malformed(
                        "provider metadata exceeds its scalar limit",
                    ));
                }
                let mut details = Vec::with_capacity(count);
                for _ in 0..count {
            ...
            3 => ProviderEvent::Complete {
                request_id: r.text()?,
                samples: r.u64()?,
            },
            _ => {
                return Err(ProviderFailure::malformed(
                    "provider stream has unknown event tag",
                ))
            }
        };
        events.push(event);
    }
    validate_events(events, request).map(|v| v.events)
}
struct Reader<'a> {
    bytes: &'a [u8],
    at: usize,
...
impl<'a> Reader<'a> {
    fn take(&mut self, n: usize) -> Result<&'a [u8], ProviderFailure> {
        let end = self
            .at
            .checked_add(n)
            .ok_or_else(|| ProviderFailure::malformed("provider frame length overflow"))?;
        let value = self
            .bytes
            .get(self.at..end)
            .ok_or_else(|| ProviderFailure::malformed("provider stream is truncated"))?;
        self.at = end;
        Ok(value)
    }
    fn byte(&mut self) -> Result<u8, ProviderFailure> {
        Ok(self.take(1)?[0])
    }
    ...
    fn text(&mut self) -> Result<String, ProviderFailure> {
        let n = self.u32()? as usize;
        String::from_utf8(self.take(n)?.to_vec())
            .map_err(|_| ProviderFailure::malformed("provider text is not UTF-8"))
    }
}
...
fn validate_events(
    ...
    if events.is_empty() {
        return Err(ProviderFailure::malformed("provider stream is empty"));
    }
    let mut sample_count = 0usize;
    let mut complete = false;
    let mut last_spec = 0u32;
    let mut seen = false;
    for (index, event) in events.iter().enumerate() {
        if complete {
            return Err(ProviderFailure::malformed("event follows final Complete"));
        }
        match event {
            ProviderEvent::Sample { spec, metric, .. } => {
                if *spec as usize >= request.specs.len()
                    || request.specs[*spec as usize].metric != *metric
                {
                    return Err(ProviderFailure::operation(
                        FailureClass::Incompatible,
                        "provider sample does not match requested spec/metric",
                    ));
                }
                if seen && *spec < last_spec {
                    return Err(ProviderFailure::malformed(
                        "provider events are not contiguous and ordered",
                    ));
                }
                seen = true;
                last_spec = *spec;
                ...
                if sample_count > MAX_SAMPLES {
                    return Err(ProviderFailure::malformed(
                        "provider emitted more than 1000000 samples",
                    ));
                }
            }
            ProviderEvent::Metadata { spec, details } => {
                if *spec as usize >= request.specs.len() || details.is_empty() {
                    return Err(ProviderFailure::malformed(
                        "provider metadata has an invalid spec or no fields",
                    ));
                }
                if detail_scalars("", details) > MAX_METADATA_SCALARS {
                    return Err(ProviderFailure::malformed(
                        "provider metadata exceeds its scalar limit",
                    ));
                }
                ...
                for (key, value) in details {
                    if key.is_empty() || previous.is_some_and(|prior| prior >= key.as_str()) {
                        return Err(ProviderFailure::malformed(
                            "provider metadata keys are not unique and sorted",
                        ));
                    }
                    if value.is_empty() {
                        return Err(ProviderFailure::malformed(
                            "provider metadata contains an empty value",
                        ));
                    }
            ...
            ProviderEvent::Unavailable {
                ...
                if *spec as usize >= request.specs.len() || reason.is_empty() {
                    return Err(ProviderFailure::malformed(
                        "provider Unavailable has invalid spec or empty reason",
                    ));
                }
                if detail_scalars(reason, details) > MAX_DETAIL_SCALARS {
                    return Err(ProviderFailure::malformed(
            ...
            ProviderEvent::Complete {
                ...
                {
                    return Err(ProviderFailure::malformed(
                        "provider Complete request id/count/finality mismatch",
                    ));
                }
                complete = true;
            }
    ...
    if !complete {
        return Err(ProviderFailure::malformed(
            "provider stream has no final Complete",
        ));
    }
    Ok(ProviderEvidence { events })
}
fn detail_scalars(reason: &str, details: &[(String, String)]) -> usize {
    reason.chars().count().saturating_add(
        details
            .iter()
            .map(|(key, value)| key.chars().count().saturating_add(value.chars().count()))
            .sum::<usize>(),
    )
}
...
pub fn unavailable_if_too_few(
    ...
    let count = evidence
        .events
        .iter()
        .filter(|e| matches!(e, ProviderEvent::Sample { .. }))
        .count();
    if let Some(ProviderEvent::Unavailable { reason, .. }) = evidence
        .events
        .iter()
        .find(|e| matches!(e, ProviderEvent::Unavailable { .. }))
    {
        return Err(
            ProviderFailure::operation(FailureClass::Unavailable, reason.clone())
                .diagnostic(budget),
        );
    }
    if count < minimum {
        return Err(ProviderFailure::operation(
            FailureClass::Unavailable,
            format!("provider returned {count} samples; policy requires {minimum}"),
        )
        .diagnostic(budget));
    }
    Ok(())
...
pub fn evaluate_provider_evidence(
    budget: &str,
    evidence_id: &str,
    context_key: &str,
...
) -> Result<Evaluation, ProviderDiagnostic> {
    ...
    for event in &evidence.events {
        match event {
            ProviderEvent::Sample {
                ...
                value,
                ..
            } if *event_spec == spec => samples.push(value.clone()),
            ProviderEvent::Unavailable {
                spec: event_spec,
                reason,
                ..
    ...
    if samples.len() < minimum_samples {
        return Err(ProviderFailure::operation(
            FailureClass::Unavailable,
            format!(
                "provider returned {} samples; policy requires {minimum_samples}",
                samples.len()
            ),
        )
        .diagnostic(budget));
    }
    jet_foundation::PerformanceBudget::evaluate(
        evidence_id,
        context_key,
        baseline_report_ids,
        &samples,
    ...
    .map_err(|reason| {
        ProviderFailure::operation(
            FailureClass::Execution,
            format!("shared evaluator rejected provider evidence: {reason}"),
        )
        .diagnostic(budget)
    })
}

pub fn evaluation_diagnostic(
    budget: &str,
    evaluation: &Evaluation,
    direction: Direction,
    baseline_report_ids: &[String],
) -> Option<ProviderDiagnostic> {
    use jet_foundation::PerformanceBudget::Evidence;
    match evaluation.evidence {
        Evidence::Pass => None,
        Evidence::Unavailable => Some(
            ProviderFailure::operation(
                FailureClass::Unavailable,
                "the shared evaluator found no compatible nonzero baseline evidence",
            )
            .diagnostic(budget),
        ),
        Evidence::Regression | Evidence::Inconclusive => {
            let state = if evaluation.evidence == Evidence::Regression {
                "regressed"
            } else {
                "is inconclusive"
            };
            let rational = |v: &Rational| format!("{}/{}", v.num, v.den);
...
mod tests {
    ...
    fn request() -> ProviderRequest {
        ProviderRequest {
            ...
            provider_hash: "2".repeat(64),
            context_hash: "3".repeat(64),
            specs: vec![ProviderSpec {
                budget_hash: "4".repeat(64),
                metric: "BenchTime".into(),
            }],
            workload: CanonicalJson::Null,
            policy: CanonicalJson::Null,
        }
    }
    fn valid_events(request: &ProviderRequest) -> Vec<ProviderEvent> {
        vec![
            ProviderEvent::Sample {
                spec: 0,
                metric: "BenchTime".into(),
                value: Rational::integer(42),
            },
            ProviderEvent::Complete {
                request_id: request.request_id.clone(),
                samples: 1,
    ...
    fn unavailable_provider(
        ...
        Ok(vec![
            ProviderEvent::Unavailable {
                spec: 0,
                reason: "probe could not observe ready event".into(),
                details: vec![],
            },
            ProviderEvent::Complete {
                request_id: request.request_id.clone(),
    ...
    fn compiler_probe_rejects_forged_workload_identity_before_execution() {
        let root = temporary("compile-identity");
        std::fs::create_dir_all(root.join("src")).unwrap();
        std::fs::create_dir_all(root.join("edits")).unwrap();
        std::fs::write(
        ...
        std::fs::write(root.join("edits/change.patch"), "patch\n").unwrap();
        let workload = compiler_probe_workload(
            &root,
            &root.join("src/run.jet"),
            "Edit",
            "cli",
            "dev",
            Some("edits/change.patch"),
        )
        .unwrap();
        let CanonicalJson::Object(mut fields) = workload else {
            panic!("workload object")
        };
        fields.insert(
            "source_tree_sha256".into(),
            CanonicalJson::String("0".repeat(64)),
        );
        let request = ProviderRequest {
            ...
            specs: vec![ProviderSpec {
                budget_hash: "4".repeat(64),
                metric: "CompileTime(P95)".into(),
            }],
            workload: CanonicalJson::Object(fields),
            policy: CanonicalJson::Null,
        };
        let cancellation = ProviderCancellation {
            cancelled: Arc::new(AtomicBool::new(false)),
    ...
    fn compiler_probe_applies_a_deterministic_one_file_edit() {
        ...
        assert_eq!(
            std::fs::read_to_string(root.join("src/run.jet")).unwrap(),
            "fn run() { print(\"fixture\") }\n"
        );
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    fn compiler_probe_rejects_unsupported_profile_before_execution() {
        let root = temporary("compile-profile");
        std::fs::create_dir_all(root.join("src")).unwrap();
        std::fs::write(
            root.join("package.jet"),
            "name: \"profile\"\nversion: \"0.1.0\"\n",
        ...
        .unwrap();
        std::fs::write(root.join("src/run.jet"), "fn run() {}\n").unwrap();
        let error = compiler_probe_workload(
            &root,
            &root.join("src/run.jet"),
            "Clean",
            "cli",
        ...
        .unwrap_err();
        assert!(
            error.contains("profile `unsupported` is unsupported"),
            "{error}"
        );
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    fn compiler_probe_rejects_pathological_source_before_hashing() {
        let root = temporary("compile-oversized-source");
        std::fs::create_dir_all(root.join("src")).unwrap();
        std::fs::write(
            root.join("package.jet"),
            "name: \"large\"\nversion: \"0.1.0\"\n",
        ...
        .unwrap();
        let source = root.join("src/run.jet");
        let file = std::fs::File::create(&source).unwrap();
        file.set_len(COMPILE_MAX_PROJECT_BYTES + 1).unwrap();
        let error =
            compiler_probe_workload(&root, &source, "Clean", "cli", "dev", None).unwrap_err();
    ...
    fn compiler_probe_identity_names_the_production_lens_for_each_profile() {
        ...
        assert!(
            jit.contains("backend=cranelift-jit;linker=cranelift-jit"),
            "{jit}"
        );
    ...
    #[cfg(target_os = "linux")]
    fn assert_last_group_gone() {
        extern "C" {
            fn kill(pid: i32, signal: i32) -> i32;
        }
        let group = LAST_ISOLATED_GROUP.load(Ordering::SeqCst) as i32;
        let deadline = Instant::now() + Duration::from_millis(100);
        while unsafe { kill(-group, 0) } == 0 && Instant::now() < deadline {
            std::thread::yield_now()
        }
        assert_ne!(
            unsafe { kill(-group, 0) },
            0,
            "isolated provider process group survived timeout"
        );
    }
    #[cfg(target_os = "linux")]
    ...
    #[cfg(target_os = "linux")]
    struct PidFd(std::fs::File);
    #[cfg(target_os = "linux")]
    impl PidFd {
        fn open(pid: i32) -> std::io::Result<Self> {
            ...
            if fd < 0 {
                ...
                Ok(Self(unsafe { std::fs::File::from_raw_fd(fd as i32) }))
            }
        }
        ...
        fn alive(&self) -> std::io::Result<bool> {
            match self.signal(0) {
                Ok(()) => Ok(true),
                Err(error) if error.raw_os_error() == Some(3) => Ok(false),
                Err(error) => Err(error),
            }
        }
    }
    ...
    fn isolated_worker_dies_with_supervisor() {
            ...
            let _ = run_isolated_bytes(
                ...
                move || {
                    std::fs::write(&path, std::process::id().to_string()).map_err(|error| {
                        ProviderFailure::operation(
                            FailureClass::Execution,
                            format!("cannot publish isolated worker PID: {error}"),
                        )
                    })?;
                    loop {
                        std::hint::spin_loop()
        ...
        let mut supervisor = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "BudgetProviders::tests::isolated_worker_dies_with_supervisor",
                "--nocapture",
            ])
            .env(CHILD_ENV, "1")
            .env(PID_PATH_ENV, &path)
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .unwrap();
        ...
        let worker = loop {
            ...
            if Instant::now() >= publish_deadline {
                ...
                return assert!(
                    false,
                    "isolated worker did not publish its PID before the deadline"
                );
            }
            assert!(
                supervisor.try_wait().unwrap().is_none(),
                "isolated worker supervisor exited before publishing its PID"
            );
            std::thread::sleep(Duration::from_millis(2));
        };
        let worker = match PidFd::open(worker) {
            Ok(worker) => worker,
            Err(error) => {
                let _ = supervisor.wait();
                let _ = std::fs::remove_file(&path);
                return assert!(false, "cannot open isolated worker pidfd: {error}");
            }
        };
        supervisor.kill().unwrap();
        supervisor.wait().unwrap();
        let exit_deadline = Instant::now() + Duration::from_secs(2);
        while worker.alive().unwrap() && Instant::now() < exit_deadline {
            std::thread::sleep(Duration::from_millis(2));
        }
        ...
        if !gone {
            ...
            while worker.alive().unwrap() && Instant::now() < cleanup_deadline {
                std::thread::sleep(Duration::from_millis(2));
            }
        }
        let _ = std::fs::remove_file(&path);
    ...
    fn file_transport_round_trips_and_rejects_hostile_frames() {
        let _guard = PROCESS_TEST_LOCK
            ...
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let req = request();
        let path = temporary("response");
        std::fs::write(&path, encode_stream(&valid_events(&req))).unwrap();
        let mut registry = ProviderRegistry::default();
        registry.register_file("fixture", path.clone()).unwrap();
        let evidence = registry
            .collect("fixture", &req, Duration::from_secs(1))
            .unwrap();
        assert_eq!(evidence.events, valid_events(&req));
        for (name, bytes) in [
            ("bad-magic", b"NOTBUDGET\n".to_vec()),
            ("truncated", {
            ...
            ("trailing", {
                let mut b = encode_stream(&valid_events(&req));
                b.push(99);
                b
            }),
        ] {
            let hostile = temporary(name);
            std::fs::write(&hostile, bytes).unwrap();
            let mut registry = ProviderRegistry::default();
            registry.register_file("hostile", hostile.clone()).unwrap();
            let error = registry
                .collect("hostile", &req, Duration::from_secs(1))
                .unwrap_err();
            assert_eq!(error.diagnostic("api").code, "E2908");
    ...
    fn in_process_panic_and_unavailable_are_separate_diagnostic_classes() {
        ...
        let mut registry = ProviderRegistry::default();
        registry
            .register_in_process("panic", panic_provider)
            .unwrap();
        let failure = registry
            .collect("panic", &req, Duration::from_secs(1))
            .unwrap_err();
        let diagnostic = failure.diagnostic("api-p99");
        assert_eq!(
            (diagnostic.code, diagnostic.what.as_str()),
            ("E2908", "performance budget operation failed")
        );
        registry
            .register_in_process("unavailable", unavailable_provider)
            .unwrap();
        let evidence = registry
            .collect("unavailable", &req, Duration::from_secs(1))
            .unwrap();
        let diagnostic = unavailable_if_too_few("api-p99", &evidence, 20).unwrap_err();
        assert_eq!(
            (diagnostic.code, diagnostic.what.as_str()),
            ("E2906", "performance budget api-p99 has no usable evidence")
        );
        assert!(diagnostic.why.contains("ready event"));
        fn hostile(
            _: &ProviderRequest,
            _: &ProviderCancellation,
        ) -> Result<Vec<ProviderEvent>, ProviderFailure> {
        ...
        let _guard = PROCESS_TEST_LOCK
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        registry.register_in_process("hostile", hostile).unwrap();
        let started = Instant::now();
        for _ in 0..25 {
            let failure = registry
                .collect("hostile", &req, Duration::from_millis(5))
                .unwrap_err();
            assert_eq!(failure.class, FailureClass::Timeout);
            assert_last_group_gone();
            assert_eq!(ACTIVE_ISOLATED_WORKERS.load(Ordering::SeqCst), 0);
        }
    ...
    fn subprocess_uses_exact_path_and_is_bounded_and_timed() {
        ...
        let req = request();
        let response = temporary("subprocess-response");
        std::fs::write(&response, encode_stream(&valid_events(&req))).unwrap();
        let script = temporary("provider.sh");
        std::fs::write(
            &script,
            format!("#!/bin/sh\ncat '{}'\n", response.display()),
        )
        ...
        let mut registry = ProviderRegistry::default();
        registry
            .register_subprocess("process", script.clone())
            .unwrap();
        assert_eq!(
            registry
                .collect("process", &req, Duration::from_secs(2))
                .unwrap()
                .events,
            valid_events(&req)
        );
        let sleeper = temporary("sleep.sh");
        std::fs::write(&sleeper, "#!/bin/sh\nsleep 5\n").unwrap();
        let mut permissions = std::fs::metadata(&sleeper).unwrap().permissions();
        permissions.set_mode(0o700);
        std::fs::set_permissions(&sleeper, permissions).unwrap();
        let mut registry = ProviderRegistry::default();
        registry
            .register_subprocess("slow", sleeper.clone())
            .unwrap();
        let started = Instant::now();
        let failure = registry
            .collect("slow", &req, Duration::from_millis(30))
            .unwrap_err();
        assert_eq!(failure.class, FailureClass::Timeout);
        assert!(started.elapsed() < Duration::from_secs(2));
        let descendant = temporary("descendant.sh");
        std::fs::write(&descendant, "#!/bin/sh\nsleep 5 &\nexit 0\n").unwrap();
        let mut permissions = std::fs::metadata(&descendant).unwrap().permissions();
        permissions.set_mode(0o700);
        std::fs::set_permissions(&descendant, permissions).unwrap();
        let mut registry = ProviderRegistry::default();
        registry
            .register_subprocess("descendant", descendant.clone())
            .unwrap();
        let started = Instant::now();
        let failure = registry
            .collect("descendant", &req, Duration::from_millis(30))
            .unwrap_err();
        assert_eq!(failure.class, FailureClass::Timeout);
        assert!(started.elapsed() < Duration::from_secs(1));
        for path in [response, script, sleeper, descendant] {
            let _ = std::fs::remove_file(path);
        }
    }
    ...
    fn file_transport_rejects_symlinks_and_scalar_overflow() {
        ...
        std::fs::write(&target, encode_stream(&valid_events(&req))).unwrap();
        #[cfg(unix)]
        {
            ...
            let mut registry = ProviderRegistry::default();
            registry.register_file("link", link.clone()).unwrap();
            assert!(registry
                .collect("link", &req, Duration::from_secs(1))
                .is_err());
        ...
        let path = temporary("detail");
        std::fs::write(&path, encode_stream(&events)).unwrap();
        let mut registry = ProviderRegistry::default();
        registry.register_file("detail", path.clone()).unwrap();
        assert!(registry
            .collect("detail", &req, Duration::from_secs(1))
            .unwrap_err()
            .reason
            .contains("512 scalars"));
    ...
    #[cfg(unix)]
    #[test]
    fn repeated_timed_file_readers_are_killed_and_reaped() {
        let _guard = PROCESS_TEST_LOCK
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let req = request();
        let path = temporary("delayed");
        std::fs::write(&path, encode_stream(&valid_events(&req))).unwrap();
        let mut registry = ProviderRegistry::default();
        registry.register_file("delayed", path.clone()).unwrap();
        FILE_READER_DELAY_MS.store(100, Ordering::SeqCst);
        let started = Instant::now();
        for _ in 0..25 {
            let failure = registry
                .collect("delayed", &req, Duration::from_millis(5))
                .unwrap_err();
            assert_eq!(failure.class, FailureClass::Timeout);
            assert_last_group_gone();
            assert_eq!(ACTIVE_FILE_READERS.load(Ordering::SeqCst), 0);
        }
        FILE_READER_DELAY_MS.store(0, Ordering::SeqCst);
        assert!(started.elapsed() < Duration::from_secs(1));
        let _ = std::fs::remove_file(path);
    }
    ...
    fn concurrent_isolated_workers_do_not_mask_file_reader_reaping() {
        ...
        let workers = (0..4)
            .map(|_| {
                std::thread::spawn(|| {
                    ...
                    registry.register_in_process("hostile", hostile).unwrap();
                    registry
                        .collect("hostile", &req, Duration::from_millis(500))
                        .unwrap_err()
                })
            })
            .collect::<Vec<_>>();
        let deadline = Instant::now() + Duration::from_secs(1);
        while ACTIVE_ISOLATED_WORKERS.load(Ordering::SeqCst) < 4 && Instant::now() < deadline {
            std::thread::yield_now()
        }
        assert!(
            ACTIVE_ISOLATED_WORKERS.load(Ordering::SeqCst) >= 4,
            "hostile isolated workers did not overlap"
        );
        let req = request();
        let path = temporary("parallel-delayed");
        std::fs::write(&path, encode_stream(&valid_events(&req))).unwrap();
        let mut registry = ProviderRegistry::default();
        registry.register_file("delayed", path.clone()).unwrap();
        FILE_READER_DELAY_MS.store(100, Ordering::SeqCst);
        for _ in 0..10 {
            ...
            assert!(
                ACTIVE_ISOLATED_WORKERS.load(Ordering::SeqCst) >= 4,
                "generic workers ended before file-reader lifecycle was checked"
            );
        }
        FILE_READER_DELAY_MS.store(0, Ordering::SeqCst);
        for worker in workers {
            assert_eq!(worker.join().unwrap().class, FailureClass::Timeout);
        }
        assert_eq!(ACTIVE_FILE_READERS.load(Ordering::SeqCst), 0);
        let _ = std::fs::remove_file(path);
    }
