use super::super::{
    describe, Diagnostic, ImplDef, Item, Parser, Span, StrTokPart, Syntax, TokKind,
};
...
impl<'a> Parser<'a> {
    ...
    fn task_string_list(
        ...
        match expr {
            crate::AST::Expr::ListLit(values, _) => values
                ...
                .map(|value| {
                    Self::job_word(value).map(str::to_string).ok_or_else(|| {
                        Self::job_metadata_error("list", "a list of strings or names", value.span())
                    })
                })
                .collect(),
            _ => Self::job_word(expr)
                ...
                .ok_or_else(|| {
                    Self::job_metadata_error("list", "a list of strings or names", marker_span)
                }),
        }
    }
    ...
    fn task_optional_string(
        ...
        match Self::job_word(expr) {
            ...
            Some(_) => Ok(None),
            None => Err(Self::job_metadata_error(
                field,
                "a string or name",
                marker_span,
            )),
            ...
            for marker in &markers {
                match marker.name.as_str() {
                    Syntax::MARKER_ABI => {
                        ...
                        let arguments = self.bound_registered_rule_arguments(marker)?;
                        let Some(crate::AST::Expr::Ident(name, span)) = arguments.parameter(0)
                        else {
                            return Err(crate::Policy::marker_argument_shape_error(
                                Syntax::MARKER_ABI,
                                marker.span,
                    ...
                    Syntax::MARKER_UNDO => {
                        if undo.is_some() {
                            return Err(Diagnostic::error(
                                "E3212",
                                "a foreign declaration may have only one `#Undo` marker"
                                    .to_string(),
                    ...
                    Syntax::MARKER_CLOSE => {
                        if close.is_some() {
                            return Err(Diagnostic::error(
                                "E3212",
                                "a foreign declaration may have only one `#Close` marker"
                                    .to_string(),
                                "one returned foreign handle has one consuming close function"
                                    .to_string(),
