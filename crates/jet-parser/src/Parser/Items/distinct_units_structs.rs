use super::super::{describe, Diagnostic, Parser, Span, StrTokPart, Syntax, TokKind};
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn at_bundle_distinct_def(&self) -> bool {
        ...
        loop {
            match self.toks.get(i).map(|t| &t.kind) {
                Some(TokKind::Hash) => match self.toks.get(i + 1).map(|t| &t.kind) {
    ...
    pub(super) fn distinct_def(
        ...
        let (base, inline_range) = match parsed_base {
            crate::AST::Type::InlineRange { base, lo, hi } => (*base, Some((lo, hi, base_ty_span))),
            base => (base, None),
        };
        ...
        for marker in &type_markers {
            self.bind_rule_fact(marker.name_span, Some(span), crate::Policy::RuleSite::Type);
        }
    ...
    pub(crate) fn expect_range_bound_int(
        &mut self,
        where_: &str,
    ) -> Result<(i64, Span), Diagnostic> {
        match self.peek().kind {
            TokKind::Int(n, _) => {
    ...
    pub(super) fn type_alias_def(
        ...
        match self.peek().kind {
            ...
            TokKind::Eq => {
                ...
                self.diags
                    .push(Diagnostic::from_row("E0378", &[], Some(equals.span)));
            }
    ...
    pub(super) fn unit_family_def(
            ...
            .map(|value| match value {
                ...
                _ => Err(Diagnostic::error(
                    ...
                    "a dimension is a new axis or a product of existing dimension names"
                        .to_string(),
                    "write `dimension` or `dimension: Mass * Length / Time / Time`".to_string(),
                    Some(value.span()),
                )),
            })
            .transpose()?;
        ...
        if matches!(dimension, Some(crate::AST::UnitDimensionDecl::Derived(_))) && base.is_none() {
            return Err(Diagnostic::error(
                "E0003",
                format!("derived dimension `{family}` has no canonical base"),
                "every derived dimension needs one member whose scale is exactly one".to_string(),
    ...
    fn unit_family_scale(
        ...
        let kind = match &self.peek().kind {
            TokKind::Ident(name) if name == "conventional" || name == "measured" => name.clone(),
            _ => {
                return self
                    .unit_family_ratio()
                    .map(|ratio| (ratio, crate::AST::UnitScaleProvenance::Rational));
            }
        };
        ...
        let provenance = if kind == "conventional" {
            ...
            let standard_uncertainty = self.unit_family_decimal("as the standard uncertainty")?;
    ...
    pub(super) fn layout_type_def(
        ...
        if matches!(self.peek().kind, TokKind::KwEnum) {
            ...
            if tag_width.is_some() {
                return Err(Diagnostic::error(
                    "E1105",
                    "A tag width applies only to enums.".to_string(),
                    "Structs have fields and padding but no discriminant tag.".to_string(),
                    "Remove `tag: …`, or put this layout on an enum.".to_string(),
                    Some(attr_span),
                ));
            }
    ...
    /// codegen (I3).
    pub(super) fn single_use_type_def(
        &mut self,
        outer_is_pub: bool,
    ) -> Result<crate::AST::Item, Diagnostic> {
    ...
    pub(super) fn must_use_type_def(
        &mut self,
        outer_is_pub: bool,
    ) -> Result<crate::AST::Item, Diagnostic> {
        let attr_start = self.peek().span;
    ...
    pub(super) fn struct_def_after_pub(
        &mut self,
        is_pub: bool,
    ) -> Result<crate::AST::StructDef, Diagnostic> {
...
fn unit_scale_error(reason: String, span: Span) -> Diagnostic {
    Diagnostic::error(
        ...
        "unit scales preserve whether a value is exact, symbolic, conventional, or measured"
            .to_string(),
        "use an exact ratio, `pi / n`, `conventional(...)`, or `measured(...)`".to_string(),
        Some(span),
    )
}
