use super::super::{
    describe, Diagnostic, Item, Marker, Parser, Span, Syntax, TagDef, TokKind, TraitDef,
};
...
impl<'a> Parser<'a> {
    ...
    pub(in crate::Parser) fn read_marker_name(&mut self) -> Result<MarkerHead, Diagnostic> {
        let negated_span = matches!(self.peek().kind, TokKind::Bang).then(|| self.bump().span);
            ...
            if !Syntax::is_signed_auto_derive(&head.name) {
                return Err(Diagnostic::error(
                    ...
                    "`!` rejects compiler generation only for structurally auto-derived traits"
                        .to_string(),
                    format!(
                        "remove `!` from `#{}`, or use it with an auto-derived trait",
                        head.name
                    ),
    ...
    fn wrong_rule_site(marker: &Marker, site: crate::Policy::RuleSite, _noun: &str) -> Diagnostic {
    ...
    fn validate_registered_rule_marker(
        ...
        if !parenthesized && rule.signature.arguments_required() && marker.name != Syntax::KW_UNSAFE
        {
            return Err(crate::Policy::marker_argument_shape_error(
                &marker.name,
                marker.span,
    ...
    pub(in crate::Parser) fn validate_registered_rule_arguments(
        &self,
        marker: &Marker,
    ) -> Result<(), Diagnostic> {
        let Some(rule) = crate::Policy::applied_rule(&marker.name) else {
    ...
    pub(in crate::Parser) fn marker_head_is_followed_by(&self, expected: TokKind) -> bool {
    ...
    fn marker_fix_source(marker: &Marker) -> Option<String> {
        fn expr_source(expr: &crate::AST::Expr) -> Option<String> {
            match expr {
                ...
                crate::AST::Expr::UnitLit { raw, suffix, .. } => Some(format!("{raw}{suffix}")),
    ...
    fn parse_marker_bracket_group(
        ...
        if group.len() == 1 {
            let mut diagnostic = Diagnostic::error(
                ...
                "brackets group two or more markers; one marker stays bare".to_string(),
    ...
    pub(in crate::Parser) fn parse_attached_marker_sequence(
        ...
        loop {
            ...
            if self.at_marker_list() {
                if !markers.is_empty() && !self.bracket_group_allows_site(self.pos + 1, site) {
                    break;
                }
                let close_index =
                    Self::skip_bracket_group(&self.toks, self.pos + 1).ok_or_else(|| {
                        Diagnostic::error(
                            "E0003",
            ...
            let Some(name) = self.marker_name_at(self.pos).map(str::to_string) else {
                break;
            };
            if !markers.is_empty()
    ...
    /// never a name list.
    pub(in crate::Parser) fn diagnose_repeated_markers(&mut self, markers: &[Marker], noun: &str) {
        for (index, marker) in markers.iter().enumerate() {
            if crate::Policy::applied_rule(&marker.name).is_none_or(|rule| rule.repeatable) {
                continue;
            }
            if !markers[..index]
                .iter()
    ...
    fn bracket_group_allows_site(&self, open_index: usize, site: crate::Policy::RuleSite) -> bool {
        ...
        loop {
            ...
            let (next, file_only) = if matches!(
                ...
                (
                    ...
                    self.bracket_group_allows_site(cursor + 1, crate::Policy::RuleSite::File)
                ...
                let Some(name) = self.marker_name_at(cursor) else {
                    break;
                };
                (
                    self.skip_bare_marker(cursor),
                    crate::Policy::rule_allows(name, crate::Policy::RuleSite::File)
                        && (!crate::Policy::rule_allows(name, crate::Policy::RuleSite::Function)
    ...
    fn parse_function_marker_sequence(&mut self) -> Result<Vec<Marker>, Diagnostic> {
        self.parse_attached_marker_sequence(crate::Policy::RuleSite::Function, "function")
    }
    ...
    pub(in crate::Parser) fn func_with_marker_list(
        &mut self,
    ) -> Result<crate::AST::Func, Diagnostic> {
        let markers = self.parse_function_marker_sequence()?;
        if markers
            .iter()
            .any(|marker| marker.name == Syntax::MARKER_FFI)
        {
    ...
    pub(in crate::Parser) fn apply_function_markers(
        ...
        self.apply_callable_markers(function, markers, crate::Policy::RuleSite::Function)
    }
    ...
    fn apply_callable_markers(
        ...
        for marker in markers {
                ...
                crate::Policy::applied_rule(&marker.name).map(|rule| rule.status)
            {
                self.diags.push(Diagnostic::error(
                    ...
                    "the applied-rule registry owns retired spellings and replacements".to_string(),
            ...
            match marker.name.as_str() {
                    ...
                    let arg = match schedule {
                        ...
                        crate::AST::Expr::Str(parts, span) if parts.len() == 1 => match &parts[0] {
                            ...
                            crate::AST::StrPart::Interp(..) => {
                                crate::AST::EveryArg::Expression(schedule.clone())
                            }
                        },
                ...
                Syntax::MARKER_WASM_EXPORT if marker.args.is_empty() => {
                    function.web_marker = Some(crate::Syntax::WebPartitionMarker::WasmExport);
                }
                ...
                Syntax::MARKER_INLINE => {
                    ...
                    } else if arguments.parameter(0).and_then(Self::marker_enum_variant)
                        == Some("Always")
                    {
                ...
                Syntax::MARKER_KERNEL => {
                    ...
                    let mode = match arguments.parameter(0).and_then(Self::marker_enum_variant) {
                ...
                Syntax::KW_STATE => {
                    let Some(state) = arguments.parameter(0).and_then(Self::marker_ident_path)
                    else {
                        return Err(crate::Policy::marker_argument_shape_error(
                            Syntax::KW_STATE,
                            marker.span,
                ...
                Syntax::MARKER_UNDO => {
                    let Some(crate::AST::Expr::Ident(inverse, span)) = arguments.parameter(0)
                    else {
                        return Err(crate::Policy::marker_argument_shape_error(
                            Syntax::MARKER_UNDO,
                            marker.span,
        ...
        if let Some(every) = &function.every {
            if !function.is_job {
                self.diags.push(Self::e0925_every_without_job(every.span));
            }
        }
        ...
        function.markers = ordered_markers
            ...
            .filter(|marker| {
                crate::Policy::applied_rule(&marker.name)
                    .is_none_or(|rule| matches!(rule.status, crate::Policy::RuleStatus::Active))
            })
            .cloned()
            .collect();
        for marker in &ordered_markers {
            self.bind_rule_fact(marker.name_span, Some(function.span), site);
        }
    ...
    pub(in crate::Parser) fn apply_unsafe_function_marker(
        ...
        let Some(value) = arguments.parameter(1) else {
            return Ok(None);
        };
    ...
    pub(in crate::Parser) fn type_def_with_any_markers(&mut self) -> Result<Item, Diagnostic> {
        let markers = self.parse_attached_marker_sequence(crate::Policy::RuleSite::Type, "type")?;
    ...
    pub(in crate::Parser) fn tag_def(&mut self, nested: bool) -> Result<TagDef, Diagnostic> {
        ...
        if !matches!(self.peek().kind, TokKind::LBrace) {
            return Err(Diagnostic::error(
                ...
                "a tag declares a dataflow fact, including where that fact is denied".to_string(),
            ...
            match field.as_str() {
                ...
                _ => {
                    return Err(Diagnostic::error(
                        ...
                        "tag bodies have required `deny` and optional `from` fields".to_string(),
