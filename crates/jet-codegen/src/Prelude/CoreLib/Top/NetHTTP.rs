...
fn jet_net_to_io_error(error: JetNetError) -> jet_std::IOError {
    let operation = match jet_net_error_operation(&error).as_str() {
        ...
        operation if operation.contains("write") || operation.contains("send") => {
        ...
        operation if operation.contains("close") || operation.contains("shutdown") => {
            jet_std::IOOperation::Close
        }
        operation if operation.contains("resolve") || operation.contains("dns") => {
    ...
    let context = jet_std::IOContext::new(
        operation,
        resource,
        jet_net_error_os_code(&error),
        Some(error.jet_show()),
    );
    match error {
...
#[cfg(unix)]
fn jet_net_scheduler_handle<T: std::os::fd::AsRawFd>(
    handle: &T,
) -> std::sync::Arc<JetSchedulerRawIoHandle> {
    ...
    let (cert, key) = config
        ...
        .map(|identity| {
            (
                ...
                identity.private_key.as_vec().clone(),
            )
        })
...
impl JetShow for JetNetError {
    fn jet_show(&self) -> String {
        let (variant, detail_message, dns_variant, dns_value) = match self {
            JetNetError::InvalidInput(detail) => (0, Some(detail.message.as_str()), None, None),
            JetNetError::PermissionDenied(detail) => (1, Some(detail.message.as_str()), None, None),
            JetNetError::AddressInUse(detail) => (2, Some(detail.message.as_str()), None, None),
            JetNetError::AddressUnavailable(detail) => {
                (3, Some(detail.message.as_str()), None, None)
            }
            JetNetError::ConnectionRefused(detail) => {
                (4, Some(detail.message.as_str()), None, None)
            }
            JetNetError::ConnectionReset(detail) => (5, Some(detail.message.as_str()), None, None),
            JetNetError::NotConnected(detail) => (6, Some(detail.message.as_str()), None, None),
            JetNetError::Closed(detail) => (7, Some(detail.message.as_str()), None, None),
            JetNetError::Timeout(detail) => (8, Some(detail.message.as_str()), None, None),
            JetNetError::Cancelled(detail) => (9, Some(detail.message.as_str()), None, None),
            JetNetError::Unsupported(detail) => (10, Some(detail.message.as_str()), None, None),
            JetNetError::TLS(detail) => (11, Some(detail.message.as_str()), None, None),
            JetNetError::Protocol(detail) => (12, Some(detail.message.as_str()), None, None),
            JetNetError::Other(detail) => (13, Some(detail.message.as_str()), None, None),
            JetNetError::DNS(JetNetDnsError::NotFound(name)) => {
                (14, None, Some(0), Some(name.as_str()))
            }
            JetNetError::DNS(JetNetDnsError::Failure(message)) => {
                (14, None, Some(1), Some(message.as_str()))
            }
        };
        jet_net_error_kernel_show(variant, detail_message, dns_variant, dns_value)
    }
}
impl JetDebug for JetNetError {
    fn jet_debug(&self) -> String {
        <Self as JetShow>::jet_show(self)
    }
}
// D-FAIL-CONV2=A: one display hook backs both the shipped impl NetError -> Err
// and user interpolation.
impl JetDisplay for JetNetError {
    fn jet_display(&self) -> String {
        <Self as JetShow>::jet_show(self)
    }
}
...
fn jet_net_io_error(
    ...
    match error.kind() {
        std::io::ErrorKind::InvalidInput | std::io::ErrorKind::InvalidData => {
            JetNetError::InvalidInput(detail)
        }
        ...
        | std::io::ErrorKind::BrokenPipe => JetNetError::ConnectionReset(detail),
...
fn jet_net_tls_io_result<T>(
    ...
    result.map_err(|message| {
        let context = jet_std::IOContext::new(
            operation,
            Some("TLS stream".to_string()),
            None,
            Some(message.clone()),
        );
        if message.starts_with("TLS protocol truncation:")
            || message.starts_with("TLS protocol error:")
        {
...
fn jet_net_dns_result<T>(result: Result<T, String>, name: &str) -> Result<T, JetNetError> {
    result.map_err(|message| {
        if message.starts_with("DNS name not found") {
            ...
            JetNetError::Cancelled(jet_net_detail(
                "dns",
                None,
                Some(name.to_string()),
                message,
            ...
            JetNetError::Protocol(jet_net_detail(
                "dns",
                None,
                Some(name.to_string()),
                message,
...
fn jet_net_error_message(error: &JetNetError) -> String {
    error.jet_show()
}
...
fn jet_net_explicit_deadline(
    deadline: &jet_std::Duration,
    operation: &str,
) -> Result<Option<JetDeadlineGuard>, JetNetError> {
        ...
        match receiver.try_recv() {
            ...
            Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                return Err(JetNetError::Other(jet_net_detail(
                    ...
                    address,
                    None,
                    "tcp connect worker stopped without a result".to_string(),
                    None,
                )));
            }
        }
...
fn jet_net_connect_addr_worker(addr: std::net::SocketAddr) -> Result<JetTCPStream, JetNetError> {
    ...
    let timeout =
        jet_deadline_remaining_ms().map(|ms| std::time::Duration::from_millis(ms.max(1) as u64));
...
fn jet_net_unix_listener_scheduler_wait(
    ...
    match jet_scheduler_wait_without_unwind(|| {
        ...
        JetSchedulerWait::Ready(()) if matches!(jet_deadline_remaining_ms(), Some(ms) if ms <= 0) => {
...
fn jet_net_tls_client_scheduler_config_deadline(
    ...
    let empty = Vec::new();
    let (cert_chain, private_key) = config
        .identity
        .as_ref()
...
fn jet_net_tls_read_bytes(
    ...
    loop {
        match jet_net_tls_io_result(
            (stream.read_step)(stream.id, limit),
            jet_std::IOOperation::Read,
        )? {
...
fn jet_net_tls_read_bytes_deadline(
    stream: &mut JetTLSStream,
    limit: i64,
    deadline: &jet_std::Duration,
) -> Result<Vec<u8>, jet_std::IOError> {
...
fn jet_net_tls_write_bytes_with_current_deadline(
    ...
    loop {
        match jet_net_tls_io_result(
            (stream.write_step)(stream.id, data),
            jet_std::IOOperation::Write,
        )? {
            Some(count) => return Ok(count),
            None => {
                jet_net_tls_io_scheduler_wait(stream, false, true, jet_std::IOOperation::Write)?
            }
        }
    }
}

fn jet_net_tls_write_bytes(
    stream: &mut JetTLSStream,
    data: &Vec<u8>,
) -> Result<i64, jet_std::IOError> {
...
fn jet_net_tls_write_all_bytes(
    stream: &mut JetTLSStream,
    data: &Vec<u8>,
) -> Result<(), jet_std::IOError> {
    let _deadline = jet_net_operation_deadline(stream.write_timeout_ms);
...
fn jet_net_tls_write_all_bytes_deadline(
    stream: &mut JetTLSStream,
    data: &Vec<u8>,
    deadline: &jet_std::Duration,
) -> Result<(), jet_std::IOError> {
    let _deadline =
        jet_net_explicit_deadline(deadline, "tls write all").map_err(jet_net_to_io_error)?;
    jet_net_tls_write_all_bytes(stream, data)
}

fn jet_net_tls_write_text(
    stream: &mut JetTLSStream,
    text: &String,
) -> Result<(), jet_std::IOError> {
...
fn jet_net_tls_close_write(
    ...
    let _deadline =
        jet_net_explicit_deadline(deadline, "tls close write").map_err(jet_net_to_io_error)?;
    loop {
        if jet_net_tls_io_result(
            (stream.close_write_step)(stream.id),
            jet_std::IOOperation::Close,
        )? {
            return Ok(());
        }
...
fn jet_net_tls_peer_identity_from_snapshot(
    ...
    let leaf = certificate_chain.first().cloned().ok_or_else(|| {
        JetNetError::TLS(jet_net_detail(
            "tls peer identity",
            None,
            None,
            "verified TLS peer chain is empty".to_string(),
            None,
        ))
    })?;
...
fn jet_net_tls_peer_identity(stream: &JetTLSStream) -> JetTLSPeerIdentity {
    stream
        .peer_identity
        .clone()
        .expect("verified TLS stream always retains peer identity")
}
...
fn jet_net_tls_ready(
    ...
    let _deadline =
        jet_net_explicit_deadline(deadline, "tls ready").map_err(jet_net_to_io_error)?;
    if matches!(jet_deadline_remaining_ms(), Some(ms) if ms <= 0) {
        return Err(jet_net_to_io_error(jet_net_deadline_timeout("tls ready")));
    }
    let want_read = matches!(
        interest,
        JetNetReadyInterest::Read | JetNetReadyInterest::ReadWrite
    );
    let want_write = matches!(
        interest,
        JetNetReadyInterest::Write | JetNetReadyInterest::ReadWrite
    );
    loop {
        let readable =
            want_read && jet_net_tls_io_result((stream.read_ready)(stream.id), operation)?;
        if readable {
            return Ok(JetNetReady {
                readable: true,
                writable: false,
            });
        }
    ...
    jet_net_pure_parse_ip(text)
        ...
        .map_err(|e| {
            JetNetError::InvalidInput(jet_net_detail(
                ...
                Some(text.clone()),
                None,
                format!("invalid IP address `{}`: {}", text, e),
                None,
            ))
        })
}
...
fn jet_net_socket_addr(host: &String, port: i64) -> Result<JetSocketAddr, JetNetError> {
    if port < 0 || port > u16::MAX as i64 {
        return Err(JetNetError::InvalidInput(jet_net_detail(
            ...
            Some(host.clone()),
            None,
            format!("invalid port `{}`: expected 0..65535", port),
            None,
        )));
    }
    ...
    let resolved = match jet_net_pure_parse_socket_addr(&text) {
        ...
        Err(_) => {
            ...
            match text.to_socket_addrs() {
                Ok(iter) => iter,
                Err(e) => return Err(jet_net_io_error("resolve socket address", Some(text), e)),
            }
            .next()
    ...
    jet_net_pure_parse_socket_addr(text)
        ...
        .map_err(|e| {
            JetNetError::InvalidInput(jet_net_detail(
                ...
                Some(text.clone()),
                None,
                format!("invalid socket address `{}`: {}", text, e),
                None,
            ))
        })
}
...
fn jet_net_tcp_connect_timeout(addr: &JetSocketAddr, ms: i64) -> Result<JetTCPStream, JetNetError> {
    jet_scheduler_world_reject_uncontrolled("network");
    jet_net_timeout(ms).map_err(|m| {
        JetNetError::InvalidInput(jet_net_detail(
            "tcp connect",
            Some(addr.inner.to_string()),
            None,
            m,
            None,
        ))
...
fn jet_net_tcp_connect_happy(
        ...
        match resolve_receiver.try_recv() {
            ...
            Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                return Err(JetNetError::Other(jet_net_detail(
                    ...
                    Some(host.clone()),
                    None,
                    "socket address resolver stopped without a result".to_string(),
                    None,
                )));
            }
        }
    ...
    if addrs.is_empty() {
        return Err(JetNetError::AddressUnavailable(jet_net_detail(
            "tcp connect",
            Some(host.clone()),
            None,
            format!("tcp connect failed: no address for `{}`", host),
            None,
        )));
    }
    ...
    loop {
        if next < ordered.len() && std::time::Instant::now() >= next_launch {
            ...
            std::thread::spawn(move || {
                ...
                let _ = attempt_sender
                    .send((addr, std::net::TcpStream::connect_timeout(&addr, timeout)));
            });
            next += 1;
            in_flight += 1;
        ...
        match receiver.try_recv() {
            ...
            Err(std::sync::mpsc::TryRecvError::Empty) => {}
            Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                unreachable!("connect sender retained")
            }
        }
...
fn jet_net_listener_local_socket_addr(
    listener: &JetTCPListener,
) -> Result<JetSocketAddr, JetNetError> {
    listener
        .inner
        .local_addr()
        .map(|inner| JetSocketAddr { inner })
...
fn jet_net_tcp_accept_deadline(
    listener: &JetTCPListener,
    deadline: &jet_std::Duration,
) -> Result<JetTCPStream, JetNetError> {
...
fn jet_net_tcp_read_bytes_deadline(
    stream: &mut JetTCPStream,
    limit: i64,
    deadline: &jet_std::Duration,
) -> Result<Vec<u8>, JetNetError> {
    let deadline_ms = deadline.as_millis();
    jet_net_timeout(deadline_ms).map_err(|message| {
...
fn jet_net_tcp_read_text_deadline(
    stream: &mut JetTCPStream,
    limit: i64,
    deadline: &jet_std::Duration,
) -> Result<String, JetNetError> {
    ...
    String::from_utf8(bytes).map_err(|error| {
        JetNetError::InvalidInput(jet_net_detail(
            ...
            None,
            None,
            format!("tcp read text failed: {}", error),
            None,
        ))
    })
}
...
fn jet_net_tcp_write_bytes_deadline(
    stream: &mut JetTCPStream,
    data: &Vec<u8>,
    deadline: &jet_std::Duration,
) -> Result<i64, JetNetError> {
...
fn jet_net_tcp_write_all_bytes(
    stream: &mut JetTCPStream,
    data: &Vec<u8>,
) -> Result<(), JetNetError> {
    let _deadline = jet_net_operation_deadline(stream.write_timeout_ms);
    ...
    while offset < data.len() {
        let wrote =
            jet_net_tcp_write_bytes_with_current_deadline(stream, &data[offset..])? as usize;
        if wrote == 0 {
...
fn jet_net_tcp_write_all_bytes_deadline(
    stream: &mut JetTCPStream,
    data: &Vec<u8>,
    deadline: &jet_std::Duration,
) -> Result<(), JetNetError> {
...
fn jet_net_tcp_write_text_deadline(
    stream: &mut JetTCPStream,
    text: &String,
    deadline: &jet_std::Duration,
) -> Result<(), JetNetError> {
...
fn jet_net_tcp_shutdown(stream: &mut JetTCPStream, how: JetNetShutdown) -> Result<(), JetNetError> {
    ...
    stream
        .inner
        .shutdown(shutdown)
        .map_err(|e| jet_net_io_error("tcp shutdown", None, e))?;
    match how {
...
fn jet_net_set_timeout(stream: &mut JetTCPStream, ms: i64) -> Result<(), JetNetError> {
    let _ = jet_net_timeout(ms).map_err(|m| {
        JetNetError::InvalidInput(jet_net_detail("set tcp timeout", None, None, m, None))
    })?;
...
fn jet_net_set_read_timeout(stream: &mut JetTCPStream, ms: i64) -> Result<(), JetNetError> {
    let _ = jet_net_timeout(ms).map_err(|m| {
        JetNetError::InvalidInput(jet_net_detail("set tcp read timeout", None, None, m, None))
    })?;
    stream.read_timeout_ms = Some(ms);
    Ok(())
}
...
fn jet_net_set_write_timeout(stream: &mut JetTCPStream, ms: i64) -> Result<(), JetNetError> {
    let _ = jet_net_timeout(ms).map_err(|m| {
        JetNetError::InvalidInput(jet_net_detail("set tcp write timeout", None, None, m, None))
    })?;
    stream.write_timeout_ms = Some(ms);
    Ok(())
}
...
fn jet_net_sendfile(stream: &mut JetTCPStream, path: &String) -> Result<i64, JetNetError> {
    ...
    let mut file = std::fs::File::open(path.as_str())
        .map_err(|e| jet_net_io_error("tcp sendfile open", Some(path.clone()), e))?;
    let _deadline = jet_net_operation_deadline(stream.write_timeout_ms);
    let mut written: u64 = 0;
    let mut buf = [0u8; 8192];
    loop {
        let n = std::io::Read::read(&mut file, &mut buf)
            .map_err(|e| jet_net_io_error("tcp sendfile read", Some(path.clone()), e))?;
        if n == 0 {
            break;
        }
...
fn jet_net_udp_set_timeout(socket: &JetUDPSocket, ms: i64) -> Result<(), JetNetError> {
    ...
    let _ = jet_net_timeout(ms).map_err(|m| {
        JetNetError::InvalidInput(jet_net_detail("set udp timeout", None, None, m, None))
    })?;
...
fn jet_net_udp_recv_from(socket: &JetUDPSocket, limit: i64) -> Result<JetUDPPacket, JetNetError> {
    ...
    if limit <= 0 {
        return Err(JetNetError::InvalidInput(jet_net_detail(
            ...
            None,
            None,
            "udp receive limit must be positive".to_string(),
            None,
        )));
    }
...
fn jet_net_udp_send_slice(
    ...
    loop {
        match socket.inner.send_to(data, addr.inner) {
            ...
            Ok(n) => {
                return Err(JetNetError::Protocol(jet_net_detail(
                    "udp send",
                    Some(addr.inner.to_string()),
                    None,
                    format!("udp send wrote {} of {} datagram bytes", n, data.len()),
                    None,
                )))
            }
            Err(error)
...
fn jet_net_udp_receive(socket: &JetUDPSocket, limit: i64) -> Result<JetUDPPacket, JetNetError> {
    ...
    if limit < 0 {
        return Err(JetNetError::InvalidInput(jet_net_detail(
            ...
            None,
            None,
            "udp receive limit must be non-negative".to_string(),
            None,
        )));
    }
    let _deadline = jet_net_operation_deadline(*socket.timeout_ms.lock().unwrap());
...
fn jet_net_udp_ready(
    ...
    return Err(JetNetError::Unsupported(jet_net_detail(
        ...
        None,
        None,
        "udp readiness backend is unavailable on this target".to_string(),
        None,
    )));
    #[cfg(unix)]
    {
...
#[cfg(unix)]
fn jet_net_unix_accept_deadline(
    listener: &JetUnixListener,
    deadline: &jet_std::Duration,
) -> Result<JetUnixStream, JetNetError> {
...
#[cfg(not(unix))]
fn jet_net_unix_accept_deadline(
    _listener: &JetUnixListener,
    _deadline: &jet_std::Duration,
) -> Result<JetUnixStream, JetNetError> {
    Err(JetNetError::Unsupported(jet_net_detail(
        "unix accept",
        None,
        ...
        match receiver.try_recv() {
            ...
            Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                return Err(JetNetError::Other(jet_net_detail(
                    ...
                    Some(address),
                    None,
                    "unix connect worker stopped without a result".to_string(),
                    None,
                )))
            }
        }
...
fn jet_net_unix_connect_deadline(
    path: &String,
    deadline: &jet_std::Duration,
) -> Result<JetUnixStream, JetNetError> {
...
fn jet_net_unix_read(stream: &mut JetUnixStream) -> Result<String, JetNetError> {
    ...
    String::from_utf8(bytes).map_err(|e| {
        JetNetError::InvalidInput(jet_net_detail(
            ...
            None,
            None,
            format!("unix read text failed: invalid UTF-8: {}", e),
            None,
        ))
    })
}
...
fn jet_net_unix_read_bytes(stream: &mut JetUnixStream, limit: i64) -> Result<Vec<u8>, JetNetError> {
    ...
    if limit <= 0 {
        return Err(JetNetError::InvalidInput(jet_net_detail(
            ...
            None,
            None,
            "unix read limit must be positive".to_string(),
            None,
        )));
    }
    let _deadline = jet_net_operation_deadline(stream.read_timeout_ms);
...
#[cfg(unix)]
fn jet_net_unix_read_bytes_deadline(
    stream: &mut JetUnixStream,
    limit: i64,
    deadline: &jet_std::Duration,
) -> Result<Vec<u8>, JetNetError> {
...
#[cfg(unix)]
fn jet_net_unix_write_all_bytes(
    stream: &mut JetUnixStream,
    data: &Vec<u8>,
) -> Result<(), JetNetError> {
    let _deadline = jet_net_operation_deadline(stream.write_timeout_ms);
    ...
    while offset < data.len() {
        ...
        if wrote == 0 {
            return Err(JetNetError::ConnectionReset(jet_net_detail(
                ...
                None,
                None,
                "unix write all failed: zero bytes written".to_string(),
                None,
            )));
        }
        offset += wrote;
...
#[cfg(unix)]
fn jet_net_unix_write_all_bytes_deadline(
    stream: &mut JetUnixStream,
    data: &Vec<u8>,
    deadline: &jet_std::Duration,
) -> Result<(), JetNetError> {
...
fn jet_net_unix_write_slice(stream: &mut JetUnixStream, data: &[u8]) -> Result<i64, JetNetError> {
    use std::io::Write;
    if stream.closed || stream.write_shutdown {
        return Err(jet_net_closed("unix write"));
    }
    loop {
        match stream.inner.write(data) {
...
#[cfg(unix)]
fn jet_net_unix_shutdown(
    stream: &mut JetUnixStream,
    how: JetNetShutdown,
) -> Result<(), JetNetError> {
    if stream.closed {
        return Err(jet_net_closed("unix shutdown"));
    }
    ...
    stream
        .inner
        .shutdown(os_how)
        .map_err(|error| jet_net_io_error("unix shutdown", None, error))?;
    match how {
        JetNetShutdown::Read => stream.read_shutdown = true,
        JetNetShutdown::Write => stream.write_shutdown = true,
        JetNetShutdown::Both => {
...
#[cfg(unix)]
fn jet_net_unix_set_timeout(
    stream: &mut JetUnixStream,
    timeout: &jet_std::Duration,
) -> Result<(), JetNetError> {
    jet_net_timeout(timeout.as_millis()).map_err(|message| {
        JetNetError::InvalidInput(jet_net_detail(
            "set unix timeout",
            None,
            None,
...
fn jet_net_unix_ready(
    ...
    if stream.closed {
        return Err(jet_net_closed("unix ready"));
    }
...
#[cfg(not(unix))]
fn jet_net_unix_ready(
    _stream: &JetUnixStream,
    _interest: JetNetReadyInterest,
    _deadline: &jet_std::Duration,
) -> Result<JetNetReady, JetNetError> {
    Err(JetNetError::Unsupported(jet_net_detail(
        "unix ready",
...
fn jet_net_dns_system_servers() -> Vec<String> {
    ...
    if let Ok(output) = std::process::Command::new("scutil").arg("--dns").output() {
        out.extend(jet_net_dns_parse_scutil(&String::from_utf8_lossy(
            &output.stdout,
        )));
    }
    ...
    if let Ok(output) = std::process::Command::new("powershell.exe")
        ...
        .output()
    {
        out.extend(jet_net_dns_parse_windows_addresses(
...
fn jet_net_dns_encode_name(out: &mut Vec<u8>, name: &str) -> Result<(), String> {
    ...
    if out.len() > 255 {
        return Err(format!(
            "invalid DNS name `{}`: encoded name exceeds 255 bytes",
            name
        ));
    }
    Ok(())
...
fn jet_net_dns_read_name(packet: &[u8], pos: &mut usize) -> Result<String, String> {
    ...
    loop {
        ...
        if len & 0xc0 == 0xc0 {
            ...
            if ptr >= packet.len() {
                return Err(
                    "network protocol error: DNS compression pointer is out of bounds".to_string(),
                );
            }
            if ptr >= p {
                return Err(
                    "network protocol error: DNS compression pointer does not point backward"
                        .to_string(),
                );
            }
            if !jumped {
...
fn jet_net_dns_parse_response(
    ...
    let minimum_record_bytes = total_records.checked_mul(11).ok_or_else(|| {
        "network protocol error: DNS record count overflows packet bounds".to_string()
    })?;
...
fn jet_net_dns_remaining(
    deadline: std::time::Instant,
    name: &str,
) -> Result<std::time::Duration, String> {
    if jet_net_dns_cancelled() {
        return Err(format!(
            "network operation cancelled during DNS lookup for `{}`",
            name
        ));
    }
...
fn jet_net_dns_tcp_exchange(
    ...
    let mut stream =
        std::net::TcpStream::connect_timeout(&server_addr, jet_net_dns_remaining(deadline, name)?)
            .map_err(|e| format!("DNS TCP connect to `{}` failed: {}", server_addr, e))?;
    let len = u16::try_from(request.len())
        .map_err(|_| "network protocol error: DNS request exceeds TCP frame".to_string())?;
    ...
    while written < framed.len() {
        stream
            .set_write_timeout(Some(jet_net_dns_io_slice(deadline, name)?))
            .map_err(|e| format!("DNS TCP timeout setup failed: {}", e))?;
        match stream.write(&framed[written..]) {
            Ok(0) => {
                return Err(
                    "network protocol error: DNS TCP connection closed while sending query"
                        .to_string(),
                )
            }
            Ok(n) => written += n,
    ...
    while prefix_read < prefix.len() {
        stream
            .set_read_timeout(Some(jet_net_dns_io_slice(deadline, name)?))
            .map_err(|e| format!("DNS TCP timeout setup failed: {}", e))?;
        match stream.read(&mut prefix[prefix_read..]) {
            Ok(0) => {
                return Err(
                    "network protocol error: DNS TCP connection closed before response length"
                        .to_string(),
                )
            }
            Ok(n) => prefix_read += n,
...
fn jet_net_dns_query(
    server: &String,
    name: &String,
    qtype: u16,
    ms: i64,
) -> Result<Vec<JetDNSWireRecord>, String> {
    jet_scheduler_world_reject_uncontrolled("network");
    ...
    socket
        .connect(server_addr)
        .map_err(|e| format!("DNS server `{}` is unreachable: {}", server, e))?;
    let mut req = Vec::new();
...
fn jet_net_dns_matching_records(
    records: &[JetDNSWireRecord],
    name: &str,
    qtype: u16,
) -> Result<Vec<JetDNSWireRecord>, String> {
    ...
    for _ in 0..16 {
        ...
        let found: Vec<_> = records
            .iter()
            .filter(|r| r.class == 1 && r.ty == qtype && r.owner.eq_ignore_ascii_case(&current))
            .cloned()
            .collect();
        if !found.is_empty() {
            return Ok(found);
        ...
        let alias = records
            .iter()
            .find(|r| r.class == 1 && r.ty == 5 && r.owner.eq_ignore_ascii_case(&current));
        match alias {
            Some(record) => {
                current = jet_net_dns_record_name(record)?
                    .trim_end_matches('.')
                    .to_string()
            }
...
fn jet_net_dns_system_lookup(name: &String, ms: i64) -> Result<Vec<std::net::SocketAddr>, String> {
    ...
    loop {
        ...
        match rx.recv_timeout(slice) {
            Ok(result) => {
                return result.map_err(|e| format!("DNS lookup for `{}` failed: {}", name, e))
            }
...
fn jet_net_dns_a_at(server: &String, name: &String, ms: i64) -> Result<Vec<JetIpAddr>, String> {
    Ok(
        jet_net_dns_matching_records(&jet_net_dns_query(server, name, 1, ms)?, name, 1)?
            .into_iter()
            .map(|r| {
                let data = &r.packet[r.rdata_start..r.rdata_start + r.rdata_len];
                if data.len() != 4 {
                    return Err("network protocol error: DNS A record is not 4 bytes".to_string());
                }
                Ok(JetIpAddr {
                    inner: std::net::IpAddr::V4(std::net::Ipv4Addr::new(
...
fn jet_net_dns_aaaa_at(server: &String, name: &String, ms: i64) -> Result<Vec<JetIpAddr>, String> {
    Ok(
        jet_net_dns_matching_records(&jet_net_dns_query(server, name, 28, ms)?, name, 28)?
            .into_iter()
            .map(|r| {
                let data = &r.packet[r.rdata_start..r.rdata_start + r.rdata_len];
                if data.len() != 16 {
                    return Err(
                        "network protocol error: DNS AAAA record is not 16 bytes".to_string()
                    );
                }
                let mut b = [0u8; 16];
...
fn jet_net_dns_txt_at(server: &String, name: &String, ms: i64) -> Result<Vec<String>, String> {
    ...
    for r in jet_net_dns_matching_records(&records, name, 16)? {
        ...
        while p < data.len() {
            ...
            let part = std::str::from_utf8(&data[p..p + len]).map_err(|_| {
                "network protocol error: DNS TXT record is not valid UTF-8".to_string()
            })?;
            s.push_str(part);
            p += len;
        }
...
fn jet_net_tcp_reply(
    mut stream: JetTCPStream,
    status: &String,
    body: &String,
) -> Result<(), JetNetError> {
    use std::io::Write;
