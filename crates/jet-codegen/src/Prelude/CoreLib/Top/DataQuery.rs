...
// wrappers preserve the original row identity through the selected indices.
//
...
pub fn jet_data_query_indices(
    ...
    let mut selected = rows
        ...
        .filter(|(_, row)| {
            ...
            jet_std::datatree_get(row, field)
    ...
    if let Some((field, descending)) = &spec.order {
        selected.sort_by(|left, right| {
            let left = jet_std::datatree_get(&rows[*left], field)
                .map(jet_analytics_cell_text)
                .unwrap_or_default();
            let right = jet_std::datatree_get(&rows[*right], field)
                .map(jet_analytics_cell_text)
                .unwrap_or_default();
...
/// Shared non-generic Prelude entry used by resident adapters.
