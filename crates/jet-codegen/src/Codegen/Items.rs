...
/// D-QUANTITY-CONVERT1=B: real destination-owned exact and rounded unit
/// conversions. Sema validates family identity and the closed rounding mode;
/// these methods own the single runtime exactness check.
pub(crate) fn emit_unit_conversions(family: &crate::AST::UnitFamilyDef, out: &mut String) {
    let defs = family.distinct_defs();
    let affine = family.base.is_some()
        && family
            .members
            .iter()
            .any(|member| member.offset != crate::AST::UnitRatio::zero());
    let member_for = |name: &str, kind: crate::AST::QuantityKind| {
    ...
    let rust_ratio = |ratio: &crate::AST::UnitRatio| {
        let value = ratio.num.to_string().parse::<f64>().unwrap_or_else(|_| {
            if ratio.num.is_negative() { f64::NEG_INFINITY } else { f64::INFINITY }
    ...
    for destination in &defs {
        let Some((_, destination_kind)) = destination.quantity else { continue };
        let Some(destination_member) = member_for(&destination.name, destination_kind) else { continue };
        let destination_offset = if affine && destination_kind == crate::AST::QuantityKind::Point {
            destination_member.offset.clone()
        } else {
            crate::AST::UnitRatio::zero()
        };
        for source in &defs {
            let Some((_, source_kind)) = source.quantity else { continue };
            if source.name == destination.name || source_kind != destination_kind {
                continue;
            }
            let Some(source_member) = member_for(&source.name, source_kind) else { continue };
            let source_offset = if affine && source_kind == crate::AST::QuantityKind::Point {
                source_member.offset.clone()
            } else {
                crate::AST::UnitRatio::zero()
            };
            let scale = source_member.scale.div(&destination_member.scale)
                .expect("validated unit scale");
            let offset = source_offset.sub(&destination_offset)
                .and_then(|value| value.div(&destination_member.scale))
                .expect("validated unit offset");
            let method = crate::Syntax::conversion_method_for_source(&source.name);
            out.push_str(&format!(
                "impl user_{destination} {{\n    pub fn user_{method}(value: user_{source}) -> Result<user_{destination}, String> {{\n        let converted = value.0 * {scale} + {offset};\n        if converted.is_finite() && converted.fract() == 0.0 {{ Ok(user_{destination}(converted)) }} else {{ Err(\"unit conversion would round\".to_string()) }}\n    }}\n    pub fn user_{method}_rounded(value: user_{source}) -> user_{destination} {{\n        let converted = value.0 * {scale} + {offset};\n        user_{destination}(converted.round_ties_even())\n    }}\n}}\n\n",
                destination = destination.name,
                source = source.name,
                scale = rust_ratio(&scale),
                offset = rust_ratio(&offset),
            ));
        }
    }
}
