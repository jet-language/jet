...
/// c109 Phase 21: a `Task<T>`/`Receiver<T>`/`Sender<T>` element type. Any covered value
...
pub(crate) fn is_covered_foreign_value_ty(ty: &Type, cx: &Cx) -> bool {
    ...
    is_prelude_struct_name(name)
        ...
        || root_prelude_rust_type_name(name).is_some()
        || service_handle_rust_type(name).is_some()
