...
fn prelude_compound_call(
    ...
    Some(match op {
        ...
        BinOp::Sub if matches!(ty, Type::Named(name) if name == crate::Syntax::TYPE_DECIMAL) => {
            format!("{root_prefix}jet_decimal_sub(&({place}), &({value}))")
        }
        BinOp::Mul if matches!(ty, Type::Named(name) if name == crate::Syntax::TYPE_DECIMAL) => {
...
fn emit_sparse_branch_tree(
    ...
    let (equals, _) = coverage_condition(jet_format!("*{jet_prefix}switch_subject == {pivot}"), cx);
    out.push_str(&format!("{pad}if {equals} {{\n"));
    emit_tir_stmts_nested(body, cx, out, indent + 1, active_cleanups);
    let middle = arms.len() / 2;
    if middle > 0 {
        let (less_than, _) =
            coverage_condition(jet_format!("*{jet_prefix}switch_subject < {pivot}"), cx);
        ...
        match cleanup {
            ActiveCleanup::Deferred(id) => {
                out.push_str(&jet_format!(
                    "{}{jet_prefix}deferred_close_{}.run();\n",
                    pad,
                    id
                ));
            }
...
fn emit_expr_with_cleanups(
    ...
    for active in cleanups.iter().rev() {
        match active {
            ActiveCleanup::Deferred(id) => {
                cleanup.push_str(&jet_format!("{jet_prefix}deferred_close_{}.run(); ", id));
            }
...
pub(crate) fn emit_tir_lambda_block(stmts: &[TStmt], cx: &Cx, out: &mut String, indent: usize) {
    ...
    match s {
        ...
        TStmt::Let {
            ...
            if let Some(bytes) = fixed_bytes {
                let backing = mangle_generated(&format!("fixed_backing_{}", name));
                out.push_str(&format!(
            ...
            if let Some(promotion) = gc_promotion {
                ...
                let value = jet_format!("{jet_prefix}gc_value_{local}");
                let site = jet_format!("{jet_prefix}gc_site_{local}");
                out.push_str(&format!(
                    "{}let {} = {};\n",
                    pad,
                    value,
        ...
        TStmt::RefutableBind {
            ...
            emit_tir_stmts_nested(fallback, cx, out, indent + 1, &mut fallback_cleanups);
            out.push_str(&format!("{}}};\n", pad));
        }
        TStmt::GcEdit {
            root,
            ...
            if *replace_all {
                out.push_str(&jet_format!(
                    "{}jet_gc::runtime_or_exit({}.edit_replacing_all_edges(&[{}], |{jet_prefix}value| {{\n",
                    pad,
                    root,
                    edges.join(", ")
                ));
                ...
                out.push_str(&jet_format!(
                    "{}jet_gc::runtime_or_exit({}.edit_edge_slot_index(\"collection\", {} as usize, &[{}], |{jet_prefix}value| {{\n",
                    pad,
                    root,
                    temp,
                    edges.join(", ")
                ...
                out.push_str(&jet_format!(
                    "{}jet_gc::runtime_or_exit({}.edit_edge_slot({:?}, &[{}], |{jet_prefix}value| {{\n",
                    pad,
                    root,
                    slot,
                    edges.join(", ")
        ...
        TStmt::Assign {
            ...
            if let TPlace::Expr(place_expr) = place {
                if let crate::Codegen::TIR::TExprKind::Deref(pointer) = &place_expr.kind {
                    ...
                    let next = match op {
                        ...
                        .unwrap_or_else(|| {
                            format!("({read}) {} {v}", op.rust_spell().expect(PRELUDE_CARRIED))
                        }),
                        None => v,
                    };
                    out.push_str(&format!(
            ...
            if let TPlace::Local(local) = place {
                ...
                if local.uninit_fixed {
                    ...
                    debug_assert!(
                        op.is_none(),
                        "fixed-list compound assignment is unsupported"
                    );
        ...
        TStmt::DeferClose {
            ...
            out.push_str(&jet_format!(
                "{}let mut {jet_prefix}deferred_close_{} = JetDeferredClose::new(move || {{ let _ = {}; }});\n",
                pad,
                id,
        ...
        TStmt::CountedLoop {
            ...
            if step.is_some() {
                out.push_str(&jet_format!(
                    "{}let mut {jet_prefix}loop_first = true;\n",
                    inner_pad
                ));
            }
            out.push_str(&format!(
            ...
            if let Some(step) = step {
                out.push_str(&jet_format!(
                    "{}if {jet_prefix}loop_first {{ {jet_prefix}loop_first = false; }} else {{\n",
                    body_pad
                ));
        ...
        TStmt::Range {
            ...
            if let Some(source) = source {
                ...
                let stride = step
                    .as_ref()
                    .map(|step| emit_expr_with_cleanups(step, cx, active_deferred_closes));
                if let Some(stride) = &stride {
                    out.push_str(&jet_format!(
                        "{}let {jet_prefix}loop_stride = {};\n",
                        range_pad,
                        stride
                    ));
                    out.push_str(&jet_format!(
                ...
                for (condition, op) in [(true, ".."), (false, "..=")] {
                    out.push_str(&jet_format!(
                        "{}{} {jet_prefix}range.exclusive {{\n",
                        range_pad,
                        if condition { "if" } else { "} else if !" }
                    ));
                    let step_suffix = if stride.is_some() {
                        jet_format!(".step_by({jet_prefix}loop_stride as usize)")
                    } else {
                        String::new()
                    };
                    out.push_str(&jet_format!(
                        "{}{}for {} in ({jet_prefix}range.start{op}{jet_prefix}range.end){step_suffix} {{\n",
                        body_pad,
                        lbl,
                        mangle(var)
                    ));
        ...
        TStmt::Break(label) => match label {
            Some(name) => out.push_str(&format!("{}break '{};\n", pad, mangle(name))),
        ...
        TStmt::BreakValue { label, value } => {
            ...
            match label {
                Some(name) => out.push_str(&format!("{}break '{} {};\n", pad, mangle(name), value)),
                None => out.push_str(&format!("{}break {};\n", pad, value)),
            }
        }
        TStmt::Continue(label) => match label {
            Some(name) => out.push_str(&format!("{}continue '{};\n", pad, mangle(name))),
        ...
        TStmt::EnumMatch {
            ...
            for arm in arms {
                ...
                emit_tir_stmts_nested(&arm.body, cx, out, indent + 2, active_deferred_closes);
                out.push_str(&format!("{}    }}\n", pad));
            }
            match else_body {
                Some(body) => {
        ...
        TStmt::RangeSwitch {
            ...
            out.push_str(&format!("{}}} else {{\n", inner_pad));
            emit_tir_stmts_nested(else_body, cx, out, indent + 2, active_deferred_closes);
            out.push_str(&format!("{}}}\n", inner_pad));
            out.push_str(&format!("{}}}\n", pad));
        }
        ...
        TStmt::IndexAssign {
            ...
            if *is_map {
                ...
                out.push_str(&jet_format!(
                    "{pad}{{ let {jet_prefix}v = {v}; jet_compute_set(&mut *({b}), &[({i})], {jet_prefix}v).unwrap_or_else(|{jet_prefix}error| jet_panic({:?}, {}, &{jet_prefix}error.jet_show())); }}\n",
                    cx.file, 0
                ));
        ...
        TStmt::IndexFieldAssign(assign) => {
            ...
            let mutate = |place: &str| -> String {
                match assign.op.and_then(|op| {
                    ...
                    Some(call) => {
                        jet_format!("let {jet_prefix}t = &mut {place}; *{jet_prefix}t = {call};")
                    }
                    None => {
                        let operator = assign.op.map_or("=".to_string(), |op| {
                            format!("{}=", op.rust_spell().expect(PRELUDE_CARRIED))
                        });
                        jet_format!("{place} {operator} {jet_prefix}v;")
                    }
                }
            };
            ...
            if assign.is_map {
                out.push_str(&jet_format!(
                    "{pad}{{ let {jet_prefix}v = {v}; let {jet_prefix}k = ({i}).clone(); \
                     let Some({jet_prefix}item) = ({b}).get_mut(&{jet_prefix}k) else {{ \
                     jet_panic({file:?}, {line}, \"map key not found\"); }}; \
                     {mutation} }}\n",
                    file = cx.file,
                    line = assign.line,
                    mutation = mutate(&jet_format!("{jet_prefix}item.{field}")),
                ));
            } else {
                ...
                out.push_str(&jet_format!(
                    "{pad}{{ let {jet_prefix}v = {v}; {mutation} }}\n",
        ...
        TStmt::IndexHookAssign {
            ...
            let v = emit_expr_with_cleanups(value, cx, active_deferred_closes);
            out.push_str(&jet_format!(
                "{pad}{{ let {jet_prefix}v = {v}; <{ty} as __jet_IndexMut>::set(&mut ({b}), {i}, {jet_prefix}v); }}\n",
            ));
        }
        ...
        TStmt::ForIn {
            ...
            match method_kind {
                Some(TForInMethod::Chars) => {
                    out.push_str(&jet_format!(
                        "{}{}for {jet_prefix}c in ({recv}).chars(){} {{\n    {}let {} = {jet_prefix}c;\n",
                        pad,
                        lbl,
                        stride_suffix,
                        pad,
                ...
                Some(TForInMethod::LinesFile) => {
                    out.push_str(&jet_format!(
                        "{}{}for {jet_prefix}raw_line in std::io::BufRead::lines(&mut ({}).inner){} {{\n",
                        pad, lbl, collection_str, stride_suffix
                    ));
                    out.push_str(&jet_format!(
                        "{}    let {} = {jet_prefix}raw_line.unwrap_or_else(|_e| {}jet_panic({:?}, {}, &_e.to_string()));\n",
                        pad,
                        mangle(var),
                        cx.root_prefix,
                        cx.file,
                ...
                Some(TForInMethod::LinesStdin) => {
                    out.push_str(&jet_format!(
                        "{}{{ let mut {jet_prefix}stdin_h = {};\n",
                        pad,
                        collection_str
                    ));
                    needs_extra_close = true;
                    out.push_str(&jet_format!(
                        "{}{}for {jet_prefix}raw_line in std::io::BufRead::lines(&mut {jet_prefix}stdin_h.inner){} {{\n",
                ...
                Some(TForInMethod::LinesProcessStream) => {
                    ...
                    out.push_str(&jet_format!(
                        "{}{}for {jet_prefix}line_result in std::iter::from_fn(|| {}jet_process_stream_next_line(&({})).transpose()){} {{\n",
                        pad,
                        lbl,
                        cx.root_prefix,
                        collection_str,
                    ...
                    out.push_str(&jet_format!(
                        "{}    let {} = {jet_prefix}line_result.unwrap_or_else(|_e| {}jet_panic({:?}, {}, &format!(\"{{:?}}\", _e)));\n",
                        pad,
                        mangle(var),
                        cx.root_prefix,
                        cx.file,
                ...
                Some(TForInMethod::EncodingReader { reader_type }) => {
                    ...
                    out.push_str(&jet_format!(
                        "{}{}for {jet_prefix}item_result in {}jet_encoding_reader_iter(|| {}{}(&mut ({}))){} {{\n",
                        pad,
                        lbl,
                        cx.root_prefix,
                        cx.root_prefix,
                    ...
                    out.push_str(&jet_format!(
                        "{}    let {} = {jet_prefix}item_result.unwrap_or_else(|_e| {}jet_panic({:?}, {}, &format!(\"{{:?}}\", _e)));\n",
                        pad,
                        mangle(var),
                        cx.root_prefix,
                        cx.file,
                ...
                Some(TForInMethod::Iterable {
                    ...
                    out.push_str(&jet_format!(
                        "{}{{ let mut {jet_prefix}it = <{coll_rust} as __jet_Iterable>::iter(({collection_str}));\n",
                        pad,
                    ));
                    out.push_str(&jet_format!(
                        "{}    {}for {jet_prefix}item in std::iter::from_fn(|| <{iter_rust} as __jet_Iterator>::next(&mut {jet_prefix}it).ok()){} {{\n",
                        pad, lbl, stride_suffix,
                    ));
                    out.push_str(&jet_format!(
                        "{}        let {} = {jet_prefix}item;\n",
                ...
                None => match var2 {
                    Some(v2) => {
                            ...
                            )
                        {
                            ...
                            let iter_form = if *by_value {
                                ...
                                jet_format!(
                                    "({}).iter_mut().map(|{jet_prefix}view| &mut **{jet_prefix}view)",
                                    collection_str
                                )
                            } else {
                            ...
                            out.push_str(&jet_format!(
                                "{}{}for ({jet_prefix}k, {jet_prefix}v) in ({}).iter(){} {{\n",
                                pad,
                                lbl,
                                collection_str,
                                stride_suffix
                            ...
                            out.push_str(&jet_format!(
                                "{}    let {} = {jet_prefix}k.clone();\n",
                                pad,
                                mangle(var)
                            ));
                            out.push_str(&jet_format!(
                                "{}    let {} = {jet_prefix}v.clone();\n",
                                pad,
                                mangle(v2)
                            ));
                        }
                    ...
                    None => {
                        if let Type::Map { key, value, .. } = &collection.ty {
                            ...
                            out.push_str(&jet_format!(
                                "{}    let {} = {} {{ {}: {jet_prefix}k.clone(), {}: {jet_prefix}v.clone() }};\n",
                                pad,
                                mangle(var),
                                tuple,
                                mangle("key"),
        ...
        TStmt::ScopeMember { kind, body } => {
            ...
            match kind {
                ...
                ScopeMemberKind::ExpectFail(expected_code) => {
                    ...
                    emit_tir_stmts_nested(body, cx, out, inner + 1, active_deferred_closes);
        ...
        TStmt::Transact {
            ...
            let effective_handle: Option<String> = match handle {
                ...
                None if !snapshots.is_empty() => Some(mangle_generated("txn")),
                None => None,
            };
        ...
        TStmt::MixedSwitch {
            ...
            if matches!(class, crate::Codegen::TIR::BranchClass::Bool2) {
                ...
                let (condition, _) =
                    coverage_condition(jet_format!("*{jet_prefix}switch_subject"), cx);
                out.push_str(&format!("{}if {condition} {{\n", inner_pad));
                emit_tir_stmts_nested(true_body, cx, out, indent + 2, active_deferred_closes);
                out.push_str(&format!("{}}} else {{\n", inner_pad));
                emit_tir_stmts_nested(false_body, cx, out, indent + 2, active_deferred_closes);
                out.push_str(&format!("{}}}\n", inner_pad));
                out.push_str(&format!("{}}}\n", pad));
                return;
            }
            if matches!(class, crate::Codegen::TIR::BranchClass::DenseInt) {
                ...
                if cx.coverage {
                    for (i, (cond, body)) in arms.iter().enumerate() {
                        let literal = match &cond.kind {
                            crate::Codegen::TIR::TExprKind::Binary { rhs, .. } => match &rhs.kind {
                                crate::Codegen::TIR::TExprKind::IntLit(value, _) => {
                                    value.to_string()
                                }
                ...
                out.push_str(&jet_format!(
                    "{}match *{jet_prefix}switch_subject {{\n",
                    inner_pad
                ));
