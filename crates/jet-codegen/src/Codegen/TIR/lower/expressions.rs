    ...
    match expr {
        Expr::Call(call) => {
            ...
            source_arg_indices(&call.args)
                .into_iter()
                .map(|index| ExprWorkChild::Arg(ExprWorkArg {
                    arg: &call.args[index],
                    mode: conventions
                        .as_ref()
...
pub(crate) fn lower_expr(e: &Expr, cx: &Cx, env: &mut LowerEnv) -> TExpr {
    ...
    if let Some(value) = expr_cache_take(e) {
        ...
        // notably a lambda capture — after the outer worklist cached its
        // local read. Refresh the structured local and type from the active
