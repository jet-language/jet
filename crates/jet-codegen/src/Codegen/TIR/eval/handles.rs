...
pub(super) fn eval_handle_with_type_and_sink(
    ...
    match op {
        ...
        THandleOp::AllocTryAlloc => {
            ...
            let (allocator, capacity, used, allocations) = match recv {
                CtValue::Struct { fields, .. } => (
                    fields.iter().find_map(|(name, value)| {
                        (name == "ctor").then(|| match value {
                            CtValue::Str(ctor) if ctor.starts_with("__JET_FIXED_INLINE:") => "Fixed",
                            CtValue::Str(ctor) if ctor.contains("JetBump") => "Bump",
                            CtValue::Str(ctor) if ctor.contains("JetPool") => "Pool",
                            CtValue::Str(_) => "Arena",
                            _ => "Arena",
                        })
                    }).unwrap_or("Arena"),
                    fields.iter().find_map(|(name, value)| {
                        (name == "ctor").then(|| match value {
                            CtValue::Str(ctor) => ctor
                                .strip_prefix("__JET_FIXED_INLINE:")
                                .and_then(|bytes| bytes.parse::<usize>().ok()),
                            _ => None,
                        })
                    }).flatten().unwrap_or(usize::MAX),
                    fields.iter().find_map(|(name, value)| {
                        (name == "used").then(|| match value {
                            CtValue::Int(used) => (*used).max(0) as usize,
                            _ => 0usize,
                        })
                    }).unwrap_or(0),
                    fields.iter().find_map(|(name, value)| {
                        (name == "allocations").then(|| match value {
                            CtValue::Int(allocations) => *allocations,
                            _ => 0,
                        })
                    }).unwrap_or(0),
                ),
                _ => ("Arena", usize::MAX, 0, 0),
            };
            let overhead = if allocator == "Fixed" {
                3 * std::mem::size_of::<usize>()
            } else {
                0
            };
            let (value, next_used) = match jet_foundation::Outcome::jet_try_alloc_value(
                value,
                used,
                capacity,
                requested_bytes,
            ...
            if let CtValue::Struct { fields, .. } = recv {
                for (name, field) in fields.iter_mut() {
                    if name == "used" {
                        *field = CtValue::Int(next_used.min(i64::MAX as usize) as i64);
                    } else if name == "allocations" {
                        *field = CtValue::Int(allocations.saturating_add(1));
                    }
                }
            }
