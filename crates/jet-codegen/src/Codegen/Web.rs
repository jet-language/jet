...
fn emit_app_shell(bundle: &ProgramBundle) -> WebArtifacts {
    ...
    let js_app = "// Generated by jet — App browser edge (D-FAIL-EDGE1).\n\
    ...
    WebArtifacts {
        ...
        wasm_rust: "// App has no browser TIR body; its JS artifact still uses the typed edge adapter.\n#[no_mangle]\npub extern \"C\" fn jet_wasm_nop() {}\n".to_string(),
        js_app,
    ...
    for stmt in body {
        match stmt {
            ...
            TIR::TStmt::ContractScope {
                ...
                emit_js_contract_scope(pre, body, post, out, funcs, file_prefix, indent)?;
            }
            TIR::TStmt::Return(Some(expr)) => out.push_str(&format!(
                "{pad}return {};\n",
                tir_js_expr(expr, funcs, file_prefix)?
