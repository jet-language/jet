...
const JS_SOURCE_FILE_MARKER: &str = "//# jet-source-file ";
const JS_SOURCE_LINE_MARKER: &str = "//# jet-source-line ";
...
pub fn emit_web(
    ...
    let (js_app, js_source_map, handlers) = emit_js_app(bundle, &funcs, &sources)?;
...
fn emit_js_app(
    ...
    let (js_app, js_source_map) = finish_js_source_map(&out, sources);
...
fn finish_js_source_map(raw: &str, sources: &[JsSource]) -> (String, String) {
    ...
    for line in raw.split_inclusive('\n') {
        ...
        if let Some(index) = marker.strip_prefix(JS_SOURCE_FILE_MARKER) {
            let index = index
                .parse::<usize>()
        ...
        if let Some(original_line) = marker.strip_prefix(JS_SOURCE_LINE_MARKER) {
            pending_line = Some(
                original_line
                    .parse::<usize>()
    ...
    for stmt in body {
        match stmt {
            TIR::TStmt::LineMarker(line) => {
                out.push_str(&format!("{pad}{JS_SOURCE_LINE_MARKER}{line}\n"));
            }
...
mod source_map_tests {
    ...
    fn source_map_markers_strip_and_encode_v3_segments() {
        ...
        let (js, map) = finish_js_source_map(raw, &sources);
