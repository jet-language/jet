...
pub fn emit_web(
    ...
    let (js_app, handlers) = emit_js_app(bundle, &funcs)?;
...
fn wasm_emit_expr(
    ...
    Ok(match &expr.kind {
        ...
        TIR::TExprKind::Clone(inner) => wasm_emit_expr(inner, funcs, file_prefix, reconstructions)?,
...
fn emit_js_app(bundle: &ProgramBundle, funcs: &[FuncWeb]) -> WebEmitResult<(String, Vec<(String, String)>)> {
    let mut out = String::from(
    ...
    let js_funcs: Vec<&FuncWeb> = funcs
        ...
        .collect();
    for f in &js_funcs {
        emit_js_fn(f, &mut out, funcs, &mut handlers)?;
    }
...
fn emit_js_fn(f: &FuncWeb, out: &mut String, all: &[FuncWeb], handlers: &mut Vec<(String, String)>) -> WebEmitResult<()> {
