...
pub fn emit_web(
    ...
    // D-FAIL-EDGE1: a native WebApp entry still passes the web build front
...
pub fn validate_web_tir_support(
    ...
    // The WebApp runtime is a native server edge. Its browser artifact uses
...
fn emit_web_app_shell(bundle: &ProgramBundle) -> WebArtifacts {
    ...
    let js_app = "// Generated by jet — WebApp browser edge (D-FAIL-EDGE1).\n\
    ...
    WebArtifacts {
        ...
        wasm_rust: "// WebApp has no browser TIR body; its JS artifact still uses the typed edge adapter.\n#[no_mangle]\npub extern \"C\" fn jet_wasm_nop() {}\n".to_string(),
        js_app,
...
fn emit_js_app(
    ...
    let wasm_main = funcs
        .iter()
        .any(|f| f.key == "run" && f.bucket == WebBucket::Wasm && auto_start);
