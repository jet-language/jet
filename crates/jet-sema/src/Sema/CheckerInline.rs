//! D-METHODMACRO1=A: checked `#Inline` / `#Inline(Always)` contracts.
//!
//! `#Inline` is a soft hint — never rejected here, never even inspected by
//! this module. `#Inline(Always)` is a checked promise: if the compiler can
...
//!     calling itself by name). Mutual recursion between two `#Inline(Always)`
...
/// E0917: `#Inline(Always) fn {name}` calls itself.
fn e0917_self_recursive(name: &str, span: Span) -> Diagnostic {
    Diagnostic::error(
        "E0917",
        format!("`{name}` calls itself, so `#Inline(Always)` cannot expand it"),
        "inlining a recursive call would either loop forever at compile time or require an \
         artificial depth cutoff — neither is a real inline."
            .to_string(),
        "drop `#Inline(Always)` (use `#Inline` as a hint), or restructure the function to be \
         non-recursive."
            .to_string(),
        Some(span),
    )
...
/// E0918: `#Inline(Always) fn {name}` had its address taken (used as a value)
...
pub(crate) fn e0918_address_taken(name: &str, span: Span) -> Diagnostic {
    Diagnostic::error(
        ...
        format!(
            "`#Inline(Always)` promises every call to `{name}` expands in place — but `{name}` is \
        ...
        format!("drop `#Inline(Always)`, or call `{name}` directly instead of through a value."),
        Some(span),
    )
}

/// E0919: `#Inline(Always) fn {name}` is too large to inline.
fn e0919_too_large(name: &str, stmt_count: usize, span: Span) -> Diagnostic {
    Diagnostic::error(
        "E0919",
        format!("`{name}` is too large for `#Inline(Always)`"),
        format!(
            "its body has {stmt_count} statements — over the {INLINE_ALWAYS_MAX_STMTS}-statement \
             ceiling `#Inline(Always)` enforces so a promised inline doesn't quietly bloat every \
             call site."
        ),
        "drop `#Inline(Always)` (use `#Inline` as a hint the compiler is free to ignore), or split \
...
/// D-METHODMACRO1=A: the local part of the `#Inline(Always)` check — direct
