...
impl<'a> Checker<'a> {
    pub(crate) fn check_lambda(
                ...
                if !info.mutable {
                    let mut diagnostic = Diagnostic::error(
                        ...
                        "changing a value inside a short function requires a `:=` binding"
                            .to_string(),
                        format!("declare `{name} := …` instead of `{name} :: …`"),
                        Some(lam.span),
                    );
                ...
                if self.interrupt_callback_depth > 0 {
                    let problem = if matches!(&cap_ty, Type::Fn { .. }) {
                        ...
                        if callback_safe {
                            ...
                            self.crossing_problem(&cap_ty, SendCrossing::InterruptCallback, false)
                                .or(Some(SendabilityProblem {
                                    root: None,
                                    path: Vec::new(),
        ...
        let saved_inferred_mut_captures = std::mem::take(&mut self.inferred_lambda_mut_captures);
        ...
        let saved_task_body_propagates = std::mem::replace(&mut self.task_body_propagates, false);
        ...
        if matches!(
            ...
            if let LambdaBody::Expr(expr) = &mut lam.body {
                    ...
                    && self.place_from_expr(expr).is_some()
                {
                    ...
                    *expr = Box::new(Expr::Place(inner, crate::AST::PlaceAccess::Read, span));
                }
            }
        }
        if inline_loop {
        ...
        let mut body_ret = match &mut lam.body {
            LambdaBody::Expr(e) => {
                ...
                let needs_value = match effective_ret.as_ref() {
                    ...
                    Some(Type::Result { ok, .. }) if matches!(ok.as_ref(), Type::Named(name) if name == "Unit") => {
                        false
                    }
                    None => false,
                    Some(_) => true,
...
fn install_inline_loop_label(body: &mut LambdaBody, label: &str, span: crate::Diagnostics::Span) {
    ...
    for stmt in stmts {
        match stmt {
            Stmt::Yield(value, span) if span.start == span.end => {
                let value = std::mem::replace(value, crate::AST::Expr::Int(0, *span, None, None));
