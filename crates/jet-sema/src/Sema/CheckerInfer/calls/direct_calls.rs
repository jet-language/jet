...
impl<'a> Checker<'a> {
    ...
    fn normalize_user_policy_arguments(&mut self, arguments: &mut [crate::AST::CallArg]) {
        for argument in arguments {
            let Expr::Call(policy) = &mut argument.expr else {
                continue;
            };
    ...
    fn check_freeze(&mut self, call: &mut Call) -> Option<Type> {
        ...
        let reason = if self.type_contains_shared(&source_ty) {
            Some(
                "the value contains `Shared`, whose lock-backed handle would keep shared mutation"
                    .to_string(),
            )
        ...
        if let Some(problem) =
            self.crossing_problem(&result_ty, crate::Sema::SendCrossing::TaskCapture, true)
        {
    ...
    fn check_callable_policy_apply(&mut self, call: &mut Call) -> Option<Type> {
        ...
        let Some(replaced) = callee_ty.replace_callable_policies(chain.clone()) else {
            self.diags.push(Diagnostic::error(
                "E0803",
                format!(
                    "`apply` can wrap only a function value, not {}",
                    callee_ty.show()
                ),
    ...
    fn check_numeric_approx(&mut self, call: &mut Call) -> Option<Type> {
        if call.args.len() != 1 {
            ...
            self.diags.push(Diagnostic::error(
                ...
                "`approx(value)` accepts possible precision loss at one integer-to-float crossing"
                    .to_string(),
        ...
        if let Some(argument) = call.args.get_mut(0) {
                ...
                || matches!(&cap_ty, Type::Named(name) if name == "Int" || name == "I64" || name == "I32"))
            {
                self.diags.push(Diagnostic::error(
                    ...
                    "bounded channels use a whole-number memory/backpressure limit".to_string(),
    ...
    fn check_call_inner(&mut self, call: &mut Call, _as_value: bool) -> Option<Option<Type>> {
        ...
        if call.name == Syntax::RESOURCE_CLOSE {
            if call.args.len() != 1 {
                self.diags.push(Diagnostic::error(
                        ...
                        "`close` consumes one value through its nominal `Close` implementation and the move marker `^`"
                            .to_string(),
                        "write `close(^resource)` with the move marker `^`".to_string(),
                        Some(call.name_span),
                    ));
                for arg in call.args.iter_mut() {
                    self.infer(&mut arg.expr);
            ...
            if arg.convention != AccessConvention::Move {
                self.diags.push(Diagnostic::error(
                        ...
                        "`Close.close(^self)` is consuming through the move marker `^`, so cleanup runs on exactly one owner"
                            .to_string(),
                        "write `close(^resource)` with the move marker `^`".to_string(),
                        Some(arg.span),
                    ));
        ...
        if call.name == Syntax::FOREIGN_PRINTLN || call.name == Syntax::FOREIGN_EPRINTLN {
            ...
            let diagnostic = Diagnostic::error(
                ...
                "`print` writes to stdout; `io.eprint` is the stderr twin in `core.term`"
                    .to_string(),
        ...
        if matches!(
            ...
            self.diags.push(Diagnostic::error(
                "E0040",
                format!(
                    "`{}` is not in Jet; use the `task` keyword instead",
                    call.name
                ),
                "Jet uses blocking tasks and channels, not async/await — simpler and race-free"
                    .to_string(),
                "write `task your_work()` or `task { … }`".to_string(),
                Some(call.name_span),
            ));
            for a in call.args.iter_mut() {
        ...
        if sig.is_c_abi {
            for (index, arg) in call.args.iter().enumerate() {
                ...
                let is_string_param =
                    matches!(sig.params.get(declaration_index), Some((_, Type::String)));
                if !is_string_param {
                    continue;
                }
            ...
            if let Some(arg_ty) = &arg_ty {
                    ...
                    )
                {
                    self.diags.push(Diagnostic::error(
                            ...
                            "create the guard with `guard_edit()` before passing it with the write-access marker `&`"
                                .to_string(),
                            Some(arg.expr.span()),
                        ));
                }
            ...
            match (param_conv, arg.convention) {
                    ...
                    if let Some((name, span)) = &bare_arg_name {
                            ...
                            && is_cloneable(param_ty, self.registry)
                        {
                            ...
                            self.diags.push(Diagnostic::error(
                                    ...
                                    format!(
                                        "write the move marker `^` (`{}{}`) to move ownership to `{}`",
                                        Syntax::SIGIL_MOVE,
                                        name,
                                        call.name
                                    ),
                ...
                (AccessConvention::Read | AccessConvention::Write, AccessConvention::Move) => {
                    self.diags.push(Diagnostic::error(
                            ...
                            "only parameters declared with the move marker `^` accept a moved value at the call site"
                                .to_string(),
                    ...
                    if let Some(info) = self.lookup(name) {
                        if matches!(info.ty, Type::Shared(_)) {
                            ...
                            self.record_memory_event(crate::Sema::MemoryEvent::new(
                                ...
                                *span,
                                format!("loop use of `{name}` auto-retains a shared reference"),
                            ));
