...
impl<'a> Checker<'a> {
    ...
    pub(crate) fn infer_code_module_call(
        ...
        if let Some(diagnostic) =
            crate::Sema::FFI::foreign_call_boundary_error(&sig, item_name, self.in_unsafe, span)
        {
        ...
        if crate::Sema::CallBinder::bind_call_args(item, &params, args, span, &mut self.diags)
            .is_none()
        {
        ...
        if sig.param_variadic.last().copied().unwrap_or(false) && sig.variadic_bounds.is_none() {
    ...
    fn infer_import_call_with_warning(
        ...
        let semantic_name =
            imported_inline_member_name(target, name).unwrap_or_else(|| name.to_string());
        ...
        if target.funcs.contains_key(&semantic_name) {
            ...
            if let Some(diagnostic) =
                crate::Sema::FFI::foreign_call_boundary_error(&sig, name, self.in_unsafe, span)
            {
                self.diags.push(diagnostic);
            }
            ...
            for (index, (arg, (pconv, pty))) in
                args.iter_mut().zip(effective_params.iter()).enumerate()
            {
                if matches!(pconv, AccessConvention::Read) && !pty.is_scalar() {
                    self.borrow_ctx = true;
                }
                ...
                let aty = pre_inferred.get(index).cloned().unwrap_or_else(|| {
                    self.with_call_access(&mut call_access, |checker| {
                        checker.check_call_argument_access(arg, *pconv, pty, !sig.is_extern);
                        let inferred = checker.infer(&mut arg.expr);
                        checker.check_call_argument_captures(&arg.expr);
                        inferred
                    })
                ...
                if let Some(aty) = aty {
                        ...
                        Type::Named(qualified) => qualified
                            ...
                            .and_then(|(alias, leaf)| {
                                target.core_imports.get(alias).and_then(|module| {
                                    if matches!(module.as_str(), "core.crypto")
                                        && matches!(
                                            leaf,
                    ...
                    let reported =
                        reads_expiring_secret_loan || self.check_type_assignable(pty, &aty, span);
