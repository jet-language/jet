    ...
    match method {
        ...
        _ => {
            let (why, fix) = if method == "free" {
                (
                    "allocator terminal release uses the same nominal `Close` protocol as every resource; it consumes through the move marker `^`".to_string(),
                    "write `close(^allocator)` with the move marker `^`; use `allocator.reset()` only when reusing its backing storage".to_string(),
                )
            } else {
