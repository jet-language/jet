...
pub(crate) fn expand_builtin_serde_items(items: &mut Vec<Item>, _diags: &mut Vec<Diagnostic>) {
    ...
    let union_names = union_defs
        .iter()
        .map(|definition| definition.name.clone())
        .collect::<std::collections::HashSet<_>>();
    items.extend(union_defs.into_iter().map(Item::Enum));
    let mut generated_items = Vec::new();
    let snapshot = items.clone();
    for item in &snapshot {
        match item {
            Item::Enum(e) if union_names.contains(&e.name) => {
                generated_items.extend(union_codec_items(e));
            }
            Item::Struct(s)
...
/// D-UNIONTYPE1=A / D-SERDE2: anonymous unions are ordinary enum items before
/// their typed codec impls are built. The representation still uses the
/// compiler's canonical union name; only the serde behavior enters through
/// this shared AST path.
fn anonymous_union_defs(
    items: &[Item],
    encode_unions: &std::collections::BTreeSet<String>,
    decode_unions: &std::collections::BTreeSet<String>,
) -> Vec<crate::AST::EnumDef> {
    fn walk_type(
        ty: &Type,
        seen: &mut std::collections::BTreeSet<String>,
        unions: &mut Vec<Vec<Type>>,
    ) {
        match ty {
            Type::Union(members) => {
                let name = crate::AST::union_enum_name(members);
                if seen.insert(name) {
                    unions.push(members.clone());
                }
                for member in members {
                    walk_type(member, seen, unions);
                }
            }
            Type::List(inner)
            | Type::Shared(inner)
            | Type::Option(inner)
            | Type::Tagged { inner, .. }
            | Type::FixedList { elem: inner, .. } => walk_type(inner, seen, unions),
            Type::Map { key, value, .. } | Type::Result { ok: key, err: value } => {
                walk_type(key, seen, unions);
                walk_type(value, seen, unions);
            }
            Type::Fn { params, ret, .. } => {
                for param in params {
                    walk_type(param, seen, unions);
                }
                if let Some(ret) = ret {
                    walk_type(ret, seen, unions);
                }
            }
            Type::Apply { args, .. } => {
                for arg in args {
                    walk_type(arg, seen, unions);
                }
            }
            Type::Tuple(fields) => {
                for (_, field) in fields {
                    walk_type(field, seen, unions);
                }
            }
            Type::InlineRange { base, .. } => walk_type(base, seen, unions),
            Type::Quantity { base, .. } => walk_type(base, seen, unions),
            _ => {}
        }
    }
        ...
        match item {
            Item::Struct(s) => {
                for field in &s.fields {
                    walk_type(&field.ty, seen, unions);
                }
            }
            Item::Enum(e) => {
                for variant in &e.variants {
                    match &variant.payload {
                        VariantPayload::Single(ty, _) => walk_type(ty, seen, unions),
            ...
            Item::Func(f) => {
                for param in &f.params {
                    walk_type(&param.ty, seen, unions);
                }
            ...
            // Nested code modules are expanded in their own scope before this
            // local pass. Their union enum must not be hoisted into the parent.
            Item::CodeModule(_) | Item::Const(_) | Item::Test(_) | Item::Bench(_) => {}
            _ => {}
        }
    }
    ...
    unions
        ...
        .map(|members| {
            ...
            crate::AST::EnumDef {
                ...
                variants: members
                    .into_iter()
                    .map(|ty| crate::AST::Variant {
                        name: crate::AST::union_member_tag(&ty),
                        name_span: span,
                        payload: VariantPayload::Single(ty, span),
                        discriminant: None,
                ...
                is_must_use: false,
                must_use_span: None,
                // D-SERDE7: the synthetic enum uses the ordinary untagged
                // enum codec path. Its payloads are the union's member types,
                // so this preserves one typed serde mechanism for named and
                // anonymous enums.
                serde_markers: vec![crate::AST::Marker {
                    name: crate::Syntax::MARKER_UNTAGGED.to_string(),
                    negated: false,
                    name_span: span,
                    args: Vec::new(),
                    arg_labels: Vec::new(),
...
fn union_codec_needs(
    ...
    fn collect(
        ...
        match ty {
            ...
            Type::InlineRange { base, .. } | Type::Quantity { base, .. } => {
                collect(base, encode, decode, encodes, decodes)
            }
            _ => {}
        }
    }
    ...
    fn requested(
        ...
        derives.iter().any(|(name, _)| name == trait_name || name == "Codable")
            || automatic.contains(type_name)
    }
    ...
    fn walk_items(
        ...
        for item in items {
            match item {
                Item::Struct(s) => {
                    let encode = requested(
                        &s.derives,
                        &auto.auto_encode,
                        &s.name,
                        crate::Generics::ENCODE,
                    );
                    let decode = requested(
                        &s.derives,
                        &auto.auto_decode,
                        &s.name,
                        crate::Generics::DECODE,
                    );
                    for field in s.fields.iter().filter(|field| {
                        !field
                            .serde_markers
                            .iter()
                ...
                // Nested code modules run this pass against their own item
                // scope, so their union codecs are not attached to the parent.
                Item::CodeModule(_) => {}
                _ => {}
            }
        }
...
fn union_codec_items(e: &crate::AST::EnumDef) -> Vec<Item> {
    // The synthetic declaration carries `#[Untagged]`, so the same ordinary
    // enum codec builder handles its typed payloads.
    enum_codec_items(e)
}
