...
pub(super) fn validate_bundle(bundle: &ProgramBundle) -> Vec<Diagnostic> {
    ...
    for module in &bundle.modules {
        for item in &module.items {
            trait_names(item, &mut traits);
        }
    }
    for module in &bundle.modules {
        for import in &module.imports {
            if !import.alias.is_empty() {
                snake(&import.alias, import.alias_span, "module alias", &mut out);
            }
            match &import.kind {
        ...
        for item in &module.items {
            item_names(item, &traits, &mut out);
        }
    }
    out
}
...
fn check(name: &str, span: Span, category: &str, out: &mut Vec<Diagnostic>) {
    ...
    let case = Syntax::name_case_for_category(category).unwrap_or_else(|| {
        jet_foundation::ice!(
            Some(span),
            "identifier category `{category}` is missing from Syntax::NAME_CASE_CATEGORIES"
        )
    });
    if name.is_empty() || name.starts_with("__") || Syntax::name_has_case(name, case) {
        return;
    }
...
fn func_names(f: &Func, category: &str, out: &mut Vec<Diagnostic>) {
    ...
    for p in &f.params {
        if p.name != Syntax::KW_SELF {
            snake(&p.name, p.name_span, "parameter", out);
        }
        if let Some(default) = &p.default {
            expr_names(default, out);
        }
    }
...
fn item_names(item: &Item, traits: &HashSet<String>, out: &mut Vec<Diagnostic>) {
    match item {
        ...
        Item::Enum(e) => {
            ...
            for v in &e.variants {
                ...
                if let VariantPayload::Named(fields) = &v.payload {
                    for f in fields {
                        snake(&f.name, f.name_span, "variant field", out);
                    }
                }
            }
            for m in &e.methods {
        ...
        Item::Trait(t) => {
            ...
            for (name, span) in &t.assoc_types {
                pascal(name, *span, "associated type", out);
            }
            for m in &t.methods {
                trait_method_names(m, out);
            }
        }
        Item::Tag(t) => {
        ...
        Item::Module(m) => {
            ...
            for import in &m.imports {
                expr_names(import, out);
            }
            for member in &m.members {
                expr_names(member, out);
            }
        }
        Item::CodeModule(m) => {
            snake(&m.name, m.name_span, "module", out);
        ...
        Item::ProtocolDecl(p) => {
            ...
            for m in &p.messages {
                ...
                for (name, _) in &m.fields {
                    snake(name, m.span, "message field", out);
                }
            }
        }
        Item::UserDerive(d) => {
        ...
        match stmt {
            ...
            Stmt::While {
                ...
                if let Some((name, span)) = label {
                    snake(name, *span, "loop label", out);
                }
                stmt_names(body, out);
            }
            ...
            | Stmt::ComptimeSwitch {
                ...
                for arm in arms {
                    expr_names(&arm.cond, out);
                    stmt_names(&arm.body, out);
                }
            ...
            | Stmt::Layout {
                ...
                snake(name, *name_span, "local", out);
                stmt_names(body, out);
            }
            Stmt::Transact {
                name,
            ...
            | Stmt::Live { body, .. }
            | Stmt::AssumeDet { body, .. } => stmt_names(body, out),
...
fn expr_names(expr: &Expr, out: &mut Vec<Diagnostic>) {
    match expr {
        ...
        | Expr::Err(inner, _)
        | Expr::Paren(inner, _)
        | Expr::Spread(inner, _) => expr_names(inner, out),
        ...
        Expr::Lambda(lambda) => {
            for param in &lambda.params {
                snake(&param.name, param.name_span, "lambda parameter", out);
            }
            match &lambda.body {
        ...
        Expr::If {
            ...
            expr_names(cond, out);
            stmt_names(then_body, out);
            expr_names(then_value, out);
            stmt_names(else_body, out);
            expr_names(else_value, out);
        }
