...
/// E0711 (D-AUTHORITY-SCOPE1 / D-AUTHORITY-WORD2): the `Abilities` handle bound by a named `#Abilities(…)` region escapes
...
pub fn e0711(handle: &str, marker: &str, span: Span) -> Diagnostic {
    Diagnostic::error(
        ...
        format!(
            "`#{}(…)` revokes the `Abilities` at the end of its block (RAII); returning, storing, or sharing `{}` would let revoked rights outlive the scope",
            marker, handle
        ),
