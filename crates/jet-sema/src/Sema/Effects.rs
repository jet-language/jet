...
pub fn effects_uncovered(inferred: &EffectSet, bound_set: &EffectSet) -> EffectSet {
    ...
    // positive function bound or a `#FX` authority.
...
impl<'a> super::Checker<'a> {
    ...
    /// function's edges and every open `#FX(…)` region.
...
/// D-EFF1: an open `#FX(…)` region's running accumulator while the checker
...
/// D-EFF1: a `#FX(…) { … }` region's accumulated effects, checked (transitively)
...
/// D-EFF1: check every `#FX(…)` region across the program against its
...
/// E0712 (D-ABILITY-NAME2): an effect used inside a `#FX(…)` region is not
...
/// E0711 (D-ABILITY-NAME2): the `Authority` handle bound by a named `#FX(…)` region escapes
...
/// E0119: a `#(…)` or `#FX(…)` list names something that isn't a known effect.
