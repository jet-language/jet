...
fn specialize_struct(
    ...
    let trait_impls = source
        ...
        .map(|block| crate::AST::TraitImplBlock {
            ...
            operator_rhs: block.operator_rhs.as_ref().map(|ty| {
...
fn expand_alias(
    ...
    for item in &template.body {
        ...
        // Re-evaluate after specialization. `source.ct` belongs to the
        // unspecialized template (and can also survive in a resumed bundle);
        // using it first can bake another instance's value into this one.
        let evaluated = crate::Comptime::evaluate_closed_value(
            &value,
            funcs,
            &HashSet::new(),
            Path::new("."),
...
pub(crate) use Expansion::{expand_generic_module_aliases, hoist_inline_module_member_types};
use Expansion::inline_type_alias;
