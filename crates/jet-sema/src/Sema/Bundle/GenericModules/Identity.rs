...
fn frame_text(out: &mut Vec<u8>, text: &str) {
    super::frame_bytes(out, text.as_bytes());
}
...
pub(super) fn type_full_key(ty: &Type) -> Vec<u8> {
    fn write(out: &mut Vec<u8>, ty: &Type) {
        ...
        match ty {
            ...
            TraitObject(names) => {
                out.push(14);
                out.extend_from_slice(&(names.len() as u64).to_be_bytes());
                for name in names {
                    frame_text(out, name);
                }
            }
    ...
    for param in params {
        match param {
            ...
            ResolvedModuleParam::Value { name, ty } => {
                out.push(1);
                frame_text(&mut out, name);
                super::frame_bytes(&mut out, &type_full_key(ty));
            }
...
pub(super) fn definition_full_key(
...
fn quoted_field(text: &str, field: &str) -> Option<String> {
    ...
    text.match_indices(&needle).find_map(|(offset, _)| {
        ...
        if boundary.is_some_and(|ch| ch.is_alphanumeric() || ch == '_') {
            return None;
        }
        let rest = text[offset + needle.len()..]
            .trim_start()
            .strip_prefix('"')?;
...
fn canonical_semver(version: &str) -> String {
    let (core_pre, build) = version
        .split_once('+')
        .map_or((version, None), |(core, build)| (core, Some(build)));
    let (core, pre) = core_pre
        .split_once('-')
        .map_or((core_pre, None), |(core, pre)| (core, Some(pre)));
    ...
    let mut canonical = parts
        .iter()
        .map(|part| part.parse::<u64>().unwrap_or(0).to_string())
        .collect::<Vec<_>>()
        .join(".");
    if let Some(pre) = pre {
        canonical.push('-');
...
fn credential_free_git_url(url: &str) -> String {
    ...
    let clean_authority = authority
        .rsplit_once('@')
        .map_or(authority, |(_, clean)| clean);
    format!(
        "{}://{}{}{}",
...
fn canonical_lock_source(
        ...
        .filter(|line| !line.is_empty() && !line.starts_with('#'))
    {
        ...
        if let Some(value) = lock_value(line, "name") {
            ...
            version = value;
        } else if let Some(value) = lock_value(line, "source") {
            source = value;
        } else if let Some(value) = lock_value(line, "locked") {
            locked = value;
        } else if let Some(value) = lock_value(line, "content-hash") {
            content_hash = value;
        }
    }
    if current {
        records.push((name, version, source, locked, content_hash));
    }
    if let Some((_, locked_version, source, locked, content_hash)) = records
        .into_iter()
        ...
        if let Some(path) = inline_lock_value(&source, "path") {
            if let Some(registry) = path.strip_prefix("registry:") {
                return format!(
                    ...
                    canonical_semver(&locked_version)
                );
            }
...
pub(in crate::Sema) fn owning_package<'a>(
...
pub(in crate::Sema) fn package_identity(
    ...
    let version = canonical_semver(
        &quoted_field(&manifest, "version").unwrap_or_else(|| "0.0.0+workspace".into()),
    );
...
pub(super) fn register_instance_fingerprint(
