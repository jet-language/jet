...
fn package_identity_uses_canonical_source_not_credentials_paths_or_formatting() {
    ...
    for path in [&project_a, &project_b, &dep_a, &dep_b] {
        std::fs::create_dir_all(path).unwrap();
    }
    std::fs::create_dir_all(project_a.join(".jet")).unwrap();
    std::fs::create_dir_all(project_b.join(".jet")).unwrap();
    std::fs::write(
        dep_a.join(crate::Syntax::PAYLOAD_FILE),
        "payload: { name: \"demo\", version: \"1.2.3\" }",
    )
    .unwrap();
    std::fs::write(
        dep_b.join(crate::Syntax::PAYLOAD_FILE),
        "payload: {\n  version: \"1.2.3\",\n  name: \"demo\"\n}\n",
    )
    .unwrap();
    ...
    assert_eq!(
        a, b,
        "formatting, credentials, timestamps, and host paths are non-semantic"
    );
    ...
    assert_ne!(
        a, changed,
        "locked git revision is semantic package source identity"
    );
...
fn same_template_path_in_different_packages_has_distinct_definition_identity() {
    ...
    std::fs::write(
        first.join(crate::Syntax::PAYLOAD_FILE),
        "payload: { name: \"first\", version: \"1.0.0\" }",
    )
    .unwrap();
    std::fs::write(
        second.join(crate::Syntax::PAYLOAD_FILE),
        "payload: { name: \"second\", version: \"1.0.0\" }",
    )
    .unwrap();
    let bundle = identity_bundle(root.clone());
    let a = definition_full_key(
        &package_identity(&bundle, &first, Some("first")),
        "src/template.jet",
        "",
        "Boxed",
    );
    let b = definition_full_key(
        &package_identity(&bundle, &second, Some("second")),
        "src/template.jet",
        "",
        "Boxed",
    );
...
fn path_lock_content_changes_definition_and_instance_identity_but_host_path_does_not() {
    ...
    std::fs::write(
        dependency.join(crate::Syntax::PAYLOAD_FILE),
        "payload: { name: \"dep\", version: \"1.2.3\" }",
    )
    .unwrap();
    let lock = |path: &str, content: &str| {
        format!("version=1\n[[package]]\nname=\"dep\"\nversion=\"1.2.3\"\nsource={{path=\"{path}\"}}\ncontent-hash=\"{content}\"\n")
    };
    let bundle = identity_bundle(project.clone());
    std::fs::write(
        project.join(crate::Syntax::UNIFIED_LOCK_FILE),
        lock("/host/a/dep", "tree-a"),
    )
    .unwrap();
    let package_a = package_identity(&bundle, &dependency, Some("dep"));
    std::fs::write(
        project.join(crate::Syntax::UNIFIED_LOCK_FILE),
        lock("/host/b/dep", "tree-a"),
    )
    .unwrap();
    assert_eq!(
        package_a,
        package_identity(&bundle, &dependency, Some("dep"))
    );
    std::fs::write(
        project.join(crate::Syntax::UNIFIED_LOCK_FILE),
        lock("/host/b/dep", "tree-b"),
    )
    .unwrap();
    ...
    assert_ne!(
        crate::SHA256::sha256_hex(&definition_a),
        crate::SHA256::sha256_hex(&definition_b)
    );
    let instance = |definition_full_key| ModuleInstanceKey {
        definition_full_key,
        parameters: vec![1],
        args: vec![vec![2]],
    };
    assert_ne!(
        crate::SHA256::sha256_hex(&instance(definition_a).bytes()),
        crate::SHA256::sha256_hex(&instance(definition_b).bytes())
    );
    let _ = std::fs::remove_dir_all(root);
}
...
#[test]
#[should_panic(
    expected = "internal compiler error: E0859 generic module instance fingerprint collision"
)]
fn different_full_keys_with_same_digest_fail_closed_before_codegen() {
    let mut registry = HashMap::new();
    let make = |full_key| crate::AST::ModuleInstanceIdentity {
        full_key,
        fingerprint: "forced-digest".into(),
        definition_id: "def".into(),
        argument_keys: Vec::new(),
        argument_values: Vec::new(),
        argument_provenance: Vec::new(),
        template_span: Span::new(0, 0),
...
fn generated_nominal_names_encode_module_alias_boundaries() {
    assert_ne!(
        module_type_name("foo", "BarBaz"),
        module_type_name("foo_bar", "Baz")
    );
    assert_ne!(
        module_type_name("foo_bar", "Baz"),
        module_type_name("fo_obar", "Baz")
    );
