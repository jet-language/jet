...
pub(super) fn check_bundle_opts_for_output_inner(
    ...
    let os_switches_before = CanonicalPass::ast_payload(bundle);
    let os_switches_identity_before = CanonicalPass::ast_identity(bundle);
    diags.extend(super::super::desugar_os_switches(bundle));
    CanonicalPass::record(
        "lowering",
        "sema.desugar-os-switches",
        "crates/jet-sema/src/Sema/OSTarget.rs",
        "ast",
        os_switches_before,
        os_switches_identity_before,
        "ast",
        CanonicalPass::ast_payload(bundle),
        CanonicalPass::ast_identity(bundle),
        "preserve",
    );
    let migrations_before = CanonicalPass::ast_payload(bundle);
    let migrations_identity_before = CanonicalPass::ast_identity(bundle);
    super::super::desugar_migrations(bundle);
    CanonicalPass::record(
        "lowering",
        "sema.desugar-migrations",
        "crates/jet-sema/src/Sema/SchemaMigration.rs",
        "ast",
        migrations_before,
        migrations_identity_before,
        "ast",
        CanonicalPass::ast_payload(bundle),
        CanonicalPass::ast_identity(bundle),
        "preserve",
    );
    let spreads_before = CanonicalPass::ast_payload(bundle);
    let spreads_identity_before = CanonicalPass::ast_identity(bundle);
    super::super::desugar_member_spreads(bundle);
    CanonicalPass::record(
        "lowering",
        "sema.desugar-member-spreads",
        "crates/jet-sema/src/Sema/MemberSpread.rs",
        "ast",
        spreads_before,
        spreads_identity_before,
        "ast",
        CanonicalPass::ast_payload(bundle),
        CanonicalPass::ast_identity(bundle),
        "preserve",
    ...
    let multi_head_before = CanonicalPass::ast_payload(bundle);
    let multi_head_identity_before = CanonicalPass::ast_identity(bundle);
    desugar_multi_head_functions(bundle, &mut diags);
    CanonicalPass::record(
        "lowering",
        "sema.desugar-multi-head-functions",
        "crates/jet-sema/src/Sema/Bundle.rs",
        "ast",
        multi_head_before,
        multi_head_identity_before,
        "ast",
        CanonicalPass::ast_payload(bundle),
        CanonicalPass::ast_identity(bundle),
        "preserve",
    ...
    for idx in 0..comptime_states.len() {
        ...
        comptime_states[idx].items = module.items.clone();
        comptime_states[idx].core_imports = ct_core_imports[idx].clone();
        comptime_states[idx].core_item_imports = ct_core_item_imports[idx].clone();
        for import in &module.imports {
            if let Some(target) = name_ledger.import_target(idx, import.span) {
                comptime_states[idx]
                    .imports
                    .insert(import.import_alias(), target);
            }
        }
        super::super::Registration::register_comptime_declarations(
            &module.items,
            &mut comptime_states[idx],
            !module.no_prelude,
        );
    }
    ...
    for (idx, module) in bundle.modules.iter_mut().enumerate() {
            ...
            .filter(|item| !matches!(item, Item::MarkerDecl(_) | Item::FactDecl(_)))
        {
            match item {
                ...
                Item::CModule(cm) => {
                    ...
                    let handle_facts = bundle
                        .cffi
                        .handle_facts
                        .iter()
                        .filter(|fact| fact.lib == cm.lib)
                        .cloned()
                        .collect::<Vec<_>>();
                    let close_adapters = bundle
                        .cffi
                        .close_adapters
                        .iter()
                        .filter(|adapter| adapter.lib == cm.lib)
                        .cloned()
                        .collect::<Vec<_>>();
                    if check_c_module_with_facts(
                        cm,
                        check_registry,
                        &handle_facts,
                        &close_adapters,
                        bundle.cffi.boundary_for(&cm.lib),
                        &mut diags,
                    ) {
                ...
                for s in &struct_infos {
                    ...
                    for (derive_name, _derive_span) in &s.derives {
                        ...
                        let layout_engine = jet_foundation::Layout::TargetLayoutEngine::new(
                            module.items.iter(),
                            jet_foundation::Layout::TargetLayout::from_build_facts(
                                &bundle.build_facts,
                            ),
                        );
        ...
        // S62 + D-LIB2: synthesis must happen before registration so the
        // synthesised Func nodes appear in both registries.
        ...
        super::super::Registration::normalize_operator_rhs(&mut module.items);
        register_type_methods(&module.items, &mut st.registry, &mut diags);
        register_patchable_methods(&module.items, &mut st.registry);
        register_cli_merge_methods(&module.items, &mut st.registry);
    ...
    if let (Some(module), Some(state)) =
        (bundle.modules.get(bundle.entry), states.get(bundle.entry))
    {
    ...
    for idx in 0..states.len() {
        ...
        // Auto-derived items are appended after the first registration pass;
        // normalize their bare operator RHS before any later consumer sees them.
        super::super::Registration::normalize_operator_rhs(&mut module.items);
        ...
        // Bundle auto-derives attach nested Equatable/Comparable methods after
        // the first TypeRegistry pass. Insert only methods that are still missing.
        register_missing_type_methods(
            &module.items,
            &mut states[idx].registry,
            &mut diags,
        );
    ...
    for (idx, module) in bundle.modules.iter().enumerate() {
        ...
        for imp in &module.imports {
            ...
            if let Some(profile) = crate::AST::target_profile_path(imp) {
                st.core_imports.insert(alias, profile);
                continue;
            }
    ...
    // Publication facts are collected by each body checker in its existing
    // TypeRegistry. Completion projects those typed rows into this shared panel
