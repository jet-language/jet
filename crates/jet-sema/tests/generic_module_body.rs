...
fn check_modules(sources: &[(&str, &str, &[(&str, usize)])]) -> (ProgramBundle, Vec<Diagnostic>) {
    ...
    for (module_idx, (path, src, targets)) in sources.iter().enumerate() {
            ...
            let import = program
                ...
                .find(|import| {
                    import.import_alias() == *alias
                        && !matches!(import.kind, jet_sema::AST::ImportKind::Unqualified { .. })
                })
                .unwrap();
        ...
        modules.push(LoadedModule {
            ...
            display: (*path).into(),
            alias: path.trim_end_matches(".jet").into(),
            imports: std::mem::take(&mut program.imports),
            items: std::mem::take(&mut program.items),
            script_body: std::mem::take(&mut program.script_body),
            source: (*src).into(),
...
"#;
    ...
    assert_eq!(
        items
            .iter()
            .filter(|item| matches!(item, Item::Struct(def) if def.name == "M5FirstBox"))
            .count(),
        1
    );
    for name in ["accepts_projection", "accepts_chain"] {
        let Item::Func(func) = items
            .iter()
            .find(|item| matches!(item, Item::Func(func) if func.name == name))
            .unwrap()
        else {
            unreachable!()
        };
...
"#,
    ...
    assert!(error_codes(&diagnostics).is_empty(), "{diagnostics:#?}");
    let names: Vec<_> = bundle.modules[0]
        .items
        .iter()
        .filter_map(|item| match item {
...
"#;
    ...
    assert!(!bundle.modules[2]
        .items
        .iter()
        .any(|item| matches!(item, Item::CodeModule(module) if module.name == "second")));
    assert!(bundle.modules[2]
        .items
        .iter()
        .any(|item| matches!(item, Item::CodeModule(module) if module.name == "different_arg")));
    assert!(bundle.modules[2].items.iter().any(
        |item| matches!(item, Item::CodeModule(module) if module.name == "different_template")
    ));
}
...
fn instance_fingerprint_is_nominal_and_ignores_body_shape() {
    ...
    assert_eq!(
        fp,
        only_instance_fingerprint(base, "pkg-b"),
        "manifest-less host paths are non-semantic"
    );

    let roots = std::env::temp_dir().join(format!(
        "jet_genmod_nominal_packages_{}",
        std::process::id()
    ));
    ...
    std::fs::write(
        package_a.join("pkg.jet"),
        "payload: { name: \"package-a\", version: \"1.0.0\" }",
    )
    .unwrap();
    std::fs::write(
        package_b.join("pkg.jet"),
        "payload: { name: \"package-b\", version: \"1.0.0\" }",
    )
    .unwrap();
    assert_ne!(
...
"#;
    ...
    assert!(items
        .iter()
        .any(|item| matches!(item, Item::Tag(t) if t.name == "M3Int4LawsAudited")));
    let Item::CodeModule(instance) = items
        .iter()
        .find(|item| matches!(item, Item::CodeModule(m) if m.name == "int_laws"))
        .unwrap()
    else {
        unreachable!()
    };
    let tagged = instance
        ...
        .iter()
        .find_map(|item| match item {
            Item::Func(f) if f.name == "audited" => Some(&f.params[0].ty),
            _ => None,
        })
        .unwrap();
    assert!(
        matches!(tagged, Type::Tagged { marker, inner } if matches!(marker, TagMarker::User(name) if name == "M3Int4LawsAudited") && **inner == Type::Int),
        "{tagged:?}"
    );
...
"#,
    ...
    assert!(
        error_codes(&diagnostics).contains(&"E2406"),
        "{diagnostics:#?}"
    );
}
...
"#,
    ...
    assert!(error_codes(&diagnostics).is_empty(), "{diagnostics:#?}");
    let inner = bundle.modules[0]
        .items
        .iter()
        .find_map(|item| match item {
            Item::CodeModule(module) if module.name == "text_outer_fixed" => Some(module),
            _ => None,
        })
        .expect("nested alias expanded into a checked module");
    let Item::Func(keep) = &inner.body.as_ref().unwrap()[0] else {
    ...
    let identity = inner
        .instance_identity
        .as_ref()
        .expect("nested applicative identity");
    assert_eq!(
        identity
            .applications
            .iter()
    ...
    let other = bundle.modules[0]
        .items
        .iter()
        .find_map(|item| match item {
            Item::CodeModule(module) if module.name == "other_outer_fixed" => Some(module),
            _ => None,
        })
...
"#,
    ...
    assert!(!plain
        ...
        .unwrap()
        .iter()
        .any(|item| matches!(item, Item::GenericModule(_) | Item::ModuleAlias(_))));
    let closed = items
        .iter()
        .find_map(|item| match item {
...
"#,
    ...
    let tests: Vec<_> = items
        ...
        .collect();
    assert_eq!(
        tests.iter().map(|t| t.name.as_deref()).collect::<Vec<_>>(),
        vec![Some("int_checks_identity"), Some("text_checks_identity")]
    );
