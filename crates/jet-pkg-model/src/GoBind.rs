...
impl std::fmt::Display for BindError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ToolFailed(detail) => write!(
                f,
                "`go build -buildmode=c-archive` rejected the source: {detail}"
            ),
        }
    }
}
...
pub fn bind(
    ...
    if functions.iter().any(|function| {
        function
            .params
            .iter()
            .any(|(_, ty)| matches!(ty, Scalar::Handle))
            || matches!(function.result, Some(Scalar::Handle))
    }) && std::mem::size_of::<usize>() != 8
    {
        return Err(BindError::Source(
            "Go `uintptr` handles need a 64-bit host ABI".into(),
        ));
    }
...
fn parse_exports(source: &str) -> Result<Vec<Function>, BindError> {
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        let Some(exported) = marker.strip_prefix("//export ").map(str::trim) else {
            continue;
        };
        if !is_ident(exported) {
            return Err(BindError::Source(format!(
                "`{exported}` is not a bindable Go export name"
            )));
        }
        let signature = lines
            .get(index + 1)
            .map(|line| line.trim())
            .ok_or_else(|| {
                BindError::Source(format!("export `{exported}` has no function declaration"))
            })?;
        let rest = signature.strip_prefix("func ").ok_or_else(|| {
            BindError::Source(format!(
                "export `{exported}` must be followed by its `func` declaration"
            ))
        })?;
        let open = rest.find('(').ok_or_else(|| {
            BindError::Source(format!("function `{exported}` has no parameter list"))
        })?;
        if rest[..open].trim() != exported {
            return Err(BindError::Source(format!(
                "export marker `{exported}` does not match function `{}`",
                rest[..open].trim()
            )));
        }
        let close = rest[open + 1..]
            ...
            .ok_or_else(|| {
                BindError::Source(format!(
                    "function `{exported}` has no closed parameter list"
                ))
            })?;
        ...
        if !raw_params.is_empty() {
            for raw in raw_params.split(',') {
                ...
                if !is_ident(name) || pieces.next().is_some() {
                    return Err(BindError::Source(format!(
                        "parameter `{}` in `{exported}` must use `name type`",
                        raw.trim()
                    )));
                }
        ...
        let result = if tail.is_empty() || tail.starts_with('{') {
            None
        } else {
    ...
    if functions.is_empty() {
        return Err(BindError::Source(
            "no supported `//export Name` Go functions were found".into(),
        ));
    }
    Ok(functions)
}
...
fn scalar(value: &str) -> Option<Scalar> {
    match value {
        ...
        "float64" => Some(Scalar::Float),
        "uintptr" => Some(Scalar::Handle),
        _ => None,
    }
}
...
fn render(lib: &str, functions: &[Function]) -> String {
    ...
    for function in functions {
        ...
        if matches!(function.result, Some(Scalar::Handle)) {
            out.push_str("Handle{ value: ");
        }
        out.push_str("abi.");
        out.push_str(&function.name);
        out.push('(');
...
fn render_public_params(out: &mut String, params: &[(String, Scalar)]) {
    ...
    for (index, (name, ty)) in params.iter().enumerate() {
        ...
        out.push_str(name);
        out.push_str(": ");
        out.push_str(if matches!(ty, Scalar::Handle) {
            "Handle"
        } else {
            ty.jet()
        });
...
fn supervised_output(
    command: &mut Command,
    timeout: Duration,
) -> Result<SupervisedOutput, BindError> {
    ...
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| BindError::IO("could not supervise `go` stdout".into()))?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| BindError::IO("could not supervise `go` stderr".into()))?;
    ...
    let status = loop {
        match child
            .try_wait()
            .map_err(|e| BindError::IO(format!("could not supervise `go`: {e}")))?
        {
            ...
            None if Instant::now() >= deadline => {
                ...
                return Err(BindError::ToolFailed(
                    "the compiler exceeded the 60 second limit".into(),
                ));
            }
    ...
    let _ = stdout_reader
        .join()
        .map_err(|_| BindError::IO("`go` stdout reader failed".into()))??;
    let stderr = stderr_reader
        .join()
        .map_err(|_| BindError::IO("`go` stderr reader failed".into()))??;
...
fn bounded_read(mut input: impl Read, limit: usize) -> Result<Vec<u8>, BindError> {
    ...
    loop {
        let count = input
            .read(&mut buffer)
            .map_err(|e| BindError::IO(format!("could not read foreign compiler output: {e}")))?;
        if count == 0 {
            break;
        }
...
fn launder(stderr: &[u8]) -> String {
    String::from_utf8_lossy(stderr)
        ...
        .map(|line| {
            line.rsplit_once(':')
                .map_or(line, |(_, detail)| detail.trim())
        })
