...
static int decode(lua_State*L,const char*input){{lua_getglobal(L,"__jet_decode");lua_pushstring(L,input);return lua_pcall(L,1,1,0)==LUA_OK;}}
static int encode(lua_State*L){{lua_getglobal(L,"__jet_encode");lua_insert(L,-2);return lua_pcall(L,1,1,0)==LUA_OK;}}
...
function __jet_decode(text) return decode(text) end
function __jet_encode(value) return encode(value,{},0) end
"#;
