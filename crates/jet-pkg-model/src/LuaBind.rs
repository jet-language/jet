...
fn discover(source: &str) -> Result<Vec<String>, BindError> {
    ...
    for name in &functions { if functions.contains(&format!("{name}_typed")) { return Err(BindError::Source(format!("Lua function `{name}` collides with generated adapter `{name}_typed`"))) } }
    Ok(functions)
}
...
fn reserved(v:&str)->bool{matches!(v,"open"|"cancel"|"close"|"Session"|"LuaError")||crate::Syntax::JET_KEYWORD_LIST.contains(&v)||crate::Syntax::JET_TYPE_LIST.contains(&v)}
