...
fn reserved(v:&str)->bool{matches!(v,"open"|"cancel"|"close"|"Session"|"TableView"|"LuaError"|"view_get_int"|"view_set_int"|"view_release")||crate::Syntax::JET_KEYWORD_LIST.contains(&v)||crate::Syntax::JET_TYPE_LIST.contains(&v)}
...
mod tests {
    ...
    #[test] fn rejects_generated_table_view_abi_names(){let error=super::discover("function view_release(input) return input end").unwrap_err();assert!(error.to_string().contains("`view_release` cannot be exported"));}
