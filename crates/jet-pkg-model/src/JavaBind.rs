    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ToolFailed(tool, detail) => {
                write!(f, "`{tool}` rejected the JVM binding input: {detail}")
            }
        }
    }
}
...
pub fn bind(
    ...
    let class = source_path
        ...
        .ok_or_else(|| {
            BindError::Source(
                "Java source needs an identifier filename matching its public class".into(),
            )
        })?;
    std::fs::create_dir_all(cache)
        .map_err(|e| BindError::IO(format!("could not create JVM binding cache: {e}")))?;
    let classes = cache.join(format!("{lib}.classes"));
    let _ = std::fs::remove_dir_all(&classes);
    std::fs::create_dir_all(&classes)
        .map_err(|e| BindError::IO(format!("could not create JVM class cache: {e}")))?;
    run(
        Command::new("javac")
    ...
    if !jvm_dir.join(jvm_name()).is_file() {
        return Err(BindError::Source(
            "provisioned OpenJDK has no embedded libjvm runtime".into(),
        ));
    }
    ...
    run(
        Command::new("cc")
            .args(["-std=c11", "-fPIC", "-c", "-I"])
            .arg(java_home.join("include"))
            .arg("-I")
            .arg(java_home.join("include").join(os_include()))
            .arg(&bridge)
            .arg("-o")
            .arg(&object),
        "cc",
    ...
    let bound = surface
        ...
        .iter()
        .map(|m| m.name.clone())
        .chain(std::iter::once("new".into()))
        .collect();
    Ok(BindResult {
        source: render_jet(lib, &surface),
        bound,
        archive,
        jvm_dir,
        provenance,
...
fn run(command: &mut Command, tool: &'static str) -> Result<Output, BindError> {
    ...
    if result.success {
        ...
        Err(BindError::ToolFailed(tool, launder(&result.stderr)))
    }
}
...
fn parse_javap(class: &str, text: &str) -> Result<Surface, BindError> {
    ...
    for pair in lines.windows(2) {
        let sig = pair[0].trim();
        let desc = pair[1].trim();
        if !sig.starts_with("public ") || !desc.starts_with("descriptor:") {
            continue;
        }
        let descriptor = desc.trim_start_matches("descriptor:").trim();
        if sig.contains(&format!(" {class}(")) || sig.starts_with(&format!("public {class}(")) {
            let (params, result) = descriptor_types(descriptor)?;
            if !matches!(result, Scalar::Void) {
                return Err(BindError::Source(
                    "JVM constructor descriptor must return void".into(),
                ));
            }
            if ctor.replace(params).is_some() {
                return Err(BindError::Source(
                    "overloaded constructors need an explicit overlay".into(),
                ));
            }
            continue;
        }
        let open = sig
            .find('(')
            .ok_or_else(|| BindError::Source("malformed javap method signature".into()))?;
        let before = sig[..open].trim();
        let name = before.split_whitespace().last().unwrap_or_default();
        if !ident(name) {
            continue;
        }
        ...
        if matches!(result, Scalar::Void) {
            return Err(BindError::Source(format!(
                "void Java method `{name}` needs a typed overlay"
            )));
        }
        methods.push(Method {
            name: name.into(),
    ...
    let ctor = ctor.ok_or_else(|| {
        BindError::Source("no supported public constructor was discovered".into())
    })?;
    if methods.is_empty() {
        return Err(BindError::Source(
            "no supported public Java methods were discovered".into(),
        ));
    }
    Ok(Surface {
        class: class.into(),
...
fn descriptor_types(value: &str) -> Result<(Vec<Scalar>, Scalar), BindError> {
    let close = value
        .find(')')
        .ok_or_else(|| BindError::Source("malformed JVM descriptor".into()))?;
    if !value.starts_with('(') {
        return Err(BindError::Source("malformed JVM descriptor".into()));
    }
    let mut params = Vec::new();
    for b in value[1..close].bytes() {
        params.push(match b {
            b'J' => Scalar::Int,
            b'D' => Scalar::Float,
            _ => {
                return Err(BindError::Source(format!(
                    "unsupported JVM descriptor `{value}`; use long/double/void"
                )))
            }
        })
    }
    let result = match value.as_bytes().get(close + 1) {
        Some(b'J') => Scalar::Int,
        Some(b'D') => Scalar::Float,
        Some(b'V') => Scalar::Void,
        _ => {
            return Err(BindError::Source(format!(
                "unsupported JVM descriptor `{value}`; use long/double/void"
            )))
...
fn render_jet(lib: &str, s: &Surface) -> String {
    ...
    for m in &s.methods {
        ...
        args(&mut o, m.params.len(), start);
        o.push_str(")\n    if abi.take_error() != 0 {");
        if matches!(m.result, Scalar::Void) {
            o.push_str(" panic(\"Java exception\") }");
        } else {
            o.push_str(" return Err(JavaError.Exception) }\n    return Ok(value)");
        }
        o.push_str("\n}\n\n");
    }
    o
}
fn params_jet(o: &mut String, p: &[Scalar]) {
...
fn args(o: &mut String, n: usize, start: usize) {
    for i in 0..n {
        if i > 0 {
            o.push_str(", ")
        }
        o.push_str(&format!("arg{}", i + start - start))
    }
}
...
"#,
    ...
    for m in &s.methods {
        o.push_str(m.result.c());
        o.push(' ');
        o.push_str(&format!("{abi}_{}(", m.name));
        let mut first = true;
        if !m.is_static {
            o.push_str("int64_t handle");
            first = false;
        }
        ...
        o.push_str("){last_error=0;JNIEnv*e=env();if(!e)");
        default_return(&mut o, m.result);
        o.push_str(&format!(
            "jclass c=(*e)->FindClass(e,\"{}\");if(!c){{fail(e);",
            c_escape(&s.class)
        ));
        default_return(&mut o, m.result);
        o.push('}');
        ...
        if !m.is_static {
            o.push_str("jobject obj=get_handle(e,handle);if(!obj)");
            default_return(&mut o, m.result);
        }
        o.push_str(&format!(
            "jmethodID id=(*e)->Get{}MethodID(e,c,\"{}\",\"{}\");if(!id){{fail(e);",
            if m.is_static { "Static" } else { "" },
            m.name,
            descriptor(&m.params, m.result)
        ));
        default_return(&mut o, m.result);
        o.push('}');
        emit_jvalues(&mut o, &m.params, 0);
        match m.result {
            Scalar::Int => o.push_str(&format!(
                "jlong value=(*e)->Call{}LongMethodA(e,{target},id,a);",
                if m.is_static { "Static" } else { "" }
            )),
            Scalar::Float => o.push_str(&format!(
                "jdouble value=(*e)->Call{}DoubleMethodA(e,{target},id,a);",
                if m.is_static { "Static" } else { "" }
            )),
            Scalar::Void => o.push_str(&format!(
                "(*e)->Call{}VoidMethodA(e,{target},id,a);",
                if m.is_static { "Static" } else { "" }
            )),
        }
        o.push_str("if((*e)->ExceptionCheck(e)){fail(e);");
        default_return(&mut o, m.result);
        o.push('}');
        if !matches!(m.result, Scalar::Void) {
...
fn params_c(o: &mut String, p: &[Scalar], _: usize) {
    for (i, t) in p.iter().enumerate() {
        if i > 0 {
            o.push(',')
        }
...
fn emit_jvalues(o: &mut String, p: &[Scalar], _: usize) {
    o.push_str(&format!("jvalue a[{}];", p.len().max(1)));
    for (i, t) in p.iter().enumerate() {
        o.push_str(&format!("a[{i}].{}=arg{i};", t.jni_field()))
    }
}
fn descriptor(p: &[Scalar], r: Scalar) -> String {
    let mut o = "(".to_string();
    for t in p {
        o.push(match t {
            Scalar::Int => 'J',
...
fn default_return(o: &mut String, t: Scalar) {
    match t {
        Scalar::Int => o.push_str("return 0;"),
        Scalar::Float => o.push_str("return 0.0;"),
        Scalar::Void => o.push_str("return;"),
    }
}
