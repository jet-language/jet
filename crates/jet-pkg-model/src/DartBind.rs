...
#[derive(Clone)] struct Function { name:String,params:Vec<Param>,result:Scalar }
...
pub fn bind(path:&Path,source:&str,lib:&str,cache:&Path)->Result<BindResult,BindError>{
    ...
    let result=BindResult{source:render_jet(lib,&functions),host_source:render_dart_host(&canonical,lib,&functions),host_rust:render_host_rust(lib,&functions),bound:functions.iter().map(|v|v.name.clone()).collect(),archive,provenance:format!("schema=jet-dart-bind-v1\nsha256={}\ndart={}\ncontract={}\n",crate::SHA256::sha256_hex(&identity),dart.display(),canonical.display())};
...
pub fn build_compute(guest_rust:&str,host_rust:&str,ffi:Option<&crate::AST::FfiLink>,clinks:&[String],lib:&str,cache:&Path)->Result<PathBuf,BindError>{
    ...
    if let Some(link)=ffi { command.arg("--extern").arg(format!("{}={}",link.crate_name,link.rlib_path.display()));if link.deps_dir.is_dir(){command.arg("-L").arg(format!("dependency={}",link.deps_dir.display()));} }
...
fn render_jet(lib:&str,functions:&[Function])->String{let abi=format!("jet_dart_{lib}");let mut out=format!("@Extern module c.{abi} {{\n    fn take_error() -> Int = \"{abi}_take_error\"\n");for f in functions{out.push_str("    fn ");out.push_str(&f.name);jet_params(&mut out,&f.params);out.push_str(" -> ");out.push_str(f.result.jet());out.push_str(" = \"");out.push_str(&format!("{abi}_call_{}\"\n",f.name));}out.push_str("}\nuse c.");out.push_str(&abi);out.push_str(" as abi\n\npub enum DartError { NotInitialized CallbackUnavailable }\n\n");for f in functions{out.push_str("pub fn ");out.push_str(&f.name);jet_params(&mut out,&f.params);out.push_str(" -> ");out.push_str(f.result.jet());out.push_str(" ? DartError {\n    result :: abi.");out.push_str(&f.name);call_args(&mut out,&f.params);out.push_str("\n    code :: abi.take_error()\n    if code == 1 { return Err(DartError.NotInitialized) }\n    if code == 2 { return Err(DartError.CallbackUnavailable) }\n    return Ok(result)\n}\n\n");}out}
