...
fn discover_functions(php: &Path, script: &Path) -> Result<Vec<String>, BindError> {
    ...
    let text = std::str::from_utf8(&output)
        .map_err(|_| BindError::Source("PHP tokenizer returned non-UTF-8 metadata".into()))?;
    let mut out = Vec::new();
    for line in text.lines() {
        if let Some(name) = line.strip_prefix("E\tARG\t") {
            return Err(BindError::Source(format!(
                "PHP function `{name}` must take one required positional argument by value"
            )));
        }
        let name = line
            .strip_prefix("N\t")
            .ok_or_else(|| BindError::Source("PHP tokenizer returned malformed metadata".into()))?;
        if !ident(name) {
            return Err(BindError::Source(format!(
                "PHP function `{name}` cannot be projected as a Jet identifier"
            )));
        }
        if reserved(name) {
            return Err(BindError::Source(format!(
                "PHP function `{name}` uses a reserved generated binding name"
            )));
        }
        if out.iter().any(|v: &String| v.eq_ignore_ascii_case(name)) {
            return Err(BindError::Source(format!(
                "PHP function `{name}` is declared more than once"
            )));
        }
    ...
    if out.is_empty() {
        return Err(BindError::Source(
            "no top-level named PHP functions were found".into(),
        ));
    }
    Ok(out)
}
...
fn lint(php: &Path, script: &Path) -> Result<(), BindError> {
    ...
    command.arg("-n").arg("-l").arg(script);
    match run_capture(&mut command, "php") {
        Ok(_) => Ok(()),
        Err(BindError::ToolFailed(_, _)) => Err(BindError::ToolFailed(
            "php",
            "the PHP source has a parse error".into(),
        )),
        Err(error) => Err(error),
    }
}
...
fn require_supported_host(unix: bool) -> Result<(), BindError> {
    if unix {
        ...
        Err(BindError::Source(
            "persistent PHP bindings require a POSIX host process supervisor".into(),
        ))
    }
}
...
"#;
    ...
    fn discovery_rejects_by_reference_variadic_default_and_multiple_arguments() {
        for source in [
            ...
            assert!(
                error
                    .to_string()
                    .contains("one required positional argument by value"),
                "{error}"
            );
        }
    }
    ...
    fn worker_loads_projected_composer_autoload_before_user_source() {
        ...
        std::fs::write(
            &autoload_path,
            "<?php function run($input) { return $input + 1; }\n",
        )
        .unwrap();
