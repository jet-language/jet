...
impl std::fmt::Display for BindError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ToolFailed(t, v) => write!(f, "`{t}` rejected the Tcl binding bridge: {v}"),
        }
    }
}
...
pub fn bind(source: &str, lib: &str, cache: &Path) -> Result<BindResult, BindError> {
    ...
    if !include.join("tcl.h").is_file() || !lib_dir.join(lib_name()).is_file() {
        return Err(BindError::Source(
            "the provisioned Tcl runtime has no embeddable headers or shared library".into(),
        ));
    }
    std::fs::create_dir_all(cache)
        .map_err(|e| BindError::IO(format!("could not create Tcl binding cache: {e}")))?;
    let stem = format!("jet_tcl_{lib}");
    let c = cache.join(format!("{stem}.c"));
    let obj = cache.join(format!("{stem}.o"));
    let archive = cache.join(format!("lib{stem}.a"));
    std::fs::write(&c, render_c(lib, source))
        .map_err(|e| BindError::IO(format!("could not write Tcl bridge: {e}")))?;
    run(
        Command::new("cc")
    ...
    let mut identity = b"jet-tcl-bind-v1\0".to_vec();
...
fn render_c(lib: &str, seed: &str) -> String {
    let abi = format!("jet_tcl_{lib}");
    format!(
        r#"#include <tcl.h>
#include <stdint.h>
...
fn tool_root(tool: &str) -> Option<PathBuf> {
    let path = std::env::var_os("PATH")?;
    for dir in std::env::split_paths(&path) {
        let candidate = dir.join(tool);
        if candidate.is_file() {
            let exe = std::fs::canonicalize(candidate).ok()?;
            return exe.parent()?.parent().map(Path::to_path_buf);
        }
    }
    None
}
