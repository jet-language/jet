...
#[cfg(test)]mod tests{#[test]fn reflection_schema_projects_typed_owned_surface(){let s=super::parse_surface(b"CLASS\tCounter\nCTOR\tint\nMETHOD\tadd\tI\tint\tint\nMETHOD\tscale\tS\tfloat\tfloat\n","Counter").unwrap();let jet=super::render_jet("counter",&s);assert!(jet.contains("pub struct Handle { value: Int }"));assert!(jet.contains("impl Handle.Close"));assert!(jet.contains("fn close(^self)"));assert!(jet.contains("pub fn close(^handle: Handle)"));assert!(jet.contains("pub fn add(handle: Handle, arg0: Int) Int DotNetError! -[FFI.DotNet]>"));assert!(jet.contains("ResourceLimit"));let cs=super::render_cs("counter",&s);assert!(cs.contains("GCHandle[Capacity]"));assert!(cs.contains("Generations[slot]"));assert!(cs.contains("jet_cs_counter_add"));assert!(cs.contains("UnmanagedCallersOnly"));}}
