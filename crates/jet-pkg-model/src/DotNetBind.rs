...
impl std::fmt::Display for BindError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ToolFailed(t, v) => write!(f, "`{t}` rejected the .NET binding input: {v}"),
        }
    }
}
...
pub fn bind(path: &Path, source: &str, lib: &str, cache: &Path) -> Result<BindResult, BindError> {
    ...
    let class = path
        ...
        .ok_or_else(|| {
            BindError::Source(
                "C# source needs an identifier filename matching its public class".into(),
            )
        })?;
    std::fs::create_dir_all(cache)
        .map_err(|e| BindError::IO(format!("could not create .NET binding cache: {e}")))?;
    let build = cache.join(format!(".dotnet-build-{lib}"));
    let _ = std::fs::remove_dir_all(&build);
    std::fs::create_dir_all(&build)
        .map_err(|e| BindError::IO(format!("could not create .NET build directory: {e}")))?;
    std::fs::write(build.join(format!("{class}.cs")), source)
        .map_err(|e| BindError::IO(format!("could not stage C# source: {e}")))?;
    std::fs::write(
        build.join("Inspect.csproj"),
        PROJECT.replace("@OUTPUT@", "Exe"),
    )
    .map_err(|e| BindError::IO(format!("could not write reflection project: {e}")))?;
    std::fs::write(build.join("Program.cs"), INSPECTOR)
        .map_err(|e| BindError::IO(format!("could not write reflection inspector: {e}")))?;
    run(
        Command::new("dotnet")
            .args(["build"])
            .arg(build.join("Inspect.csproj"))
            .args([
                "-c",
                "Release",
                "--nologo",
    ...
    let inspected = run(
        Command::new("dotnet")
            .arg(build.join("bin/Release/net8.0/Inspect.dll"))
            .arg(class),
        "dotnet",
    )?;
    let surface = parse_surface(&inspected.stdout, class)?;
    std::fs::remove_file(build.join("Program.cs"))
        .map_err(|e| BindError::IO(format!("could not replace reflection inspector: {e}")))?;
    std::fs::write(build.join("Bridge.cs"), render_cs(lib, &surface))
        .map_err(|e| BindError::IO(format!("could not write managed .NET bridge: {e}")))?;
    std::fs::write(
        build.join("Inspect.csproj"),
        PROJECT.replace("@OUTPUT@", "Library"),
    )
    .map_err(|e| BindError::IO(format!("could not write managed bridge project: {e}")))?;
    run(
        Command::new("dotnet")
            .args(["build"])
            .arg(build.join("Inspect.csproj"))
            .args([
                ...
                "-v:q",
                "--disable-build-servers",
                "-p:AssemblyName=JetBinding",
            ]),
        "dotnet",
    )?;
    let managed = cache.join(format!("{lib}.dotnet"));
    let _ = std::fs::remove_dir_all(&managed);
    std::fs::create_dir_all(&managed)
        .map_err(|e| BindError::IO(format!("could not create managed runtime cache: {e}")))?;
    let assembly = managed.join("JetBinding.dll");
    std::fs::copy(build.join("bin/Release/net8.0/JetBinding.dll"), &assembly)
        .map_err(|e| BindError::IO(format!("could not retain managed bridge assembly: {e}")))?;
    let runtime_config = managed.join("JetBinding.runtimeconfig.json");
    std::fs::write(&runtime_config, RUNTIME_CONFIG)
        .map_err(|e| BindError::IO(format!("could not write .NET runtime config: {e}")))?;
    let hostfxr = find_hostfxr()?;
    let stem = format!("jet_cs_{lib}");
    let bridge = build.join(format!("{stem}.c"));
    let object = build.join(format!("{stem}.o"));
    let archive = cache.join(format!("lib{stem}.a"));
    std::fs::write(
        &bridge,
        render_c(lib, &surface, &hostfxr, &assembly, &runtime_config),
    )
    .map_err(|e| BindError::IO(format!("could not write native hostfxr bridge: {e}")))?;
    run(
        Command::new("cc")
            .args(["-std=c11", "-fPIC", "-c"])
            .arg(&bridge)
    ...
    let mut identity = b"jet-dotnet-bind-v1\0".to_vec();
...
fn parse_surface(bytes: &[u8], expected: &str) -> Result<Surface, BindError> {
    let text = std::str::from_utf8(bytes).map_err(|_| {
        BindError::Source("the .NET reflection inspector returned non-UTF-8 metadata".into())
    })?;
    let mut class = None;
    let mut ctor = None;
    ...
    for line in text.lines() {
        ...
        match f.first().copied() {
            Some("CLASS") if f.len() == 2 => class = Some(f[1].to_string()),
            Some("CTOR") => {
                let params = parse_scalars(&f[1..], "constructor")?;
                if ctor.replace(params).is_some() {
                    return Err(BindError::Source(
                        "overloaded C# constructors need an explicit overlay".into(),
                    ));
                }
            }
            Some("METHOD") if f.len() >= 4 => {
                if !ident(f[1]) {
                    continue;
                }
                if !names.insert(f[1].to_string()) {
                    return Err(BindError::Source(format!(
                        "overloaded C# method `{}` needs an explicit overlay",
                        f[1]
                    )));
                }
                let result = Scalar::parse(f[3]).ok_or_else(|| {
                    BindError::Source(format!(
                        "C# method `{}` uses an unsupported return type; use long or double",
                        f[1]
                    ))
                })?;
                let params = parse_scalars(&f[4..], f[1])?;
                methods.push(Method {
                    name: f[1].into(),
                    is_static: f[2] == "S",
                    result,
            ...
            Some("METHOD") => {
                return Err(BindError::Source(
                    "malformed .NET reflection metadata".into(),
                ))
            }
            _ => {}
        }
    ...
    let class = class.ok_or_else(|| {
        BindError::Source(format!("public C# class `{expected}` was not discovered"))
    })?;
    let ctor = ctor.ok_or_else(|| {
        BindError::Source("no supported public C# constructor was discovered".into())
    })?;
    if methods.is_empty() {
        return Err(BindError::Source(
            "no supported public C# methods were discovered".into(),
        ));
    }
    Ok(Surface {
        class,
...
fn parse_scalars(values: &[&str], member: &str) -> Result<Vec<Scalar>, BindError> {
    values
        .iter()
        .map(|v| {
            Scalar::parse(v).ok_or_else(|| {
                BindError::Source(format!(
                    "C# member `{member}` uses an unsupported parameter type; use long or double"
                ))
            })
        })
        .collect()
...
fn render_cs(lib: &str, s: &Surface) -> String {
    ...
    o.push_str(&abi);
    o.push_str("_take_error\")] public static long ");
    o.push_str(&abi);
    o.push_str(
        "_take_error(){var v=error;error=0;return v;}\n    [UnmanagedCallersOnly(EntryPoint=\"",
    );
    o.push_str(&abi);
    o.push_str("_close\")] public static void ");
    o.push_str(&abi);
    o.push_str("_close(long raw){try{Release(raw);}catch{error=2;}}\n    [UnmanagedCallersOnly(EntryPoint=\"");
    o.push_str(&abi);
    o.push_str("_new\")] public static long ");
    o.push_str(&abi);
    o.push_str("_new(");
    cs_params(&mut o, &s.ctor);
    o.push_str("){try{var value=new ");
    o.push_str(&s.class);
    o.push('(');
    args(&mut o, s.ctor.len());
    o.push_str(");return Store(value);}catch(InsufficientMemoryException){error=3;return 0;}catch{error=1;return 0;}}\n");
    for m in &s.methods {
        let entry = format!("{abi}_{}", m.name);
        o.push_str("    [UnmanagedCallersOnly(EntryPoint=\"");
        o.push_str(&entry);
        o.push_str("\")] public static ");
        o.push_str(m.result.cs());
        o.push(' ');
        o.push_str(&entry);
        o.push('(');
        if !m.is_static {
            o.push_str("long handle");
            if !m.params.is_empty() {
                o.push(',')
            }
        }
        cs_params(&mut o, &m.params);
        o.push_str("){try{return ");
        if m.is_static {
            o.push_str(&s.class)
        } else {
            o.push_str("Get(handle)")
        }
        o.push('.');
        o.push_str(&m.name);
        o.push('(');
        args(&mut o, m.params.len());
        o.push_str(");}catch{error=1;return default;}}\n");
    }
    o.push_str("}\n");
    o
}
fn cs_params(o: &mut String, p: &[Scalar]) {
    for (i, t) in p.iter().enumerate() {
        if i > 0 {
            o.push(',')
        }
        o.push_str(t.cs());
        o.push_str(&format!(" arg{i}"))
    }
}
fn args(o: &mut String, n: usize) {
    for i in 0..n {
        if i > 0 {
            o.push(',')
        }
...
fn jet_params(o: &mut String, p: &[Scalar]) {
    for (i, t) in p.iter().enumerate() {
        if i > 0 {
            o.push_str(", ")
        }
        o.push_str(&format!("arg{i}: {}", t.jet()))
    }
}

fn render_c(lib: &str, s: &Surface, hostfxr: &Path, assembly: &Path, config: &Path) -> String {
    let abi = format!("jet_cs_{lib}");
    let mut wrappers = String::new();
    wrappers.push_str(&format!("int64_t {abi}_take_error(void){{typedef int64_t(*F)(void);F f=(F)entry(\"{abi}_take_error\");return f?f():1;}}\nvoid {abi}_close(int64_t h){{typedef void(*F)(int64_t);F f=(F)entry(\"{abi}_close\");if(f)f(h);}}\n"));
    wrappers.push_str(&format!("int64_t {abi}_new("));
    c_params(&mut wrappers, &s.ctor);
    wrappers.push_str("){typedef int64_t(*F)(");
    c_types(&mut wrappers, &s.ctor);
    wrappers.push_str(&format!(");F f=(F)entry(\"{abi}_new\");return f?f("));
    args(&mut wrappers, s.ctor.len());
    wrappers.push_str("):0;}\n");
    for m in &s.methods {
        wrappers.push_str(m.result.c());
        wrappers.push(' ');
        wrappers.push_str(&format!("{abi}_{}(", m.name));
        let types = m.params.clone();
        if !m.is_static {
            wrappers.push_str("int64_t handle");
            if !m.params.is_empty() {
                wrappers.push(',')
            }
        }
        c_params(&mut wrappers, &m.params);
        wrappers.push_str("){typedef ");
        wrappers.push_str(m.result.c());
        wrappers.push_str("(*F)(");
        if !m.is_static {
            wrappers.push_str("int64_t");
            if !m.params.is_empty() {
                wrappers.push(',')
            }
        }
        c_types(&mut wrappers, &types);
        wrappers.push_str(&format!(");F f=(F)entry(\"{abi}_{}\");return f?f(", m.name));
        if !m.is_static {
            wrappers.push_str("handle");
            if !m.params.is_empty() {
                wrappers.push(',')
            }
        }
        args(&mut wrappers, m.params.len());
        wrappers.push_str(if m.result == Scalar::Float {
            "):0.0;}\n"
        } else {
            "):0;}\n"
        });
    ...
    NATIVE_C
        .replace("@HOSTFXR@", &c_escape(&hostfxr.to_string_lossy()))
        .replace("@ASSEMBLY@", &c_escape(&assembly.to_string_lossy()))
        .replace("@CONFIG@", &c_escape(&config.to_string_lossy()))
        .replace("@TYPE@", "JetBinding.EntryPoints, JetBinding")
        .replace("@WRAPPERS@", &wrappers)
}
fn c_params(o: &mut String, p: &[Scalar]) {
    for (i, t) in p.iter().enumerate() {
        if i > 0 {
            o.push(',')
        }
...
fn c_types(o: &mut String, p: &[Scalar]) {
    for (i, t) in p.iter().enumerate() {
        if i > 0 {
            o.push(',')
        }
...
fn find_hostfxr() -> Result<PathBuf, BindError> {
    let root = std::env::var_os("DOTNET_ROOT")
        .map(PathBuf::from)
        .or_else(|| {
            std::fs::canonicalize(which("dotnet")?)
                .ok()
                .and_then(|p| p.parent().map(Path::to_path_buf))
        })
        .ok_or(BindError::ToolMissing("DOTNET_ROOT"))?;
    let dir = root.join("host/fxr");
    let mut versions = std::fs::read_dir(&dir)
        .map_err(|e| BindError::IO(format!("could not inspect provisioned hostfxr: {e}")))?
    ...
    versions
        ...
        .map(|p| p.join(name))
        .find(|p| p.is_file())
        .ok_or_else(|| BindError::Source("the provisioned .NET SDK has no hostfxr runtime".into()))
}
...
fn run(command: &mut Command, tool: &'static str) -> Result<Output, BindError> {
    ...
    if result.success {
        ...
        Err(BindError::ToolFailed(
            tool,
            launder(&result.stdout, &result.stderr),
        ))
    }
}
...
fn launder(stdout: &[u8], stderr: &[u8]) -> String {
    let text = format!(
        "{}\n{}",
    ...
    if let Some(line) = text
        .lines()
        .map(str::trim)
        .find(|s| s.contains(": error CS"))
    {
        if let Some((_, tail)) = line.split_once(": error CS") {
            if let Some((_, detail)) = tail.split_once(": ") {
                let detail = detail.rsplit_once(" [").map_or(detail, |v| v.0);
    ...
    text.lines()
        ...
        .find(|s| {
            !s.is_empty()
                && !s.starts_with("Determining projects")
                && !s.starts_with("Restore")
                && !s.starts_with("Build FAILED")
                && !s.starts_with("The build failed")
        })
        .map(|s| {
            s.rsplit_once(": ")
                .map_or(s, |x| x.1)
        ...
        .unwrap_or_else(|| "the managed tool returned a failure status".into())
}

#[cfg(test)]
mod tests {
    #[test]
    fn reflection_schema_projects_typed_owned_surface() {
        let s=super::parse_surface(b"CLASS\tCounter\nCTOR\tint\nMETHOD\tadd\tI\tint\tint\nMETHOD\tscale\tS\tfloat\tfloat\n","Counter").unwrap();
        let jet = super::render_jet("counter", &s);
        assert!(jet.contains("pub struct Handle { value: Int }"));
        assert!(jet.contains("impl Handle.Close"));
        ...
        assert!(cs.contains("GCHandle[Capacity]"));
        assert!(cs.contains("Generations[slot]"));
        assert!(cs.contains("jet_cs_counter_add"));
