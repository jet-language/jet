...
pub(crate) fn render_supervisor_c(abi:&str,executable:&Path,worker:&Path,script:&Path,wrappers:&str,exec_args:&str)->String{format!(r#"#include <errno.h>
#include <fcntl.h>
#include <poll.h>
#include <pthread.h>
#include <signal.h>
...
static const char executable_path[]="{}";static const char worker_path[]="{}";static const char script_path[]="{}";
...
static void reap(pid_t pid){{if(pid>0){{if(kill(-pid,SIGKILL)<0&&errno==ESRCH)kill(pid,SIGKILL);while(waitpid(pid,0,0)<0&&errno==EINTR){{}}}}}}
...
{wrappers}"#,c_escape(&executable.to_string_lossy()),c_escape(&worker.to_string_lossy()),c_escape(&script.to_string_lossy()))}
...
fn reserved_jet_function(v:&str)->bool{matches!(v,"open"|"cancel"|"close"|"Session"|"PowerShellError")||crate::Syntax::JET_KEYWORD_LIST.contains(&v)||crate::Syntax::JET_TYPE_LIST.contains(&v)}
