...
#[derive(Debug,Clone,Copy)] enum Scalar { Int,Float }
...
#[derive(Debug,Clone)] struct Constraint { low:String,high:String }
#[derive(Debug)] struct Param { name:String,scalar:Scalar,constraint:Option<Constraint> }
#[derive(Debug)] struct Routine { name:String,symbol:String,params:Vec<Param>,result:Scalar }
...
pub fn bind(spec_path:&Path,spec:&str,lib:&str,cache:&Path)->Result<BindResult,BindError>{
    ...
    let source=render_jet(lib,&routines);let bound:Vec<String>=routines.iter().map(|v|v.name.clone()).collect();let constraints=routines.iter().flat_map(|r|r.params.iter().filter_map(|p|p.constraint.as_ref().map(|c|format!("{}.{}={}..{}",r.name,p.name,c.low,c.high)))).collect::<Vec<_>>().join(",");let _=std::fs::remove_dir_all(&build);
    ...
    Ok(BindResult{source,bound,archive,runtime_dir,provenance})
}

fn parse(source:&str)->Result<Vec<Routine>,BindError>{
    let text=source.lines().map(|v|v.split("--").next().unwrap_or("")).collect::<Vec<_>>().join(" ");
    let mut constraints:BTreeMap<String,(Scalar,Constraint)>=BTreeMap::new();let mut routines=Vec::new();let mut names=BTreeMap::new();let mut symbols=BTreeMap::new();
    for raw in statements(&text){let statement=raw.split_whitespace().collect::<Vec<_>>().join(" ");if statement.is_empty(){continue}let lower=statement.to_ascii_lowercase();
        ...
        let Some(function_at)=lower.find("function ") else {
            if lower.contains("export") {
                return Err(BindError::Source("exported Ada procedures are unsupported; export scalar functions instead".into()))
            }
            continue
        };
        if !lower.contains("export"){continue}
        let convention_is_c=lower.split("convention =>").skip(1).any(|tail|tail.trim_start().split([',',';']).next().unwrap_or("").trim()=="c");
        if !convention_is_c{
            return Err(BindError::Source("exported Ada functions must use `Convention => C`".into()))
        }
...
fn gnat_runtime_dir()->Result<PathBuf,BindError>{let output=capture(Command::new("gnatls").arg("-v"),"gnatls")?;let text=String::from_utf8_lossy(&output);text.lines().map(str::trim).map(|v|v.trim_end_matches('/')).find(|v|v.ends_with("/adalib")&&Path::new(v).is_absolute()&&Path::new(v).join(gnat_runtime_name()).is_file()).map(PathBuf::from).ok_or_else(||BindError::Source("the provisioned GNAT runtime library directory was not found".into()))}
fn gnat_runtime_name()->&'static str{if cfg!(target_os="macos"){"libgnat.dylib"}else{"libgnat.so"}}
...
mod tests{
    ...
    fn generated_source_uses_current_callable_and_effect_shapes(){
        let source=render_jet("geo",&[Routine{
            name:"double_lat".into(),
            symbol:"geo_double".into(),
            params:vec![Param{name:"lat".into(),scalar:Scalar::Float,constraint:Some(Constraint{low:"-90.0".into(),high:"90.0".into()})}],
            result:Scalar::Float,
        }]);
