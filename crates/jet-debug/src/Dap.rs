...
fn run_io(
    ...
    loop {
        ...
        let next = take_next_request(&mut requests, server.has_pending_resume());
...
struct DapServer {
    ...
    /// Jet-level exception kind for the current stopped snapshot. Backend
    /// panic/error text never crosses the adapter boundary.
...
struct ProjectedFrame<'a> {
    position: usize,
    frame: &'a RawFrame,
    jet_line: Option<usize>,
    raw: bool,
}
...
fn project_frames<'a>(
    ...
    rust_file: &str,
    show_raw_frames: bool,
) -> Vec<ProjectedFrame<'a>> {
    let mut projected = frames
        .iter()
        .enumerate()
        .filter_map(|(position, frame)| {
            let jet_line = map.jet_line_for_file(&frame.rust_file, rust_file, frame.rust_line)?;
            Some(ProjectedFrame {
                position,
                frame,
                jet_line: Some(jet_line),
                raw: false,
            })
        })
        .collect::<Vec<_>>();
    if show_raw_frames {
        projected.extend(
            frames
                .iter()
                .enumerate()
                .map(|(position, frame)| ProjectedFrame {
                    position,
                    frame,
                    jet_line: map.jet_line_for_file(&frame.rust_file, rust_file, frame.rust_line),
                    raw: true,
                }),
        );
    }
...
/// Name a stopped DAP thread from its first verified Jet frame.  LLDB's
/// numeric thread id identifies the control-plane selection, not a Jet task;
/// exposing it as `task N` would make a native worker number look like a
/// language-level identity.  The stack and source mapping remain authoritative
/// for the task view.
fn jet_thread_name(thread_id: u32, backtrace: &str, map: &LineMap, rust_file: &str) -> String {
    if thread_id == 1 {
        return "main".to_string();
    }
    let function = Inferior::parse_frames(backtrace)
        .into_iter()
        .find(|frame| {
            map.jet_line_for_file(&frame.rust_file, rust_file, frame.rust_line)
                .is_some()
        })
        .map(|frame| Inferior::safe_jet_func(&frame.func));
    match function.as_deref() {
        Some(function) if function != "<native frame>" => {
            format!("task {function} (stopped)")
        }
...
impl DapServer {
    ...
    fn settle_resume(&mut self) -> io::Result<()> {
        let (_, result) = self.take_resume_result()?;
        result.map(|_| ())
    }

    fn take_resume_result(&mut self) -> io::Result<(ResumeMode, io::Result<ResumeResult>)> {
        let event = self.wait_resume()?;
        self.inf = Some(event.inferior);
        Ok((event.mode, event.result))
    }
    ...
    /// Close the native target and report whether the close was proven. A
    /// terminated adapter is not proof that LLDB detached or killed its
    /// debuggee; callers must keep that distinction in the DAP event stream.
    ...
    /// Report the backend's resolved state after a target becomes available.
    /// A source-map hit is not enough: optimized code can leave LLDB with a
    /// pending breakpoint, and the editor must receive that truth as a DAP
    /// breakpoint event rather than retain a stale pre-launch `verified` bit.
    fn emit_breakpoint_statuses(&mut self, out: &mut impl Write, statuses: &[bool]) {
        let specs = self.pending_specs.clone();
        let client_ids = self.client_breakpoint_ids.clone();
        let source_path = self.source_path();
        for (index, spec) in specs.iter().enumerate() {
            let id = client_ids.get(index).copied().unwrap_or(0);
            let verified = statuses.get(index).copied().unwrap_or(false)
                && spec.condition.as_deref().is_none_or(valid_condition)
                && spec
                    .hit_condition
                    .as_deref()
                    .is_none_or(valid_hit_condition);
            let breakpoint = if verified {
                format!(
                    "{{\"id\":{id},\"verified\":true,\"line\":{},\"source\":{{\"path\":\"{}\"}}}}",
                    self.dap_line(spec.line),
                    json_escape(&source_path)
                )
            } else {
                format!(
                    "{{\"id\":{id},\"verified\":false,\"line\":{},\"source\":{{\"path\":\"{}\"}},\"message\":\"E2235: no stoppable Jet statement at {}:{}\"}}",
                    self.dap_line(spec.line),
                    json_escape(&source_path),
                    json_escape(&source_path),
                    self.dap_line(spec.line)
            ...
            self.event(
                out,
                "breakpoint",
                &format!("{{\"reason\":\"changed\",\"breakpoint\":{breakpoint}}}"),
            );
        }
    }
    ...
    fn select_resume_thread(
        ...
        let Some(inf) = self.inf.as_mut() else {
            return Err((22034, "Jet debugger has no target"));
        };
    ...
    fn restart_target(&mut self, out: &mut impl Write, request_seq: i64) {
        let was_configuring = self.state == State::Configuring;
        if self.resume_task.is_some() {
            let settled = self.settle_resume();
        ...
        let (source_ids, statuses) = match Self::install_breakpoints(
            &self.map,
            &self.rust_file,
        ...
        self.emit_breakpoint_statuses(out, &statuses);
        if !was_configuring {
    ...
    fn begin_resume(&mut self, out: &mut impl Write, command: &'static str, mode: ResumeMode) {
        ...
        self.resume_stop_requested = false;
    ...
    fn handle(&mut self, msg: &JSONValue, out: &mut impl Write) -> Option<()> {
        ...
        match command {
            ...
            "setBreakpoints" => {
                ...
                let resume_after_replacement =
                    self.state == State::Running && !self.resume_stop_requested;
                if resume_after_replacement && self.resume_task.is_some() {
                    let (mode, result) = match self.take_resume_result() {
                        Ok(event) => event,
                        Err(error) => {
                            self.respond_backend_error(
                                ...
                                command,
                                &error,
                                "the native debugger could not settle before replacing source breakpoints",
                            );
                            self.terminate_backend(
                                out,
                                &error,
                                "the native debugger lost the running session while replacing source breakpoints",
                            );
                            return Some(());
                        }
                    };
                    match result {
                        Ok(result) => self.emit_resume(out, result, mode),
                        Err(error) => {
                            self.respond_backend_error(
                                out,
                                request_seq,
                ...
                if self.state == State::Terminated || self.terminal_failure {
                    self.respond_jet_error(
                        out,
                        request_seq,
                        command,
                        22034,
                        "the native debugger target terminated before source breakpoints could be replaced",
                    );
                    return Some(());
                }
                ...
                if resume_after_replacement {
                    self.state = State::Running;
                    let mode = if self.entry_breakpoint_id.is_some() {
                        ResumeMode::Launch
                    } else {
                        ResumeMode::Continue
                    };
                    self.begin_resume(out, "continue", mode);
                }
                Some(())
            }
            "configurationDone" => {
            ...
            "threads" => {
                ...
                let threads = match self.inf.as_mut().map(Inferior::thread_backtraces) {
                    Some(Ok(threads)) => threads
                        .into_iter()
                        .filter_map(|(thread, backtrace)| {
                            let frames = Inferior::parse_frames(&backtrace);
                            has_jet_frame(&frames, &self.map, &self.rust_file).then(|| {
                                (
                                    thread.id,
                                    jet_thread_name(
                                        thread.id,
                ...
                if threads.is_empty() {
                    self.respond_jet_error(
                        ...
                        command,
                        22039,
                        "the stopped target has no verified Jet task",
                    );
                    return Some(());
                }
            ...
            "stackTrace" => {
                ...
                if !has_jet_frame(&frames, &self.map, &self.rust_file) {
                    self.respond_jet_error(
                        ...
                        command,
                        22039,
                        "the stopped target has no verified Jet frame",
                    );
                    return Some(());
                }
                ...
                let projected =
                    project_frames(&frames, &self.map, &self.rust_file, show_raw_frames);
                let total_frames = projected.len();
                    ...
                    .take(levels.unwrap_or(usize::MAX))
                {
                    if projected.raw {
                        let id = self
                            .references
                            .issue_raw_frame(thread_id, projected.position);
                        entries.push(format!(
                            "{{\"id\":{},\"name\":\"[raw] {}\",\"source\":{{\"path\":\"{}\"}},\"line\":{},\"column\":{}}}",
                            id,
                            json_escape(&projected.frame.func),
                            json_escape(&projected.frame.rust_file),
                            self.dap_line(projected.frame.rust_line),
                            self.dap_column(1)
                        ));
                    } else if let Some(jline) = projected.jet_line {
                        let id = self.references.issue_frame(thread_id, projected.position);
                        entries.push(format!(
                            "{{\"id\":{},\"name\":\"{}\",\"source\":{{\"path\":\"{}\"}},\"line\":{},\"column\":{}}}",
                            id,
                            json_escape(&Inferior::safe_jet_func(&projected.frame.func)),
                            json_escape(&self.source_path()),
                            self.dap_line(jline),
                            self.dap_column(1)
                        ));
            ...
            "continue" | "next" | "stepIn" | "stepOut" => {
                ...
                if let Err((id, message)) = self.select_resume_thread(args) {
            ...
            "pause" => {
                ...
                if self.resume_task.is_some() {
                    self.resume_stop_requested = true;
            ...
            "exceptionInfo" => {
                ...
                if let Some(kind) = self.last_exception.as_deref() {
                    let (exception_id, description) = if kind == "panic" {
                        ("JetPanic", "Jet panic".to_string())
                    } else if kind == "error" {
                        ("JetError", "Jet error".to_string())
                    } else {
                        (kind, format!("Jet target stopped on {kind}"))
                    };
                    self.respond(
                        out,
                        request_seq,
                        ...
                        true,
                        &format!(
                            "{{\"exceptionId\":\"{}\",\"description\":\"{}\"}}",
                            json_escape(exception_id),
    ...
    fn apply_pending_breakpoints_and_run(&mut self, out: &mut impl Write) {
        ...
        let (ids, statuses) = match Self::install_breakpoints(map, &rust_file, &lines, inf) {
            Ok(result) => result,
            Err(_error) => {
                self.terminate_with_diagnostic(
                    out,
        ...
        loop {
            ...
            let bt_text = match result {
                ResumeResult::Exited { status, signal } => {
                    ...
                    if self.cleanup_after_terminal_state(out) {
                        self.event(out, "terminated", "{}");
                    }
                    return;
                }
            ...
            let exception_kind = self.exception_kind(&bt_text).map(str::to_string);
            self.last_exception = exception_kind.clone().map(|kind| {
                if kind == "signal" {
                    self.last_signal
                        .clone()
            ...
            if mode != ResumeMode::Pause {
                if let Some(kind) = exception_kind.as_deref() {
    ...
    fn exception_kind(&self, backtrace: &str) -> Option<&'static str> {
        // Only the LLDB stop banner carries the reason.  Searching every
        // frame makes ordinary Jet stops look like panics because the Rust
        // runtime's catch_unwind helpers are present in otherwise normal
        // stacks.
        let reason = backtrace
            .lines()
            .find(|line| line.contains("stop reason ="))
            .unwrap_or("")
            .to_ascii_lowercase();
        if self.last_signal.is_some() || reason.contains("stop reason = signal ") {
            Some("signal")
        } else if reason.contains("panic") {
            Some("panic")
        } else if reason.contains("exception")
            || reason.contains("fatal error")
            || reason.contains("exc_")
        {
            Some("error")
        } else {
            None
...
fn diagnostic_details(id: i64, format: &str) -> (&'static str, &'static str, &'static str) {
    match id {
        ...
        22039 => (
            "E2235",
            "the requested thread has no verified Jet source location",
            "select a stopped Jet task or move the breakpoint to a mapped Jet statement",
        ),
        _ => (
            "E2239",
...
mod tests {
    ...
    #[test]
    fn stack_projection_pages_over_unmapped_native_frames() {
        ...
        let jet_frames = project_frames(&frames, &map, "main.rs", false);
        assert_eq!(jet_frames.len(), 2);
        assert_eq!(jet_frames[0].position, 1);
        assert_eq!(jet_frames[0].jet_line, Some(2));
        assert_eq!(jet_frames[1].position, 2);
        assert_eq!(jet_frames[1].jet_line, Some(3));

        let second_page = jet_frames.iter().skip(1).take(1).collect::<Vec<_>>();
        assert_eq!(second_page[0].frame.rust_line, 4);
        assert_eq!(second_page[0].jet_line, Some(3));

        let raw_frames = project_frames(&frames, &map, "main.rs", true);
        assert_eq!(raw_frames.len(), 5);
        assert!(!raw_frames[0].raw);
        assert!(!raw_frames[1].raw);
        assert!(raw_frames[2].raw);
        assert_eq!(raw_frames[0].position, 1);
        assert_eq!(raw_frames[1].position, 2);
        assert_eq!(raw_frames[2].position, 0);
    }

    #[test]
    fn async_task_threads_use_their_jet_frame_name() {
        let map = LineMap::build("// jet:line 6\nlet value = 1;\n");
        let backtrace =
            "frame #0: 0x1 bin`__jet_work at main.rs:2:1\nframe #1: 0x2 bin`__jet_run at main.rs:9:1\n";

        assert_eq!(
            jet_thread_name(2, backtrace, &map, "main.rs"),
            "task work (stopped)"
        );
    }
    ...
    #[test]
    fn queued_snapshot_requests_wait_for_resume_to_settle() {
        let mut requests = VecDeque::from([
            parse_dap_request(r#"{"seq":1,"type":"request","command":"threads","arguments":{}}"#)
                .unwrap(),
            parse_dap_request(
                r#"{"seq":2,"type":"request","command":"stackTrace","arguments":{}}"#,
            )
            .unwrap(),
            parse_dap_request(r#"{"seq":3,"type":"request","command":"pause","arguments":{}}"#)
                .unwrap(),
            parse_dap_request(r#"{"seq":4,"type":"request","command":"restart","arguments":{}}"#)
                .unwrap(),
            parse_dap_request(
                r#"{"seq":5,"type":"request","command":"setBreakpoints","arguments":{}}"#,
            )
            .unwrap(),
        ]);

        let control = take_next_request(&mut requests, true).expect("running control request");
        assert_eq!(
            json_get(&control, "command").and_then(json_str),
            Some("pause")
        );
        let restart = take_next_request(&mut requests, true).expect("restart request");
        assert_eq!(
            json_get(&restart, "command").and_then(json_str),
            Some("restart")
        );
        let breakpoints =
            take_next_request(&mut requests, true).expect("breakpoint replacement request");
        assert_eq!(
            json_get(&breakpoints, "command").and_then(json_str),
            Some("setBreakpoints")
        );
        assert!(take_next_request(&mut requests, true).is_none());
        assert_eq!(requests.len(), 2);

        let first = take_next_request(&mut requests, false).expect("queued request");
        assert_eq!(
            json_get(&first, "command").and_then(json_str),
            Some("threads")
        );
    }
    ...
    #[test]
    fn breakpoint_status_events_publish_backend_resolution() {
        let mut server = test_server(State::Configuring);
        server.pending_specs = vec![
            BreakpointSpec {
                line: 2,
        ...
        server.client_breakpoint_ids = vec![7, 8];
        let mut output = Vec::new();
        server.emit_breakpoint_statuses(&mut output, &[false, true]);
        let text = String::from_utf8(output).unwrap();
        assert!(
            text.contains("\"event\":\"breakpoint\""),
            "missing DAP breakpoint change events: {text}"
        );
        assert!(
            text.contains("\"id\":7,\"verified\":false"),
            "unresolved native breakpoint must be reported as pending: {text}"
        );
        assert!(
            text.contains("\"id\":8,\"verified\":true"),
            "resolved native breakpoint must be reported as verified: {text}"
        );
    }
    ...
    #[test]
    fn dap_exception_info_keeps_jet_panic_error_and_signal_meaning() {
        let mut server = test_server(State::Stopped);
        server.last_exception = Some("panic".to_string());
        let panic = dap_call(
            &mut server,
            r#"{"seq":1,"type":"request","command":"exceptionInfo","arguments":{}}"#,
        );
        assert!(panic.contains("\"exceptionId\":\"JetPanic\""), "{panic}");
        assert!(panic.contains("\"description\":\"Jet panic\""), "{panic}");

        server.last_exception = Some("error".to_string());
        let error = dap_call(
            &mut server,
            r#"{"seq":2,"type":"request","command":"exceptionInfo","arguments":{}}"#,
        );
        assert!(error.contains("\"exceptionId\":\"JetError\""), "{error}");
        assert!(error.contains("\"description\":\"Jet error\""), "{error}");

        server.last_signal = Some("SIGABRT".to_string());
        server.last_exception = Some("SIGABRT".to_string());
        let signal = dap_call(
            &mut server,
            r#"{"seq":3,"type":"request","command":"exceptionInfo","arguments":{}}"#,
        );
    ...
    #[test]
    fn unwind_frames_do_not_turn_breakpoints_into_jet_panics() {
        let server = test_server(State::Running);
        let backtrace = concat!(
            "* thread #1, stop reason = breakpoint 1.1\n",
            "    frame #0: 0x1 bin`__jet_run at main.rs:2:1\n",
            "    frame #1: 0x2 bin`std::panic::catch_unwind at panicking.rs:1:1\n",
            "    frame #2: 0x3 bin`core::panic::unwind_safe::AssertUnwindSafe at unwind_safe.rs:1:1\n",
        );

        assert_eq!(server.exception_kind(backtrace), None);
    }
    ...
    #[test]
    fn dap_pause_does_not_expose_an_unmapped_native_frame() {
        let mut server = test_server(State::Running);
        let mut output = Vec::new();
        server.emit_resume(
            &mut output,
            ResumeResult::Stopped(
                "frame #0: 0x0000 `runtime_helper at generated.rs:99:1\n".to_string(),
            ),
            ResumeMode::Pause,
        );
        let text = String::from_utf8(output).expect("DAP output is UTF-8");
        assert_eq!(server.state, State::Terminated);
        assert!(!text.contains("\"event\":\"stopped\""));
        assert!(text.contains("E2235") || text.contains("E2234"), "{text}");
    }
