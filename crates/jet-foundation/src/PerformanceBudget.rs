...
fn validate_measurement(value:&CanonicalJson)->Result<(),String>{
    let f=object(value,"measurement",&["budget_id","budget_spec","budget_spec_sha256","source","metric","target_class","unit","direction","provider","comparison","enforcement","context_key","policy","samples","statistics","history","baseline","decision"])?;
    text(&f["budget_id"],"measurement.budget_id")?;if !matches!(f["budget_spec"],CanonicalJson::Object(_)){return Err("measurement.budget_spec is not an object".into());}hex64(&f["budget_spec_sha256"],"measurement.budget_spec_sha256")?;verify_stable_id(&f["budget_spec"],text(&f["budget_spec_sha256"],"measurement.budget_spec_sha256")?).map_err(|_|"measurement budget_spec_sha256 mismatch".to_string())?;text(&f["source"],"measurement.source")?;validate_metric(&f["metric"])?;text(&f["target_class"],"measurement.target_class")?;text(&f["unit"],"measurement.unit")?;one_of(&f["direction"],"measurement.direction",&["lower_is_better","higher_is_better"])?;validate_provider(&f["provider"])?;validate_comparison(&f["comparison"])?;one_of(&f["enforcement"],"measurement.enforcement",&["warn","fail"])?;hex64(&f["context_key"],"measurement.context_key")?;nullable(&f["policy"],validate_policy)?;for q in array(&f["samples"],"measurement.samples")?{validate_rational(q,true,"measurement sample")?;}nullable(&f["statistics"],validate_statistics)?;nullable(&f["history"],validate_history)?;nullable(&f["baseline"],validate_baseline)?;nullable(&f["decision"],validate_decision)
}
