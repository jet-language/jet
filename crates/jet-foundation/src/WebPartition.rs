...
pub fn web_abi_type(
    ...
    crate::Diagnostics::Diagnostic::error(
        ...
        "web exports and imports only admit ABI-safe types (scalars, `String`, `List`/`Map` of ABI-safe values, and `#[Codable]` structs/enums)"
            .to_string(),
        "use a scalar, `String`, a `List`/`Map` of ABI-safe values, or a `#[Codable]` struct/enum whose fields are ABI-safe"
            .to_string(),
        span,
    )
}
