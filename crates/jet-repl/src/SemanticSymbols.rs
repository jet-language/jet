...
pub const SYMBOLS: &[Symbol] = &[
    ...
    member!("List", "remove", "List.remove(value: T, by: RemoveBy = .Val) => T?", "Removes the first equal value; `.Slot` selects positional removal."),
    member!("List", "count", "List.count(value: T) => Int", "Counts items equal to value."),
    member!("List", "extend", "List.extend(other: [T])", "Appends every item from other in order."),
    member!("List", "concat", "List.concat(other: [T]) => [T]", "Returns this list followed by other."),
    member!("List", "indexed", "List.indexed() => Iter<(idx: Int, item: T)>", "Pairs each item with its index."),
    member!("List", "indexes", "List.indexes() => Iter<Int>", "Yields every valid Int index."),
    ...
    member!("Map", "keys", "Map.keys() -> Iter<K>", "Lazily yields every key in map order."),
    member!("Map", "values", "Map.values() -> Iter<V>", "Lazily yields every value in map order."),
