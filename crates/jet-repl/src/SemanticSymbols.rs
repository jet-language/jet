...
macro_rules! member {
    ($owner:literal, $name:literal, $signature:literal, $summary:literal) => {
        Symbol {
            identity: concat!($owner, ".", $name),
            module: "core.collections",
            owner: Some($owner),
            member: $name,
            signature: $signature,
            summary: $summary,
            example: concat!($owner, ".", $name, "(...)"),
            provenance: "builtin",
        }
    };
}
...
pub const SYMBOLS: &[Symbol] = &[
    ...
    member!(
        "List",
        "indexed",
        "List.indexed() -> Iter<(idx: Int, item: T)>",
        "Pairs each item with its index."
    ),
    member!(
        "List",
        "indexes",
        "List.indexes() -> Iter<Int>",
        "Yields every valid Int index."
    ),
    member!(
        "List",
        "zip",
    ...
    member!(
        "Map",
        "add_new",
        "Map.add_new(key: K, value: V) -> Bool",
        "Adds only when the key is absent."
    ),
    member!(
        "Map",
        "remove",
