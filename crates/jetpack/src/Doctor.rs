...
impl Report {
    ...
    pub fn to_json(&self) -> String {
        let checks = self
            ...
            .join(",");
        format!(
            "{{\"kind\":\"jetpack.doctor\",\"status\":{},\"checks\":[{}]}}",
            JSON::quote(self.health().word()),
            checks
        )
    }
}
...
fn check_hangar(roots: &Store::Roots) -> Check {
    ...
    let rd = match fs::read_dir(&hangar) {
        Ok(rd) => rd,
        Err(e) => {
            return broken(
                "hangar",
                format!("unreadable ({})", e.kind()),
                "restore read access to the Jetpack root",
            )
        }
    };
    ...
    for next in rd {
        let ent = match next {
            ...
            Err(e) => {
                return broken(
                    "hangar",
                    format!("directory enumeration failed ({})", e.kind()),
                    "restore read access to every Hangar object",
                )
            }
        };
        let path = ent.path();
        ...
        if !meta_path.is_file() {
            return degraded(
                "hangar",
                format!("object `{id}` has no metadata"),
                "run `jetpack hangar recover`, then realize it again if no committed record exists",
            );
        }
        let Some(entry) = known.get(id.as_ref()) else {
            return degraded(
                "hangar",
                format!("object `{id}` has malformed metadata"),
                "run `jetpack hangar recover`, then realize the package again",
            );
        };
        ...
        if !Path::new(&entry.out).exists() {
            return degraded(
                "hangar",
                format!("object `{id}` points to a missing output"),
                "realize the package again",
            );
        }
        match Store::try_entry_output_hash(roots, entry) {
            Ok(actual)
                if !entry.envelope.output_hash.is_empty()
                    && actual == entry.envelope.output_hash => {}
            Ok(_) => {
                return degraded(
                    "hangar",
...
fn registry_endpoints() -> Vec<(String, String)> {
    ...
    if let Ok(url) = std::env::var("JET_REGISTRY_URL") {
        if !url.is_empty() {
            found.insert("jet".to_string(), url);
        }
    }
    for (key, url) in std::env::vars() {
        if let Some(name) = key
            .strip_prefix("JET_REGISTRY_")
            .and_then(|s| s.strip_suffix("_URL"))
        {
            if !name.is_empty() && !url.is_empty() {
                found.insert(name.to_ascii_lowercase(), url);
            }
        }
    }
    if found.is_empty() {
        found.insert("jet".into(), "https://github.com/jet-lang/registry".into());
    }
...
fn check_registry(online: bool) -> Check {
    ...
    if skipped > 0 {
        ok(
            "registry",
            format!(
                "{} configured; network probe skipped (offline-safe default)",
                endpoints.len()
            ),
        )
    } else {
        ok(
            "registry",
            format!("{} configured endpoint(s) reachable", endpoints.len()),
        )
    }
}
...
fn endpoint_reachable(url: &str, online: bool) -> Result<bool, &'static str> {
    ...
    if let Some(path) = url.strip_prefix("file://") {
        return if Path::new(path).exists() {
            ...
            Err("local index missing")
        };
    }
    if !url.contains("://") {
        return if Path::new(url).exists() {
            Ok(true)
        } else {
            Err("local index missing")
        };
...
fn git_registry_probe(url: &str) -> Result<(), &'static str> {
    ...
    let mut child = git
        ...
        .stderr(Stdio::null())
        .spawn()
        .map_err(|_| "git registry client unavailable")?;
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                return if status.success() {
                    ...
                    Err("HTTPS registry rejected probe")
                }
            }
            ...
            Ok(None) => {
                let _ = child.kill();
                let _ = child.wait();
                return Err("HTTPS registry probe timed out");
            }
...
fn check_locks(project: &Path, roots: &Store::Roots) -> Check {
    let dirs = [
        roots.root.join(".locks"),
        Store::managed_dir(project).join(".locks"),
    ];
    ...
    for dir in dirs {
        let rd = match fs::read_dir(&dir) {
            ...
            Err(_) => {
                unknown.insert(format!("{} (directory unreadable)", dir.display()));
                continue;
            }
        };
        for next in rd {
            let ent = match next {
                Ok(ent) => ent,
                Err(_) => {
                    unknown.insert("directory entry unreadable".to_string());
                    continue;
                }
            };
            let path = ent.path();
            ...
            match super::RuntimePolicy::lock_state(&path) {
                Ok(super::RuntimePolicy::LockState::Held) => {
                    held.insert(ent.file_name().to_string_lossy().into_owned());
                }
                Ok(
                    super::RuntimePolicy::LockState::Absent | super::RuntimePolicy::LockState::Idle,
                ) => {}
                Err(_) => {
                    unknown.insert(ent.file_name().to_string_lossy().into_owned());
                }
            }
        }
    }
    if !unknown.is_empty() {
        degraded(
            "locks",
            format!(
                "{} lock(s) could not be probed: {}",
                unknown.len(),
                unknown.into_iter().collect::<Vec<_>>().join(", ")
            ),
            "restore lock-file access and filesystem advisory-lock support",
        )
    } else if !held.is_empty() {
        degraded(
            "locks",
            format!(
                "{} lock(s) currently held: {}",
                held.len(),
                held.into_iter().collect::<Vec<_>>().join(", ")
            ),
            "wait for active Jetpack operations, then rerun `jetpack doctor`",
        )
    } else {
        ok("locks", "kernel advisory locks readable")
    }
}
...
fn check_cache(roots: &Store::Roots) -> Check {
    let now = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_secs();
    let stale = Store::list_read_only(roots)
        .into_iter()
        .filter(|e| e.last_used_at == 0 || now.saturating_sub(e.last_used_at) > STALE_AFTER)
        .count();
    if stale == 0 {
        ok("cache", "no objects unused for more than 30 days")
    } else {
        degraded(
            "cache",
...
fn check_signing_key() -> Check {
    ...
    if !secret.is_file() || !public.is_file() {
        return degraded(
            "signing",
            format!("signing key for `{registry}` is missing or incomplete"),
            "run `jet registry keygen` before publishing",
        );
    }
...
fn degraded(name: &'static str, detail: impl Into<String>, fix: impl Into<String>) -> Check {
    Check {
        name,
...
mod tests {
    ...
    fn lock_health_uses_kernel_state_not_pid_or_file_contents() {
        ...
        assert!(check_locks(&root, &roots)
            .detail
            .contains("directory unreadable"));
