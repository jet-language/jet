...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_peer_credentials_prove_user_namespace_boundary_without_subuid_helper() {
        use std::os::unix::net::UnixStream;
        use std::time::{Duration, Instant};

        let socket = std::env::temp_dir().join(format!(
            "jet-shared-store-peer-listener-{}",
            std::process::id()
        ));
        let _ = fs::remove_file(&socket);
        let child = std::process::Command::new("unshare")
            ...
            .args([
                "--exact",
                "Store::Broker::tests::broker_peer_probe_listener_child",
                "--nocapture",
            ])
            .env("JET_BROKER_LISTENER_PROBE_SOCKET", &socket)
            .spawn();
        let Ok(mut child) = child else {
            return;
        };
        ...
        if !socket.exists() {
            let status = child.wait().unwrap();
            eprintln!("note: user namespace listener unavailable: {status}");
            return;
        }
        UnixStream::connect(&socket).unwrap();
        let status = child.wait().unwrap();
        assert!(status.success(), "user namespace peer probe failed: {status}");
        let _ = fs::remove_file(socket);
    }

    #[cfg(target_os = "linux")]
    #[test]
    fn broker_peer_probe_listener_child() {
        let Some(socket) = std::env::var_os("JET_BROKER_LISTENER_PROBE_SOCKET") else {
            return;
        };
        use std::os::unix::net::UnixListener;
        let listener = UnixListener::bind(socket).unwrap();
        let (stream, _) = listener.accept().unwrap();
        let peer = peer_uid(&stream).unwrap();
        let own = current_uid().unwrap();
        println!("user-namespace peer uid={peer}, listener uid={own}");
        assert_ne!(peer, own);
    }
}
