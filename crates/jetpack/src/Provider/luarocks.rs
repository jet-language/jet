...
impl Provider for LuaRocksProvider {
    fn validate_cache_authority(
        ...
        let current = cache_authority(ctx)?;
        super::ensure_locked_authority("LuaRocks", &repository, &locked, &current)
    }
    ...
    fn cache_expectation(
        ...
        let (output, source_hash, _repository, _locked_authority, env) =
            crate::Lock::luarocks_realization(project, &spec.raw)?;
        let authority = cache_authority(ctx).ok()?;
    ...
    fn realize(
        ...
        for name in &order {
            ...
            let source_path = scratch
                .path
                .join(format!("{}-{source_name}", rockspec.name));
            authority
        ...
        let root = specs.get(root_name).ok_or_else(|| {
            error(format!(
...
fn resolve_rockspec(
    ...
    if !active.insert(name.to_string()) {
        return Err(error(format!(
            "LuaRocks dependency cycle includes `{name}`"
        )));
    }
    ...
    for dep in deps {
        ...
        resolve_rockspec(
            &dep.name, &selected, repository, authority, manifest, scratch, specs, active,
        )?;
    }
    active.remove(name);
    Ok(())
...
fn parse_rockspec(raw: &str) -> Result<Rockspec, ProviderError> {
    ...
    if source_hash.len() != 64 || !source_hash.bytes().all(|b| b.is_ascii_hexdigit()) {
        return Err(error(format!(
            "rockspec `{name}` needs a SHA-256 source hash"
        )));
    }
    ...
    if modules.is_empty() {
        return Err(error(format!(
            "rockspec `{name}` has no installable modules"
        )));
    }
    for (module, path) in &modules {
        ...
        {
            return Err(error(format!(
                "rockspec `{name}` contains unsafe or native module mapping `{module}` = `{path}`"
            )));
        }
    }
    Ok(Rockspec {
...
fn parse_dependencies(raw: &str) -> Result<Vec<Dependency>, ProviderError> {
    ...
    for item in string_values(raw)? {
        ...
        for requirement in rest.split(',').map(str::trim).filter(|v| !v.is_empty()) {
            ...
            {
                return Err(error(format!(
                    "unsupported LuaRocks constraint `{requirement}`"
                )));
            }
...
fn dependency_order(
    ...
    fn visit(
        ...
        let spec = specs
            .get(name)
            .ok_or_else(|| error(format!("missing resolved LuaRocks dependency `{name}`")))?;
        for dep in &spec.dependencies {
            if dep.name != "lua" {
                visit(&dep.name, specs, active, done, out)?;
            }
        }
        active.remove(name);
...
fn preserve_and_install(
    ...
    for (module, relative) in &artifact.spec.modules {
        ...
        if !source.is_file() {
            return Err(error(format!(
                "module `{module}` source `{relative}` is missing"
            )));
        }
...
fn validate_archive(path: &Path) -> Result<(), ProviderError> {
    ...
    for entry in listing.lines() {
        if entry.is_empty() || !safe_relative(Path::new(entry)) {
            return Err(error(format!(
                "LuaRocks archive contains unsafe path `{entry}`"
            )));
        }
    }
    ...
    if !verbose.status.success() {
        return Err(error(
            "LuaRocks source archive types could not be inspected",
        ));
    }
    ...
    for entry in verbose.lines() {
        if matches!(entry.as_bytes().first(), Some(b'l' | b'h')) {
            return Err(error(
                "LuaRocks source archives may not contain symbolic or hard links",
            ));
        }
    }
    Ok(())
...
fn write_runtime_files(out: &Path) -> Result<(), ProviderError> {
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(&wrapper, std::fs::Permissions::from_mode(0o755)).map_err(
            |e| {
                error(format!(
                    "could not make Lua runtime wrapper executable: {e}"
                ))
            },
        )?;
    }
...
fn safe_url_basename(url: &str) -> Result<String, ProviderError> {
    ...
    if !valid_component(value) && !value.ends_with(".tar.gz") {
        return Err(error(format!(
            "LuaRocks source URL has unsafe archive name `{value}`"
        )));
    }
...
fn ensure_platform_supported(platforms: &[String]) -> Result<(), ProviderError> {
    ...
    let positive = platforms
        .iter()
        .filter(|v| !v.starts_with('!'))
        .collect::<Vec<_>>();
    let denied = platforms
        .iter()
        .any(|v| v == &format!("!{host}") || (!cfg!(target_os = "windows") && v == "!unix"));
    let allowed = positive.is_empty()
        || positive
            .iter()
            .any(|v| v.as_str() == host || (!cfg!(target_os = "windows") && v.as_str() == "unix"));
    if denied || !allowed {
        Err(error(format!(
            "LuaRocks package does not support host platform `{host}`"
        )))
    } else {
        Ok(())
    }
...
fn assignment_string(raw: &str, key: &str) -> Result<String, ProviderError> {
    let value = assignment_value(raw, key)
        .ok_or_else(|| error(format!("rockspec has no `{key}` field")))?;
    parse_quoted(value).map(|v| v.0)
}

fn assignment_table<'a>(raw: &'a str, key: &str) -> Result<&'a str, ProviderError> {
    assignment_table_optional(raw, key)
        .ok_or_else(|| error(format!("rockspec has no `{key}` table")))
}
...
fn table_string(raw: &str, key: &str) -> Result<String, ProviderError> {
    let value =
        assignment_value(raw, key).ok_or_else(|| error(format!("table has no `{key}` field")))?;
    parse_quoted(value).map(|v| v.0)
}

fn table_table<'a>(raw: &'a str, key: &str) -> Result<&'a str, ProviderError> {
    let value =
        assignment_value(raw, key).ok_or_else(|| error(format!("table has no `{key}` table")))?;
...
fn parse_quoted(raw: &str) -> Result<(String, usize), ProviderError> {
    ...
    let quote = raw
        ...
        .first()
        .copied()
        .ok_or_else(|| error("missing quoted value"))?;
    if quote != b'\'' && quote != b'"' {
        return Err(error("provider metadata requires quoted string values"));
    }
...
fn balanced_table(raw: &str) -> Result<(&str, usize), ProviderError> {
    ...
    let raw = raw.trim_start();
    if !raw.starts_with('{') {
        return Err(error("provider metadata requires a table"));
    }
...
fn keyed_strings(raw: &str) -> Result<BTreeMap<String, String>, ProviderError> {
    ...
    for line in raw.lines() {
        ...
        let Some((key, value)) = line.split_once('=') else {
            return Err(error(format!("unsupported module entry `{line}`")));
        };
        let key = key.trim();
        ...
        if out.insert(key.clone(), value).is_some() {
            return Err(error(format!("duplicate module `{key}`")));
        }
    }
    Ok(out)
}
...
fn manifest_versions(raw: &str, name: &str) -> Result<Vec<String>, ProviderError> {
    ...
    let package = keyed_table(repository, name)
        .ok_or_else(|| error(format!("LuaRocks manifest has no package `{name}`")))?;
    ...
    if versions.is_empty() {
        Err(error(format!(
            "LuaRocks manifest has no versions for `{name}`"
        )))
    } else {
        Ok(versions)
    }
...
fn require_manifest_version(raw: &str, name: &str, version: &str) -> Result<(), ProviderError> {
    if manifest_versions(raw, name)?.iter().any(|v| v == version) {
        Ok(())
    } else {
        Err(error(format!(
            "LuaRocks manifest has no `{name}` version `{version}`"
        )))
    }
}
    ...
    for pattern in patterns {
        ...
        while let Some(found) = raw[cursor..].find(&pattern) {
            ...
            if let Some(value) = rest.strip_prefix('=').map(str::trim_start) {
                if let Ok((table, _)) = balanced_table(value) {
                    return Some(table);
                }
            }
            cursor = start;
...
impl Scratch {
    fn new(store: &Path) -> Result<Self, ProviderError> {
        ...
        let path = store
            ...
            .join(format!("luarocks-{}-{nonce}", std::process::id()));
        std::fs::create_dir_all(&path)
            .map_err(|e| error(format!("could not create LuaRocks scratch: {e}")))?;
        Ok(Self { path })
    }
}
...
mod tests {
    ...
    fn manifest_resolution_is_exact_and_semver_aware() {
        ...
        assert_eq!(
            manifest_versions(raw, "jetdep").unwrap(),
            vec!["1.0-1", "1.5-1"]
        );
    ...
    fn real_two_rock_install_executes_locks_replays_and_rejects_drift_and_tamper() {
        ...
        let base =
            std::env::temp_dir().join(format!("jet-luarocks-provider-{}", std::process::id()));
        let _ = fs::remove_dir_all(&base);
        let repo = base.join("repo");
        let source = base.join("source");
        fs::create_dir_all(&repo).unwrap();
        fs::create_dir_all(&source).unwrap();
        let dep_archive = archive_source(
            &source,
            &repo,
            "jetdep",
            "return { value = function() return 41 end }\n",
        );
        ...
        *TEST_REPOSITORY.write().unwrap() = Some(format!("file://{}", repo.display()));
        ...
        let spec = crate::RefSpec::classify_in("jetapp#version=2.0-1@luarocks", &table).unwrap();
        let online = Ctx {
            fixtures: None,
            store_dir: &store,
            offline: false,
        ...
        let output = Command::new(&wrapper)
            .args(["-e", "io.write(require(\"jetapp\").value())"])
            .output()
            .unwrap();
        assert!(
            output.status.success(),
            "installed Lua closure failed: {}",
