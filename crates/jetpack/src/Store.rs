...
pub enum IngestError {
    PathLaw(super::Envelope::PathLawError),
    Mutated(String),
    Invalid(String),
    Io(String),
}
...
impl IngestError {
    ...
    pub fn report(&self, theme: &super::Output::Theme) {
    ...
    let result = (|| {
        ...
        for (name, src) in &req.outputs {
            ...
            .map_err(|msg| {
                ...
                {
                    IngestError::PathLaw(super::Envelope::PathLawError {
                        code: "archive",
...
fn reject_semantic_xattrs_tree(root: &Path) -> Result<(), IngestError> {
    fn walk(path: &Path) -> Result<(), IngestError> {
        ...
        if meta.is_file() {
            super::Envelope::check_xattrs(path, false).map_err(IngestError::Invalid)?;
        }
        if meta.is_dir() {
    ...
    if meta.file_type().is_dir() {
        ...
        for ent in fs::read_dir(src).map_err(|e| IngestError::Io(e.to_string()))? {
            ...
            #[cfg(unix)]
            {
                ...
                if let Err(err) = super::Envelope::validate_path_component(&bytes) {
        ...
        if let Err(err) = super::Envelope::reject_casefold_collisions(&names) {
            return Err(IngestError::PathLaw(err));
        }
    } else if meta.file_type().is_symlink() {
