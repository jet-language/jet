...
const PROMPT_LABEL_ENV: &str = "JETPACK_PROMPT_LABEL";
...
fn enter_with_mode(theme: &Theme, env: &Env, kind: ShellKind, clean: bool) -> i32 {
    ...
    // Keep the user-controlled label out of shell source. The generated
    // prompt reads this value through a fixed, quoted print operation so
    // prompt substitution cannot execute label contents.
    cmd.env(PROMPT_LABEL_ENV, &env.label);
    if theme.color {
    ...
    match kind {
        ShellKind::Bash => {
            ...
            BrandedShell {
                ...
                env_vars: vec![(PROMPT_LABEL_ENV.to_string(), label.to_string())],
        ...
        ShellKind::Zsh => {
            ...
            BrandedShell {
                ...
                env_vars: vec![
                    ...
                    (PROMPT_LABEL_ENV.to_string(), label.to_string()),
                ],
                cleanup_file: None,
...
fn bash_rc(_label: &str, path: PromptPathMode, strip: PromptStripMode) -> String {
    ...
    let mut rc = format!(
        "[ -f /etc/bash.bashrc ] && . /etc/bash.bashrc\n\
         ...
         if [ \"${{NO_COLOR+x}}\" = x ]; then\n\
           PS1='{status_prefix}$(printf \"%s\" \"$JETPACK_PROMPT_LABEL\") {path_escape} > '\n\
         else\n\
           PS1='{status_prefix}{S}\u{1b}[{accent}m{E}$(printf \"%s\" \"$JETPACK_PROMPT_LABEL\"){S}\u{1b}[0m{E} {S}\u{1b}[{border}m{E}{path_escape}{S}\u{1b}[0m{E} {S}\u{1b}[{success}m{E}❯{S}\u{1b}[0m{E} '\n\
         fi\n"
    );
...
fn zsh_rc(_label: &str, path: PromptPathMode, strip: PromptStripMode) -> String {
    let path_escape = match path {
    ...
    let mut rc = format!(
        "[ -f \"$HOME/.zshrc\" ] && source \"$HOME/.zshrc\"\n\
         ...
         if [[ ${{+NO_COLOR}} = 1 ]]; then\n\
           PROMPT=$'{status_prefix}$(print -r -- \"$JETPACK_PROMPT_LABEL\") {path_escape} > '\n\
         else\n\
           PROMPT=$'{status_prefix}%{{\\033[{accent}m%}}$(print -r -- \"$JETPACK_PROMPT_LABEL\")%{{\\033[0m%}} %{{\\033[{border}m%}}{path_escape}%{{\\033[0m%}} %{{\\033[{success}m%}}❯%{{\\033[0m%}} '\n\
         fi\n"
    );
...
fn fish_init(_label: &str, path: PromptPathMode, strip: PromptStripMode) -> String {
    ...
    let mut rc = format!(
        "set -g __jetpack_build_status 'never run'; set -g __jetpack_test_status 'never run'; set -g __jetpack_kind ''; set -g __jetpack_command ''; set -g __jetpack_started 0; set -g __jetpack_spinner_pid ''; \
         ...
         if set -q NO_COLOR; printf '%s ' \"$JETPACK_PROMPT_LABEL\"; echo -n ({path_expr}); echo -n ' > '; \
         else; printf '\\033[{accent}m%s\\033[0m ' \"$JETPACK_PROMPT_LABEL\"; \
...
mod tests {
    ...
    fn bash_rc_sets_label() {
        ...
        assert!(rc.contains(PROMPT_LABEL_ENV));
    }
    ...
    fn prompt_rc_has_plain_no_color_fallback() {
        ...
        assert!(rc.contains("printf \"%s\" \"$JETPACK_PROMPT_LABEL\""));
        assert!(rc.contains(PROMPT_LABEL_ENV));
    }
    ...
    fn hostile_prompt_labels_are_data_in_generated_shells() {
        ...
        assert!(!bash.contains(hostile));
        assert!(bash.contains("printf \"%s\" \"$JETPACK_PROMPT_LABEL\""));
        ...
        assert!(!fish.contains(hostile));
        assert!(fish.contains("printf '%s ' \"$JETPACK_PROMPT_LABEL\""));
        ...
        assert!(!zsh.contains(hostile));
        assert!(zsh.contains("print -r -- \"$JETPACK_PROMPT_LABEL\""));
    }

    #[test]
    fn hostile_prompt_labels_are_not_reexpanded_in_real_shells() {
        let path = std::env::var("PATH").unwrap_or_default();
        let backtick = '\u{60}';

        let bash_marker = unique_tmp("jetpack-bash-prompt-injection");
        let bash_backtick_marker = unique_tmp("jetpack-bash-prompt-backtick");
        let bash_hostile = format!(
            "label; $(touch {}) {backtick}touch {}{backtick} \\ %",
            shell_single_quote(&bash_marker.display().to_string()),
            shell_single_quote(&bash_backtick_marker.display().to_string()),
        );
        let bash_rc_path = write_temp(
            "jetpack-bash-prompt-injection-rc",
            &bash_rc("ignored", PromptPathMode::Short, PromptStripMode::Off),
        );
        let _ = pty(
            &format!(
                "env -i HOME=/ PATH={} TERM=dumb NO_COLOR=1 JETPACK_PROMPT_LABEL={} bash --noprofile --rcfile {} -i",
                shell_single_quote(&path),
                shell_single_quote(&bash_hostile),
                shell_single_quote(&bash_rc_path.display().to_string()),
            ),
            "exit 0\n",
            true,
        );
        assert!(!bash_marker.exists(), "bash executed a command from the label");
        assert!(
            !bash_backtick_marker.exists(),
            "bash executed a backtick command from the label"
        );
        let _ = std::fs::remove_file(bash_rc_path);

        let zsh_marker = unique_tmp("jetpack-zsh-prompt-injection");
        let zsh_backtick_marker = unique_tmp("jetpack-zsh-prompt-backtick");
        let zsh_hostile = format!(
            "label; $(touch {}) {backtick}touch {}{backtick} \\ %",
            shell_single_quote(&zsh_marker.display().to_string()),
            shell_single_quote(&zsh_backtick_marker.display().to_string()),
        );
        let zdir = write_temp_dir("jetpack-zsh-prompt-injection-zdotdir");
        std::fs::write(
            zdir.join(".zshrc"),
            zsh_rc("ignored", PromptPathMode::Short, PromptStripMode::Off),
        )
        .unwrap();
        let _ = pty(
            &format!(
                "env -i HOME=/ PATH={} TERM=dumb NO_COLOR=1 JETPACK_PROMPT_LABEL={} ZDOTDIR={} zsh -d -i",
                shell_single_quote(&path),
                shell_single_quote(&zsh_hostile),
                shell_single_quote(&zdir.display().to_string()),
            ),
            "exit 0\n",
            true,
        );
        assert!(!zsh_marker.exists(), "zsh executed a command from the label");
        assert!(
            !zsh_backtick_marker.exists(),
            "zsh executed a backtick command from the label"
        );
        let _ = std::fs::remove_dir_all(zdir);

        let fish_marker = unique_tmp("jetpack-fish-prompt-injection");
        let fish_backtick_marker = unique_tmp("jetpack-fish-prompt-backtick");
        let fish_hostile = format!(
            "label; $(touch {}) {backtick}touch {}{backtick} \\ %",
            shell_single_quote(&fish_marker.display().to_string()),
            shell_single_quote(&fish_backtick_marker.display().to_string()),
        );
        let _ = pty(
            &format!(
                "env -i HOME=/ PATH={} TERM=dumb NO_COLOR=1 JETPACK_PROMPT_LABEL={} fish --no-config -C {} -i",
                shell_single_quote(&path),
                shell_single_quote(&fish_hostile),
                shell_single_quote(&fish_init(
                    "ignored",
                    PromptPathMode::Short,
                    PromptStripMode::Off,
        ...
        assert!(!fish_marker.exists(), "fish executed a command from the label");
        assert!(
            !fish_backtick_marker.exists(),
            "fish executed a backtick command from the label"
        );
    }
    ...
    fn bash_prompt_receipts_are_colored_in_a_real_pty() {
        ...
        let bash = pty(
            &format!(
                "env -i HOME={} PATH={} TERM=xterm-256color SHELL=/bin/bash HISTFILE=/dev/null JETPACK_PROMPT_LABEL={} bash --noprofile --rcfile {} -i",
