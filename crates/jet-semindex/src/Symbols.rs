    ...
    for import in imports {
        match &import.kind {
            jet_foundation::AST::ImportKind::Unqualified {
                ...
                let source_import = imports.iter().chain(module.imports.iter()).find(|candidate| {
                    !matches!(candidate.kind, jet_foundation::AST::ImportKind::Unqualified { .. })
                        && candidate.import_alias() == *module_alias
                });
                let imported_module = source_import.and_then(|source_import| match &source_import.kind {
            ...
            | jet_foundation::AST::ImportKind::Module(_, _) => {
                ...
                symbols.push(SemanticSymbol {
                    identity: import_identity(module, lexical_scope, &alias),
                    name: alias.clone(),
