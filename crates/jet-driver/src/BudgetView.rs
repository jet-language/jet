...
impl BudgetProjection {
    ...
    pub fn render_text(&self) -> String {
        ...
        if self.facts.is_empty() {
            out.push_str("  no compatible canonical report\n");
        }
...
pub fn read_compatible(root: &Path, sources: &[(String, String)]) -> BudgetProjection {
    ...
    let Ok(entries) = fs::read_dir(&dir) else {
        return BudgetProjection::default();
    };
    let wanted = sources
        .iter()
        .map(|(path, digest)| (normalize(path), digest.as_str()))
    ...
    for path in paths {
        ...
        let metadata = match fs::symlink_metadata(&path) {
            ...
            Err(error) => {
                rejected.push(format!("{label}: unreadable: {error}"));
                continue;
            }
        };
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            rejected.push(format!("{label}: not a regular no-follow report"));
            continue;
        }
        if metadata.len() > 16 * 1024 * 1024 {
            rejected.push(format!("{label}: report exceeds 16 MiB projection limit"));
            continue;
        }
        ...
        let Ok((report_id, evidence_id, members, measurements)) = report_parts(&report) else {
            rejected.push(format!("{label}: report projection fields are invalid"));
            continue;
        };
        if members.is_empty()
            || !members.iter().all(|(path, digest)| {
                wanted
                    .get(&normalize(path))
                    .is_some_and(|wanted| *wanted == digest)
            })
        {
            continue;
        }
        for measurement in measurements {
            let Ok(fact) = measurement_fact(&report_id, &evidence_id, measurement) else {
                rejected.push(format!(
                    "{label}: measurement projection fields are invalid"
                ));
                continue;
            };
    ...
    BudgetProjection {
        facts: by_budget.into_values().collect(),
        rejected,
    }
}
...
fn report_parts(
    value: &CanonicalJson,
) -> Result<(String, String, Vec<(String, String)>, &[CanonicalJson]), ()> {
    ...
    let members = array(subject.get("member_sources"))?
        .iter()
        .map(|value| {
            let member = object(value)?;
            Ok((
                text(member.get("path"))?.to_string(),
                text(member.get("sha256"))?.to_string(),
            ))
        })
    ...
    Ok((
        ...
        evidence_id,
        members,
        array(content.get("measurements"))?,
    ))
}
...
fn object(value: &CanonicalJson) -> Result<&BTreeMap<String, CanonicalJson>, ()> {
...
fn array(value: Option<&CanonicalJson>) -> Result<&[CanonicalJson], ()> {
    if let Some(CanonicalJson::Array(value)) = value {
        Ok(value)
    } else {
        Err(())
    }
}
fn text(value: Option<&CanonicalJson>) -> Result<&str, ()> {
    if let Some(CanonicalJson::String(value)) = value {
        Ok(value)
    } else {
        Err(())
    }
