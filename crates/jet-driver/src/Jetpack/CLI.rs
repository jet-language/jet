...
fn realize_ref(
    ...
    match realize_ref_outcome(theme, roots, flags, table, spec) {
...
fn compose_env(theme: &Theme, roots: &Roots, flags: &Flags, plan: &RunPlan) -> Result<Env, i32> {
    ...
    for spec in &plan.refs {
        match realize_ref_outcome(theme, roots, flags, &plan.table, spec) {
            RefOutcome::Realized(entry, _state) => {
...
fn cmd_build(theme: &Theme, parsed: &Parsed) -> i32 {
    ...
    let mut holes = Vec::new();
    for spec in &plan.refs {
        match realize_ref_outcome(theme, &roots, &parsed.flags, &plan.table, spec) {
            RefOutcome::Realized(entry, state) => {
                realized_refs.push(entry.reference);
                match state {
