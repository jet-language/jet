...
fn math_binomial(n: i64, k: i64) -> Option<i64> {
    if n < 0 || k < 0 || k > n {
        return None;
    }
    let k = k.min(n - k);
...
fn math_ilogb(x: f64) -> Option<i64> {
...
fn math_ldexp(x: f64, exp: i64) -> f64 {
...
fn math_next_after(x: f64, toward: f64) -> f64 {
...
fn math_ulp(x: f64) -> f64 {
    ...
    let bits = x.to_bits();
    f64::from_bits(bits + 1) - x
}

fn math_significand(x: f64) -> f64 {
    if x == 0.0 || !x.is_finite() {
        return x;
    }
    let bits = x.to_bits();
...
fn math_gamma(x: f64) -> f64 {
    ...
    if x <= 0.0 {
        ...
        return std::f64::consts::PI
            / ((std::f64::consts::PI * x).sin() * math_gamma(1.0 - x));
    }
    const G: f64 = 7.0;
    const C: [f64; 9] = [
        0.99999999999980993,
    ...
    if z < 0.5 {
        return std::f64::consts::PI
            / ((std::f64::consts::PI * z).sin() * math_gamma(1.0 - z));
    }
    z -= 1.0;
    let mut xacc = C[0];
    for i in 1..9 {
...
pub fn apply_core_call(
    ...
    match (module, method) {
        ...
        ("core.math", "cmp") => {
            ...
            Ok(CtValue::Int(math_cmp(a, b)))
        }
        ("core.math", "next_after") => {
            let a = as_ct_float(one(0)?, span)?.as_f64();
            let b = as_ct_float(one(1)?, span)?.as_f64();
            Ok(CtValue::Float(CtFloat::f64(math_next_after(a, b))))
        }
        ("core.math", "ldexp" | "scaleb") => {
            ...
            Ok(CtValue::Float(CtFloat::f64(math_ldexp(x, exp))))
        }
        ("core.math", "ilogb") => {
            let x = as_ct_float(one(0)?, span)?.as_f64();
            Ok(match math_ilogb(x) {
                Some(e) => CtValue::Some(Box::new(CtValue::Int(e))),
                None => CtValue::None(Type::Int),
            })
        }
        ("core.math", "logb") => {
            Ok(CtValue::Float(CtFloat::f64(math_logb(
                as_ct_float(one(0)?, span)?.as_f64(),
            ))))
        }
        ("core.math", "significand") => {
            Ok(CtValue::Float(CtFloat::f64(math_significand(
                as_ct_float(one(0)?, span)?.as_f64(),
            ))))
        }
        ("core.math", "ulp") => {
            Ok(CtValue::Float(CtFloat::f64(math_ulp(
                as_ct_float(one(0)?, span)?.as_f64(),
            ))))
        }
        ("core.math", "erf") => {
            Ok(CtValue::Float(CtFloat::f64(math_erf(
                as_ct_float(one(0)?, span)?.as_f64(),
            ))))
        }
        ("core.math", "erfc") => {
            Ok(CtValue::Float(CtFloat::f64(
                1.0 - math_erf(as_ct_float(one(0)?, span)?.as_f64()),
            )))
        }
        ("core.math", "gamma") => {
            Ok(CtValue::Float(CtFloat::f64(math_gamma(
                as_ct_float(one(0)?, span)?.as_f64(),
            ))))
        }
        ("core.math", "lgamma") => {
            let g = math_gamma(as_ct_float(one(0)?, span)?.as_f64());
        ...
        ("core.math", "frexp") => {
            let x = as_ct_float(one(0)?, span)?.as_f64();
            let exp = math_ilogb(x).unwrap_or(0);
