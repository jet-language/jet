...
pub fn apply_repl_authorized_core_call_with_type(
    ...
    if !granted {
        return Err(Diagnostic::error(
            ...
            "REPL host effects require both lexical `#Caps` authority and invocation policy; no host operation ran"
                .to_string(),
            format!(
                "wrap this operation in `#Caps(caps: {}) {{ ... }}`; interactive sessions then prompt, while non-TTY sessions also need `--allow-{}`",
                request.root,
                request.root.to_ascii_lowercase()
            ),
            Some(span),
