...
impl AclProjection {
    fn create(
        ...
        let source_writable = output_dir.is_none();
        if !source_writable {
...
fn read_pipe(handle: RawHandle) -> thread::JoinHandle<io::Result<Vec<u8>>> {
...
fn backend_policy(output_dir: Option<&Path>, share_network: bool) -> String {
    format!(
...
pub(crate) fn run(
    ...
    env: &BTreeMap<String, String>,
    share_network: bool,
) -> Result<BackendOutput, super::execution_runtime::NativeSandboxError> {
    if std::env::var_os("JETPACK_FAKE_SANDBOX").is_some() {
        return Err(super::execution_runtime::NativeSandboxError::Unsupported(
    ...
    let stdout_thread = read_pipe(stdout_read.into_raw());
    let stderr_thread = read_pipe(stderr_read.into_raw());
    let wait_failed = unsafe { WaitForSingleObject(process.raw(), INFINITE) } == WAIT_FAILED;
    let wait_error = wait_failed.then(io::Error::last_os_error);
    let terminate_error = if wait_failed {
        let _ = job.terminate();
        None
    } else {
        job.terminate().err()
    ...
    Ok(BackendOutput {
        ...
        policy: backend_policy(output_dir.as_deref(), share_network),
    })
}

fn io_error(error: io::Error) -> super::execution_runtime::NativeSandboxError {
    super::execution_runtime::NativeSandboxError::Io(error.to_string())
}

fn real_directory(path: &Path) -> Result<PathBuf, super::execution_runtime::NativeSandboxError> {
