...
/// Registered unit-family conversion metadata is parsed as arbitrary-precision
/// source integers. Keep ordinary E0007 integer limits everywhere else.
fn retain_non_metadata_integer_errors(toks: &[Token], diags: &mut Vec<Diagnostic>) {
    ...
    while i + 2 < significant.len() {
        if !matches!(significant[i].kind, TokKind::Hash)
            || !matches!(&significant[i + 1].kind, TokKind::Ident(name) if jet_foundation::Registry::is_exact_integer_metadata_marker(name))
