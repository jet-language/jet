...
fn jet_jit_list_sort_date(list: i64) {
    Concurrency::with_runtime_mut(|rt| {
        let Some(mut pairs) = clone_list_ints_with_runtime(rt, list)
            .into_iter()
            .map(|handle| {
                let index = usize::try_from(handle.checked_sub(1)?).ok()?;
                let value = match rt.time_values.get(index)?.as_ref()? {
                    crate::Time::TimeValue::Date(value) => value.clone(),
                    _ => return None,
                };
                Some((value, handle))
            })
...
host_fns! {
    ...
    list_sort_date: "jet_jit_list_sort_date" => jet_jit_list_sort_date: sig_sort;
    ...
    checked_bit_set_new: "JetBitSet::new" => jet_jit_bit_set_new: sig_new;
    ...
    checked_bit_set_has: "jet_bit_set_has" => jet_jit_bit_set_has: sig_list_eq;
