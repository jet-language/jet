...
`tests/agent_workloads/SHA256SUMS` fixes all task inputs and declared outputs. It holds 35 rows, and the test asserts that exact count. The test compares the recursive file set below `inputs/` and `expected/` with all checksum rows. An extra, missing, or changed file fails. The checksum reader also rejects an absolute path, a path with `..` or an empty part, a duplicate path, and a row it cannot split.
...
| --- | --- | --- | --- |
| Repository search and edit | `repository-marker-scan`, `repository-semantic-edit` | Linux proved; macOS declared native but not run; Windows cannot count Bash as native | #1173 |
| Build, test, debug, and Git | `git-diff-review`, `build-test-failure-recovery` | Linux Git diff inspection and compile-check recovery proved; debug not run; macOS declared native but not run; Windows cannot count Bash as native | #1166, #1173 |
...
| --- | --- | --- |
| `manifest_is_complete_frozen_and_non_vacuous` | Success | The manifest schema, the frozen task set, the 35 checksum rows, and the presence of every declared input, expected output, and adapter source. |
...
| `equivalent_adapters_complete_declared_tasks` | Integration | All ten tasks across all four adapters, cold and warm: exit status, exact stdout, output stability, unchanged input authority, no undeclared scratch residue, and adapter agreement on the declared outcome. |
