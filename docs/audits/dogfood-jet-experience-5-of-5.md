...
| F52 stale packaged Scheduler A01/T1 ICE and current exact-duplicate diagnostic regression | bug/compiler | Closed by `#2415`: raw/archived witness `docs/audits/raw/2393-r1/A01/T1/jet.json` preserves the stale packaged `fact registry law violation: \`Scheduler\` is registered twice; one table means one row per name` ICE; fresh replay receipt `docs/audits/raw/2393-r1/A01/T1/rerun-2026-09-02-jet.json` records the integrated binary, source digest, one `Scheduler` row, one distinct `Target.Scheduler` row, and the complete check/AOT/default/dev/interpreter rerun with no ICE and byte-identical output. Durable regression: `tests/marker_declarations.rs::repeated_scheduler_fact_registration_is_stable_and_conflicts_are_diagnostic`; exact duplicate Scheduler fact registration stays diagnostic without conflating `Scheduler` and `Target.Scheduler`. The matched `#2393`/`#2394` rerun is sealed in the receipt; no owner row remains unresolved. |
| F53 malformed A10/T4 input handling | bug/hostile-input | Closed by `#2419`: raw witness `docs/audits/raw/2393-r1/A10/T4/jet.json`; fresh matched Jet/Rust receipts `docs/audits/raw/2393-r1/A10/T4/rerun-2026-09-02-jet.json` and `docs/audits/raw/2393-r1/A10/T4/rerun-2026-09-02-rust.json` record the durable fixture tree `tests/fixtures/fresh_agent_t4/**`, including `truncated.tsv`, `empty-key.tsv`, `extra-column.tsv`, `unknown-key.tsv`, `duplicate-key.tsv`, `invalid-count.tsv`, and `missing-required-key.tsv`. Valid output is byte-identical across default/dev/interpreter/AOT (AOT when `rustc` exists); every malformed case exits 1 with empty stdout and deterministic line-aware stderr carrying What/Why/Fix, with matched Jet/Rust output. The complete `#2393`/`#2394` rerun records the malformed-input policy; no unowned testimony remains. |
