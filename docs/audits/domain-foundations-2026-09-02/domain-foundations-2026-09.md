...
## Current implementation index (2026-09-05)

This append-only index names the current source locations for the eight first-party battery packages and examples. It makes no completion claim for rows whose implementation receipts are still being integrated. Runtime/build/golden proof is unrun for every row and belongs to #2919; historical probe paths and findings above remain intact.

| Area | Current battery package | Current executable example | Runtime/build/golden proof |
|---|---|---|---|
| Web | `crates/jetpack/batteries/web/package.jet` | `examples/features/web/battery/package.jet`, `run.jet`, `web/package.jet`, `web/client.jet`, `public/index.html`, `web/index.html` | Unrun |
| Games | `crates/jetpack/batteries/games/package.jet` | `examples/features/game/battery/package.jet`, `run.jet`, `assets/`, `runs/demo.jetreplay` | Unrun |
| CLI and scripts | `crates/jetpack/batteries/cli/package.jet` | `examples/features/cli/battery/package.jet`, `run.jet` | Unrun |
| Data analysis | `crates/jetpack/batteries/data/package.jet` | `examples/features/collections/data_battery/package.jet`, `run.jet`, `analysis.jetnb` | Unrun |
| Backend services | `crates/jetpack/batteries/backend/package.jet` | `examples/features/net/backend_battery/package.jet`, `run.jet` | Unrun |
| AI and ML | `crates/jetpack/batteries/ml/package.jet` | `examples/features/math/ml_battery/package.jet`, `run.jet` | Unrun |
| GUI applications | `crates/jetpack/batteries/gui/package.jet` | `examples/features/ui/battery/package.jet`, `run.jet` | Unrun |
| Embedded | `crates/jetpack/batteries/embedded/package.jet` | `examples/embedded/battery/package.jet`, `run.jet` | Unrun |

Web source receipt (separate from this location index): `agent://BatteryWeb` reports source completion for the Web package and example; runtime/build/golden proof remains unrun.
