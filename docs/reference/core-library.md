...
| Built-in `request_id` middleware | Shipped: `server.request_id(mux)` keeps a valid inbound `x-request-id` or assigns one, echoes it on the response, and feeds access events |
