...
| Request target and Host authority | Partial: origin-form and HTTP(S) absolute-form route through the same path; absolute authority must match Host after case, IPv6, percent-hex, and default-port normalization. HTTP/1.1 requires exactly one valid Host, while HTTP/1.0 may omit it; mismatch, userinfo, fragments, unsupported target forms, and malformed or ambiguous authorities fail with 400 and close before dispatch. CONNECT authority-form remains open. |
