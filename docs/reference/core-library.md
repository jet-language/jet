...
| `req.proxy(url)` | `HttpClientReq` | Use an explicit proxy; malformed proxy URLs fail before transport, and env proxies are honored by default |
| `req.send()` | `HttpClientResp ? String` | Execute the request; malformed response status or header framing returns a stable Jet error |
