...
| Host authority | Shipped: HTTP/1.1 requires exactly one valid reg-name, IPv4, bracketed IPv6, or IPvFuture authority with an optional numeric port; HTTP/1.0 may omit Host, while duplicate, empty, malformed, ambiguous, or out-of-range authorities fail with 400 and close before dispatch |
