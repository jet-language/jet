...
| `Client.new().retries(.Safe/.Idempotent/.None)` | `HttpClient` | Stale pooled-connection retry before request bytes / first-byte Io (D-HTTP-CLIENT2): default unset is Safe (GET/HEAD/OPTIONS/TRACE); `.Idempotent` opts in PUT/DELETE; `.None` disables; max one attempt; never status-based; POST/PATCH never auto-retry |
...
| Redirects, cookies, forms, multipart, proxy, phase timeouts | Shipped: request builder methods above including per-request dns/tls/write/first_byte overrides; Client exposes typed `.proxy(HttpProxy)`, `.tls(TlsClientConfig)`, `.redirects(.Follow.{ max:, same_origin_credentials: })`, `.allow_http_downgrade(Bool)`, and `.retries(.Safe/.Idempotent/.None)` (default Safe); ambient `@Context(deadline:)` upper-bounds send totals; live CustomOnly HTTPS trust, HTTPS→HTTP deny/opt-in, same-origin credential strip on Follow, request `.first_byte_timeout` overriding Client phase budgets, and stale-pool safe GET retry / POST no-retry / `.retries(.None)` / `.retries(.Idempotent)` opt-in proven |
...
| Transparent Content-Encoding decoding | Shipped for the native client: gzip is advertised and decoded by default; decoded responses hide Content-Encoding/Length while `raw_content_encoding()` remains inspectable; `.raw_encoding()` opts out; unsupported encodings fail closed (exact `http_client_law`) |
...
| Pooling and HTTP/2 | Shipped for the native client: shared `Client` pools HTTP/1.1 keepalive after drained bodies and multiplexed HTTP/2 sessions (ALPN + explicit h2c); concurrent streams open without holding the connection mutex across HEADERS waits; exact `http_client_law` interop covers reuse, HPACK hostility, and TLS ALPN |
