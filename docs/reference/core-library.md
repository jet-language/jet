...
| `req.proxy(url)` | `HttpClientReq` | Use an explicit proxy; malformed proxy URLs and refused proxy tunnels return stable Jet errors, and env proxies are honored by default |
