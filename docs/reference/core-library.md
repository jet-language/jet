...
| Redirects, cookies, forms, multipart, proxy, phase timeouts | Shipped: request builder methods above including per-request dns/tls/write/first_byte overrides; Client exposes typed `.proxy(HttpProxy)`, `.tls(TlsClientConfig)`, `.redirects(.Follow.{ max:, same_origin_credentials: })`, and `.allow_http_downgrade(Bool)`; ambient `@Context(deadline:)` upper-bounds send totals; live CustomOnly HTTPS trust, HTTPS→HTTP deny/opt-in, same-origin credential strip on Follow, and request `.first_byte_timeout` overriding Client phase budgets proven |
