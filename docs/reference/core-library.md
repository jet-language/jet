...
| Typed JSON both ways | Shipped (D-HTTP-JSON1=A): `resp.json<T>(limit)` on the client, `req.json<T>()` on the server, and `server.json(status, value)` for answers. All three ride the `#Codable` path, each returns one Result, and the raw text and byte paths stay unchanged for experts. |
