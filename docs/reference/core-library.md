...
| Middleware engine | Partial: the one Handler wrapper chain is declaration-order outermost-first, supports request/response mutation and short-circuiting, contains panics, and runs for routed, automatic OPTIONS, plaintext pipelines, TLS HTTP/1.1, and cleartext HTTP/2. The real Jet AOT example is executable; the dev/JIT status remains listed in `tests/jit_gaps.txt`. |
| Built-in `request_id` middleware | Shipped on the approved surface: `server.request_id(mux)` preserves one 1–128-byte visible-ASCII inbound `x-request-id` or assigns a fresh bounded ID, exposes it to access events, and echoes it unless the response already chose one. Plaintext pipelines get distinct IDs; TLS HTTP/1.1 and cleartext HTTP/2 use the same wrapper. |
| Built-in `recover` middleware | Partial: the shared dispatch boundary contains handler and middleware panics and maps them to a redacted internal error on every shipped transport. A separately installable `recover` wrapper and incident publication policy have no approved public spelling yet. |
...
| Bounded streaming bodies | Partial: plaintext HTTP/1 request bodies are pulled from the socket only as the handler consumes the shared Body; unknown-length responses pull at most 64 KiB per write and use chunked framing, socket backpressure, write-idle deadlines, and source cancellation. HTTP/2 request/response bodies stream with flow-control backpressure and cancellation. Gzip request decoding and TLS-backed HTTP/1 requests remain bounded-buffered, and the public compatibility `server.response` surface has no streaming-body or trailer builder yet. |
| Transparent Content-Encoding decoding | Partial: plaintext and TLS HTTP/1.x requests decode native gzip after transfer framing, remove the stale Content-Encoding/Length view, reject unsupported coding with 415 and malformed gzip with 400, and cap encoded plus decoded bodies at the server limit with 413. HTTP/2 request decoding and response compression middleware remain open. |
| Graceful shutdown | Shipped for plain HTTP/1.x, cleartext HTTP/2, and TLS-backed HTTP/1.1: `Server.bind`/`serve`/`shutdown(grace)` stop accepts, send HTTP/2 GOAWAY with the accepted last-stream id, drain active work until grace, cancel stragglers, refuse new streams/requests (including TLS keep-alive reuse), and return bounded report counts |
| Pooling and HTTP/2 | Partial: native cleartext HTTP/2 server serving is shipped; client pooling/HTTP/2 transport is tracked under D-HTTP-CLIENT2 |
