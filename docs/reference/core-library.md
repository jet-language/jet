...
| Middleware engine | Partial: the one Handler wrapper chain is declaration-order outermost-first, supports request/response mutation and short-circuiting, and runs routed responses plus automatic 400/404/405 and redacted 500 recovery responses through the same boundary. Handler errors/panics and middleware factory/runtime errors/panics fail closed without escaping private details. The boundary is shared by plaintext pipelines, TLS HTTP/1.1, and cleartext HTTP/2. The real Jet AOT example is executable; the dev/JIT status remains listed in `tests/jit_gaps.txt`. |
| Built-in `request_id` middleware | Shipped on the approved surface: `server.request_id(mux)` preserves one 1–128-byte visible-ASCII inbound `x-request-id` or assigns a fresh bounded ID, exposes it to access events, and correlates router and recovery responses unless the response already chose one. Plaintext pipelines get distinct IDs; TLS HTTP/1.1 and cleartext HTTP/2 share the same response boundary. |
