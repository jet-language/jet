...
| Middleware engine | Partial: the one Handler wrapper chain is declaration-order outermost-first, supports request/response mutation and short-circuiting, and runs routed responses plus automatic 400/404/405 and redacted 500 recovery responses through the same boundary. Each composed layer is total: handler errors/panics and middleware factory/runtime errors/panics become bounded responses that reached outer wrappers can observe. An outer short-circuit does not run inner middleware. The boundary is shared by plaintext pipelines, TLS HTTP/1.1, and cleartext HTTP/2. The real Jet AOT example is executable; the dev/JIT status remains listed in `tests/jit_gaps.txt`. |
| Built-in `request_id` middleware | Shipped on the approved surface as an ordinary declaration-ordered Handler wrapper: `server.request_id(mux)` preserves one 1–128-byte visible-ASCII inbound `x-request-id` or assigns a fresh bounded ID, exposes it to access events, and correlates router and recovery responses that reach its layer unless the response already chose one. Plaintext pipelines get distinct IDs; TLS HTTP/1.1 and cleartext HTTP/2 share the same wrapper. |
...
| HTTP message trailers | Shipped (D-HTTP-TRAILER-API1=A): `req.trailers()` succeeds only after Body EOF and returns ordered repeated fields or empty headers; `resp.trailers(^headers)` is the sole response builder and consumes its Headers argument. Forbidden fields fail validation. HTTP/1.1 and TLS emit an upfront generated `Trailer` declaration with chunked framing; HTTP/1.0 fails before publishing; native HTTP/2 accepts and emits trailing HEADERS with END_STREAM. |
