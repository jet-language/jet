...
| HTTP/1 response framing | Partial: only HTTP/1.0 and HTTP/1.1 requests reach handlers; unsupported versions close with 505. HEAD preserves the corresponding selected representation's Content-Length and metadata without sending body bytes, including error responses and pipelines; 1xx/204/304 publish neither body bytes nor Content-Length. Streaming, trailers, and HTTP/2 remain open. |
