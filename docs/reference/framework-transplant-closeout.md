...
| D-VALIDATE-DECODE1 | open-conflict | `crates/jet-sema/src/Sema/Registration/Serde.rs`; `crates/jet-codegen/src/Codegen/TIR/emit/functions.rs`; `crates/jet-codegen/src/Prelude/CoreLib/Top/EncodingTraits.rs`; `crates/jet-codegen/src/Prelude/Core/FieldError.rs`; `crates/jet-codegen/src/Prelude/CoreLib/Top/DataFmt.rs` | `tests/corelib.rs`; `tests/encoding_parity.rs`; `examples/features/serde/validate.jet`; `tests/corelib_parts/derives.rs:1097-1285` | `#1161`; owner ballot required against D-MIGRATE3 | Codec failures use `Result<T, [FieldError]>`, but D-MIGRATE3 ratifies `decode_traced => DecodeResult<T>` as a second successful-result API. The retired codec `DecodeError` envelope has no alias or bridge. No migration claims closure until the owner resolves the conflict. `AuthError::DecodeError` is a token-JSON error variant, not a codec result contract. `examples/features/serde/hand_codec.jet` remains unusable evidence: the corpus gate records it `aot_broken` (AOT exit 1 against a golden expecting 0), so no tier comparison ran for the hand-codec path. |
