...
| `core.net` / `core.net.tls` | stream owned | `Net` | `NetError` | may block | Linux proven; macOS/Windows E9 | reachable Core runtime + audited host ABI |
...
| `core.compute` | `Tensor` owned | pure on CPU; `GPU` for explicit accelerator placement | `ComputeError` | sync | native CPU; macOS/iOS Metal; Linux CUDA/Vulkan; browser WebGPU provider | shared Prelude CPU oracle + explicit F32 Metal/CUDA/Vulkan bridges; browser WebGPU provider |
