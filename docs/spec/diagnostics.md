...
| E3204 | Two different `use` forms refer to the same C library `{lib}`. | S59 allows one bring-in per C lib per file — either `use "{header}" as alias`, `use c.{lib} as alias`, or `use c.[{lib} as alias]`, not both. | Remove one line; keep the form that matches your workflow. |
