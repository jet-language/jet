...
| E2307 | A string view would outlive its owning `String`, lacks one stable public source, or reaches an operation that needs an owned `String`. | `View<str>` follows the same inferred receiver/parameter/static provenance rule as `View<T>`; local and temporary owners die too soon, while an owned-`String` use cannot preserve a zero-copy borrow. | Keep the owner alive and use a stable named `View<str>` boundary, or materialize an owned `String` with `~view`. |
...
| E2702 | Crypto API misuse. | After syntax, effects, and types succeed, a compiler-known `core.crypto` argument violates a public cryptographic bound. Live reasons include an explicit expert AEAD nonce list whose length differs from the selected algorithm's exact nonce length and a literal HKDF-SHA256 output length outside 0..8160 bytes. | Pass the exact bound named by the diagnostic; use `core.crypto.seal` for managed nonces. |
