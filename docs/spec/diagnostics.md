...
| E0215 | sema  | a Cell guard is stored in an unsupported aggregate or captured by a lambda (D-LOCALCELL1=A) |
...
| Code | What | Why | Fix |
|------|------|-----|-----|
| E1101 | A spawned task captures a value it does not own. | Tasks run concurrently and may outlive the scope that created them; shared mutable state is not allowed. | Give the task ordinary owned data. Copyable values copy at closure creation; other owned values move. Use a channel to send results back. |
...
| E2305 | A `View<T>` would outlive its owner, or sema cannot stabilize a public owner source for each returned view slot. | Each returned or stored view slot must keep one receiver, parameter, or static source through every return path, call, generic instantiation, field projection, and closed trait implementation; different output slots may name different owners. A local or temporary owner dies too soon; open/dynamic dispatch, function values, lambdas, and list/tuple storage erase that relationship; overwriting a view field could change it. | Keep each owner alive, return or store the view through a named boundary with a stable source per output slot, or cross the boundary with an owned copy (`~view`). |
| E2307 | A string view would outlive its owning `String`, lacks one stable public source, reaches an operation that needs an owned `String`, or an owned `String` is used where `View<str>` is required. | `View<str>` follows the same inferred receiver/parameter/static provenance rule as `View<T>`; local and temporary owners die too soon; an owned-`String` use cannot preserve a zero-copy borrow; filling a `View<str>` slot needs `.trim()`/`.after()`/`.before()` (or a tracked string-view binding), not a plain owned place. | Keep the owner alive and use a stable named `View<str>` boundary, bind a string window, return a `View` of the owning element, or materialize an owned `String` with `~view`. |
...
| E0215 | A Cell guard is stored inside an unsupported value or captured by a lambda. | A Cell guard is a temporary loan handle. Storing it inside another value could keep the loan after its local scope ends. | Keep the guard in a local name or a tuple, and use `.map(...)` or `.split(...)` for projections. |
