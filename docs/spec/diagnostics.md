...
4. When a fix names one exact source replacement, author its `TextEdit` at the
   ...
   change `fix_edits`.
   The registry also supplies `fix_edits[].safety` from D-REPORT-FIXGRADE1=D;
   an edit has no default class. Its `applicability` is `safe` for the two
   auto-apply classes and `suggested` for the remaining classes. An editor may
   show a suggested edit, but unattended `jet fix` does not apply it.
...
7. Run the focused snapshot test without update mode, review the diff, then use
   ...
   `jet run` and the interpreter; do not park a missing tier in a hand ledger.
   Update relevant spec/docs, then run
...
Lint warnings use the same shape with `Warning [L02xx] (lint_name):` instead of
`Error [E02xx]:`; the stable lint name sits beside its rendered code. Lints do not block compilation; the driver prints them
...
`pub` stays exempt. `jet fix` removes a literal-only unused local binding, an
empty private function, or a plain unused import. It renames the name when
removal is not provably side-effect free. A team may promote any of these rows
...
| E0029 | parse | two access markers on one parameter (D-CAP7/D-MEM1) |
...
| E0056 | parse | *retired by D-S14-PAUSE* (was: `mut` permission keyword teaching) |
...
| E0075 | sema | a collecting loop uses a break payload instead of its List result (D-LOOPSTATE1) |
| E0076 | sema | ordinary-loop exits omit a result or use incompatible payload types (D-LOOPSTATE1) |
| E0077 | parse | a scoped grant uses the retired body-binding form (D-ARROW-UNIFY1) |
...
| E0080 | parse | a value-returning braced callable is missing its body arrow (D-CALLABLE-ONE1=A, D-BODY-ARROW1=B) |
...
| E0138 | sema  | an operation used on a nominal `distinct` type whose operation bundles don't grant it (D-CAPBUNDLE1) |
...
| E0205 | sema  | `self.field = v` without the write-access marker `&` on the receiver (D-MUTSELF1) |
...
| E0209 | sema  | a named binding passed where it would be silently cloned — Move-param arg without the move marker `^`, or a std constructor consuming a borrowed value (D-MEM1/S2; hard error, was lint `L0201`) |
...
| E0213 | sema  | the write-access marker `&` is not on a place — a name plus maximal field/index/range projections (D-SHAPE-PLACE1=A) |
...
| E0218 | sema  | `mem.pin` was given a value instead of a write window made with the write-access marker `&` (D-PIN1=A) |
...
| L0204 | jet   | a `flake.nix`/`devenv.nix` field `jet os bridge flake` couldn't translate into `env.*` form (U16) |
| L0205 | jetpack | build sandboxing is unavailable and trusted substitution is allowed by policy (U28, D-JPK-SANDBOX2) |
...
| E0320 | parse | teaching: dotted literal construction → dotless `Type{ … }` / `{ … }` (D-LIT-DOT1) |
...
| E0383 | parse | teaching: source loops join a binding and source with `in`, not a comma (D-LOOP-IN1=A) |
| E0384 | parse | teaching: expression-position membership uses `.contains(x)`, not `in` (D-LOOP-IN1=A) |
...
| E0386 | parse | a bare distributed atom group is a direct operand of `&&` or `||` (D-ARMHEAD-PAREN1=A) |
...
| E0375 | sema  | retired `#Default` on a field; write `field: T{expr}` (D-DEFAULT-SHAPE1=B) |
...
| E0502 | sema  | type can't be a map key (D-MAP-KEY1)     |
...
| E0751 | sema  | Panic is a deny-only effect row (D-PANICROOT1=A) |
...
| E0772 | sema   | a call matches more than one callable body, so positional binding cannot choose safely (D-CALLPOS1) |
...
| E3261 | parse | `#Import(c)` function uses a non-canonical body marker |
...
| E0921 | sema  | a reachable call violates a `!Mem.Alloc`, `!Mem.Rc`, or bounded `!Mem.Alloc(above: N)` denial; reports the source operation, full call path, denial, and provenance (D-MEM-FACTS1/D-AUTHORITY-MEM1) |
...
| E0929 | parse | body-level `derive X;` is retired; request the derive with `#X` and reserve `derive` for provider definitions (D-ONCE-DERIVE1=A) |
...
| E1333 | sema/jetpack | a typed environment fact, language-pack selection, dotenv declaration, or Git hook path is invalid or conflicting (D-ENV-LIFECYCLE2/D-ENV-LANGPACK1/D-ECO6) |
| E1334 | jetpack | an explicit workspace member is missing or is not a Package directory (D-ECO-MEMBERS1) |
| E1335 | sema/jetpack | an environment integration, package-generation provider fact, or executable lease handoff is conflicting, lossy, or invalid (D-ENV-INTEGRATIONS1/D-JPK-PROFILE1/D-JPK-EXECLEASE1) |
| E1336 | jetpack | an environment image cannot project a service, verified package output, or extra file (D-ENV-IMAGE1) |
...
| E1234 | jet   | `jet registry publish` refused: the version is already reserved in the registry index — versions remain immutable after a yank (D-VERSION1) |
| E1235 | jet   | `jet registry publish`/`jet registry yank` couldn't reach the registry index, or the endpoint contains embedded credentials or URL parameters |
...
| E1238 | jet   | a build recipe or staged plan fragment violated its realized-tool, sandbox, authority, or finite-graph contract (D-JPK-ADAPTER1, D-JPK-DYNAMICPLAN1) |
...
| E1260 | jet   | a sandbox's exported `pub fn` isn't one homogeneous `Int`, `Float`, `Bool`, or `Text` shape (D-PLUGIN-EXPORT1=A) |
...
| E1268 | jetpack | a remote OCI base or bare/unsupported push reference needs a verified registry transport; local `file://` layouts and explicit HTTP(S) pushes are supported (U14, D-JPK-IMAGE1) |
...
| E1272 | jetpack | one or more package refs lack a supported native Nix compatibility output (U23, D-JPK-NIXSTORE1) |
| E1273 | jetpack | a recipe-backed package build failed at a logged build step (U27, D-JPK-BUILDDBG1) |
| E1274 | jetpack | no persisted Store record or build explanation exists for the requested package/ref (U27, D-JPK-BUILDDBG1) |
| E1275 | jetpack | no enforceable native sandbox is available for a local executable action (U28, D-JPK-SANDBOX2) |
...
| E1294 | jet | `jet run <entry> -- <name>` names no `#Job fn` in the entry (D-JPK-TASKRUN1, D-JOB-SUBCMD1) |
...
| E1297 | jetpack | `jetpack tool install` bin name collides with a project `#Job fn` (JPK-TOOL-COLLIDE) |
| E1298 | jetpack | `jetpack tool` ref names an external provider with no hangar realization path yet (JPK-TOOL-PROVIDER) |
...
| E1315 | jetpack | Hangar Store v2 ingest, reproducibility certification, or executable lease handoff aborted (source mutated during race-safe copy, unsupported special object/xattr, invalid lease service state, digest mismatch on verify, or divergent producer result) (E4-JP1, E4-JP18, D-JPK-EXECLEASE1) |
| E1316 | jetpack | ambiguous or unmatched typed package variant selection (E4-JP15, D-JPK-VARIANT1) |
| E1317 | jetpack | a direct CLI ref uses a retired provider-first order, the retired `path@` prefix, or the public `@nixpkgs` source spelling (D-JPK-REF1, D-JPK-SNIXREUSE1) |
...
| E0386 | A bare distributed atom group is a direct operand of `&&` or `||`. | A `|` arm head is one distributed group, so its boundary must stay visible before a Boolean operator applies. | Write `(atom | atom) && guard` or `(atom | atom) || guard`; the compiler can insert these parentheses. |
...
| E1333 | a typed environment fact is invalid or unusable | Language packs use one catalog with explicit host, platform, license, and required-tool facts. Dotenv paths stay inside the project, expert allowlists make secret handling explicit, and Git hook paths stay inside the project and name an existing directory. | Fix the language selection/catalog fact, use a project-relative file and `Dotenv{ file, allow, secrets }` with valid variable names, or create an in-project directory for `git_hooks_path`. |
...
| E1335 | an environment integration, package-generation provider fact, or executable lease handoff is conflicting, lossy, or invalid | Integrations and package generations lower into shared typed facts; an executable child may use only a verified leased output, so a path that escapes the lease or a failed handoff cannot run. | Merge the declarations, use a supported package ref, or re-realize the package so Jetpack can hand only its verified lease to the child. |
| E1336 | an environment image cannot project a service, verified package output, or extra file | D-ENV-IMAGE1 projects selected services through one generated OCI supervisor. An image that omits declared services, selects a disabled or unknown service, or lacks an executable in the verified Hangar layer fails closed; an absent, empty, conflicting, or wrong-platform package output, or a missing, unsafe, or secret extra file cannot be copied into an image. | Select the declared enabled service in the image, realize its executable package output for the image target, or choose a regular project-relative non-secret extra file and run `jet image` again. |
...
| E0319 | `` `step` `` is not allowed in a range arm — range arms test a band, not a sequence. | A stride modifies a loop range to skip values (`loop i in 0..10, 2`); an arm head like `1..10` just checks whether the subject is between 1 and 10. A stepped range is not a contiguous band and can't be used for membership testing. | Remove the stride; to match only multiples of N, use a full condition: `subject >= lo && subject <= hi && subject % n == 0 ->`. |
...
| E1110 | `task` has no lexical or parameter task group, uses the wrong lexical group, or `Group` is stored or captured by an escaping lambda. | Structured spawning uses the active lexical `task.group` or a direct `Group` parameter. A group may flow down the call stack, but it cannot become stored state or escape its scope. | Write `task work()` in the active group, or pass the group to `fn helper(group: Group)`; do not store or capture it. |
...
| E0041 (`Mutex`/`RwLock`/`mutex`/`lock`) | `` `<name>` is not in Jet; share data through channels `` | Jet avoids shared mutable state: tasks communicate by sending messages, not sharing memory. | Create `channel<T>()`, and use `sender.send`/`loop value in receiver -> ...`. |
...
| E2305 | A declared `View<T>` would outlive an owner, or sema cannot stabilize a public owner set for each declared view slot (D-MEM-VIEWRET1/D-MEM-COPYSEM1). | Declared view slots keep a bounded set of possible receiver, parameter, or static sources through every return path, call, generic instantiation, aggregate projection, function value, lambda, and trait implementation. Read-only views entering non-view owning slots copy by default; `copies: .Explicit` restores this refusal. | Keep every possible owner alive, preserve the declared view boundary, or remove `copies: .Explicit` / write `~view` when an owning destination is intended. |
| E2307 | A declared string view would outlive an owning `String`, or sema cannot stabilize its public owner provenance (D-MEM-VIEWRET1/D-MEM-COPYSEM1). | `View<str>` follows the same inferred owner-set rule as `View<T>`. Local and temporary owners still die too soon; `copies: .Explicit` refuses the owning materialization needed at a non-view destination, while an owned `String` still cannot fill a declared `View<str>` slot. | Keep every possible owner alive, use a proven `View<str>` boundary, write `~view` at an explicit-copy boundary, or remove `copies: .Explicit` for the default owning copy. |
...
| E0218 | `mem.pin` needs a write window made with the write-access marker `&` into the place being pinned. | A pin promises one storage location will not move, so it has to name that location with the write-access marker; see the sigil reading rule above. | Write `mem.pin(&place)` with the write-access marker `&`. |
...
| E2410 | `E2410: missing required field `{field}`` (runtime `FieldError`). | Decoding into a `#Codable`/`#Decode` type found no wire value for a required field and the field has no declaration default (`T{expr}`) and isn't optional (D-SERDE5, D-DEFAULT-SHAPE1=B). | Mark the field optional (`T?`), give it a declaration default (`field: T{expr}`), or fix the input so the key is present. Compose with `??` to supply a fallback. |
| E2411 | `{Type}` can't be serialized / decoded, or a union member has no compiler-known wire shape. | Only a structurally qualifying type gets a compiler-generated codec (by default or through `#Codable`/`#Encode`/`#Decode`), unless it supplies a hand-written codec. Anonymous-union decoding also needs each member's outer wire shape; a custom or imported decoder does not declare one (D-SERDE1, D-UNIONTYPE1). | Add a compiler-derived codec, use a configured tagged enum or `#CodableAsBase` distinct type for a union member, or remove the unsupported type from the encoded value (for example, with `#Skip`). |
...
| E2422 | Compiler-only `Never` cannot be a function return or parameter type. | `Never` records a non-returning path; it is not a value type in a public function contract (D-NEVER1). | Remove `Never` from the return or parameter type; use `()` for a result with no information. |
...
Enforced SemVer, resolver conflicts, signed-feed audit advisories, freshness,
trust, and integrity verification live here. E26xx is the block for M8. These fire from the
...
| E2608 | registry package name `{name}` is blocked: {reason} | Package names are trust labels. Jet compares each new name with existing names, folds common homoglyphs, and reserves misleading suffixes so a lookalike cannot borrow an established package's identity. | Choose a different package name than `{reference}`, remove the reserved suffix if present, then publish again. |
| E2609 | `{package}#{version}` is not mature under the advisory policy. | A trusted signed feed records the first immutable inclusion of this `{source_class}` release; the configured maturity window has not elapsed. | Wait for the maturity window, or add a reviewed exact `policy.exceptions` record for `{package}#{version}` with an id, reason, and expiry. |
...
| E-APP-TARGET-FEATURE | App feature `.{feature}` needs target `{required}`, but the resolved target is `{target}`. | App is one type across targets, but each feature must be supported by the target selected by `#Target` or the package manifest; rejecting it in sema keeps AOT, JIT, interpreter, and web codegen on one meaning (I9). | Choose a build target that supports `.{feature}` (for example `#Target(JS)` for browser features or a native target for server features), or remove that feature. |
...
| E3625 | replay capture cannot model `{operation}` | The reachable operation has no bounded replay adapter, including opaque, native, task, or unsupported effect boundaries. | Route the operation through a supported deterministic input, or remove it from the captured target. |
...
| L2901 | This `#Test` block has no assertions. | A test with no `assert`, `assert_eq`, or `expect(…).snapshot()` call cannot find bugs — it always passes. | Add at least one assertion, or remove the test if it only exercises compilation. |
...
A failure report leads with the root failure and puts the `?` trail under it
(E3002); `jet explain E3001` explains the report format and the D-OBS2
safe-locals policy.

| Code | What | Why | Fix |
|------|------|-----|-----|
| E3001 | `panic: {msg}` | The program hit a `panic`, `assert`, or `assert_eq` call that failed, a bounds/key check triggered at runtime, or another program-side stop. The shared report boundary owns the program-side stop; an unhandled entry error is a returned report, not E3001. Jet file and line are shown in Jet terms — never generated-Rust terms (I2). | Fix the logic that led to the failure. Program-side stops exit 70 through the shared boundary; unhandled entry errors print their report and exit 1; exit 101 is reserved for Jet defects. |
| E3002 | `Trail [E3002] (N hops via ?, origin first):` followed by one numbered `{fn} ({file}:{line})` line per hop, each with its note after an em dash and `×N` when the same hop repeats. | Each failed `?` joins the failure's trail, so the report can show where the failure started and which callers forwarded it. | Read the root failure above the trail first — it says what went wrong. Then read the trail from hop 1, the origin, down to the last hop at the entry. |
...
| E3104 | Retired allocator-specific use-after-free diagnostic. | Terminal release now uses ordinary consuming `close(^allocator)` through the move marker `^`. | Follow E0121 and acquire a new allocator after close. |
...
| E0355 | A scoped policy is unknown, conflicts, widens an inherited constraint, or is attached at a prohibited scope. | One compiler-owned matrix resolves package → module → function → block while keeping the full declaration chain. Audited authority stays at its sound site. | Use `#Policy(gc)`, `#Policy(explicit_units)`, `#Policy(copies: .Explicit)`, or `#Policy(sentries: .Off)`; write memory floors as `!Mem.*` effect denials. |
...
| L0508 | This arrow loop body computes a value and drops it. | A statement-position loop arrow discards its body's value; the loop takes write access (&) for items when it updates the source. | Bind the loop with `::` to collect its values, or write `loop v in &values -> v *= 2` so it takes write access (&) for items. |
...
| E0375 | `#Default` on a field is retired. | Field absence and construction defaults ride the field's type as `field: T{expr}` (D-DEFAULT-SHAPE1=B). | Write `field: T{expr}` instead of `#Default(expr)`. |
...
| E0380 | This loop needs a named source binding. | An implicit subject is one collection item; a scalar source or nested loop needs a distinct named binding. | Write `loop item in source -> ...`; give nested loops named bindings. |
...
| E0161 | `{what}` | Increment and decrement edit the binding or field in place; see the sigil reading rule above (D-INCR1). | Declare with `:=` or mark the parameter with the write-access marker `&` if the function should change it. |
...
| E3261 | `#Import(c)` function uses a non-canonical body marker. | `#Import(c)` binds a compile-time C symbol; `->` is a Jet function-body marker and cannot declare a foreign symbol. | Write `#Import(c) fn name(args) Return = "symbol"`. |
...
| Code | What | Why | Fix |
|------|------|-----|-----|
| E0043 | `jet install` isn't a Jet command | Jet uses `jet fetch` to download and link dependencies | run `jet fetch` to install all dependencies listed in package.jet |
...
Executable lease planning and handoff use the existing E1335 and E1315 rows;
they do not add a syntax diagnostic or a second lease error family. A
read-only audit boundary failure with no more-specific code uses E1340.
`jetpack audit` remains read-only: a malformed lease or closure projection is
reported through E1340 and is left for `jetpack hangar recover`. The rendered
failure keeps its What, Why, and Fix lines and ends with
`More: jet-lang.dev/e/E1340`.
...
| E1234 | `{name}` {version} is already reserved in the registry index. | Published versions are immutable (D-VERSION1) — a version can never be overwritten or reused after a yank, so anyone who already locked it keeps building the exact same bytes. | Bump the version in `package.jet` and publish again. A yank hides a version from new resolution; it does not free the version number for reuse. |
| E1235 | Couldn't reach the registry index at `{url}`. | The Git registry transport rejects URL userinfo and query/fragment parameters before any network operation. Otherwise the Git operation failed because of network, authentication, or stale local-clone state; authentication comes from the host Git credential provider with path-scoped helper requests, and rendered endpoint details are redacted. | Remove credentials and URL parameters, configure the host Git credential provider for the registry path, then check network access or set `JET_REGISTRY_URL` to a reachable mirror. |
...
| E1238 | Build recipe or staged plan action is not a valid declared action graph. | Build tools must be realized adapter `deps:` packages, and staged plan fragments must use only declared inputs, tools, effects, platform facts, non-overlapping outputs, and acyclic dependencies; a build never falls through to host state or recursive resolution (D-JPK-ADAPTER1, D-JPK-DYNAMICPLAN1). | Realize every named tool, declare every input and authority fact, fix the output/dependency graph, and retry the stage. |
...
| E1260 | A sandbox's exported function has an unsupported signature. | Every parameter and the return type of a sandbox export must use the same Component Model scalar: `Int`, `Float`, `Bool`, or `Text`. | Narrow the signature to one homogeneous scalar shape, or drop `pub` if this function isn't meant to be called across the sandbox boundary. |
...
| E1268 | `` `jet image <name>` cannot use remote OCI reference `<ref>`. `` | D-JPK-IMAGE1: local OCI layouts are copied only after digest validation; explicit HTTP(S) publish uses the host-owned registry adapter, while remote base pulls and bare registry names remain separate trust boundaries. | Use `--push file:///path/to/layout` or `--push https://registry.example/repository:tag`; copy a remote base into a verified local layout first. |
...
| E1272 | `{count}` package refs lack a supported Nix compatibility output. | The listed refs need pinned compatibility outputs. Jetpack does not invoke an installed Nix executable for package realization, so native core and adapted packages can proceed while unsupported Nix refs are reported as holes. | Provide a pinned fixture or verified Hangar output, or replace the listed refs with native sources/adapters; `jetpack add <ref> --adapt` drafts an adapter snippet. |
...
| E1274 | No package record or build attempt exists for `{pkg}`. | `jet explain` reads the Hangar Store and persisted Jetpack realization attempts. If neither record exists on this machine, there is no local package explanation. | Run `jet build <ref>` first; for diagnostic-code help, keep using `jet explain E1234`. |
...
| E1293 | `` lint `{name}` (`{code}`) is denied by policy: {what} `` | `` {why}. This team's `policy.lints.deny` in `package.jet` turns this warning into a build failure (D-LINTPOLICY1 — the override law); it stays a warning everywhere `package.jet` doesn't opt in. The listed lint name matches. `` | Use the underlying lint fix ({fix}), or remove `{name}` from `policy.lints.deny` if this team no longer wants the wall. |
| E1294 | no job named `{name}`. | `jet run <entry> -- <name>` only invokes `#Job fn`s (D-JPK-TASKRUN1, D-JOB-SUBCMD1). | Mark a function `#Job`, or check the spelling; the diagnostic lists declared jobs. |
...
| E1297 | `` `{bin}` is already a job in {path} `` | JPK-TOOL-COLLIDE: `jetpack tool install` would project `{bin}` onto `~/.jet/bin`, but this project already declares `#Job fn {bin}` — the project job wins here, so the global tool would be shadowed. | Install under a different bin name with `jetpack tool install <ref> --as <other>`, or use `jetpack use <ref> -- <command>`. |
| E1298 | `` tool provider `{source}` isn't available yet `` | JPK-TOOL-PROVIDER: `jetpack tool` accepts external providers (`npm`, `pypi`, `cargo`, …) so the surface is discoverable, but that provider has no hangar realization path yet — Jet never silently skips. Built-ins that work today include `nixpkgs` and `github`; local paths are bare. | Use a built-in ref (`name@nixpkgs`, `owner/repo@github`) or a bare local path, or wait for the `{source}` provider to land. |
...
| E1315 | hangar ingest, executable lease handoff, or digest verification failed | Race-safe no-follow ingest re-stats open handles and aborts if the source mutates; the authenticated executable lease protocol rejects invalid keys, frames, owner locks, snapshots, and receipts before a child starts; verify re-hashes and compares the envelope digest; independent producer disagreement remains in `private/unreproducible/<action-key>.json` and cannot satisfy trusted cache policy. | Re-run realization or ingest against a stable tree; if the error names executable lease state, repair the lease service state and retry without deleting the last good Hangar object; inspect the structured reproducibility report and fix the nondeterminism. |
...
| E1317 | A Jetpack CLI ref uses a retired order, path-provider prefix, or `@nixpkgs` source spelling. | D-JPK-REF1 puts the package or upstream target before `@` and the source after it; D-JPK-SNIXREUSE1 makes Jetpack public while retaining nixpkgs as lock and receipt provenance. | Write `target@provider`, a bare local path, or rewrite `package@nixpkgs` exactly as `package@jetpack`. |
...
| L0204 | `{field}` in `{file}` has no `env.*` equivalent yet. | `jet os bridge flake` (U16) is a best-effort translator; some `flake.nix`/`devenv.nix` fields (`shellHook`, multiple named devShells, `buildInputs` vs `nativeBuildInputs`) have no ratified `env.*` spelling. | Review the generated shim and add `{field}`'s effect by hand if you need it — the shim is a starting point, not a full translation. |
| L0205 | Build sandboxing is unavailable; local executable actions will be refused after substitution/remote resolution. | D-JPK-SANDBOX2 requires a strong sandbox for fetched or transitive executable actions. Jetpack may use a trusted substitute or approved remote builder, but never an unsandboxed local action. | Provide a trusted substitute or approved remote builder, or enable the native sandbox. |
...
| `fix_edits` | array | Machine-projected replacements. Each edit has `file`, `span`, `new_text`, and `safety`. |
| `fix_edits[].safety` | string | One class from D-REPORT-FIXGRADE1. The grade is required for every edit. |
...
`why`, `fix`, `applicability`, `fix_edits`, and `cause`. Each `fix_edits` entry
also carries its safety class. The `codeDescription.href` value is
...
editor-only edit path. `jet fix` applies formatting and behavior-preserving
edits by default. `jet fix --all` applies every safety class.
...
`!Mem.Alloc`, `!Mem.Rc`, and `!Mem.Alloc(above: N)` are effect-row
prohibitions. Sema checks every reachable call, including dependency code,
against the function's denial. A violation points to the source allocation or
other incompatible operation, prints the full call path, and names the denial
and its provenance. Open-world dispatch must have a sealed target set or a
signed dependency summary; otherwise the strict fact is unprovable and
rejected.

| What | Why | Fix |
|------|-----|-----|
| `{operation}` at `{source}` violates the denial `{fact}` declared at `{declaration}`. | `{source}` is reachable through `{call_path}` from code governed by `{fact}`; the diagnostic includes the signature provenance. | Remove or replace the incompatible operation, call an implementation whose transitive facts satisfy the denial, or move the call outside the denial. |
| `{fact}` cannot be proved through the open dispatch at `{source}`. | A strict transitive denial cannot assume that an unknown future target avoids allocation, reference counting, or excess arena use. | Seal the target set, consume a signed dependency summary that proves the denial, or move the dispatch outside the denial. |

Implementation note: D-AUTHORITY-MEM1/B and D-AUTHORITY-MEM2/A tombstone the
friendly floor words. No E0922 is allocated for that migration.
...
### E0929 — body-level derive is retired (D-ONCE-DERIVE1=A)

The ratified spelling law leaves `derive` with one job: defining a provider
(`derive T.Trait { … }`). A type requests a built-in or user derive with
the marker before its declaration.

| What | Why | Fix |
|------|-----|-----|
| `` `derive {Trait}` inside a type body is retired ``. | Derive requests have one marker spelling, while `derive` defines providers. | Write `#{Trait}` before the type. |
...
| `` `#Bench` is a retired claim spelling ``. | measurement is a mode of `#Test`, so Jet has one claim family. | Write `#Test("name") { .measure { … } }` and run `jet test --measure` (D-CLAIM-BENCH1=A). |
...
| What | Why | Fix |
|------|-----|-----|
| `` `!{name}` is not a signed auto-derive trait ``. | `!` rejects compiler generation only for structurally auto-derived traits. | Remove `!` from `#{name}`, or use it with an auto-derived trait. |
