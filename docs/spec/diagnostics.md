...
| E2609 | `{package}#{version}` is not mature under the advisory policy. | A trusted signed feed records the first immutable inclusion of this `{source_class}` release; the configured maturity window has not elapsed. | Wait for the maturity window, or add a reviewed exact `{package}#{version}` exception with a reason, reviewer, and expiry. |
