...
| E0979 | `jet os` was given no host to apply. | D-JPK-REF1 amends D-JPK-OSHOST1: a bare host selects `system.<host>` in `./config.jet`; `host@root` selects an exact external root. | Write `jet os switch laptop` or `jet os switch laptop@../machines`. |
