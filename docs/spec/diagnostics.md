...
| E1317 | A Jetpack CLI ref uses retired `provider@target` or `path@target` order. | D-JPK-REF1 puts the package or upstream target before `@` and the source after it; local paths are bare. | Put the target or package first: write `target@provider`, or write a local `./`, `../`, or `/` path without `path@`. |
