...
| E1315 | jetpack | Hangar Store v2 ingest or reproducibility certification aborted (source mutated during race-safe copy, unsupported special object/xattr, digest mismatch on verify, or divergent producer result) (E4-JP1, E4-JP18) |
...
| E1315 | hangar ingest aborted / digest mismatch / unreproducible action rejected | Race-safe no-follow ingest re-stats open handles and aborts if the source mutates; unsupported special files and semantic xattrs (without an explicit platform artifact kind) are rejected; verify re-hashes and compares the envelope digest; independent producer disagreement remains in `private/unreproducible/<action-key>.json` and cannot satisfy trusted cache policy. | Re-run ingest against a stable tree, or inspect the structured reproducibility report, fix the nondeterminism, and run a fresh build from trusted source. |
