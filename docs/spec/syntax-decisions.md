...
**The owner has final say on all user-facing syntax.** Only ratified entries
define the public language surface; agents must not invent syntax.

This file records one final entry per decision. Canonical truth is
`crates/jet-foundation/src/Syntax.rs` plus this file (D-CANON-SOURCE1).
Retired forms get ordinary syntax errors under D-S14-PAUSE.

Vocabulary: [Jet vocabulary](vocabulary.md).

## Ratified

<!-- BEGIN GENERATED LEXICAL LEDGER -->

Generated by `scripts/agent/lexical-ledger.mjs` from `crates/jet-foundation/src/Syntax.rs::LEXICAL_LEDGER`.
...
naming the canonical form. **D-S14-PAUSE / D-TEACHING-LAYER1=A**: the
teaching layer is disabled; retired spellings get ordinary syntax errors, and
stale teaching fixtures are deleted. **D-CAP10**: one definition per name
(E0105); no overloading — capability disambiguation is call-site sigils on a
single definition.

**D-S14 teaching diagnostics** *(diagnostic-only; no new Jet syntax)*: the
shared foreign-word table recognizes common declaration words (`fun`, `func`,
`def`, `function`) and binding words (`let`, `let mut`, `var`, `const`, `val`)
only when their surrounding shape proves a declaration or binding. The
diagnostic names the one Jet form — `fn`, `::`, or `:=` — and supplies a source
edit when the rewrite is mechanical. A function-return colon gets the existing
`->` edit. These words never enter the grammar; ordinary dead-value cases keep
E0003's existing wording.
...
card #1495)*: the current tree keeps `core.files`,
...
**D-CORE-USELIST1=A — Bracketed grouped use** *(ratified 2026-08-06)*: grouped imports use `use path.[a as x, b]`. A missing
...
to `()`. This decision does not add or change tuple syntax. D-NEVER1=C's
compiler-only bottom rule is amended by D-NEVER2=B below; `Never` is now
public only in callable success-return slots.

**D-NEVER2=B — named non-returning contract** *(ratified 2026-09-02, card
#2437)*: the existing `Never` failure-domain name may also occupy a callable
success-return slot: `fn usage() Never`, `fn serve(addr: String) Never
!NetError`, and `fn() Never`. `Never` remains uninhabited in ordinary value
positions, including parameters, fields, stored values, and generic arguments
(the generic case is out of scope and needs its own decision). A declared
`Never` function must have no normal path: falling off the end, bare `return`,
and `return value` are diagnostics; `return Err(e)` is valid in `Never !E`.
Inference remains for unannotated functions, and public always-diverging
functions receive an advisory lint. For `Never !E`, `f() ?? x` has the type
of `x` because the success route cannot produce a value. This is a semantic
return-position use, not a new token.
...
**D-LOOPREAD1=A** *(ratified 2026-09-04, card #2896)* leaves this syntax
unchanged: a state-loop initializer runs once, so a condition that reads an
unchanged header binding can repeat forever. Prefer
`loop line in io.stdin().lines()` for repeated reads, or stop from a body-first
test with `if test { break }`.
...
`&self`/`^self` receivers. A `&` or `^` call-site marker inside a lambda body
keeps its write-lend or take meaning, so the lambda captures that binding by
write lend or by take, never as a hidden consuming move (card #2758).
Passing an unmarked parameter is allocation-free and
...
including the package's own code; E1220 names the offending package, function,
and effect. `grants: { "dep": [Effect] }` per-dep escape; malformed block E1221.
...
**D-PLACE1=A — Layout alignment and lock-free scalar cells** *(ratified)*:
reuse the one `T.@layout` fact, including its exact
`T.@layout.size` and `T.@layout.alignment` projections; no `size_of` or
`align_of` aliases enter the language. `#Layout(c, align(N))` puts alignment
inside the existing layout family. The portable form accepts positive
power-of-two byte values from 1 through 1048576 inclusive. The explicit
target-supported expert form is `#Layout(c, align(target, N))`; it removes
the portable ceiling but still requires a positive power of two supported by
the selected target. Effective alignment is the maximum of the requested and
natural alignments; C field order and padding rules remain unchanged, with
trailing size and stride padded to the effective alignment.

**D-ATOMIC-WIDTH1=A — One exact atomic value family** *(ratified)*:
`Atomic<T>` is safe Core for `Bool`, `I32`, `U32`, `I64`, `Int`, and `U64`.
Fixed-width values use their direct one-word representation; `Int` keeps
exact integer semantics and grows through the same canonical family rather
than saturating or silently falling back to a lock. The surface has
`load`, `store`, `add`, `compare_exchange`, `publish`, and `observe`; all
ordinary operations are sequentially consistent, while `publish` and
`observe` are the intent-named release/acquire pair. No method accepts a
memory-order argument or exposes a relaxed operation. Unsupported targets
reject the declaration rather than silently using a lock fallback.

**D-LAYOUT-ALIGN1=A — Portable and target alignment modes** *(ratified)*:
`align(N)` is the portable profile rule; `align(target, N)` is source-local
expert syntax whose acceptance is owned by sema and the selected target
profile. The compiler never clamps a requested ABI alignment or silently
turns an expert request into the portable form.
...
`from_array`/`to_array`
(E2510/E2511). Raw intrinsics behind `#Unsafe`. Built-in lane/linalg
operators keep fixed shapes (`T.splat(x)` lifts a scalar into every lane);
user-defined operators, including mixed operand pairs, use the D-OPDEF1 hook
traits as extended by D-OPMIX1=D.
...
**D-TEST-HISTORY1=A**: `testing.histories<Command>`
generates typed operation sequences, compares independently obtained public
observations, and shrinks failures to valid replayable histories. Foundation
`TestingHistory` owns generation and shrinking across applicable execution
tiers. Use the existing comparison, controlled-world, and replay contracts;
do not add another scheduler, comparison schema, or fixture-specific runner.
The model must not call the production behavior, directly or transitively.
Unknown opaque dependencies cannot certify model independence.

**D-TEST-STRATEGY1=A** *(ratified 2026-09-07, #2973)*: choose a strategy per
test through the optional `strategy: HistoryStrategy<Command>` argument.
Omission uses the compiler-derived type-valid strategy. An explicit strategy
replaces derivation; the two never merge. Ordinary functions may return a
reusable strategy value. The immutable value requires all five fields:
`generate`, `rebuild`, `valid`, `bounds`, and `distributions`.

```jet
rules :: testing.HistoryStrategy<Command>(
    generate: generate_bookings, rebuild: rebuild_bookings,
    valid: valid_bookings, bounds: booking_bounds,
    distributions: booking_weights)
testing.histories<Command>(seed: 41, cases: 500,
    strategy: rules, model: reference, actual: production,
    observe: public_state)
```

`generate` receives `HistoryRng`, `case_index: Count`, and `max_steps: Count`,
and returns optional `TypedHistoryCase<Command>`. `rebuild` receives
`HistoryCase` and returns optional `TypedHistoryCase<Command>`. `valid` receives
`TypedHistoryCase<Command>` and returns `Bool`. These use the existing checked
callback mechanism. `HistoryRng` exposes deterministic index and bounded-integer
draws. The compiler supplies the lossless `Command` codec; users do not write
a second codec.

The public records project existing Foundation data, not another wire format:

- `TypedHistoryCase<Command>` contains `case: HistoryCase` and
  `commands: [Command]`.
- `HistoryCase` contains `case_id`, `seed`, `operations`, and `schedule`.
- `HistoryOperation` contains `index`, `name`, `arguments`, `creates`,
  `consumes`, `preconditions`, `depends_on`, `task`, and `event`.
- `HandleId`, `TaskId`, and `EventId` retain logical identities, never raw host
  pointers. `HistoryPrecondition` retains `HandleLive`, `HandleState`,
  `TaskCompleted`, and `EventAvailable`.
- `HistoryBounds` exposes `max_steps`, `max_resources`, `max_shrink_attempts`,
  and `max_discarded_cases`. The existing `cases` argument remains the case
  bound. `HistoryDistribution` contains `operation` and `weight`.

Strategies declare handle lifetimes, domain preconditions, and task/event
dependencies. Invalid generated actions are counted as discarded and do not
run. Rebuild failure rejects a shrink candidate. Every accepted shrink keeps
validity and the same failure observation.

Save effective bounds, weights, discards, concrete operations, and schedule.
Replay identity binds source, strategy, tool, and target; a seed alone is not
identical replay after an artifact changes. Use the existing secret-redaction
policy. Unknown oracle dependencies, zero useful cases, invalid schedules,
exhausted bounds, unavailable resources, cancellation, and timeout never pass.
This is bounded exploration, not universal proof.
...
- **D-FFI-R1=A**: `r.*` (root reserved by D-DATA-BRIDGE1) — sidecar
  Rserve-style worker; `data.frame` ↔ ordinary typed row lists through the
  framed JSON channel. The earlier `core.data.Table` wording is superseded by
  D-QUERY-RETAIN1; CRAN remains the provider and plots return as SVG values
  (D-DATA-PLOT1-compatible). Its effect clause is `-[FFI.R]>`.
...
**D-FFI-CONTRACT1=A — one foreign contract, evidence per obligation**
*(ratified 2026-09-05, #2933)*: extend the existing
binder descriptor, not a second contract registry. Each ownership, lifetime,
effect, and failure fact records its evidence basis, checker, assumptions,
and exact artifact coverage. Distinguish proved, enforced, contained, trusted,
and unknown facts. Declarations, flags, target, generator, loaded implementation,
and transitive callbacks and native dependencies participate in identity and
invalidation. Optional source analysis and compiler integration supply evidence
to the same binding generator. A safe wrapper does not prove its foreign
implementation safe; missing facts remain unknown and native trust retains
its existing audited boundary.

**D-FFI-AUTO1=A — proved adaptations with checked conditions**
*(ratified 2026-09-05, #2933)*: generate an adaptation
only from an established rule and its satisfied contract. Pointer/count
collapse requires the exact count unit and meaning, native-width checks, and
buffer-extent checks. Preserve identity, aliasing, ordering, partial success,
failure distinctions, callback behavior, and effects. Do not silently copy,
retry, change placement, or invent an error translation. Established error
translations may be automatic. Missing facts preserve a checked native shape
where possible, otherwise refuse with the missing obligation.
`jet inspect bind <binding> --explain` shows the selected plan and evidence;
`jet bind <binding> --shape native` selects the native shape without weakening
memory checks; `jet bind <binding> --freeze` pins that plan; and
`jet bind --policy frozen` rejects unreviewed project-wide changes. Freeze
binds the exact inputs and selected plan, not merely a library name.

**D-FFI-GUEST1=A — host-native facades over stable bridges**
*(ratified 2026-09-05, #2933)*: extend existing Library
output and export rows for C, C++, Rust, Zig, Go, Python, and JavaScript.
Use selected C-compatible shims and supported host-runtime interfaces;
component projection remains available where appropriate. Cover both
directions with the same eligible contract. Preserve native identity,
ownership, integer ranges, encoding, enums, errors, and runtime roots;
require explicit adaptation where a host cannot express the contract.
The ratified resource example uses generation-checked views: it observes
20, then `ExpiredView` after owner replacement, then 50 through a fresh view.
Close consumes the owner exactly once; views do not own foreign storage.
A Rust true-borrow projection is an explicit stronger alternative, not a
silent change to which programs compile. The Node example explicitly selects
an asynchronous facade; no generator silently changes scheduling.

**D-FFI-CALLBACK2=A — managed registration with explicit completion**
*(ratified 2026-09-05, #2933)*: generate an owning
registration and a borrowed event for each accepted callback invocation.
`event.stop()` requests stop without waiting on its own callback.
`source.unsubscribe(^sub)` consumes the registration and returns a task that
completes only after native shutdown acknowledgment and all accepted calls
finish. Captures remain alive until then; a request to stop is not completion.
Validate lifetime, thread affinity, reentrancy, effects, retained native access,
and failure obligations before exposing a safe facade. Cancellation preserves
partial effects and implies neither rollback nor retry. Panics and exceptions
must not cross incompatible unwind boundaries. A native implementation that
never acknowledges shutdown does not authorize premature release.
...
- **D-QUERY-RETAIN1=A**: `core.data` exposes one typed `Query<T>` over ordinary
  list results, checked readers, and checked SQL. `collect` materializes an
  ordinary `[T]`; `plan` inspects the ordered operations without materializing.
  The maintained query surface is typed `filter`, stable `sort_by`, `map`,
  `min`, `max`, duplicate-preserving `inner_join`/`left_join`, and `group_by`
  with `Group<K, V>` reductions. `DataStream<T>` remains one-shot and
  fallible, and `DataLoader<T>` remains typed and stateful. Public
  `Table<T>`, `Series<T>`, `LazyFrame<T>`, and `DataGroup` carriers are retired.
This supersedes the earlier `D-DATAFRAME1` carrier wording; historical
alternatives and review evidence remain historical records.
...
`package.jet` `target:` > file marker. `#HTML(Path{"path.html"})` names a companion
...
**D-FFI-BUILD1=A — explicit preview and acceptance activate imported builds**
*(ratified 2026-09-05, #2933)*:
preserve D-BUILDLEGACY1 optional graph import and independent host builds.
`jet build --import cmake:build --preview` presents a candidate without
activating it. `jet build --import cmake:build --accept <plan-digest>` activates
that exact reviewed digest. Ordinary `jet build` never accepts new import
intent. Compiler-driver adoption remains distinct from full build ownership;
a compilation database is not a complete build graph. Tools, custom actions,
inputs, outputs, environment, unsupported edges, and any retained inner foreign
...
**D-JPK service preset** *(2026-08-22; no new syntax)*:
...
- **D-WD8**: jetos activation always has plan/diff, with VM proof and rollback
  ...
  changes.
- **D-WD9**: Jet has a typed data floor over ordinary lists, one deferred
  `Query<T>` carrier, checked readers and loaders, maintained keyed updates,
  stats, and plotting; Python/R/GPU bridges cover gaps and expose native
  replacement status.
  - **D-DATA-SURFACE1 (=A, ratified 2026-07-06, #237)**: ordinary lists,
    typed `Query<T>`, typed readers/loaders, stats, and plot builders live
    behind one beginner import, `core.data`; expert modules may sit below it
    with the same operation names.
  ...
  - **D-DATA-PLOT1 (=A, ratified 2026-07-06, #237)**: Core plotting starts with
    first-party deterministic SVG plus a text backend over ordinary lists of
    typed `Group<K, V>` values; bitmap export may layer on the same model
    later. The shipped line family is `line_text(groups, options)` /
    `line_svg(groups, options)`; `DataLineOptions` carries title, x/y labels,
    point markers, an optional horizontal reference, line style, color, and
    legend. Bar and line renderers use deterministic output and the same
    checked geometry rules on every execution tier.
...
codec) or one scalar named `search`/`query` (query codec) is the search input;
a parameter named `data` receives the route loader's decoded value. `.loader`
attaches to the route registered immediately before it, names that route's
handler, takes the same path/search inputs, and returns a Codable value or
`!E`. Any other input, a non-`WebPage` handler return, or a loader out of
place is E2810. The App builds one WebRouter from these facts; navigation,
preload, cache, abort, and pending/error/not-found boundaries run there, and
a served page carries the encoded route data as `#jet-route-data`.

**D-DX-SERVERFN1=A — derive typed server calls from App actions**: `.action`,
`.form`, and `.data` register one checked server-function boundary and expose
the same endpoint to native forms and scripted clients. Sema proves the input,
success, and error contracts are serializable before code generation; the
generated fact carries the stable endpoint, HTTP method, wire types, declared
effects, retry/idempotency policy, and dependent-data revalidation keys.
Each boundary composes the canonical middleware vocabulary in this order:
context, auth, CSP, logging, observability. Mutating calls use same-origin
CSRF by default. These hooks are one ordered request/function pipeline, not a
second RPC registry. Native forms remain usable
without JavaScript; hydration adds pending/result state and invokes explicit
dependent-data revalidation. Development inspection receives metadata only;
release output never ships a dev panel or request/response secrets.
...
**D-JOB-SUBCMD1=C**: D-JOB-SUBCMD1=C is wired
...
law; see the D-CONC entries above.
...
`tests/decisions.rs` checks ratification tags from the canonical `Syntax.rs`
sources, rejects provisional source flags and missing keyword/sigil decision
adjacency, and requires superseded IDs to retain tombstone provenance.
...
### Marker and effect declarations
...
Owner rulings recorded without ballots: the strategy-attachment unification was withdrawn on the owner's pushback (allocator, container, type, and scope-attribute are different things on purpose; `#Policy(gc)` keeps its one-line shape).
...
D-NAME-REFLECT1=A, D-NAME-SIGIL1=A** *(card #1625)*.
...
D-CHOOSE-HEADS1=A, D-CHOOSE-FNBODY1=A** *(card #1651)*. D-CHOOSE-FIND1=A already records the
...
- **D-CHOOSE-HEADS1=A** — the table proofs check multi-head coverage and
  ...
  shadowed head. The surface does not change; any later table sugar requires a
...
**2026-08-13 — D-FAIL-EXIT1=A / D-FAIL-EDGE1=A — default-fallible entry and
process exit** *(cards #1533/#1536)*:
The default-fallible `fn run()` uses the generic entry report and exit-1 path.
Program-side stops and explicit process termination cross the shared Prelude
boundary; target adapters carry that report in their native shape. No source
spelling changed.
...
**2026-08-13 — D-META-USER1=A / D-META-DSL1=A** *(cards #1540, #1542)*: a
`marker` declaration may have an ordinary checked body; without one it records
facts, and with one it may reject or emit additive items. A checked text block
uses the closed `#Name { … }` boundary and resolves the name through the
declaration registry; `#SQL<Row>` and `#HTML { … }` use their declared `.Block`
rows. The old marker-only DSL whitelist is retired by the syntax-chore lane.

**2026-08-13 — D-CONF-MODULE1=A** *(card #1524)*: declared profile settings
seed the shared typed build-fact snapshot. A closed `@build.settings.<name>`
expression is folded by the same Tier-0 evaluator used for generic-module
value arguments, and its normalized value is part of the existing instance
fingerprint. Semantic instance facts retain the profile and declaration
provenance; no execution tier reads settings.
...
All six D-REPORT decisions are ratified as option A. This exact slate extends
D-FAIL-BREACH1's one-renderer rule to every report moment and surface.
D-REPORT-HOME1 defines the typed report-row home, and D-REPORT-MACHINE1 defines
the JSON envelope; the outcomes below are durable report law.
...
**2026-08-14 — D-FMT-SIMPLIFY1=A** *(ratified 2026-08-06)*: `jet fmt` keeps
its default one-style output. The opt-in `jet fmt --simplify` mode applies R1:
a comment-free braced function body containing one `return` is emitted as the
canonical `::` one-line body when it fits width 100; wide or commented bodies
remain braced. The rewrite round-trips through the parser's span-free AST
identity check and is stable on the second pass.

**2026-08-14 — D-MAPSPACE1=A** *(ratified 2026-08-06)*: type-position map
shorthand emits `[K:V]`. Map literal entries and fields keep `key: value`
spacing. The parser and generated grammar remain whitespace-agnostic;
formatter output and in-repo examples use the one canonical type spelling.

## The one-crossing slate (D-BOUND-*, ratified 2026-08-07, card #1655)

The boundary audit's rulings: all nine decisions are ratified as option A.
Tower is the decision home; this block records each outcome. The surface text
each outcome renders lives with its subject — the typed-head entries above,
`spec.md` § Boundary crossings and § Data-schema binders, and the registered
diagnostic rows — and is not repeated here. Four outcomes carry amendments,
named in their bullets: SINK1 discloses the F9 types-not-markers note, RAW1
narrows D-UNIFYLIT1's head-body lexing to backslash handling and leaves
D-REGEX-LIT1 untouched, PROV1 amends D-AUTHORITY-MANIFEST1's trust block, and
TAINT1 names its off-switch spelling.

- **D-BOUND-LAW1=A — one crossing law**: nothing foreign becomes Jet silently. Every comptime, build, link, and run crossing names its schema and leaves its fact. The five-column grid — time, schema, checker, evolution, fact — is spec law. A new boundary feature names one cell or waits for an owner ruling; the ratified literal, manifest, dependency, link, wire, validation, migration, fact, and trust rules are recorded instances of that grid, not separate mechanisms.
- **D-BOUND-HEAD1=A — checked URL, Path, and DateTime heads**: `URL{"…"}`, `Path{"…"}`, and `DateTime{"…"}` are checked typed values, not fallible runtime constructors; an invalid head is E0155. A URL hole is percent-encoded, a Path hole is one encoded component, and a DateTime head takes no hole. Runtime strings keep their existing `parse`/`from` constructors. No amendment: the forms ride D-UNIFYLIT1, D-DOTCTOR3, and D-CORE-PATH1.
- **D-BOUND-RAW1=A — typed head bodies own their escapes**: inside any `Type{"…"}` body a backslash is literal text owned by that head's grammar, so a checked pattern reads as written. **Amendment**: this narrows D-UNIFYLIT1's head-body lexing, and backslash handling only — quote, brace, multiline-closing, and hole rules do not move, and D-REGEX-LIT1 stays untouched, so Regex still refuses interpolation. Plain strings keep the four-entry escape table.
- **D-BOUND-SINK1=A — retired marker contract** *(superseded by D-TEXTHEAD-TYPE1=A, card #2185)*: the former `marker Name on [.Text] { check … hole … }` route is decision history only. The ordinary nominal type and `CheckedText` implementation now own the checked-text sink; the planned sink registry remains deleted.
- **D-BOUND-TAINT1=A — origin facts clear on typed decode**: `net.*`, `fs.read*`, `env.*`, process output, and FFI marshalling seed a `$origin.*` fact in the one ratified flow store. The fact spreads with no ceremony and clears on a successful typed decode or typed construction, because a value that just proved a schema is not raw attacker text. `SQL.raw`, `HTML.raw`, and `Sh.raw` refuse origin-marked text with E0721; `#Scrub` stays the word for a hand-written sanitizer, and `jet inspect gates` lists every gate. The fact is sema-only and is erased before every execution tier. **Off-switch**: one typed settings row on the D-CONF rail refuses seeding project-wide; the proposed spelling is `settings: { origin_facts: off }`, the final key is the owner's, and the row stays unbuilt until that spelling is ratified (I7).
- **D-BOUND-EVOLVE1=A — published schemas keep unknown wire fields**: a `#PublishedSchema` codec keeps the fields it does not recognize and re-emits them in their original relative order, so an old binary cannot eat a new field during a rolling deploy. Plain `#Codable` keeps today's drop and `#DenyUnknownFields` keeps its refusal with E2412 — refuse, drop, preserve, one word each. This extends the D-MIGRATE1/D-MIGRATE4 bookkeeping and changes no known-field decoding, marker behaviour, or FieldError shape.
- **D-BOUND-BIND1=A — the binder verb reads data schemas**: `jet inspect bind json|csv|sql|xml|proto <file>` writes visible ordinary Jet source under a provenance header that records the command, the input, its `sha256`, the format, and each inference rule applied. Regeneration is explicit and nothing imports the result for you, so D-NAME-FILES1=C holds; the same verb still binds language headers.
- **D-BOUND-UNDO1=A — a foreign call inside `#Transact` declares its undo**: the compiler cannot read a foreign body, so an `extern` call inside `#Transact` is E0746 unless its binding carries `#Undo(inverse)`; the named inverse joins the ratified rollback path and runs in reverse order with the other rollback actions. The error names the whole fix vocabulary: hoist the call out of the block, or declare the inverse. **Amendment**: this extends the D-TXN2-era irreversible set, which was exactly Net, FS, and Exec, and it rides D-ROLLBACK-TRAIT instead of adding a transaction mechanism.
- **D-BOUND-PROV1=A — dependency provenance has one read surface and a chosen posture**: `jet inspect provenance` prints one lock-backed record per dependency — locked hash, transparency entry, publisher identity, build attestation — each marked enforced, verified, or recorded, with the same record behind the human and JSON output. Integrity stays enforced by E1204. **Amendment**: the `require: none | logged | attested` row is new vocabulary inside D-AUTHORITY-MANIFEST1's ratified trust block; `none` is the default, so an unattested dependency still resolves until a project raises its floor.

I9 requires the Prelude to own the crossing behaviour — the checked-text
interpolation kernel, ordered wire merge, taint erasure, rollback hook, and
provenance record — while AOT, Cranelift JIT, and interpreter engines call the
same `jet_*` symbols rather than re-encoding the rules.

The only user-typeable syntax in this slate is HEAD1's three typed values,
RAW1's body lexing, and UNDO1's `#Undo` marker row. SINK1's former marker form
is retired by D-TEXTHEAD-TYPE1=A; this slate adds no other spelling.
...
**2026-09-04 — D-LOOPREAD1=A** *(card #2896; ratified 2026-09-04)*: a
state-loop header initializer runs once. If its condition reads the unchanged
binding and another turn is reachable, L0529 warns; an unconditional
`break`/`return` suppresses it. The grammar does not change: use
`loop line in io.stdin().lines()` for repeated reads and stop with
`if test { break }` in the body.
...
The six A outcomes below are the operative record for the
structure-program-is-a-value slate. They are ratified law.
...
## Whole-language audit slate (2026-09-02)

These twenty-two decisions were ratified on 2026-09-02. Each row records the
picked rule and its technical rationale.
fact instances of one checked program, and the evidence records of one run. One typed query contract reads all
three. Every projection (check, test, prove, inspect, dev, receipts) uses it. Evidence grade lives on claims
only; rights keep their gate kind and provenance and never receive a grade. Time is a value on the query, not
a column on the schema. **Technical:** Amends D-FACT-LAW1=B by adding the evidence clause for claims and the
shared-query clause for tools. Does not fold FactRegistry (Facts.rs:337-494) into Registry.rs; both stay
behind the contract. Registration rows stay &'static. A parity test proves that every existing tool answer can
be reproduced through the contract before any tool switches. 
- **D-RIGHTS1=C —** Purity is the empty row. #Replayable lowers to the row that excludes Time, Rand, Net, and
IO. A compile-time block holds the row that allows only Mem. Every spelling stays and one walker checks them.
The transaction rule stays its own checker because it is a duty: an irreversible effect owes an undo. Which
effects count as irreversible becomes a declared fact that checker reads. The command-line spelling is decided
in D-RIGHTS-CLI1, not here. **Technical:** Deletes Sema/Purity.rs:77-99 and the replayable walker at
Sema/Effects.rs:1480-1565. The comptime purity stage split and its impure_builtin leaf table
(crates/jet-comptime/src/Comptime/Purity.rs:78-123) become one row read by the shared walk. Amends D-REPLAY1
and D-SHAPE8 as lowerings. D-TXN1-4 and D-BOUND-UNDO1 unchanged. Precondition: a differential matrix runs
purity, replayable, comptime, and invocation cases through both the old walkers and the row walker and shows
identical verdicts before the old walkers are deleted. 
- **D-RIGHTS-CLI1=B —** allow and deny take comma-separated effect names, roots or leaves. The words match
authority.holds in package.jet. Precedence: names expand to their leaves first, then deny wins; naming the
same effect in both flags is a usage error. Jet reads these two flags before the program's own flags; a
program passes literal --allow to itself after --. The old twenty flags stop with a message that prints the
exact replacement. **Technical:** --allow=FS.Read,Time --deny=Net. Amends the D-CLI-GLOBAL1 flag set; reserves
the two names against #CLI structs with a diagnostic. No all sentinel: the pure preset stays jet eval --pure,
which lowers to the empty row under D-RIGHTS1. Scripts outside the repository are migrated by hand; jet fix
does not edit shell files. 
- **D-RIGHTS-DIAG1=B —** Every rights report uses one frame: the call, the chain of callables it reaches the
right through, the scope chain, the nearest scope that could grant it. The repairs with their safety class.
The code stays distinct so a tool can tell a package denial from a transaction rule. A package grant is never
an automatic edit: rights only widen by a written word, so that repair is graded needs-review; removing the
call or tightening a row can be behavior-preserving. E0750 keeps its own frame because its repair is to fix
the effect name. **Technical:** Adds a denial_kind field and the chain to the jet.report/v1 objects of E0740,
E0712, E1803, E3521, E3401, E3403, E0725, E0746. E0921 (amends their I4 snapshots and D-REPORT-MACHINE1 detail
fields, no code retires). fix_edits use the five ratified safety classes (D-REPORT-FIXGRADE1=D).

- **D-CLAIM1=C —** jet test runs examples, doctests, property tests, and, new, the contracts of effect-free
functions with inputs generated from their #Pre, within a bounded budget. jet prove stays the ratified
umbrella; with --lens solver (the only producer-enabling lens, D-PROVE-LENS1) it adds solver evidence: it
proves the implication precondition implies postcondition, never the precondition alone. Both commands write
the same evidence report. jet fuzz folds into jet test --grade=generated --iterations=N. Lenses stay
presentation views. A contract that cannot be generated (an effectful function, a precondition that names
another function, a type without a generator) is reported as not generated, with the reason. **Technical:**
Amends D-JPROOF1 additively: ProofReport gains an evidence summary per claim (producer, outcome, count) under
a version bump; D-PROVE-LENS1 unchanged; the jet fuzz surface folds into jet test. Eligibility: default
generation only for callables whose inferred row is -[]>; effectful contracts need a #Test fn with fixtures.
Default budget: 1,000 generated inputs per contract, settable per package. 
- **D-GRADE1=D —** Every evidence record is stored as producer, outcome, and count: solver proved, generated
passed 4000, examples passed 1, runtime check present. A claim keeps all its records; nothing is overwritten.
The report shows the strongest rung by one acceptance order used only for package floors: proved above
generated(n) above examples(n) above checked above unchecked. Gates such as #Unsafe are not claim evidence and
are reported by their own kind in the rights plane. **Technical:** Evidence record = { producer, outcome,
count, record_id }. Summary word = strongest rung reached. Counts are part of the word: generated(4000). No
static rung: compiler facts are not claims. No assumed rung: gates keep their kind. 
- **D-GRADE-POLICY1=A —** policy.claims.min names the floor with its count. It tightens only, like every
policy field. A claim below the floor is a diagnostic whose fix names the command that can raise it when a
generator exists, or the property test to write when one does not. **Technical:** Amends the
D-PACKAGE-POLICY-SCOPE1 field set with claims: .{ min: <rung> }. Rungs per D-GRADE1 option D; a count is part
of the rung, no max field, obeys tighten-only. 
- **D-SHAPE-ONE1=A —** Every format reads the same field facts, and database rows and FFI layout join now.
**Technical:** Rejected by the rival review: database migration from a field diff has no table identity or
transaction contract, and FFI shares only layout and skip with the others. 
- **D-SHAPE-PROJECT1=A —** args.decode<T>() decodes a struct from the process arguments anywhere in a program.
It lowers through the same core.args builder that fn run(args: T) uses, so help and diagnostics are
byte-identical. env.decode<T>() and its parameters are unchanged. A new combinator applies the ratified
precedence outside the entry function: an explicit flag wins, then an environment value, then the field
default (D-CLI-GLOBAL1). A present but invalid value is a FieldError, never treated as absent. **Technical:**
core.args gains decode<T>() with the same signature family as env.decode. Binds to the D-CLI builder floor
(syntax-decisions.md:183-196); no second parser. Precedence combinator: Config.merge(args, settings) returns T
![FieldError] with the stated order. 
- **D-LIVE1=A —** Saving a file swaps the changed functions into the running program. #Persist values survive
as ratified. A layout change to a pinned type still triggers the ratified announced restart, unless
D-LIVE-PERSIST1 selects migration for #PublishedSchema types. Every save prints one line naming swapped
functions, kept values, and any restart. jet dev --restart refuses swap for the session; dev: .{ swap: false }
refuses it for the package. **Technical:** Amends D-DEV4 and D-DEVMODE1 defaults (swap first, rerun only when
swap is impossible). D-HOTSWAP1 layout rule unchanged. Module mutation law unchanged: #Persist remains the one
writable module binding (D-PERSIST-DEVSTATE1=A). Precondition carded before the flip: a long-lived edit test
on the JIT and interpreter engines with callers mid-execution, proving swap keeps program meaning (I9).

- **D-LIVE-PERSIST1=A —** When the changed type is #PublishedSchema and a migration covers the change, the
pinned value migrates in place through the same step function that decode uses, and the save line says so.
When the type is not published or no migration covers the change, the ratified announced restart happens and
the line prints the migration block to write. A rename of the pinned binding resets its state, and the line
says so. **Technical:** Extends D-MIGRATE4's step functions to resident values. D-HOTSWAP1 restart stays the
fallback. Identity stays module path plus binding name (D-PERSIST-DEVSTATE1), no new marker.

- **D-VERDICT-LOOP1=D —** Every row keeps the ratified report line. A new typed no_fix_reason field, mutually
exclusive with fix_edits, holds a reviewed reason and the next action. A coverage baseline per plane never
decreases: a new row without fix_edits or a reason fails the registry today; existing rows are classified
plane by plane. Each plane closes when its rows are all covered. Structured-edit coverage is reported
separately from reason coverage. Machine status is one stable object on pass and fail; human progress lines
stay plain as ratified. **Technical:** Amends D-REPORT-MACHINE1 additively: one optional typed field,
no_fix_reason, joins the report object under a schema version bump to jet.report/v2. The one-object-per-line
law is unchanged. Its envelope clause is unchanged: envelopes are status data holding these objects. Five
safety classes unchanged (D-REPORT-FIXGRADE1=D). no_fix_reason = { kind: behavior | design | ambiguous, next:
<text or link> }. Status object: { schema: "jet.status/v1", action, ok, reports: [...] } for every command in
--json mode, including infrastructure failures. Amends D-DX1 (status envelope) and the I4 registry guard
(baseline rule). 
- **D-CLI-ONE1=A —** jet inspect types, rights, claims, shapes, structure, build, gates. Each takes a file or
package, or --live PID for a running process, or --replay ARTIFACT for a record. Ledger views fold into these;
task actions (schema, codemod, sbom, bind, logs, info, outdated, digest, env, expand, output, provenance,
semindex, impact, graph, query, explain-build, compiler, audit, live) keep their spelling. report and dossier
fold into inspect build, jet explain stays. **Technical:** Fold table: facts→types, authority→rights,
unsafe→gates, guarantees→rights, structure→structure (kept name), gates→gates (kept name), report→build,
dossier→build, live <pid>→--live PID on every plane. Retired names: facts, authority, unsafe, guarantees,
report, dossier (six). Scope grammar: jet inspect <plane> [TARGET] [--live PID | --replay ARTIFACT]; TARGET
defaults to the current package; --live and --replay are exclusive. Amends D-DX4 and D-SHAPE-CLI-COMPLETE1.
jet help prints the old name and its new home for one edition. 
- **D-RECORD1=B —** Every artifact keeps its ratified bytes. One index under .jet/records lists them with one
identity key (target inputSha256, tool, engine) and cross-links a proof to the replay it consumed and the
receipt it produced. Jet dev captures the ratified safe replay (clock only) on every run, within a project
retention budget. Release never captures without the ratified flag. jet build --verify rebuilds from the
receipt inputs and reports identical bytes or the first differing input. When a target reaches randomness,
input, or the network, `jet dev` shows the ratified preflight once per session and asks for the phrase
`capture sensitive`. Consent holds for that session while the target, the reachable authorities, hosts and
streams, byte limit, disclosures, and destination stay the same; any change asks again. Each reload is one run
with at most one artifact, mode 0600, under the budget. A session without a terminal runs unrecorded for such
a target and says so; explicit `--record` and `--capture-sensitive` keep their E3627 refusal. **Technical:**
...
authorities, hosts and streams, byte limit, residual disclosures, and destination; any inequality re-prompts.
Non-TTY dev prints `capture: skipped (Net at net_app.jet:9); no terminal to consent` and continues; `jet prove
--capture`, `--record`, and `--capture-sensitive` keep E3627 before any effect. 
- **D-RECORD-SPELL1=D —** `jet prove TARGET --replay ARTIFACT` stays the proof consumer and `jet debug
--replay=NAME` the debugger's. `jet prove TARGET --save ARTIFACT` copies the replay under
`.jet/records/saved/<artifact_id>` and lists it in the index as a claim named `replay <artifact_id>`. Every
`jet prove` of that target replays saved claims first. Two saves of one artifact are one claim; `--unsave
<id>` removes it. A saved claim guards a path, not a build. Each record is matched by an anchor that survives
edits elsewhere: the function that made the request, the record kind, and its order within that function. An
edit that does not touch that function keeps the claim alive. Operation, arguments, and request fields are
still compared, as ratified. Verdicts: passed when every record is consumed and the expectation holds. Failed
when the fault returns or the output differs. Diverged (E3623) when an anchor is missing or ambiguous, or a
record does not match. Vacuous when the fault's function no longer exists. Every verdict except passed names
`--unsave` or a fresh capture, and only passed lets `jet prove` exit clean. **Technical:** Adds `--save` and
`--unsave` to jet prove; no new command, no new file kind, no change to jet run. Saved claims carry producer
replay in the evidence report (D-CLAIM1 option C) and run before doctests, which amends D-PROVE-SEM1 producer
ordering. Amends the D-JREPLAY1 identity and outcome law for the saved-claim consumer only; plain `--replay`
keeps exact identity and E3621. Anchor = (function stable id, record kind, ordinal of that kind within the
function), stored in the index at save time beside the immutable artifact. Re-binding needs a unique
anchor-to-site mapping; absence or ambiguity is E3623. Renaming or removing the request's function breaks its
anchor: the claim reports diverged, or vacuous when it was the fault's function. Expectation is fixed at save
time from the recorded outcome. `no-fault` (recorded runtime_stop): passed when the outcome kind is exit with
any status; failed on any runtime_stop, naming the code. `same-output` (recorded exit): passed when kind,
status, stdout and stderr lengths, and hashes all match; failed on the first differing field. Sensitive-form
records match by their ratified request digests. The index row stores the recorded identity, the identity last
passed under, the fault anchor, and the expectation. 
- **D-TIMETRAVEL2=B —** Backward stepping is a jump to an earlier event over
  deterministic replay. `jet debug TARGET --replay=NAME` opens a capture and
  permits scrubbing, jumping, and skipping by re-executing from the start with
  recorded values. Reverse step (`back N`) is the corresponding jump; `back 3`
  from event 11 means `jump 8`. Replay adapters must preserve byte-identical
  locals for repeated jumps and across adapters. Checkpoints may optimize the
  operation without changing its semantics; forward replay remains the
  fallback when reverse stepping is unavailable.
- **D-OPENTABLE1=D —** Tables with a Jet twin are generated from it with a
  drift guard; tables without a twin keep one Rust home under a guard against
  duplicate copies. The guarded surfaces include effect roots and leaves,
  marker sites, fact reads, encoding formats, taint sinks, transaction
  irreversibility, the Core dispatcher, ambient routes, the export classifier,
  the CLI inventory, report enums, and the tier list. Irreversibility is
  declared on FS.Write, Net, Exec, and FFI. Non-derivable rows remain explicit
  declared facts. Generation runs over committed declarations without a
  bootstrap cycle: the lexer that parses declarations is not generated from
  them.
- **D-RAWSTR1=A —** Jet currently gives backticks no meaning; this option assigns them to raw `String`
literals. Inside, backslashes, quotes, braces, line endings, and indentation stay exactly as typed; no escape
is processed and `{name}` is not evaluated. A group of one or more adjacent backticks opens the literal, and
the next group with the same count closes it; groups with other counts stay text. This is the Markdown
code-span rule. When the text starts and ends with a space, Jet removes one space from each end, unless the
text is only spaces. That space rule lets a value begin or end with a backtick: type one space just inside
each backtick group. To keep an edge space, type one extra space at each end. The empty `String` keeps its
ordinary spelling `""`, because adjacent backticks merge into one opening group. A typed Windows line ending
stays carriage return plus line feed. The result is the same ordinary `String` as `""`, with every String
operation, including joining with a normal string. The lexer, the part of the compiler that splits source into
tokens, only looks for the closing backtick group; a backslash never starts an escape. **Technical:** New
token in Lexer/Tokens.rs; Syntax.rs entry with this decision ID (I7). Amends S8 (interpolation off inside) and
S20 (escapes off inside); S70's multi-line trimming does not apply inside a fence. Fence rule: the opening
fence is the maximal contiguous run of N backticks at the token start. The literal ends at the next maximal
contiguous run whose length is exactly N; runs of any other length are content. Padding rule: when the content
begins and ends with an ASCII space and is not all spaces, one space is stripped from each end. Jet borrows
only these two rules from CommonMark; its line-ending normalization does not apply, and the String keeps every
byte, so CRLF stays CRLF. A fence inside an ordinary string hole is one nested expression, and braces inside
it do not close the hole. The formatter invariant is identical decoded bytes after reparse. An unterminated
fence is reported with its opening line. 
- **D-TRAILCOMMA1=A —** Every comma list accepts a trailing comma: parameters, arguments, list and map
literals, constructor bodies, marker groups, effect rows, import lists, type arguments, and loop headers. jet
fmt writes a trailing comma when the list spans lines and removes it when the list fits on one line. A comment
after the last item counts as part of the multi-line form. **Technical:** Parser list productions all use the
same
optional-final-comma rule; formatter list renderers use source delimiters and comments to choose multiline
layout. 
- **D-SEMI1=A —** An explicit semicolon is the ratified E0373 with a
  behavior-preserving edit: a line break when code follows on the same line,
  removal at line end. The parser keeps a boundary at the same position so one
...
- **D-MARKERARGS1=A —** The companion page argument is a checked Path value. A malformed path fails at compile
time with the Path grammar's diagnostic. **Technical:** Marker signature change in Prelude/Markers.jet
(D-MARKSIG1): HTML(page: Path). Migration: jet fix rewrites #HTML("x") to #HTML(Path{"x"}).
