...
| 2026-06-30 | D-DISPLAY-SHAPE | **Display hook is `fn display(self) -> String`** (A): the user-facing display protocol is a single method `fn display(self) -> String` on the type; no trait impl syntax. Interpolation `{}` calls it. Confirmed no default; must be explicit. **Ratified — not yet implemented.** c51 | owner |
| 2026-06-30 | D-DEBUG-REDACT | **Debug redaction uses `#[Redact]` field marker** (A): fields that should not appear in debug output are marked `#[Redact]`; auto-derived Debug replaces their value with `"[redacted]"`. No per-call redaction controls. **Ratified — not yet implemented.** c51 | owner |
