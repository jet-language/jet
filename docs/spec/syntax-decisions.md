...
**S80 — Error carrier & fallible `run`** *(D-ERR2, D-S80-RUN1, amended by
D-FAIL-ERROR1/D-FAIL-EXIT1)*: default `Err` carries message, optional code,
and optional cause. `Err("msg")`, `Err("msg", code: "CFG404")`, and
`Err("msg", cause: e)` build it. `fn run()` is fallible by default; an expert
may pin `fn run() ? E`. `Error` is deleted, not aliased. Cross-type `?`
