...
type annotation before the sigil (`ratio: Float #= ratio: Float=  3.14`, `count: Int #= ratio: Float #= 3.14`, `count: Int=  0`).
...
omit per-item `pub`. **Ratified (D-VISDEFAULT2=A, 2026-06-28):** marker is `#PubFile`;
exceptions inside that file use `priv`. S18 private-by-default remains the global baseline.
...
`**impl Type { ... }`** block (Rust-style layout, Jet-owned semantics), or
as a top-level external inherent method `**fn Type.method(self)**`
(D-EXTMETH1). All forms are equivalent; pick whichever keeps the file readable. A
...
**D-INCR1 — C-style increment/decrement (M3)** *(ratified 2026-06-30)*:
`**++` `--`** in all four forms (`++x`, `x++`, `--x`, `x--`). Prefix
returns the updated value; postfix returns the old value. Operand must be
a mutable integer lvalue (`:=` binding, `~` parameter, or `~self` field).
Sized integers trap on overflow like `+= 1` / `-= 1` (D-NUMOPS2). Indexed
slots (`xs[i]++`) are rejected (same as compound `+=` on an index). Float
and other non-integer types are rejected (E0162). Immutable bindings are
rejected (E0161). A second spelling beside S17 compound assignment —
owner chose C transliteration ergonomics over I8 uniformity on this site.
...
**Surface (D-CFFI2-SYN, ratified 2026-06-16; amended by
D-CFFI-SYNTAX-REOPEN and D-CFFI-CANON1):**
...
Retired teaching spellings: `@extern module`, `#extern module`, `@bindgen
module`, and `#bindgen module`. Rejected: bare `extern c raylib { }` globals
(S59 provisional A); shadow-only override (overlay must merge with bindgen);
two `use` forms for the same C lib in one file. Rust FFI (S50) unchanged. See
[`decision-ballots.md`](decision-ballots.md).
...
the `;`. See `examples/features/basics/method_chain.jet`. The pipe operator `|>` is
...
column, and is idempotent. See `examples/features/basics/multiline_string.jet`.
...
**S81 — `?continue` loop skip** *(ratified 2026-06-16; superseded 2026-06-29 by D-ORRETURN-CANON1=A)*:
The `?continue` spelling is retired. Canonical form is `expr ?? continue` (see D-ORRETURN-ERG1 / D-ORRETURN-CANON1).
...
example `examples/features/effects/single_use.jet`. **Open follow-on (deferred, needs a
...
golden-verified). `tests/ui/typestate_wrong_state`, `examples/features/types/typestate.jet`,
...
`examples/features/effects/taint.jet`, `tests/taint.rs`. **One spelling fork deferred:** the
...
non-parameter or non-callback is E0748. Example: `examples/features/effects/effect_levers.jet`.
...
name: Type #= val` / `name: Type :=  val`) `=` also
...
### Marker family, CLI flags, syntax wave (ratified 2026-07-01)

Fourteen ballots. Four ratify status-quo or no-syntax outcomes: **D-UFCS1** (B — no UFCS;
`fn Type.name(self)` extensions stay the one method spelling), **D-POINTERCHAIN1** (A — no
compact pointer cast/deref chain; explicit `mem.cast_ptr<T>(p)` + postfix `p.*` inside
`#Unsafe`, plus a Jai-transliteration doc), **D-ERRCTX1** (D — automatic `?`-propagation
trace in dev builds + stdlib `.context("…")` for human wording; no new grammar), and
**D-ANY-JAI1** (A — trait-bounded heterogeneous varargs `parts: ...Renderable` over
D-VARIADIC1's grammar + reflect handles as the tooling floor; owner's two follow-ups both
answered without new syntax: reflect handles (option B) already ship inside A, and requiring
a trait *set* is ratified S45 `<T: A + B>` bounds — `fn f<T: Renderable + Serializable>(parts: ...T)`).
General `Any` stays rejected per D-DYNAMIC-TYPE1. The rest mint surface, below.

> **Note on ballot examples:** the c8syntaxwave cards' code samples use illustrative
> v5-proposal notation (`::` bindings, `type X :: Base`). Live syntax is D-BIND3
> (`#=`/`:=`) and the D-DIST1/D-DIST3 `distinct` machinery; entries below are written in
> current law.

**D-MARKER-FAMILY1 — two-plane sigil law: `@` contracts / `#` directives / `$` splices**
*(ratified 2026-07-01, option B)*: `@` may only precede a declaration and states a checkable
contract about it — a promise the reader or checker can verify (`@Pure`, `@MustUse`,
`@Codable` are the ballot's canonical examples). `#` never states a contract: it changes what
compiles, when code runs, what is legal in a region, or supplies a compile-time value — and it
may appear inside types/expressions where `@` never does (`#Unsafe(...)`, `#Test`, `#(Fs)`,
`[T#N]`, `pkg#1.2.3`, `#Caller()`). `$` stays splice-only. Reverses D-ATTR1's blanket
"everything is `#`" stance (2026-06-19) for the contract subset only. Loop-label suffix `@`
(D-LOOPLABEL2) is a different grammatical slot and unaffected. **Two follow-up ballots gate
implementation** (raised 2026-07-01, open): **D-CONTRACTCASE1** — casing of the `@` plane
(D-CASING1's PascalCase law is textually scoped to `#`-markers; this batch itself is split:
the ballot's gist writes `@Pure`/`@MustUse`, but D-PREPOST1/D-PERSIST1/D-CAPBUNDLE1 ratified
lowercase `@pre`/`@persist`/`@numeric`); **D-MARKERMOVE1** — the exact reclassification list
(which existing `#` markers move to `@`; `#Numeric`, `#PublishedSchema`, `#Redact`, and the
field-level serde markers are genuinely ambiguous under the law and are not renamed by
guess). No existing marker is renamed until both are decided.

**D-CLIFLAG1 — typed entry-signature CLI parsing** *(ratified 2026-07-01, option A)*: giving
the entry function a typed parameter opts it into CLI parsing — `fn run(args: ServeArgs)`
derives `--flag` names from `ServeArgs` fields, defaults from field defaults, `--help` text
from field-level doc markers; an enum parameter derives subcommands. `fn run()` stays
zero-ceremony. Bad-flag input is an ordinary Jet diagnostic (what/why/fix). Same
derive/reflection machinery as `#Codable` (D-SERDE4). An explicit `cli.parse<Args>(...)`
call remains the library floor for non-entry parsing (REPLs, plugins, tests). Mints a
struct-level CLI-derive marker and a field-level doc marker; their spellings ride the
D-CONTRACTCASE1/D-MARKERMOVE1 follow-ups.

**D-UNITLIT1 — unit-suffix numeric literals** *(ratified 2026-07-01, option A)*: a numeric
literal may carry a unit suffix resolving to a `#UnitFamily` member imported into scope —
`500ms`, `12.50usd`. Sugar over the existing D-QUAL3/D-DIST3 distinct-type path, not a new
type. Suffix resolution is import-scoped. No implicit cross-unit conversion: `1s + 500ms`
compiles only if the family declares the conversion, else a diagnostic names the missing
conversion fn. A suffix shaped `e`+digits stays reserved for float-exponent notation and may
never be a unit name. **Supersedes D-LITSUFFIX-SCOPE** (2026-06-30), which had closed the
literal-suffix idea in favor of `px.{100}` dot-construction — that construction form remains
valid; this adds the literal spelling for `#UnitFamily` members.

**D-TRAILBLOCK1 — trailing block call argument** *(ratified 2026-07-01, option A)*: when a
call's final parameter is a function type, the call may end with a bare `{ }` block standing
in for that last lambda argument — `ui.button("Save") { prefs.save() }` instead of
`ui.button("Save", () => prefs.save())`. v1 is zero-parameter blocks only; a lambda binding
parameters keeps the `(x) => …` form inside the parens (deliberately dodging the Kotlin
`it` / Swift `$0` implicit-name debate). No new sigil: a `{` directly after a call's `)` is
the trailing zero-arg lambda — Jet has no bare-block statements, so the slot is free. The
formatter owns one canonical layout (implementation).

**D-PREPOST1 — `@pre`/`@post` function contracts** *(ratified 2026-07-01, option A)*: a
signature may carry `@pre(condition, "message")` (a claim about the arguments) and/or
`@post(condition, "message")` (a claim about `result`). Conditions are pure expressions —
no effects — enforced by the same purity checker as `#Pure`. Checked in **every** build by
default; a build-policy escape may strip them per module for measured hot paths (explicit
opt-out, not a debug/release split — the C-assert tradition was rejected). A violated clause
raises a diagnostic quoting the clause at the call site. Complements the open D-RANGETYPE1
(a range type states a fact about one value's shape; pre/post state a relation between
arguments and result). Spelling is lowercase as ratified; final casing rides D-CONTRACTCASE1.

**D-EFFBUDGET1 — package effect budget: always-on report + opt-in enforcement** *(ratified
2026-07-01, option D)*: with zero config, every `jet build` prints a one-line summary of the
effects the dependency graph uses and records per-dependency effect provenance in the lock
file. An `effects: { allow: […], deny: […] }` block in `pkg.jet` turns on whole-graph
enforcement — the build fails naming the exact dependency path and function when a transitive
dep needs an effect outside the list. `grants: { "dep": [Effect] }` is the per-dependency
escape hatch, recorded in the lock file so exceptions are an auditable diff. Shares the
closed ten-effect vocabulary (D-EFF4). Distinct from the open D-BUILDPOLICY1 (build-*time*
code authority). Manifest keys, not language grammar.

**D-PERSIST1 — `@persist` dev-hot-reload state survival** *(ratified 2026-07-01, option A)*:
a module-level binding marked `@persist` survives a `jet dev` hot reload instead of resetting;
unmarked state resets as today. Identity is module path + binding name (rename = fresh state,
predictable). On a reload that changes the value's type layout, the dev runtime attempts a
Codable-style re-decode, falling back to reinit + a one-line warning — never a crash. Purely
dev-tier: release semantics are identical with or without the marker. Persist-everything-by-
default (B) rejected as carrying live sockets/handles across reloads by default. Implementation
rides the JIT hot-reload runtime (E3 exit criterion); surface ratified ahead so runtime work
builds against a fixed contract. Casing rides D-CONTRACTCASE1.

**D-DESTRUCT1 — struct destructuring: bindings + if-table patterns, mandatory `..` for
partial** *(ratified 2026-07-01, option A)*: the dot-construction shape (D-DOTCTOR1/2) runs
backwards. In binding position, `.{ id, severity: sev } #= incident` binds `id` and
`severity` (renamed `sev`). The same shape works as an if-table pattern head —
`.{ kind: "page", target, .. } -> page(target)` — matching on named field values and binding
the rest. **`..` is mandatory whenever the pattern does not name every field**, making
exhaustiveness visible at each destructure site. This **amends S74** (2026-06-15,
implemented), which shipped struct destructuring irrefutable with no partiality marker —
existing partial destructures need `..` added when this lands (a breaking spelling change to
shipped syntax; migration pass across examples/tests required). Nesting allowed one level;
the formatter discourages deeper towers.

**D-CHAINCMP1 — chained comparisons** *(ratified 2026-07-01, option A)*: `0 <= sev < 10`
desugars to `0 <= sev && sev < 10` with the shared middle operand evaluated exactly once.
Only same-direction chains of `<`/`<=`/`>`/`>=` are legal, any length; a mixed-direction
chain (`a < b > c`) is a compile error naming the direction break (closing off the C
silent-bool-comparison footgun). `a < b < c` is already a type error today, so no existing
program changes meaning. `==`/`!=` chains excluded (use table dispatch). No new token.

**D-CAPBUNDLE1 — named capability bundles for nominal types** *(ratified 2026-07-01, option
A)*: a nominal distinct type (today `Usd #= distinct Decimal` via D-DIST1/D-DIST3; the
ballot's `type Usd :: Decimal` spelling is v5 illustration, not minted here) starts with no
inherited operators; tagging it with capability bundles re-exposes a curated slice of the
base type's operations while preserving nominal identity (`Usd + Usd -> Usd`; `Usd + Eur`
still a type error). Four bundles ship: `@numeric` (`+ - * /`, ordering, same-type only),
`@comparable` (`==`/`<`, hash/sort), `@printable` (interpolation/display),
`@codable_as_base` (encode/decode via the base wire representation). Bundles stack.
Per-operator grant lists (B) rejected as new pseudo-syntax. **Relationship to the shipped
`#Numeric` marker (D-DIST3/D-QUAL3) is not resolved by this ballot** — the reconciliation
(and whether `#Numeric` becomes `@numeric`) is part of the open D-MARKERMOVE1 follow-up;
neither spelling changes until it's decided. Casing rides D-CONTRACTCASE1.
...
| 2026-06-30 | D-INCR1 | C-style `++`/`--` (all four forms) on mutable integer lvalues; prefix returns new value, postfix returns old; index rejected | owner |
...
| 2026-06-25 | D-BUILDPROFILE1 | **build profiles** (A — named, flag-selected): a package's `build { }` surface defines named profiles (`release`/`debug`/`ci` as `Build.{ optimize: …, debug_info: …, small: …, panic: …, features: […], env: { … } }`); the active profile is chosen by an explicit flag (`--release` sugar for `--profile=release`, general `--profile=<name>`), never by ambient environment — same commit + same flag ⇒ byte-identical binary on every machine. Blessed names `release`/`debug`/`ci` carry built-in defaults; manifest entries override them; others are user-defined. Ambient-env selection (B, the `CMAKE_BUILD_TYPE` footgun, an ungated ambient read on the default build) rejected as it contradicts D-CTEFFECT1. **Ratified — implemented 2026-06-30 (c16iwukt)** | owner |
...
| 2026-06-25 | D-WORKSPACE1 | **monorepo index → a Jet `Workspace` surface** (B — fully computable, owner-modified from rec A): retire the root `jetpack.toml` monorepo index for a `module workspace` (in `workspace.jet`, parallel to `env.jet`/`system.jet`), so the whole project is ONE grammar (Jet) instead of two and the `sources:` double-home across `jetpack.toml`+`env.jet` ends. Owner chose **B (full power)** over the rec A (restricted `find()`-only + generated lock): the `members:` field may run arbitrary `comptime` (members derived from anything — `find("./packages") + comptime gen()`), consistent with the owner's computed-modules / pure-eval direction. Accepted trade: external tools must evaluate Jet to know the layout — mitigated by the resolver still emitting generated `[[workspace_member]]` entries in `.jet/lock` for the common case. Keep-`jetpack.toml` (C, two languages) rejected. **Ratified; partial impl 2026-06-29**: `workspace.jet` with `module workspace { members: [...] }` and `members: find("./packages")` is implemented, tested in `tests/workspace.rs`, and demonstrated in `examples/workspace/`; the generated member index is folded into `.jet/lock` per D-WORKSPACELOCK1. Arbitrary comptime member expressions and sparse subtree fetch remain on c90. | owner |
| 2026-06-25 | D-METADEPTH1 | **v1 user-metaprogramming ceiling** (A — reflection/derives only): user code may READ type info (S56 reflection) and author derives that generate behavior (`derive T.Encode`) on top of the compiler-provided built-ins (S55, e.g. `#[Codable]`), but may NOT rewrite arbitrary code, inject AST, or define macros. The no-macros non-goal is **load-bearing**, not provisional (Owner Q4). A⊂B⊂C is a ladder revisitable post-self-host: B (a read-only `lint`-style rejection pass — Marcus's "no panic in net") and C (full Jai message loop / user macros) stay OFF the v1 table and rise only by a future vote. Honors I3 (codegen dumb), I8 (small surface), and pairs with D-CTCODEGEN1= A (generated code re-enters sema). Per owner, **full Jai (option C) is tracked as a frozen card** for later consideration. Devil's-advocate analysis (full-Jai vs A across beginner/student/power-user/enterprise) recorded on the ballot card (Owner Q5): "inside the language" removes only a toolchain seam, not Jai's costs (local reasoning dies; still build-time RCE/non-reproducible); A is the safe-by-default, forward-compatible floor. **Ratified** (S56 user-authored-derive surface tracked as a board card; built-in derives already shipped). | owner |
...
| 2026-06-27 | D-CODECS1 | **compression codecs** (A — gzip + zstd now, brotli later): standalone codec APIs ship as `core.compress.gzip` and `core.compress.zstd`, separate from archive containers; brotli is deferred to a follow-on. Bootstrap implementations use the owner-approved stdlib bridge posture (runtime/package side, hash-pinned, native-ize obligation; never in compiler `Source/`). Package-only (B) and archive-only (C) rejected. **Ratified — implemented 2026-07-01** (c67/cviw4t7; plan `compression-codecs`; examples 187/188). | owner |
| 2026-06-27 | D-PUBPKG1 | **package-scoped visibility** (A — `pub(package)`): add a visibility tier between private and public. `pub(package)` exposes an item to packages in the same payload/workspace package boundary while hiding it from downstream consumers. `internal` (C) rejected to avoid a new keyword; private+`pub` only (B) rejected because it over-exposes shared internals. **Implemented 2026-06-29**: parser accepts `pub(package)` on visibility-bearing items/imports and rejects other `pub(…)` with E0411; sema permits same-package access and rejects downstream package access; public API/freeze surfaces exclude package-scoped items. c74/c12iwkw3. | owner |
| 2026-06-27 | D-SWIZZLE1 | **vector swizzles** (A — member swizzles read + write): vector lane names and swizzles (`v.xyz`, `v.wzyx`, `v.xy = .{...}`) are blessed member access on vector types, including lvalue swizzles. Read-only (B), method-only (C), and defer (D) rejected. Overlapping write swizzles must be diagnosed rather than miscompiled. **Implemented 2026-06-29**: sema `field_type` + write overlap (E3110/E3111); TIR read/write lowering; example `155_swizzle.jet` + ui snapshots. c82/c1aix1yl. | owner |
| 2026-06-27 | D-PROP2 | **effect prohibition spelling** (A — `#(!Net)` inside the existing effect set): a leading `!` before an effect name means this function and every reachable callee must not use that effect. Dedicated `#Forbid(Net)` (B) and `#(-Net)` (C) rejected. **Implemented 2026-06-29** (parser effect-list negation + sema inverse propagation seeded from declared positives + E0749 + ui snapshot + golden example 152). c18 | owner |
| 2026-06-27 | D-PROTO2 | **protocol declaration spelling** (A — `protocol Name { client -> server: Msg(...) }`): a top-level `protocol` block declares the ordered conversation and generates `.Client`/`.Server` handles over the existing linear + typestate machinery. `session` (B) and hand-written `state`/`#Transition` (C) rejected. **Ratified — implemented** (`protocol` parser, E0154, sema expansion, `examples/features/concurrency/protocol.jet`, `tests/ui/protocol_out_of_order.jet`). c20 | owner |
| 2026-06-27 | D-VARIADIC1 | **variadics and spread** (A — `...` for all three jobs): variadic parameters use final-position `name: ...T`, call spread uses `f(...xs)`, and list spread uses `[...a, x, ...b]`. List-only (B), variadic-only (C), and continued deferral (D) rejected. This is distinct from S75 fan-out, which calls once per element. **Implemented 2026-06-29** (c93): lexer/parser/sema/codegen, E1310–E1312, `examples/features/basics/variadics_spread.jet`, ui snapshots. | owner |
| 2026-06-27 | D-SEMINDEX1 | **stable semantic-index API** (A — versioned public API over compiler seam crates): expose symbols, references, types, call graph, and effects through a stable public query surface instead of private LSP internals. Internal-only (B) and defer (C) rejected. **Implemented** (`crates/jet-semindex`, `jet semindex --json`, schema snapshot tests). c96 | owner |
| 2026-06-27 | D-DBDRIVER1 | **generic database driver interface** (A — Driver trait + parameterized-only API, SQLite first): `core.db` grows a backend-neutral driver layer that accepts SQL plus separate parameters and exposes no raw string-execute escape. Per-database-only APIs (B) and async-gated deferral (C) rejected. **Ratified — implemented 2026-07-01** (core.db `DbConnection`/`DbValue` types + SQLite implementation + example 128; transaction hooks are explicit `.begin/.commit/.rollback`, not `#Transact` reuse — that mechanism is value-snapshot rollback, not SQL transactions). c117 | owner |
...
| 2026-06-28 | D-RANDSPLIT1 | **PRNG and CSPRNG APIs are structurally separated** (A — distinct namespaces + typed outputs + misuse lint): seedable deterministic randomness and cryptographic randomness must be separate enough that crypto misuse is caught. **Ratified — implemented 2026-06-29**: `core.random.bytes(n)` (SplitMix64 PRNG, seedable) and `core.crypto.random.bytes(n)` (/dev/urandom CSPRNG) added. Both return `[Int8N]`. `core.crypto.random` registered as known core module; both carry `Effect::Rand` (non-deterministic in `#Pure` fns). W0410 lint code reserved; name-based misuse lint is a follow-up requiring call-context correlation. Example `examples/features/crypto/random_api_split.jet`. c52 | owner |
...
| 2026-06-28 | D-BIGINT1 | **Core BigInt** (A — explicit construction, no auto-promotion): add arbitrary-precision integers as an explicit Core type; fixed-width `Int` arithmetic does not silently promote. **Implemented 2026-06-30** (`BigInt` type, `BigInt(…)`/`BigInt("…")` constructors, E0130–E0133, `examples/features/text/bigint.jet`, ui snapshot `bigint_int_mix`). c65 | owner |
| 2026-06-28 | D-PQCRYPTO1 | **crypto envelope gets algorithm-agility seam now** (A — PQ algorithms later): design crypto envelope APIs so hybrid/PQ primitives can slot in without breaking users, but do not ship PQ algorithms yet. **Implemented 2026-06-30** (versioned `JETC` envelope header + algo id; `crypto.open` reads all supported algos; migration example 176). c71/c64 | owner |
...
| 2026-06-28 | D-TYPEALIAS1 | **transparent type aliases return narrowly** (B — `alias X = Y` for generic shortcuts only): aliases are for shortening long generic type spellings, not for primitive/unit newtypes. **Implemented 2026-06-29** (example `159_type_alias.jet`, ui tests `type_alias_ok`/`type_alias_no_params`). c76 | owner |
| 2026-06-28 | D-MATURITY1 | **maturity tags are doc-only now** (B — propagation later): `#Experimental`/`#Tested`/`#Hardened` are not semantic markers yet; capture as documentation convention until effect-like propagation is justified. **Ratified — implemented 2026-06-29**: parser accepts all three tags before `fn`/`pub fn` (same-line and next-line), silently erasing them; no AST field, no sema effect, no codegen effect. Docs at `docs/reference/maturity-tags.md`. c79 | owner |
...
| 2026-06-27 | D-RACEWIN1 | **success race folds into structured-concurrency combinators** (A): try-both/keep-winner is a nursery-level `race`/`any` operation, not a standalone primitive. **Implemented 2026-06-30** (`g.race`, example `167_race_cancel.jet`; c126). c103 | owner |
...
| 2026-06-27 | D-WEBBACKEND1 | **web backend is JS DOM + WASM logic hybrid** (A): view/DOM work emits JS; compute/pure logic can compile to WASM. **Implemented 2026-07-01** (reactive-ui-stack Phase 7): `Codegen/Web.rs`'s JS emission now covers the reactive-UI slice — `core.ui` backend calls (`null_backend`/`node`/`constraint`/`rect`/`key_event`, `measure`/`layout`/`paint`/`commands`/`on_event`), `core.reactive` (`reactive.signal(x)` → `jetDom.makeSignal`, `.get()`/`.set()`), `ui.reactive_render(() => {...})` → `jetDom.reactiveRender`, lambdas (arrow-fn emission for the one closure shape UI render loops need), `if`/`else`/`else if`, `for`/`loop … in`, string interpolation, and the existing `#WasmExport`/`#Js` WASM-bridge split (unchanged from M2). Still narrow, not general Jet→JS: no closures capturing by `take`, no structs/enums beyond flat-Int ABI flattening, no match/try. `reactive.derived`/`effect`/`computed` remain native-only (not wired to JS). Verified end-to-end: `examples/features/web/ui_web_reactive.jet` (signal-driven DOM render loop) compiles via `emit_web` and actually runs under `node`, producing a deterministic accumulated paint-command log (`tests/web_build.rs::web_reactive_dom_snapshot_roundtrip`); same source also runs natively (golden.rs) with identical stdout. c123/c134 Phase 7 | owner |
...
| 2026-06-27 | D-USERDERIVE1 | **user-authored derives + typed reflection ship** (A): build the ratified derive/reflection surface within the existing `derive T.Trait` + `T.reflect()` ceiling. **Ratified; derive output done, reflection hardening remains on c129.** c129 | owner |
| 2026-06-27 | D-REACTCORE1 | **reactivity is an explicit `#Reactive` scope marker** (D): no global spreadsheet semantics; the compiler recognizes `#Reactive fn`/`#Reactive {}` and lowers to `core.reactive` tracking. **Implemented 2026-06-29** (parser/sema/codegen, example 161, E2914). c132 | owner |
| 2026-06-27 | D-RENDERTGT1 | **render-target backend trait seam exists before concrete backends** (A): define the backend abstraction before web/native/embedded/TUI implementations. **Implemented 2026-06-29** (`core.ui` module, `JetBackend` trait, geometry/event types, `NullBackend` + `TuiBackend`, `ui.reactive_render`; examples 162/165; tests/ui_backend.rs). c133 M1–M2 | owner |
...
| 2026-06-26 | D-BINDEXPLICIT1 | **explicit-type binding: mutability marker hugs the name** (A — `name: Type #= name: Type =  val` / `name: Type #= name: Type =  val` / `name: Type :=  val`): the explicit-typed binding collapses to one marker fused to the name (`@`=immutable, `:`=mutable), a bare type, and a plain `=` that binds-or-reassigns depending on the name's marker (the Go/Odin/Pascal `:=` family). The inferred `name @ : name: Type =  val` / `name: Type :=  val`): the explicit-typed binding collapses to one marker fused to the name (`@`=immutable, `:`=mutable), a bare type, and a plain `=` that binds-or-reassigns depending on the name's marker (the Go/Odin/Pascal `:=` family). The inferred `name #= val` / `name : : name: Type =  val` / `name: Type :=  val`): the explicit-typed binding collapses to one marker fused to the name (`@`=immutable, `:`=mutable), a bare type, and a plain `=` that binds-or-reassigns depending on the name's marker (the Go/Odin/Pascal `:=` family). The inferred `name #= val` / `name := val` forms are unchanged. Amends **S4** (annotation position) and reopens **D-BIND2**'s `=`-is-reassignment-only invariant. Status-quo two-marker form (B ? type with `:`, mutability with `#=`/`:=`) rejected. **Implemented 2026-06-26** (parser: `sigil_binding` handles `name: Type #= name: Type #= val` / `name: Type :=  val`): the explicit-typed binding collapses to one marker fused to the name (`@`=immutable, `:`=mutable), a bare type, and a plain `=` that binds-or-reassigns depending on the name's marker (the Go/Odin/Pascal `:=` family). The inferred `name #= val` / `name := val` forms are unchanged. Amends **S4** (annotation position) and reopens **D-BIND2**'s `=`-is-reassignment-only invariant. Status-quo two-marker form (B ? type with `:`, mutability with `#=`/`:=`) rejected. **Implemented 2026-06-26** (parser: `sigil_binding` handles `name: Type #= val` and `name: Type #= name: Type #= val` / `name: Type :=  val`): the explicit-typed binding collapses to one marker fused to the name (`@`=immutable, `:`=mutable), a bare type, and a plain `=` that binds-or-reassigns depending on the name's marker (the Go/Odin/Pascal `:=` family). The inferred `name #= val` / `name := val` forms are unchanged. Amends **S4** (annotation position) and reopens **D-BIND2**'s `=`-is-reassignment-only invariant. Status-quo two-marker form (B — type with `:`, mutability with `#=`/`:=`) rejected. **Implemented 2026-06-26** (parser: `sigil_binding` handles `name: Type #= name: Type = val` and `name: Type #= name: Type #= val` and `name: Type := val`; formatter: `fmt_binding` emits new form; E0988 teaching error for old prefix label form; Syntax.rs; migrated all explicit-typed bindings in examples; re-blessed snapshots). c29 | owner |
...
| 2026-06-26 | D-DECIMAL1 | **exact Decimal type + float-money lint** (A — `Decimal` in Core + default-on `L0504`): ship an arbitrary-precision base-10 `Decimal` in `core.numeric` plus a default-on lint that fires when a money-named field (price/cost/amount/total/fee/balance/tax) holds a float, suppressible with `#[allow(float_money)]`. **Implemented 2026-06-30** (`Decimal`/`core.numeric.decimal`, L0504 + `tests/ui_lint/float_money`, `examples/features/text/decimal.jet`). c65 | owner |
...
| 2026-06-26 | D-OWNCOMP1 | **copy-in-and-own UI component distribution** (B — `jetpack add` copies source the user owns): UI components are copied into the user's tree and owned outright (shadcn-style), no version lock; patches don't auto-propagate (user re-runs add or diffs). Fits the ownership ethos and I8 (one distribution path). Block-until-the-stack-is-ratified (A) and locked versioned dependency (C — conflicts with the ownership ethos) rejected. The reactive-UI stack itself (**D-REACTUI1**) is still open and gates what these components target. **Ratified — gated on D-REACTUI1** (the reactive-UI stack); the `jetpack add` copy mechanism lands once the stack is chosen. **Implemented 2026-07-01** (reactive-ui-stack Phase 4): `jetpack add <Component>` — an exact-case match against a fixed starter catalog (Button/Label/Input/Container), checked before the existing `<source>:<package>` ref path so the two CLI surfaces never collide — copies real, embedded `.jet` source into `./components/<Name>.jet`, refusing to overwrite an existing (possibly user-edited) file. Canonical source lives at `crates/jet-driver/src/Jetpack/components/*.jet` (real, independently runnable Jet files, `include_str!`-embedded, same technique as the Rust prelude); dispatch in `crates/jet-driver/src/Jetpack/Components.rs` + `CLI.rs::cmd_add_component`. Example `192_ui_component_kit.jet` composes all four over one `View` tree. c61 | owner |
...
| 2026-06-26 | D-CRYPTOENV1 | **misuse-resistant crypto envelope** (A — `seal`/`open` + `sign`/`verify` as the blessed default): ship a high-level envelope (libsodium model) that hides nonce/IV/mode selection; raw AES/ChaCha/RSA primitives require an explicit expert import, making misuse a deliberate opt-in (I1). Raw-primitives-direct (B — maximum footgun surface) and hold-for-post-quantum-agility (C — delays a useful safe-by-default API) rejected. **Implemented 2026-06-30** (`core.crypto.seal`/`open`/`sign`/`verify` via RustCrypto FFI bridge; `core.crypto.expert` raw AEAD behind `#Unsafe`; E0510/E0511; examples 174–177). c64 | owner |
...
| 2026-06-28 | D-STDRUBRIC1 | **stdlib ergonomic-law checklist proceeds** (A): create a Core/std API review rubric for naming, fallibility, ownership, allocation, docs, and examples before broad API expansion. **Ratified — implemented 2026-06-29** (`docs/spec/stdlib-api-laws.md`, 8 laws + PR template + known-drift table; truthfulness gate added). c44 | owner |
...
| 2026-06-28 | D-TTLVAL1 | **TTL/rotting values proceed as typed library capability** (A): expiring secrets/values get a first-class typed API instead of ad hoc timestamp fields. **Implemented 2026-06-30** (`core.time.expiring.Expiring<T>`, `core.secrets.Rotting<T>`, injectable `Clock` + `Duration`, fallible `get(clock)`, E0511 on `force`). c59/c64 | owner |
...
| 2026-06-28 | D-COROUTINE1 | **coroutine substrate stays internal; control rides task handles** (A): no user-facing coroutine/generator primitive is added for concurrency; experts use task-handle controls (`pause`/`resume`/`cancel`/`trace`) on the canonical task/runtime path. **Implemented 2026-06-29** (task handle controls in sema/TIR/prelude runtime + example 157). c101 | owner |
...
| 2026-06-28 | D-DEADLINE1 | **deadline propagation rides context/taskgroups** (A): deadlines/cancellation flow through `#Context` and structured taskgroups instead of globals. **Implemented 2026-06-29** (`#Context(deadline: …)` parser/sema, runtime context guard, wait/IO checks at task join, channel receive, `time.sleep`, TCP read/write with runtime E3003 report, example 158 + ui snapshot). c112 | owner |
...
| 2026-06-28 | D-REFLECT1 | **reflection read API proceeds** (A): expose type/member/marker facts through the ratified typed reflection surface. **Implemented 2026-06-29** as part of c130/D-METAREFLECT1 rich reflection. c130 | owner |
...
| 2026-06-30 | D-CONCSELECT1 | **scoped select is fluent on taskgroups** (A): `g.select().recv(...).read(...).after(...).wait()?` is the chosen select surface. **Ratified — implemented 2026-06-30** (parser/sema/TIR/codegen + `168_select_channel.jet`; `.read(stream)` stub). c36 | owner |
| 2026-06-28 | D-TASKSCOPE1 | **structured taskgroup scope** (A): task spawning is scoped by a nursery/taskgroup that owns child handles and cancellation. **Ratified — implemented** (`131_taskgroup.jet`; scope-exit cancel+join). c102 | owner |
| 2026-06-28 | D-WORKSPACELOCK1 | **workspace lock stays unified under `.jet/lock`** (A): monorepo workspace resolution shares the existing lock path instead of adding `.jet/workspace.lock`. Implemented 2026-06-29: workspace resolution updates `[[workspace_member]]` entries inside `.jet/lock` while preserving package pins. c90 | owner |
...
| 2026-06-28 | D-IGNORERET2 | **explicit discard is `.drop("reason")` plus scoped suppressions** (A): a `#MustUse` value is intentionally ignored with a method-style `.drop("reason")`; broader generated-code or wrapper cases use a visible `#Suppress(MustUse)` marker. Silent ignore remains rejected. **Ratified — implemented 2026-06-29**: `.drop("reason")` recognized in sema as discard terminal (suppresses E0402); requires non-empty string literal (E0407). `#Suppress(MustUse) { … }` lexical scope suppressor added to parser/AST/sema/codegen/comptime. Example `154_discard_fallible.jet`, ui snapshots. c37 | owner |
...
| 2026-06-28 | D-REFSTRUCT1 | **stored reference fields use `#Ref(Label)`** (B, owner amendment): a stored reference field is marked `#Ref(arena) field: T`; sema proves the named owner relationship and no `use core.mem` gate is required. The older bracket spelling is rejected. **Implemented** (c26 / c147). |
| 2026-06-28 | D-GENMOD2 | **generic modules use unified `<...>` parameters** (A): type and value module parameters share angle brackets; annotation kind distinguishes type bounds (`K: Hash`) from value parameters (`capacity: Int`), and instantiation mirrors declaration (`Lru<String, 32>`). Supersedes D-GENMOD1's placeholder paren syntax. **Implemented 2026-06-30** (parser `module Name<params> { … }` + `module Alias = Target<args>`; sema expands aliases into `CodeModule` with type substitution in fn signatures; E0850–E0854 + ui snapshots; example `170_generic_modules.jet`). Value-param bounds, cycle detection, and non-fn items remain follow-ups (E0852/E0853/E0855 staged). c91 | owner |
...
| 2026-06-28 | D-VISDEFAULT2 | **file-scope marker is `#PubFile` with `priv` exceptions** (A): `#PubFile` flips a file to public-by-default; `priv` marks exceptions inside that file. **Implemented 2026-06-30** (parser visibility defaults, teaching diags E0412–E0418, formatter, examples/ui). c127 | owner |
...
| 2026-06-28 | D-ENUMDOT2 | **leading enum dot extends to value position** (A): `.Red` is allowed anywhere an expected enum type is known; `Color.Red` remains valid everywhere. **Implemented 2026-06-29** (parser: leading-dot case in `expr_unary_inner`, `EnumLit{type_name:""}` sentinel; sema: resolved from `expected_type` with E0330 fallback; expected_type propagated for `Named`/`Option` function params). c145 | owner |
...
| 2026-06-28 | D-RENDERTGT2 | **render backend trait is measure-layout-paint** (A): platform backends implement `measure`, `layout`, `paint`, and a separate event entry point, giving UI code one portable backend seam. **Implemented 2026-06-29** (`JetBackend` + `NullBackend`/`TuiBackend` in `Prelude/Ui.rs`; `ui.reactive_render`; examples 162/165; tests/ui_backend.rs). c133 M1–M2 | owner |
| 2026-06-28 | D-SIGNAL1 | **reactive primitives are `Signal<T>`, `Computed<T>`, and `Effect`** (A): `#Reactive` lowers onto a small explicit library vocabulary rather than creating a second language. **Implemented 2026-06-29** (`reactive.computed` alias, `Computed<T>` type name, `#Reactive` scope). c132 | owner |
| 2026-06-29 | D-WEBKIND1 | **first browser WASM target is `wasm32-unknown-unknown` plus generated JS loader** (A): the web backend emits browser-focused WASM and a JS loader/runtime seam first; WASI/component targets are not the first shipping browser contract. **Implemented 2026-07-01** (reactive-ui-stack Phase 7, extending the M2 compute-export bridge already covered by example 164): the loader/bridge path was already real; this phase confirmed and exercised it for a UI (not just compute) example — `jet_dom_runtime.js::instantiateWasm` + `marshalAbi`/`unmarshalAbi` unchanged, no new WASM surface was needed since `195_ui_web_reactive.jet` is pure-JS orchestration (no `#WasmExport` functions). c123 | owner |
| 2026-06-29 | D-DOMGEN1 | **generated JS uses a tiny first-party DOM runtime shim** (A): web codegen calls a stable `JetDom`-style shim for create/update/event wiring instead of raw repeated DOM boilerplate or a full virtual DOM runtime. **Implemented 2026-07-01** (reactive-ui-stack Phase 7): `crates/jet-codegen/src/Prelude/DomRuntime.js` — confirmed the M2 shim (`createBackend`/`measure`/`layout`/`paint`/`commands`/`onEvent`/`makeNode`/`makeConstraint`/`makeRect`/`makeKeyEvent`) was already real (mirrors the Rust `JetNullBackend`, in-memory command log — no actual browser `document`/DOM API touched anywhere, by design: there's no real DOM under plain `node`, and no jsdom-equivalent is vendored, I6-adjacent). Added the piece that was genuinely missing for a *reactive* UI example: `makeSignal`/`reactiveRender`, a dependency-tracked observer stack mirroring the Rust `JetSignal`/`jet_reactive_effect` prelude (`Prelude/CoreLib.rs`) — `.get()` inside a running effect subscribes it, `.set()` re-runs subscribers synchronously. Zero new dependencies. c123 | owner |
...
| 2026-06-30 | D-EXTMETH1 | **external inherent methods use `.` connector** (B — `fn Point.dist(self)`): a method defined outside the type's block spells its attachment with `.` (`fn Point.dist(self, other: Point) -> F64`), matching D-IMPLDOT1's `impl Type.Trait` separator and reading naturally as "Point's dist". The `::` connector (`fn Point::dist`) was rejected as an extra mental rule. Orphan rule: type must be defined in the same source module. **Implemented 2026-06-30** (parser normalizes top-level external methods into inherent impl blocks; duplicate/field collision checks reuse E0105; retired `~~` emits E0325; example `100_external_methods.jet`). c92 | owner |
| 2026-06-30 | D-CTBLOCKEXPOSE1 | **comptime block bindings leak into enclosing scope** (A): bindings declared inside `comptime { }` are visible in the surrounding runtime scope as `$name`; the block is a compile-time evaluation fence, not an isolation barrier. This enables `$name` splices to reference values computed at comptime without re-declaring them. Strict-isolation (B — block-locals invisible outside, $name can't reference them) rejected. **Implemented 2026-06-30** (comptime block returns bindings to sema scope; runtime `$name` lowers as a serialized literal; E2713 for missing comptime name; example 125). c94 | owner |
| 2026-06-30 | D-UITREE1 | **UI tree uses typed constructors** (A): view items are constructed with dot-construction syntax (`.Button.{ label: "OK" }`) giving full type inference and autocompletion, not string-tag builders. **Implemented 2026-07-01**: named-payload enum variants (S30, 2+ fields incl. a single named field) now construct with `.Variant.{ field: val }` / `Type.Variant.{ field: val }`, reusing the D-DOTCTOR1 struct dot-brace sigil (parser `enum_lit_named_fields`, `Parser/Expressions.rs`; codegen pattern-match field-name fix, `Codegen/Statement.rs`); example `190_ui_view_tree.jet` (typed `View` tree + diff-based rerender over `core.reactive`). c134 | owner |
| 2026-06-30 | D-STYLESHAPE1 | **style is a flat `Style` record** (A): all visual properties live in one flat `Style` record passed to view items; no nested sub-records per property group. Matches the constraint-layout typed-axis approach (D-LAYOUT1). **Implemented 2026-07-01**: `Style`/`Color`/`Length`/`LengthUnit` are plain user-space Jet types (`struct`/`enum`, dot-constructed via D-DOTCTOR1/D-UITREE1) — zero new core-opaque types or sema/codegen plumbing, same recipe as `View`. `Style { color: Color, width: Length, height: Length }`, `Color { r: Int, g: Int, b: Int }`, `Length { value: Float, unit: LengthUnit }` with `LengthUnit { Px }` (only unit this phase needs). A `#UnitFamily(length){px}` distinct-Float unit was tried first (cheaper Jet-source surface) but hit a pre-existing TIR-codegen coverage gap — `field_ty_covered`/`struct_is_covered` (`crates/jet-codegen/src/Codegen/TIR/subset.rs`) don't recognize a `distinct` type as a coverable struct-field type, so any struct with a distinct-typed field ICEs at codegen; reproducible standalone, unrelated to UI/styles, filed as a follow-up rather than patched inline. `JetPaintCmd`'s backend paint-command color isn't reachable/settable from Jet source yet (`paint()` hardcodes its fill in Rust), so no backend (`Prelude/Ui.rs`) changes were load-bearing for this phase. Example `191_ui_typed_style.jet`. c134 | owner |
| 2026-06-30 | D-MOTIONTIME1 | **motion timing uses injectable Clock** (A): animation/motion durations and delays come from the same injectable `Clock` as the rest of Jet's time surface (D-TIMEDEPTH1), enabling deterministic animation tests. **Implemented 2026-07-01**: `core.motion`-flavored `tween(from, to, elapsed_ms, duration_ms) -> Float` is plain user-space Jet (smoothstep ease, clamped at both ends) — zero new core-opaque types or sema/codegen plumbing, same recipe as `View`/`Style`. Wired through `reactive`: the elapsed-time reading lives in a `Signal<Int>` fed by explicit reads of the same injectable `core.time.clock(seed)` used by D-DET1 elsewhere (no global/ambient time signal — that was the axis this decision settled); a `reactive.derived` recomputes the eased position whenever that signal changes, so the motion value is a pure function of `(target, elapsed)` and is never `.set()` directly (no imperative frame mutation). Deterministic golden example `193_ui_motion.jet`: seeds a `Clock` at 0, ticks it by fixed steps, and asserts the exact interpolated value at each step, including clamping past the animation's duration. c134 | owner |
| 2026-06-30 | D-A11YGATE1 | **accessibility is a lint gate: `jet lint --a11y`** (B): a11y issues (missing roles, unlabeled controls, low contrast) surface as lints under `jet lint --a11y` rather than blocking compilation; projects opt-in to zero-a11y-warnings as a CI gate. Compile-errors (A — over-strict for interactive iteration) rejected. **Implemented 2026-07-01**: `JetAriaRole` (`Button`/`TextInput`/`Label`/`Container`, mirroring the Phase-4 starter kit) is a new core-opaque type (`Prelude/Ui.rs`); `JetUiNode` gains `role: Option<JetAriaRole>` (`label` doubles as the accessible name — WAI-ARIA accname model, no separate field). `ui.node_role(label, w, h, role)` is the a11y-checked sibling of the existing roleless `ui.node(label, w, h)` (kept unchanged so examples 162/163/165 don't break); `ui.aria_role_button/text_input/label/container()` mint role values. Keyboard focus routing: `NullBackend`/`TuiBackend` gain `set_focus_group([UiNode])` + `focused_label()`; a `Tab` key event (`on_event`) advances a focus index that wraps, everything else falls through to the prior key/resize handling unchanged. Two new static, literal-only sema lints (`Severity::Lint`, never `Error`) — **E2930** (an interactive-role `ui.node_role(...)` call with a literal empty label) and **E2931** (two `ui.node_role(...)` calls inline in the same `set_focus_group([...])` list sharing a literal non-empty label, WCAG 2.5.3). Both are always computed during ordinary sema like any lint, but a CLI-layer filter (`Source/CmdDevTools.rs::visible_lints`) strips them from `jet build`/`jet run`/`jet check`/`jet emit` output; only the new `jet lint --a11y <file>` subcommand (`Source/CmdDevTools.rs::run_lint_a11y`) prints them, exiting non-zero when it finds one (verified: the same file with an a11y violation exits 0 under `jet run`/`jet build`, non-zero only under `jet lint --a11y`). Snapshot coverage: `tests/ui_lint/a11y_unlabeled_control.{jet,warn}` (E2930), `tests/ui_lint/a11y_duplicate_label.{jet,warn}` (E2931). Golden example `194_ui_a11y.jet` demonstrates Tab cycling focus between two labeled buttons (a11y-clean, zero lint warnings). "Low contrast" (the third D-A11YGATE1 example category) was skipped — two solid, honest checks beat a shaky third. c134 | owner |
| 2026-06-30 | D-DISPLAY-SHAPE | **Display hook is `fn display(self) -> String`** (A): the user-facing display protocol is a single method `fn display(self) -> String` on the type; no trait impl syntax. Interpolation `{}` calls it. Confirmed no default; must be explicit. **Implemented 2026-06-30** (synthetic `Display` trait, E0915, L0520 migration lint). c51 | owner |
| 2026-06-30 | D-DEBUG-REDACT | **Debug redaction uses `#[Redact]` field marker** (A): fields that should not appear in debug output are marked `#[Redact]`; auto-derived Debug replaces their value with `"[redacted]"`. No per-call redaction controls. **Implemented 2026-06-30** (field attr + codegen `jet_debug` body). c51 | owner |
| 2026-06-30 | D-ITER-HOOK | **Iterable dual-tier hook** (A): beginners use `.each`/`.to_list()` on custom collections; experts implement `Iterable` + `Iterator` for zero-copy `loop x in mytype`. Built-in `[T]`/`Map` keep native `loop`. **Implemented 2026-06-30** (synthetic traits, TIR `TForInMethod::Iterable`, example 180). c41 | owner |
| 2026-06-30 | D-INDEX-HOOK | **Index dual-tier hook** (A): built-in List/Map keep `[]`; custom types use `.get`/`.set` by default; experts implement `Index`/`IndexMut` for bracket syntax. **Implemented 2026-06-30** (synthetic traits, TIR index hooks, example 181). c41 | owner |
| 2026-06-30 | D-LITSUFFIX-SCOPE | **No literal-suffix hook — units use dot-constructor** (A): close the literal-suffix protocol item; `#UnitFamily` units construct as `px.{100}` per D-DOTCTOR1. **Ratified — doc-only confirmation; no compiler hook.** c41 | owner |
...
| 2026-06-29 | D-S25-RETIRE1 | **retire S25 `||`/`&&` comparison distribution** (A): `||` and `&&` no longer reuse the nearest comparator for bare values. Comparator alternatives are single `|` (`x == 1 | 2` in an arm head, or `1 | 2` under an inferred comparator); `||`/`&&`/`==`/`!=` remain boolean arm condition syntax. **Ratified — implemented 2026-06-29.** c154 | owner |
| 2026-06-29 | D-BIND3 | **binding sigils: `#=` / `:=` / `: Type #=` / `: Type :=`** (A): immutable inferred `name #= value`; mutable inferred `name := value`; explicit immutable `name: Type #= value`; explicit mutable `name: Type := value`; `::`, `@=`, `name: Type :`, and `name: Type =` retired (E0991/E0998). Supersedes D-BIND2 and D-BINDEXPLICIT1. **Ratified — implemented 2026-06-29.** | owner |
| 2026-06-29 | D-BIND-CANON1 | **binding syntax stays current law** (A): canonical bindings are `name #= value`, `name: Type #= value`, and `name: Type #== value`; older alternatives are cleanup targets. **Ratified ? confirms D-BIND3 for reconciliation.** c154 | owner |
...
| 2026-06-29 | D-CFFI-CANON1 | **C FFI marker family is `#Extern` plus `#Bindgen` only** (A): delete `@extern`, `#extern`, `@bindgen`, and `#bindgen` fixtures/usages; generated binding modules use PascalCase too. **Implemented 2026-06-30** (old spellings are E0060 teaching recovery only). c154 | owner |
...
| 2026-06-29 | D-ORRETURN-CANON1 | **early-exit fallbacks use the `?? <control>` family only** (A): canonical forms are `expr ?? return`, `expr ?? continue`, and `expr ?? break`; delete `?return`, `?continue`, and `?break` fixtures/usages. **Ratified — implemented 2026-06-29** (parser: `OrFallback::Break`/`Continue`; sema loop-depth gate E0115; codegen `break`/`continue` emit; example 150; ui snapshots). c153 | owner |
| 2026-06-29 | D-LOOP-SEMICOLON1 | **keep semicolons in counted-loop headers** (A — reversed 2026-06-29): the three-part form `loop init; condition; afterthought { }` is canonical, confirming D-LOOP-SURFACE-REOPEN=B. Semicolons are legal only inside this header, not as general statement separators. Prior outcome C (separator redesign) is reverted. **Ratified — implemented 2026-06-29** (`Stmt::CountedLoop`, example 100, fmt stability test). c154 | owner |
...
| 2026-06-29 | D-CORENS-CANON1 | **`core.*` is the only standard namespace spelling** (A): delete `std.*`, `jet.*`, `jet.core`, `core.json`, and old namespace fixtures; no aliases or pre-Epoch-6 teaching diagnostics. **Ratified — implemented 2026-06-29** (purged dead `core.*`/`jet.*` aliases from CheckerCoreLib, Effects, Purity; corelib test updated). c154 | owner |
...
| 2026-07-01 | D-MARKER-FAMILY1 | **two-plane sigil law: `@` contracts / `#` directives / `$` splices** (B): `@` precedes a declaration and states a checkable contract about it (`@Pure`, `@MustUse`, `@Codable`); `#` never states a contract — it changes what compiles, when it runs, what is legal, or supplies a compile-time value (`#Unsafe`, `#Test`, `#(Fs)`, `[T#N]`, `pkg#1.2.3`, `#Caller()`); `$` stays splice-only. Reverses D-ATTR1's "everything is `#`" stance for the contract subset — each currently-`#` marker that states a declaration promise is a reclassification *candidate*, not an automatic rename (worked list in the Ratified entry). One marker family (A) and `$` as marker+splice (C) rejected. **Implementation gated on two follow-up ballots raised 2026-07-01: D-CONTRACTCASE1 (casing of the `@` plane — D-CASING1 is `#`-scoped) and D-MARKERMOVE1 (exact move list; `#Numeric`/`#PublishedSchema`/`#Redact`/serde field markers ambiguous).** c7markerfamily | owner |
| 2026-07-01 | D-CLIFLAG1 | **typed entry-signature CLI parsing** (A): the entry function's parameter is the flag surface — `fn run(args: ServeArgs)` derives `--flag` names from fields, defaults from field defaults, help from doc markers, subcommands from an enum parameter; `fn run()` untouched. Same derive/reflection layer as `#Codable` (D-SERDE4). Explicit `cli.parse<Args>(...)` (B) ships underneath as the library floor for non-entry parsing; builder-only status quo (C) rejected. Marker spellings ride D-CONTRACTCASE1/D-MARKERMOVE1. c7cliflag | owner |
| 2026-07-01 | D-ANY-JAI1 | **trait-bounded heterogeneous varargs, not a top type** (A, owner-clarified): `parts: ...Renderable` (D-VARIADIC1 grammar, sugar over `<T: Renderable>(parts: ...T)`) monomorphizes per call site, type-checking every argument against the trait. Owner follow-ups answered without new syntax: (1) A & B together — yes, reflect handles (`reflect.Value`/`Data`, D-METAREFLECT1/D-ENC-DYN1) are already inside option A as the tooling floor; (2) trait *sets* — yes, ratified S45 `<T: A + B>` bounds already require a specific set (`fn log_all<T: Renderable + Serializable>(parts: ...T)`), shape already shipping in D-COLLBREADTH1 (`Set<T: Hash + Eq>`). General `Any` (C) stays rejected per D-DYNAMIC-TYPE1. c7jaiany | owner |
| 2026-07-01 | D-POINTERCHAIN1 | **no compact pointer cast/deref chain; docs only** (A): keep explicit `mem.cast_ptr<T>(p)` + postfix `p.*` inside `#Unsafe(...)`; add a worked doc translating the Jai `slot.value_pointer.(*bool).* = true` idiom. Typed reflection setters (B) and a compact chain (C — hides two unsafe ops in one expression) rejected. No syntax minted. c7pointerchain | owner |
| 2026-07-01 | D-UNITLIT1 | **unit-suffix numeric literals from in-scope `#UnitFamily`** (A): `500ms`, `12.50usd` resolve against imported family members; no implicit cross-unit conversion (mixed-unit arithmetic needs a family-declared conversion fn or diagnostics); `e`+digits stays reserved for float exponents, never a unit. **Supersedes D-LITSUFFIX-SCOPE** (2026-06-30, c41), which closed the suffix idea in favor of `px.{100}` dot-construction — construction stays valid; this adds the literal spelling. Constructor-only (B) and core-only suffixes (C) rejected. c8syntaxwave | owner |
| 2026-07-01 | D-TRAILBLOCK1 | **zero-parameter trailing block argument** (A): a call whose final parameter is a function may end with a bare `{ }` block — `ui.button("Save") { prefs.save() }` — in place of the last lambda argument. v1 zero-parameter only; parameter-binding lambdas stay inside the parens (dodges the Kotlin-`it`/Swift-`$0` debate). No new sigil: `{` directly after a call's `)` is the trailing zero-arg lambda (no bare-block statements exist, slot free). Parameterized trailing closures (B) and reject-forever (C) rejected. c8syntaxwave | owner |
| 2026-07-01 | D-PREPOST1 | **`@pre`/`@post` signature contracts** (A): `@pre(cond, "msg")` claims about arguments, `@post(cond, "msg")` about `result`; conditions pure (same checker as `#Pure`); checked in **every** build by default, per-module build-policy strip is the explicit opt-out (rejects debug-only asserts, option B). Violation = named-clause diagnostic at the call site. Complements open D-RANGETYPE1 (value shape vs argument/result relation). Body-assert status quo (C) rejected. Lowercase as ratified; casing rides D-CONTRACTCASE1. c8syntaxwave | owner |
| 2026-07-01 | D-EFFBUDGET1 | **package effect budget: always-on report + opt-in enforcement + per-dep grants** (D): every `jet build` prints an effect summary and records per-dependency effect provenance in the lock file, zero config; `effects: { allow/deny }` in `pkg.jet` turns on whole-graph enforcement failing the build with the offending dependency path; `grants: { "dep": [Effect] }` is the audited per-dep escape. Ten-effect vocabulary shared with D-EFF4. Distinct from open D-BUILDPOLICY1 (build-time authority). Allow-list-only (B) and report-only (C) rejected. Manifest keys, not grammar. c8syntaxwave | owner |
| 2026-07-01 | D-PERSIST1 | **`@persist` opt-in dev hot-reload state survival** (A): a `@persist` module-level binding survives `jet dev` reloads; identity = module+name; layout change → Codable-style re-decode, fallback reinit + one-line warning, never a crash; release semantics identical. Persist-by-default with `@fresh` opt-out (B) rejected (footgun as default: live sockets/handles carried silently). Implementation rides the E3 JIT hot-reload runtime; surface fixed now. Casing rides D-CONTRACTCASE1. c8syntaxwave | owner |
| 2026-07-01 | D-UFCS1 | **no UFCS; explicit `fn Type.name(self)` extensions stay the one spelling** (B, status quo): a free function never becomes a method automatically. Full UFCS (A) and pipeline `\|>` (C) rejected as second/third call spellings for the same job — I8, grep-hostile, import-sensitive resolution. No change. c8syntaxwave | owner |
| 2026-07-01 | D-DESTRUCT1 | **struct destructuring: bindings + if-table patterns, mandatory `..` for partial** (A): dot-construction runs backwards — `.{ id, severity: sev } #= incident` binds fields (with rename); same shape as if-table pattern head `.{ kind: "page", target, .. } -> page(target)`; `..` **mandatory** when not every field is named. **Amends S74** (2026-06-15, implemented — was irrefutable with no partiality marker): existing partial destructures gain a required `..`, a breaking spelling change needing a migration pass across examples/tests. One nesting level; formatter discourages towers. Binding-only (B) and status quo (C) rejected. Lands with D-DOTCTOR1/2 implementation. c8syntaxwave | owner |
| 2026-07-01 | D-CHAINCMP1 | **same-direction chained comparisons** (A): `0 <= sev < 10` → `0 <= sev && sev < 10`, middle operand evaluated once; only monotone `<`/`<=`/`>`/`>=` chains legal; mixed direction = compile error naming the break; `==`/`!=` excluded (table dispatch). `a < b < c` is a type error today so no meaning change. No new token. Reject-forever (B) rejected. c8syntaxwave | owner |
| 2026-07-01 | D-CAPBUNDLE1 | **named capability bundles grant curated base operations to nominal types** (A): a distinct nominal type (D-DIST1/D-DIST3 machinery; ballot's `type X :: Base` spelling is v5 illustration, nothing minted there) starts inert; `@numeric`/`@comparable`/`@printable`/`@codable_as_base` re-expose curated base-operation slices preserving nominal identity (`Usd + Usd -> Usd`, `Usd + Eur` error). Stackable. Per-operator grant lists (B) and hand-written operators (C) rejected. **`#Numeric` (D-DIST3/D-QUAL3) overlap deliberately unresolved — reconciliation is part of open D-MARKERMOVE1;** casing rides D-CONTRACTCASE1. c8syntaxwave | owner |
| 2026-07-01 | D-ERRCTX1 | **automatic `?`-propagation trace + stdlib `.context()`** (D): every `?` crossing is recorded in dev/debug builds (cheap span push) so an unhandled failure prints the full propagation path with file:line, zero user code; `fs.read(path).context("loading config {path}")?` adds a lazily-evaluated human boundary message; renderer shows both in one chain. Release keeps origin + explicit contexts (policy may retain full traces). Stdlib method, no new grammar. Context operator on `?` (B) and no-context (C) rejected. c8syntaxwave | owner |
| 2026-07-01 | D-METADERIVE1 | **user-derive spelling amended by owner: `derive T.Wire`** (was `derive Wire for T`): the type parameter leads, joined to the trait name with a dot — reads with the dot-member grain (`T.reflect()`, `T.{ }`) and drops the `for` keyword from the derive grammar. Clean break, no alias; typing the old form is teaching error **E2714**. Parser/sema messages, examples 128–130, ui + fmt fixtures, diagnostics.md respelled. | owner |
| 2026-07-01 | D-METHODMACRO1 | **methods stay ordinary functions; no macro-method call rewriting** (A): a Jet method call is always a plain function call — no Jai-style call-site macro expansion, ever. The two wants behind macro-methods get honest homes: guaranteed inlining via checked `@Inline`/`@InlineAlways` contracts (`@InlineAlways` that cannot inline is a compile error naming why — casing per D-CONTRACTCASE1=A), and DSL/codegen power arrives in E4 metaprogramming, explicit and auditable. Blessed hooks (Iterable/Index/Display) stay sema-owned traits. c7methodmacro | owner |
| 2026-07-01 | D-DYNARRAY1 | **no `[..]T` spelling; list family stays `[T]`/`[K,V]`/`[T#N]`; zero-copy comes as `View<T>`** (A): Jai needs `[..]T` because its `[]T` means a non-owning view; Jet's `[T]` already IS the owning growable list, so a second spelling would be pure duplication (I8). The genuine gap — cheap non-owning slices — is filled by a `View<T>` library type riding the stored-ref machinery: compiler-tracked owner, zero-copy windows that cannot dangle, copy-slices stay the safe beginner default. No new grammar. c7dynarray | owner |
| 2026-07-01 | D-METADEPTH2 | **rung B granted: read-only whole-program reflection + structured diagnostics from the build entry** (B): the selected build entry (D-BUILDENTRY1=B) receives a post-sema read-only snapshot of the checked program (program → packages → types/functions, scaling the D-METAREFLECT1 TypeInfo surface) plus a diagnostic-emission API whose signature structurally requires code + what/why/fix (I4 by construction). No mutation, no macros, never runs at module import time; dependencies get no rule execution in your build. Rung C stays frozen on c154. c1nixrpd | owner |
| 2026-07-01 | D-BUILDENTRY1 | **compile-time build entrypoint is `fn build(b: BuildContext)` via the lifecycle verb map** (B): `jet build` runs the root's `fn build(BuildContext)` when defined, else the batteries default pipeline. The typed `BuildContext` parameter is the visible authority boundary. Aligns with the lifecycle-verbs shape (`jet run` → `fn run()`, `jet dev` → `fn dev()`) without depending on it. No name magic. c1nixrpd | owner |
| 2026-07-01 | D-BUILDPOLICY1 | **build-time code gets tiered authority, `BuildContext`-only by default** (A): Tier 1 is pure + locked by default; Tier 2 requires `#Impure("reason")` + CLI/project/org permission + provenance. Dependencies never get Tier 2 unless explicitly granted. c1nixrpd | owner |
| 2026-07-01 | D-BUILDSCOPE1 | **`fn build` lives in the unit's existing definition file at every rung** (A): single file — beside `fn main`; package — inside `pkg.jet`; workspace — inside `workspace.jet`. Zero new filenames; grant chain (flag → pkg.jet `build:` block → workspace policy block) mirrors containment; a workspace `fn build` may add workspace targets and cap members, never mutate a member's plan. c1nixrpd | owner |
| 2026-07-01 | D-BUILDGEN1 | **build-generated Jet source is derived state under `.jet/generated/`, never committed** (A): generated modules are build products addressed by module name, materialized as real files under `.jet/generated/<package>/` (LSP go-to-def lands in them, debugger steps through them, `jet explain-build` diffs them). Lock records source-input + output hashes so CI verifies without committing. c1nixrpd | owner |
| 2026-07-01 | D-OSTARGET1 | **native OS platform gating extends the existing `#Target` marker with `Os.*` variants** (A): `#Target(Os.Linux)` / `Os.Macos` / `Os.Windows` join Browser/Wasm/Js as instances of the ONE environment marker (I8). Sema rejects mixing a web variant and an OS variant on the same item — two axes, one marker, mutually exclusive values. Unblocks native `#Extern` backends (reactive-ui Phase 8). Implementation plan: reactive-ui-stack.md Phase 9.3. c2qj06uq | owner |
| 2026-07-01 | D-WEBDEFAULT1 | **default build target: `#Target(Web)` file-level marker + pkg.jet `target: "web"` field, CLI flag always wins** (A): third value of the existing `#Target` family — default backend selection, not a partition ceiling. Precedence: `--target=<x>` CLI > pkg.jet `target:` > file-level `#Target(Web)`. **Implemented 2026-07-01** (built during Phase 7 dogfooding, ratified after). Formatter emission + fmt stability test still owed (Phase 9.1). c2qj06uq | owner |
| 2026-07-01 | D-HTMLPAIR1 | **explicit companion-HTML marker `#Html("path.html")`, same family as `#Target(Web)`** (A): precedence for web builds — explicit `#Html(...)` wins; else the sibling `<stem>.html` convention (kept for existing examples); else the generic auto-generated page. A referenced path missing on disk is a build-time error naming the file, never a silent fallback. **Implemented 2026-07-01** (Phase 7 dogfooding, ratified after). Formatter + fmt stability test owed (Phase 9.2). c2qj06uq | owner |
| 2026-07-02 | D-JPK-OSNAME1 | **the OS name is `jetos` — working title confirmed as product name** (A): literal and canon-consistent with jet/jetpack. Owner reviewed two menus (aviation-canon + jet-prefixed/plain-word rounds) and kept jetos. Trademark sweep remains a pre-release task. c9jetpackgates | owner |
| 2026-07-02 | D-MARKERMOVE2 | **`@Pure` is one spelling everywhere: allowed in the single type-position bound slot** (B): the @ plane law gains one carve-out — a contract marker may prefix a function TYPE to state a bound (`fn map(items: [Int], f: @Pure fn(Int) -> Int)`). "Declarations only" becomes "declarations, plus contract bounds on function types". Resolves marker-family plan gate G1; `#Pure` type-position spelling migrates with the plane. c7markerfamily | owner |
| 2026-07-02 | D-MARKERMOVE3 | **all built-in derive markers move to the @ plane; user derives stay #** (B): `@Debug`, `@Comparable`, `@Summarize` join `@Codable`/`@Encode`/`@Decode` as capability promises; user derives (`derive T.Wire { … }` bodies, applied as `#[Wire]`) remain # generation machinery. The built-in/user line is the plane line. Resolves marker-family plan gate G3, reversing its stay-# default for built-ins. c7markerfamily | owner |
