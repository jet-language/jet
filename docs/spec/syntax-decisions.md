...
**D-CASE-CHROME1=C / D-CASE-PROSE1=A — user-facing case law** *(ratified
2026-08-25, card #2203)*: short UI chrome that acts as a label uses Title Case;
a summary or other text that describes an action, state, reason, or next step
uses sentence case. Section headers, table headers, status labels, buttons,
menus, tabs, and panel titles are labels. Help command and flag summaries are
descriptions. All three diagnostic payload lines — What, Why, and Fix — are
prose and use sentence case. A leading identifier, flag, path, package ref,
keyword, type name, or quoted code fragment keeps its own case and is never
force-capitalized. JSON keys and values, file paths, environment variable
names, package refs, and every other machine-parsed string are excluded and
must remain byte-identical. The complete surface matrix and Title Case word
rule are in [ui-case-law.md](ui-case-law.md).
...
**D-RESULT-OPTION-CANON1**: `?T` always means Optional; an explicit fallible
contract owns the `!` prefix (D-FAILURE-FOUNDATION1).
...
**S7 — Propagation**: fallible calls propagate implicitly. The only postfix
journey form is contextual `?(text)`, which adds a lazy context frame.

**S34 — Failure-returning return**: the failure surface is
`[?Success] [!ErrorUnion]`; a missing error contract is the beginner route and
means the implicit default `Err` error. A unit-fallible declaration writes its
explicit contract alone (`fn save(path: String) !IOError`), and value-returning
signatures use `fn load() Config !E` or `fn load() ?Config !E`. Lowers to Rust
...
`code`, and `cause` fields. `Error` is deleted, not aliased. `T !Err` is the
explicit default-error contract. An `Err(e)` in a result-arm context
...
**D-ERRCTX1 — Error context** *(amended by D-FAIL-CTX1)*: contextual `?(text)`
carries a lazy string note. Every fallible call joins the failure journey
automatically, and each explicit `?(text)` adds one frame on every tier. The
`.context` method and the internal error-context lowering sentinel are
...
error type is `Err`; an explicit default contract is `T !Err`, while omitted
contracts use it implicitly. This amends S80's builder spelling
...
library failure up implicitly, or pin `fn run() !Err`. `Prelude/Errors.jet` holds the
declarations; sema injects only the conversions a module's fallible calls
...
Each fallible call joins the failure journey on every tier, whether it has an
explicit note or not. Notes use D-ERRCTX1's lazy interpolation rule. The
`.context` method and the internal error-context lowering sentinel are
deleted.
...
2026-08-06, card #1534; spelling amended by D-FAILURE-FOUNDATION1=A)*:
`fn save(path: String) !IOError` has no body arrow or unit. `fn sync() !Err`
pins the default error explicitly; omitting the contract is the implicit
beginner route. A value-returning signature uses `fn load() Config !E`. This
retires S80's arrow-and-unit form.
...
local-rollback behavior (D-TXN1–4) is unchanged. The `#Transact` block body runs
exactly once; the commit takes every touched `Shared` handle's lock in one fixed
address order, applies the buffered writes, and releases -- no retry. D-CONC-STM1=A
below is the operative statement of that law.
...
is not `TaskHandle.outcome()` or `TaskHandle.status()` and does not restore
`trace()` or `exception()`. It amends D-COROUTINE1 and retires
...
D-ONCE-AT1=D; the `@if` branch is reaffirmed by D-STRUCT-ONCE1=A; ratified
2026-08-06, card #1537)*: ordinary foldable expressions
...
The `@if` branch is the earlier D-WHEN1 surface owned by S57. D-STRUCT-ONCE1=A
reverses the intervening same-day ruling that erased `@if` in favor of plain
`if`; this paragraph is the reverse amendment link, and `@if` remains current.
...
The required test gate also leaves candidate-bound `jet.ci-evidence.v1`
receipts. Each receipt records candidate, runner, source/workflow and toolchain
identity, command exit, and captured stdout/stderr. GitHub uploads the receipt
with `always()` and `if-no-files-found: error`, so a failed, missing, or partial
gate cannot become a green change or erase its audit trail. The test-target
inventory is read twice and a mismatch fails as nondeterministic. The CI test
gate has no release-signature requirement; release publication remains a
separate gate.
...
Service observations read the typed workflow `TaskStatus` and `TaskOutcome`
facts and export through the existing structured `core.log` sink; they do not
parse string outcomes or exceptions and add no second telemetry mechanism.
...
typed endpoints from signed, generation-versioned directory
...
All six D-REPORT decisions are ratified as option A. This is one exact slate:
one bullet records each outcome, with no alternative outcome left active. The
slate extends D-FAIL-BREACH1's one runtime report family and one renderer to
every report moment and surface. D-REPORT-HOME1 amends the I4 `Adding a
diagnostic` process; D-REPORT-MACHINE1 amends D-DX1. The card numbers below
record existing outcome owners or implementation slices. They do not claim
that any card is complete.

- **D-REPORT-LAW1=A — one law for every report (handoff #1712).** Every error, warning, stop, test failure, and tool failure from any Jet tool across compile, run, test, tool, web, and editor is a registered report with one code, What, Why, and Fix, rendered by one renderer per surface. Status, progress, success, ledger, and usage lines stay plain. Amendment: D-FAIL-BREACH1's one-renderer rule now covers every report moment. Required guards: one UI snapshot for each owned report surface, and I9 proof that one condition has one report with the same meaning and text under AOT, `jet run`, and the interpreter; the registry or Prelude owns the text, and engines only marshal it.
- **D-REPORT-HOME1=A — typed rows are the one text home (handoff #1806).** A typed row is the only home for code, severity, moment, What/Why/Fix templates, named holes, and structured fixes. The registry generates the spec, error pages, and `jet explain`; no renderer or emitter keeps a second copy. I4 `Adding a diagnostic` now requires the row, its snapshot, and both-direction coverage. Required guards: the card's UI snapshot proves the complete row-backed report, and I9 proves the same row-backed meaning under AOT, `jet run`, and the interpreter.
- **D-REPORT-SEV1=A — two severities and the ratified words (handoff #1678).** Blocking and advisory are the only levels. `Error`, `Stop`, and `Warning` remain the frame words; a contained failed assertion is `Stop` in the E3001 family, not a new `Fail` word or code block. Moment is row metadata, not a new severity. The duplicate severity enum is deleted; E and L prefixes stay, and W0410 stays the frozen exception. Required guards: UI snapshots cover each changed severity form, and I9 proves identical severity meaning and frame text under AOT, `jet run`, and the interpreter.
- **D-REPORT-TEST1=A — a failed test shows the full report (handoff #1684).** A failed assertion or golden check prints the full report frame under the test ledger, through the same renderer used by the stop path, with registered code, location, and caret. A golden failure states whether the file is missing or the output changed and includes its diff. Passing output stays unchanged. `JETTEST2` proof records stay separate from JSON reports. Required guards: one UI snapshot covers the failed-test frame, and I9 proves the E3001 report is the same under AOT, `jet run`, and the interpreter.
- **D-REPORT-MACHINE1=A — one JSON line for every report (handoff #1713).** Each report is one complete `jet.report/v1` JSON object per line, with schema, moment, severity, code, What, Why, Fix, detail, location, span, `fix_edits`, cause, and clears fields when applicable. Command status envelopes remain status data; every report inside them uses this object. Amendment: D-DX1 changes `message` to `what`, `suggestions` to `fix_edits`, and `schema_version` to `schema`. The failed-test example uses registered code **E3001**, never provisional E3020. Required guards: UI/machine snapshots cover the exact emitted object, and I9 proves one condition emits the same report meaning under AOT, `jet run`, and the interpreter; no engine invents a second JSON shape or report field meaning.
- **D-REPORT-EDITOR1=A — the editor gets the whole report (handoff #1807).** LSP uses What as `message` and carries Why, Fix, `fix_edits`, cause, the `jet://explain/<CODE>` link, and related locations in structured fields. Code actions reuse the row-owned edit used by `jet fix`; no editor path parses prose or keeps editor-only text. Required guards: LSP/UI snapshots cover the full payload and action, and I9 proves the same report meaning and recovery data under AOT, `jet run`, and the interpreter.
...
**2026-08-19 — D-STRUCT-PLANE1=A** *(card #2052; ratified 2026-08-19)*:
Structure is a fact plane: liveness, lifecycle, and import-edge rows join the
one fact registry, use one gate ledger, and erase before runtime; `jet inspect
structure` reads that ledger through the structure lens. It adds no spelling,
separate lint plane, or external analyzer; it amends no prior decision and
retires no separate mechanism.

**2026-08-19 — D-STRUCT-LIVE1=A** *(card #2052; ratified 2026-08-19)*:
Unused imports, bindings, private functions, and unreachable `pub(package)` or
application-target exports are warning verdicts under D-LINTPOLICY1, with
delete-or-rename-to-`_name` (D-NAME-SIGIL1) as the repair and library `pub`
exempt. It adds no spelling or new suppression mechanism, and amends or retires
none.

**2026-08-19 — D-STRUCT-LIFE1=A** *(card #2052; ratified 2026-08-19)*:
The lifecycle ladder is `_name`, `pub _name`, `pub`, `#Deprecated(since:,
use:, removed_in:)`, then a named removed delta; any package may mark a `pub`
item, with `removed_in` dormant until editions exist. It opens the Core-only
E2001/E2002/L2001 area to all packages and retires the two duplicate Core
tables into ordinary markers.

**2026-08-19 — D-STRUCT-EDGE1=A** *(card #2052; ratified 2026-08-19)*:
Packages may write `boundaries: { deny: [{ from: "...", to: "..." }] }` for
their own import edges, with one trailing `*` subtree wildcard per side;
resolution names violating edges and zero-match rules warn. It adds the
manifest key `boundaries`, keeps the policy narrowing law, and does not merge
with the existing effect vocabulary; it amends no prior source decision.

**2026-08-19 — D-STRUCT-POLICY1=A** *(card #2052; ratified 2026-08-19)*:
Users may declare nominal package-scoped settings through checked bodies,
`pub policy audit(topic: String) { wrap(call) { ... } }`, which observe typed
...
test/bench blocks from closed literal or comptime-pure tables, including the
...
suffix zone `T E!` (or `T !` for the default `Err`); call-site `?` propagation
and `??` keep their current meanings. The infix `T ! E` and `T ? E` spellings
are retired and teach the suffix form.
...
is a suffix zone: `[Success?] [ErrorUnion!]`, one `!` mark on the error type or
on an explicit parenthesized union — `Entry? StoreError!`,
...
**2026-08-21 — D-LOOP-IN1=A** *(card #2153; ratified directly in chat)*: the
source loop joins binding to source with the keyword `in`:
`loop user in users if user.active -> user.name`,
`loop (key, value) in map { audit(key) }`, `loop n in 0..10, 2` (the stride
stays the optional comma third clause and is now the only comma in the
header). This amends D-LOOP-COMMA1=A; D-LOOP-GUARD1=A, D-LOOP-SUBJECT1=A,
D-LOOP-STMT-ARROW1=C, and the stride rule of S19/S22 are untouched, as are
...
Generated by `scripts/agent/decision-index.mjs` from the Tower store: 662 ratified decisions.
...
| `D-1912-NAME1` | A | `c0jmdp7v` |
| `D-1913-LIVENESS1` | C | `c0eexc32` |
| `D-ABILITY-NAME2` | A | `c14vk3sif` |
...
| `D-ARMHEAD-PAREN1` | A | `c0an426i` |
| `D-ARROW-RESPELL1` | A | `c0an426i` |
...
| `D-ERRSIGIL1` | A | `c0uyc9is` |
| `D-ERRSUFFIX1` | B | `c0an426i` |
...
| `D-FAILURE-FOUNDATION1` | A | `cp3zupnt` |
...
| `D-JPK-SECRETCOMPOSE1` | D | `c0gq3vxb` |
| `D-JPK-SECRETMETA1` | B | `c0gq3vxb` |
| `D-JPK-SERVICEDEPTH1` | A | `c0nliloo` |
| `D-JPK-SNIXREUSE1` | A | `c087yjjx` |
...
| `D-PLATFORM-EVIDENCE1` | D | `c0uy7emy` |
...
| `D-STATE-HOME1` | A | `c13gfyz5l` |
| `D-STATE-NS1` | A | `c0ts6lkx` |
| `D-STATE-TERMINAL1` | A | `c11y6itr6` |
| `D-STM1` | A | `c0n6vj96` |
...
| `D-SUBJECT-COHERE1` | A | `c0an426i` |
| `D-SYNC1` | A | `c0n6vj96` |
| `D-TAG-SURFACE1` | A | `c0cv24hu` |
| `D-TAG-UNIFY1` | A | `c0cv24hu` |
| `D-TAIL-RETURN1` | A | `cxf6x7ap` |
| `D-TAINT2` | A | `c0n6vj96` |
...
| `D-TEXTHEAD-TYPE1` | A | `c12uacgqe` |
| `D-TIMERES1` | A | `c0s7isu9` |
| `D-TRACK-ORIGIN1` | A | `cv62zydw` |
...
| `D-VERDICT-2157-1` | Jetpack gets a local unofficial package catalog that needs no keys and carries no signing gua… | `c00s3cle` |
| `D-VERDICT-2188-1` | Any verb that reads or executes Jet source belongs to the jet binary. Any verb that acquires,… | `c024o546` |
| `D-VERDICT-2189-1` | jetpack env is the canonical spelling for entering a project environment. jet env becomes a d… | `c09vy29f` |
| `D-VERDICT-2189-2` | The visible prompt label inside a Jetpack shell is jetpack, not jet. The environment belongs … | `c09vy29f` |
| `D-VERDICT-2190-1` | Every package reference is written package@source#version. The hash slot answers which versio… | `c0gziu7u` |
| `D-VERDICT-2191-1` | A jet verb that needs the project environment while outside it fails with a coded diagnostic … | `c0o7ipzb` |
| `D-VERDICT-2192-1` | jetpack update determines its scope from the current directory: inside a jetpack project it u… | `c0vprzqp` |
| `D-VERDICT-2193-1` | Every tool installed by jetpack is projected as a jetpack shim rather than a hardlink into a … | `c02wcldf` |
| `D-VERDICT-2193-2` | There is no shim. Installed tools keep their current projection and jetpack detects drift on … | `c02wcldf` |
| `D-VERDICT-2194-1` | Realization displays one live aggregate line carrying the current phase with counts, transfer… | `c0a3h9ta` |
| `D-VERDICT-2195-1` | The hangar verb groups everything that manipulates the store rather than the project. It abso… | `c0h7q2i8` |
...
## Operative structure slate — D-STRUCT-* (ratified 2026-08-19, card #2052)

The six A outcomes below are the operative record for the structure-program-is-a-value slate.
They are ratified law; implementation follows on separate cards.

- **D-STRUCT-PLANE1=A — structure is a fact plane.** Liveness, lifecycle, and
  import-edge facts enter the one registry under D-FACT-LAW1. Their safe
  directions and gates are recorded once: referenced / `_name`, fewer promises
  or more warnings honored / policy allow, and fewer edges / an on-record
  manifest rule edit. A row without both a direction and a gate fails the
  compiler guard; `jet inspect structure` reads the same gate ledger, and all
  rows erase before runtime.
- **D-STRUCT-LIVE1=A — four liveness verdicts.** Unused imports, unused
  bindings, unused private functions, and unreachable `pub(package)` or
  application-target exports warn under D-LINTPOLICY1 and never fail by
  default. Delete the name or rename it to the existing `_name` form; library
  `pub` remains exempt, the first three checks are file-local, and export
  reachability uses the package's explicit import graph.
- **D-STRUCT-LIFE1=A — one lifecycle ladder.** `_name` is internal, `pub _name`
  is soft-public, `pub` is stable, `#Deprecated(since: "1.2", use: "parse")`
  is retiring, and removal is a named delta; `removed_in:` waits for editions
  and is warn-only before then. The consumer warning carries the replacement,
  `jet fix` handles a plain rename, and the duplicated Core-only deprecation
  rows in `jet-pkg-model/Manifest.rs` and `Sema/Edition.rs` are replaced by
  ordinary markers using the existing L2001/E2002 path.
- **D-STRUCT-EDGE1=A — manifest-owned import boundaries.** A package may write
...
- **D-STRUCT-POLICY1=A — users may author settings.** The declaration form is
  ...
  arguments/results, and adds visible effects without changing its signature.
  The setting is nominal and package-scoped, unknown names identify their
  declaring package, and the body has no AST, macro, or signature-edit power.
  This extends D-CALLPOLICY1's vocabulary; `#Policy(...)` and `apply(...)` do
  not change.
- **D-STRUCT-ONCE1=A — `@loop` reaches program declarations.** Existing
  `@loop` may appear in marker bodies, top-level impl-item position, and test
  or bench declarations, over literal or comptime-pure closed tables, with
  each generated declaration checked as if written. The ruling amends
  D-GENMOD-VALUE1 for closed type-list bindings and D-META-CODE1 for
  interpolated declaration names; it creates no type at a use site, keeps the
  impl set closed, and exposes expansion through `jet inspect expand`.
  It also reverses the intervening same-day plain-`if` ruling: the earlier
  S57/D-WHEN1 entry points back to this decision, and `@if` remains the live
  compile-time branch spelling.
