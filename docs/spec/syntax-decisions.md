...
| 2026-06-19 | D-NARG-D2 | **defaults may reference earlier params** (A): `fn box(w: Int, h: Int = w)` allowed. Owner: hard work on the backend so the frontend feels magic, while exposing expert tools. **Ratified, not yet implemented** (current default-fill treats defaults as self-contained; extend to allow earlier-param refs) | owner |
