...
| 2026-06-30 | D-UITREE1 | **UI tree uses typed constructors** (A): view items are constructed with dot-construction syntax (`.Button.{ label: "OK" }`) giving full type inference and autocompletion, not string-tag builders. **Ratified — gated on reactive UI stack implementation.** c134 | owner |
| 2026-06-30 | D-STYLESHAPE1 | **style is a flat `Style` record** (A): all visual properties live in one flat `Style` record passed to view items; no nested sub-records per property group. Matches the constraint-layout typed-axis approach (D-LAYOUT1). **Ratified — gated on reactive UI stack implementation.** c134 | owner |
| 2026-06-30 | D-MOTIONTIME1 | **motion timing uses injectable Clock** (A): animation/motion durations and delays come from the same injectable `Clock` as the rest of Jet's time surface (D-TIMEDEPTH1), enabling deterministic animation tests. **Ratified — gated on reactive UI stack implementation.** c134 | owner |
| 2026-06-30 | D-A11YGATE1 | **accessibility is a lint gate: `jet lint --a11y`** (B): a11y issues (missing roles, unlabeled controls, low contrast) surface as lints under `jet lint --a11y` rather than blocking compilation; projects opt-in to zero-a11y-warnings as a CI gate. Compile-errors (A — over-strict for interactive iteration) rejected. **Ratified — not yet implemented.** c134 | owner |
