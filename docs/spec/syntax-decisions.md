...
| 2026-06-24 | D-SIMD2 | SIMD lane construction & access surface (A — method reduce): adopt the community-converged surface — constructor `F32x4(1.0,…)`, splat `F32x4.splat(x)`, lane index `v[i]`, element-wise operators `+`/`-`/`*`/`/`, reduce `v.sum()` with general `v.reduce(#Add)`/`v.reduce(#Max)`; `[F32#4]` round-trips via `from_array`/`to_array` (D-FIXARR1 bridge). Operator overloading is blessed on the built-in lane types ONLY (closed compiler-provided family — no user-defined `+`), which also unblocks D-LINALG1 Option D. Defer named lanes (`v.x/.y/.z/.w`, Option C) to a graphics follow-on. Plan: sidequests/math-linalg-simd.md. c94 | owner |
