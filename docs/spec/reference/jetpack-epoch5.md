...
One typed graph covers packages, environments, services, and foreign flake
inputs. The graph is data; Jetpack performs realization, process control, file
writes, and lock updates from those facts.
...
Language pack entries are typed catalog records. Each entry declares its
package references, host/platform facts, license, command mappings, and
required commands. Enabled records expand into ordinary package references;
disabled records remain visible in the plan and trust fingerprint without
blocking on missing tools. Unsupported hosts or missing required commands fail
closed during expansion; Jet never creates a partial PATH or claims an absent
tool.

Contributors use the same `LanguagePackCatalog` as built-in entries. Registration
rejects duplicate names, incomplete package/host/platform/license/tool facts,
empty command mappings, malformed venv packages, invalid environment-variable
names, and required tools without a command. Contributions cannot create a
second resolver or an untracked PATH shortcut.
...
The service constructor registry supplies named presets with package, executable,
port, argument, readiness, and state facts. Host supervision, image projection,
and discovery use that same registry; no second service runner or shell-string
preset table exists. A named preset may be overridden with explicit `ports`,
`run`, or `ready` fields without changing the supervisor or cleanup path.
...
The private native evaluator is bounded before parsing: source input, tokens,
memory, expressions, imports, and JSON depth have finite limits. Imports require
explicit project-root authority, and only pinned Stage A systems are accepted.
...
Fixed-output fetchers use verified `file:` sources through explicit fetch
authority and return canonical store-path contexts. Cross-system package
identities use explicit pinned target authority. Local external flakes use
explicit `path:` provider authority. Remote providers, dynamic derivations, and
IFD require authority or staging outside this boundary and are rejected rather
than becoming empty or guessed values.
