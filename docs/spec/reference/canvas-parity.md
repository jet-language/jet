...
- [Stmt::Val metadata] status=graph source-backed metadata: `#Meta` projects to binding-node `meta` JSON and Details shows its category/tunable facts (D-CANVASMETA1); scalar, enum, reference, collection, and nested values use source-backed Details editors, covered by `tests/canvas.rs::canvas_details_editor_facts_and_checked_value_edits`, `tests/canvas.rs::canvas_details_collection_values_keep_inline_provenance_and_refuse_bad_nested_source`, `details-scalar-enum-reference-editors`, and `details-collection-nested-editors`.
- [Func metadata] status=graph source-backed metadata: `#Meta` projects to function `meta` JSON and the function Details view shows its category/tunable facts (D-CANVASMETA1).
