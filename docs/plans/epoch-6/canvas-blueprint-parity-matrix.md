...
| Node model | Large graph virtualization and LOD | Canvas renders visible graph regions and low-zoom title-bar nodes for large projections. | claimed | interaction:tests/canvas_scenarios.rs::big_project_perf_budget — fixture and pan pass; zoom reaches 233.3 ms against the 120 ms maximum, so later interaction checks and repeated clean-run proof remain open |
