...
| Pins and wires | Multiple exec IN (convergence) | N incoming exec wires converge on one node over source-backed statement spans. | shipped | interaction:tests/canvas_scenarios.rs::exec_convergence_preview; D-CANVAS-MULTIEXEC1=E no-write preview with extract selected by default and warned duplication |
...
| Debugger | Step, next, continue, stop | Debug controls create, resume, finish, and stop one live session through the production route. | shipped | protocol:tests/web_dev.rs::jet_dev_web_exposes_canvas_panel_and_graph; interaction:tests/canvas_scenarios.rs::debug_live_session |
| Debugger | Breakpoints/watches | Local source-span debug state drives bounded breakpoints and watches without editing source. | shipped | protocol:tests/web_dev.rs::jet_dev_web_exposes_canvas_panel_and_graph; projection:crates/jet-devserver/src/Canvas/debug_source_git.rs |
...
| Learning | Node docs and first-run overlay | Source doc comments, type explanations, pin hover text, and a dismissible local first-run overlay guide Canvas without changing source. | shipped | interaction:tests/canvas_scenarios.rs::node_docs_pointer_hover; interaction:tests/canvas_scenarios.rs::canvas_onboarding_tour |
