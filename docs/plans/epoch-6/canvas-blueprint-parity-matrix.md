...
| Workbench | My Canvas component tree and palette | Files, functions, and current-function variables are source-backed tree items; empty-canvas and pin gestures open the canonical Flow/Variables/Project/Core palette. | shipped | interaction:tests/canvas_scenarios.rs::component_tree_and_palette |
| Workbench | Built-in method search | Palette/context search includes source-checked built-ins and ordinary Jet functions with pin metadata. | shipped | interaction:tests/canvas_scenarios.rs::palette_insert_catalog_sweep; interaction:tests/canvas_scenarios.rs::palette_insert_core_fn; interaction:tests/canvas_scenarios.rs::palette_insert_imported_alias_function; unit:tests/canvas.rs::canvas_actions_preserve_nested_imported_module_callee |
...
| Workbench | Shift-add, ctrl-toggle, marquee select | Modifier clicks and marquee drag build multi-selection over projected nodes. | shipped | interaction:tests/canvas_scenarios.rs::selection_marquee_modifiers_local_move |
...
| Workbench | Cut/copy/paste/duplicate | Source transactions duplicate or move source-backed selections; staged fallback remains local until connected. | shipped | interaction:tests/canvas_scenarios.rs::clipboard_copy_paste; interaction:tests/canvas_scenarios.rs::clipboard_paste_as_staged; interaction:tests/canvas_scenarios.rs::clipboard_mixed_selection_staged_fallback; interaction:tests/canvas_scenarios.rs::clipboard_duplicate_undo_redo; interaction:tests/canvas_scenarios.rs::clipboard_stale_selection_refusal; interaction:tests/canvas_scenarios.rs::clipboard_refuses_entry_selection; unit:tests/canvas.rs::canvas_source_paste_is_renamed_checked_and_stale_atomic |
| Workbench | Inspector/details panel | Inspector reflects source-backed function, variable, nested-value, and node properties; every editable field has a live checked transaction. | shipped | interaction:tests/canvas_scenarios.rs::details_scalar_enum_reference_editors; interaction:tests/canvas_scenarios.rs::details_collection_nested_editors; interaction:tests/canvas_scenarios.rs::click_select_details; unit:tests/canvas.rs::canvas_details_collection_values_keep_inline_provenance_and_refuse_bad_nested_source |
| Workbench | Collection and nested Details editors | Lists, maps, and nested values expose source-backed fields. Every rendered field has a live apply operation. Validation, stale revisions, Escape, blur, undo, redo, and reload keep source truth intact. | shipped | interaction:tests/canvas_scenarios.rs::details_collection_nested_editors; unit:tests/canvas.rs::canvas_details_collection_values_keep_inline_provenance_and_refuse_bad_nested_source |
...
| Node model | Branch, switch, loops | Jet `if`, dispatch, and `loop` forms project and insert from palette without opaque fallbacks; a staged branch materializes at the saved exec target; pattern-arm rows are editable source transactions. | shipped | interaction:tests/canvas_scenarios.rs::branch_insertion_targets; interaction:tests/canvas_scenarios.rs::palette_insert_flow_variable_project_core; interaction:tests/canvas_scenarios.rs::pattern_arm_add_edit_remove; interaction:tests/canvas_scenarios.rs::pattern_arm_invalid_refused; projection:#272/#274, tests/canvas.rs |
...
| Node model | Large graph virtualization and LOD | Canvas renders visible graph regions and low-zoom title-bar nodes for large projections. | shipped | interaction:tests/canvas_scenarios.rs::big_project_perf_budget — generated 300-function project passed Chromium open, pan, zoom, selection, palette, file-switch frame budgets plus culling hit/focus/minimap/source checks on two clean runs |
...
| Pins and wires | Multiple exec OUT (generalized) | Switch arms, loop body/done, and early returns project as N labeled exec outs. | shipped | projection:crates/jet-devserver/src/Canvas/graph_json.rs::add_execution_overlay; protocol:test:canvas_projects_labeled_execution_outputs_for_loops_and_early_returns |
| Pins and wires | Multiple exec IN (convergence) | N incoming exec wires converge on one node over source-backed statement spans. | shipped | interaction:tests/canvas_scenarios.rs::exec_convergence_apply; interaction:tests/canvas_scenarios.rs::exec_convergence_explicit_strategies; interaction:tests/canvas_scenarios.rs::exec_convergence_escape; interaction:tests/canvas_scenarios.rs::exec_convergence_stale; interaction:tests/canvas_scenarios.rs::exec_convergence_refuses_out_of_scope; interaction:tests/canvas_scenarios.rs::exec_convergence_structured_join; D-CANVAS-MULTIEXEC1=E checked extract/helper/duplicate choices with recoverable refusal and ordinary undo |
...
| Events | Event dispatchers/interfaces | `core.event` Event/Hook calls project dispatcher emit/subscription/lifetime/EventTrace facts from source. | shipped | interaction:tests/canvas_scenarios.rs::events_panel_authoring; protocol:tests/canvas.rs::canvas_projects_event_dispatchers_without_masquerading_runtime_facts |
| Interfaces | Trait/impl authoring | Jet traits project Blueprint-interface parity facts and create ordinary checked impl stubs. | shipped | interaction:tests/canvas_scenarios.rs::traits_panel_authoring; protocol:tests/canvas.rs::canvas_projects_trait_impl_authoring_and_writes_impl_stub |
...
| Debugger | Debug session selector | Canvas selects the one source- and revision-bound live session and exposes its executing tier. | shipped | protocol:tests/web_dev.rs::jet_dev_web_exposes_canvas_panel_and_graph; interaction:tests/canvas_scenarios.rs::debug_live_session |
| Debugger | Step, next, continue, stop | Debug controls create, resume, finish, and stop one live session through the production route. | shipped | protocol:tests/web_dev.rs::jet_dev_web_exposes_canvas_panel_and_graph; interaction:tests/canvas_scenarios.rs::debug_live_session; interaction:tests/canvas_scenarios.rs::debug_breakpoints_run_control_gestures |
| Debugger | Breakpoints/watches | Local source-span debug state drives bounded breakpoints and watches without editing source. | shipped | protocol:tests/web_dev.rs::jet_dev_web_exposes_canvas_panel_and_graph; interaction:tests/canvas_scenarios.rs::debug_breakpoints_run_control_gestures; projection:crates/jet-devserver/src/Canvas/debug_source_git.rs |
| Debugger | Active node/wire pulse | A running overlay maps the active stop back to current graph source spans; finished/stopped sessions have no pulse. | shipped | protocol:tests/web_dev.rs::jet_dev_web_exposes_canvas_panel_and_graph; projection:crates/jet-canvas/src/js/graph-rendering.js |
| Debugger | Call stack and trace | One bounded session response projects current Jet call-stack and trace facts. | shipped | protocol:tests/web_dev.rs::jet_dev_web_exposes_canvas_panel_and_graph; projection:crates/jet-devserver/src/Canvas/debug_source_git.rs |
...
| Accessibility | Keyboard-only authoring | Keyboard commands cover search, action palette, alignment/tidy, bookmarks, run, undo/redo, graph/source toggle, and node nudge with focus-visible/reduced-motion support. | shipped | interaction:tests/canvas_scenarios.rs::workspace_keyboard_view_state; interaction:tests/canvas_scenarios.rs::promote_pin_keyboard_gesture; interaction:tests/canvas_scenarios.rs::conversion_keyboard_gesture; interaction:tests/canvas_scenarios.rs::math_expression_keyboard_edit; interaction:tests/canvas_scenarios.rs::keyboard_cheat_sheet_accessibility_states |
| Learning | Checked docs and teaching states | Checked source docs drive node hovers; the first-run overlay and empty/loading recovery states teach the next safe action without changing source. | shipped | interaction:tests/canvas_scenarios.rs::node_docs_pointer_hover; interaction:tests/canvas_scenarios.rs::canvas_onboarding_tour; interaction:tests/canvas_scenarios.rs::canvas_teaching_empty_states |
...
| Source control | Dirty/stale/conflict markers | Graph transactions guard against stale source revisions; diagnostics are tagged to the checked revision so stale problem overlays clear on reproject. | shipped | interaction:tests/canvas_scenarios.rs::bubble_appears_and_clears; projection:#265, tests/canvas.rs::canvas_stale_transaction_conflicts_without_writing; unit:jet-devserver::Canvas::source_model::tests::compare_and_publish_preserves_source_on_conflict |
