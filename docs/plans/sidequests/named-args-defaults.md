...
- **Parser** — `param()` (src/parser/items.rs) parses a trailing `= expr` default
  via `expr_no_struct_lit()`. `call_arg()` (src/parser/exprs.rs) parses a `name:`
...
- **Sema (free functions)** — `FuncSig` carries `param_info: Vec<(String, bool)>`
  and `defaults: Vec<Option<Expr>>` (src/sema/mod.rs — `FuncSig`), populated in
  `func_to_sig` (src/sema/mod.rs). `check_call` (src/sema/checker_infer.rs):
...
- **Fmt (params)** — `fmt_param` emits ` = default` (src/fmt/items.rs).
...
4. **L2401 covers free functions, not methods.** The advisory lint (positional
   ...
   src/sema/registration.rs and src/sema/bundle.rs (the `f.is_pub` param loop),
...
1. **Lift labels + defaults to methods.** Add `param_info` and `defaults` to
   `MethodSig` (src/sema/mod.rs); populate in `func_to_method_sig`
   (src/sema/mod.rs). In `check_method_args` (src/sema/checker_items.rs), run the
   same label-match (offset by `self`) and default-fill logic `check_call`
   (src/sema/checker_infer.rs) already has. Factor the shared logic into one
...
4. **Extend L2401 to methods.** Already fires on free `pub fn`s in
   src/sema/registration.rs / src/sema/bundle.rs; extend the same param loop to
...
`src/sema/{mod,checker_items,checker_infer,registration,bundle}.rs` (MethodSig +
check_method_args + shared label/default helper, L2401-on-methods),
`src/parser/{items,exprs}.rs` (only if a decision adds grammar),
