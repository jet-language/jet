...
| #991 | live | `crates/jetpack/src/Store/Archive.rs`, `crates/jetpack/src/CLI/package_hangar_vendor.rs`, `tests/jetpack_hangar_store_v2.rs` | `jet hangar copy` takes a source snapshot, verifies the signed archive, and publishes the closure through destination staging and one closure transaction; repeated copies are deterministic and conflicting destination bytes stay live. |
