...
| #517 | live | `crates/jetpack/src/Store/Archive.rs`, `tests/jetpack_hangar_store_v2.rs` | One locked signed-archive repair path verifies and stages before publication, quarantines the prior object, restores on import failure, and recovers crash leftovers without following symlinks. |
| #992 | live | `crates/jetpack/src/Store/Archive.rs`, `crates/jetpack/src/CLI/package_hangar_vendor.rs`, `tests/jetpack_hangar_store_v2.rs` | `jetpack hangar export`, `import`, and `copy` use one signed archive backbone: closure snapshots are verified before publication, destination staging and closure registration are atomic, repeated operations are deterministic, and conflicting bytes stay live. |
