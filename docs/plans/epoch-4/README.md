...
| U12 | `D-JPK-SERVICE1` | A | `services:` in `env.*`; jetpack supervises project-local processes; `jet services up/down/health/logs/wait`; `jet dev` health-gates before running code. |
...
| U22 | `D-JPK-GC1` | B (amended 2026-07-03) | Hangar disk contract: auto-GC ages out unreferenced objects (30d default, opportunistic, no daemon) + manual `jetpack hangar clean` (GC + hangar optimize: hardlink/dedup, `nix store optimise` equivalent, one pass) + honest `jetpack hangar du`; lockfile/generation-reachable never collected; zero-/tmp guarantee golden-tested; build scratch hangar-scoped and crash-cleaned. |
| U23 | `D-JPK-NONIX1` + `D-JPK-NIXSTORE1` | A/D | No-Nix machines: everything Nix-free realizes; package refs without a pinned compatibility output fail with one E12xx naming them; Jetpack has no installed-Nix process fallback and never holds realized packages hostage. |
...
| U27 | `D-JPK-BUILDDBG1` | A | Failed builds: `--shell-on-fail` shell inside preserved scratch at the failing step (sole exception to U22 cleanup; GC-swept), `jet explain <ref>` (Store identity, provider facts, dependency/closure edges, liveness roots, and rebuild checks), and `jet logs <pkg>` persisted per-step with `--json`. |
