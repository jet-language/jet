...
### Phase 8 — Native backends (Linux/GTK4 DONE, verified 2026-07-03)
AppKit/Win32/GTK via C FFI. Each implements `JetBackend`. Platform-gated with `#Target(Os.*)` + `comptime if build.os`.

**Linux/GTK4 shipped** (unblocked by D-UIDEVSHELL1=A): `core.ui.gtk_backend()` is a real, working `JetBackend` over libgtk-4 — a retained-mode widget API. `label`/`button` create real GtkWidgets and return an integer handle; `set_text`/`set_size`/`set_color` mutate a live widget (Px via `gtk_widget_set_size_request`, `#RRGGBB` fills via a scoped `GtkCssProvider`); `on_click(id, handler)` wires a button's `clicked` signal via `g_signal_connect_data`; `present(title)` opens the window and runs the GLib main loop. The flagship example is a **live counter**: the button click sets a `reactive.signal`, the `ui.reactive_render` effect re-runs and calls `set_text` on the label in place — the same reactive core the web/DOM backend uses (I8, one reactive model). All gtk C-ABI calls live in the vetted `crates/jet-codegen/src/Prelude/UiGtk.rs` `jet_gtk` module (I1: user code writes no low-level tier; `tests/golden.rs` strips `mod jet_gtk` from its I1 scan like the S58 C-FFI shims). The module is emitted only on a Linux target that constructs the backend (`Codegen/mod.rs::uses_gtk_backend`). Selection is the shipped `comptime if build.os` dispatch (D-OSTARGET2=B). The native link (`-lgtk-4 …`) is named by `use c.gtk4` through the S59 `pkg-config gtk4` path; `flake.nix`'s devShell provides `gtk4` + `pkg-config`. Missing gtk4 at build time is the existing **E3201** (names the fix). `jet run examples/features/ui/ui_native_linux.jet` (inside `nix develop`) opens the window; `JET_UI_HEADLESS=1` skips it (every widget op is a safe no-op) so tests and display-less CI terminate. Verification: golden runs the example with `JET_UI_HEADLESS=1`; `tests/cross.rs::gtk_backend_*` structurally proves the window/widget/click/style code emits + links against gtk4; a real `jet build` links libgtk-4 (`ldd` shows it).

**Not yet**: macOS/AppKit + Windows/Win32 backends (their `comptime` arms print a "not yet" line — honest degrade). Menus, motion, a11y beyond labels remain out of the v1 native floor. `jet dev` hot-reload is still web-only (`Source/CmdDevWeb.rs`); a native app is run with `jet run`/`jet build`, not `jet dev` (native hot-reload is a follow-up).
