...
| `maturity-markers` | `@` contract marker | `@Experimental`, `@Tested`, `@Hardened` | D-MATURITY1=B | `ATTR_EXPERIMENTAL`, `ATTR_TESTED`, `ATTR_HARDENED`; `MaturityTag` on `Func`; parser `at_maturity_fn`/`bump_maturity_marker`; fmt preserves; E0062 for retired `#` spelling; `examples/features/syntax/maturity_tags.jet` | Shipped doc-only (no sema propagation) |
