...
| D-DEVR-WITNESS1 | Witness + countersign model; CI = cold witness (`jet verify --cold`); team share via cache bindings | full witness model / local witnesses only / decline | amends D-CLI-SURFACE1=B (new `verify` verb; settles the word against `jetpack hangar verify`); rides D-JPK-CACHEAUTH1=D signing and D-JPK-REMOTE1=D trust domains |
