...
D-CAP2 (which explicitly declined a *third sigil* — "no third sigil"),
D-DROP-WORD1, D-MARK-DISCARD1, D-MEM1.
...
- **I — duration spelled 3 ways:** `#Every(5min)`, `.timeout(2s)`,
  `time.ms(n)` (core F13).
- **J — derive spelled 3 ways:** `@[Codable]`, `#[Wire]`, in-body `derive`
  line (core F9).
...
- **Two-plane `@`/`#` law**, *once the assignments are correct* — the intuition
  "promise vs instruction" is sound (D-MARKER-FAMILY1). Fix the leaks, keep the
  law.
...
- **Naming laws that already hold** — `has` for membership (D-API-CONTAINS1),
  ...
  don't disturb them.
- **Injectable-capability convention** — `time.clock(seed)`/`random.rng(seed)`
  mint a value, passed as an ordinary typed param, no DI container. Uniform
  today (stdlib §8).
...
**(f) value-lifecycle — the plane law forces one shape.** Duplicating,
consuming, and discarding are all *operations on a value* → the method plane. All
three collapse onto `.verb()`, matching the already-shipped `.drop()`:

```jet
ops.transform(pool, input.copy(), 5000)   // was `copy input` prefix keyword
handle.consume()                          // was consume(handle) free builtin
result.drop("reason")                     // already a method
```

`input.copy()` reads left-to-right in argument position and carries its own
call-site signal (the `.copy()` is visibly *on* `input`), which the bare `copy`
prefix never did. No new sigil is introduced, respecting D-CAP2's "no third
sigil." Class G fixed by rule.
...
`result.drop("reason")` → `input.copy()` ✓. Estimated
predict-right ~90% across classes, vs ~60% today.
...
**Adopt Candidate 2 (Plane-total) as the language-wide shape law, and treat the
manifest as the strict data-plane instance of that same law** — i.e.
register-split's "manifest is pure data" is not a second law but what
plane-total *looks like* when the domain is a manifest (data plane + value
literals only). This is one coherent job→plane function, not two.

- **Why:** it is the only candidate that fixes all four highest-blast classes
  (A/B/C/D) *and* keeps every bright spot — it repairs the two-plane law instead
  of replacing it, keeps keywords for control flow (the owner's constraint),
  keeps marker terseness and the magic/named-arg dual facet (I8), and states the
  product-vs-sum and ctor-verb rules the current surface only implies. Highest
  guessability (~90% sibling-predict vs ~60% today) at the lowest conceptual
  cost, which is exactly priority #2.
- **Concretely, the value-lifecycle family (Class G) is respelled to the
  method plane:** `copy x` → `x.copy()`, `consume(x)` → `x.consume()`, joining
  the already-shipped `result.drop("reason")`. The `copy` prefix keyword is
  retired; `ops.transform(pool, copy input, 5000)` becomes
  `ops.transform(pool, input.copy(), 5000)`. This is the strongest guessability
  play — it unifies the whole family onto the one shape already in the language
  (`.drop()`), reads left-to-right in argument position, and adds no sigil
  (honoring D-CAP2's "no third sigil"). A postfix operator/field form was
  considered and rejected: it would be a fourth punctuation for a job the method
  plane already owns.
- **Tradeoff:** it keeps two marker glyphs and a small bounded set of I8 DSL
  islands (`.view`, `#Test` members), so "gap-free" is "gap-free except a
  published, closed exception list" — an honest asterisk, absorbed by making the
  exception list itself part of the law (I9 below).
- **Why not Value-uniform (owner's seed):** it fixes construction beautifully
  ...
  invariant), regresses marker terseness into field bags, and only covers
  values — leaving every declaration and control-flow shape untouched. It is the
  right *instinct for construction* (one literal shape) and plane-total adopts
  that instinct as its value-literal plane, without paying its costs elsewhere.
- **Why not Declaration-uniform:** kills the beginner `packages: { hello:
  executable }` one-liner (priority #2), mangles `alias`/`distinct`, and leaves
  construction and markers unsolved.
- **Why not pure Register-split:** fixes only manifests, forbids the ratified
  `hook` escape and the computed-module direction, and regresses the beginner
  manifest — so it is kept only as the *manifest profile* of plane-total, not as
  the whole law.
...
defect, not a carve-out, and any bounded exception (an I8 DSL island) is listed
in the law itself, closed and named.
...
the one ref sigil. Branch / loop / return → keyword. A manifest is the data
plane: `field: value` and value literals only. If a construct maps to two
shapes, fix the boundary — never special-case.
...
|---|---|---|---|
| **D-SHAPE1** | **Ratify the plane-total shape law + invariant I9** (§4/§5) — the job→plane function, the closed DSL-island exception list, the manifest-as-data-plane profile | — | the disease framing itself; gates all below |
| **D-SHAPE2** | Marker-plane repair: move `#Sanitizer`/`#Replayable` → `@`, one handle-binding slot (keyword-name), one derive spelling | D-SHAPE1 | core F1, F2, F9 |
| **D-SHAPE-LIFECYCLE** | Value-lifecycle onto the method plane: **retire the `copy` prefix keyword → `x.copy()`**, `consume(x)` → `x.consume()`, aligning both with shipped `result.drop("reason")` — respell shipped `ops.transform(pool, copy input, …)` call sites | D-SHAPE1 | core F6 |
| **D-SHAPE3** | Construction law: product→`.{}`, sum→`.Variant()`, mechanical `new`/`from`/`over` choice, retire bare `T(…)` + module factories (`time.clock`→`Clock.new`), unify Option/Result variant casing | D-SHAPE1 | core F4, F5, F17; stdlib 1, 2 |
| **D-SHAPE4** | Stdlib naming/resource law: `eq`/`equal` split, `watcher.*` verb-first, RAII-vs-`.close()` rule, duration unification (`5min`/`2s`/`time.ms`) | D-SHAPE1, D-SHAPE3 | stdlib 2, 3, 4; core F13 |
| **D-SHAPE5** | Manifest data-plane profile — **reworks the HELD D-ECO1–19 wave**: `overlay`→`module overlay.`/`Overlay.{}`, resolve bare `executable{}`, fix U8-vs-U10 nesting, `Recipe.cargo`→`.Cargo` variant, one block/colon rule | D-SHAPE1, D-SHAPE3 | manifest F1, F2, F3, F7, F8 |
| **D-SHAPE6** | CLI grammar law + **fix the spec/impl drift**: pick the noun-verb namespacing (or ratify flat and amend the spec), implement/align `jet inspect …`/`jet registry …`, unify `jetos`/`jetpack` depth, retire self-relabeling aliases | D-SHAPE1 | manifest F5, F6 |
| **D-SHAPE7** | External-ref sigil unification: one signature across `provider@target`, version-pin, CLI `:` ref, and the three `D-MONOREF1` member-ref forms | D-SHAPE1 | manifest F4, F9; core F8 |
...
| **SHAPE-HYGIENE1** *(card, no owner gate)* | Registry doc casing (`#layout`→`#Layout`), `U11`/`U13`/`U18` citation drift → canonical headers, `W`-prefix diagnostics note — pure correction, can start now | — | core F12; manifest F10; diagnostics note |

Order: **D-SHAPE1 ratifies first** (nothing else is a real decision until the
law exists). Then the application ballots fan out; SHAPE-HYGIENE1 is a
correction that can run in parallel immediately. D-SHAPE5 explicitly supersedes
the held D-ECO wave rather than deciding it as-proposed — the ecosystem
spellings are re-derived from the ratified law, not chosen ad hoc.
