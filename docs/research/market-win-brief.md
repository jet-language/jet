...
6. **Hardened first-party package universe is a strategy pillar (ratified 2026-08-28 grill session).** Jet launches with a large first-party package universe (order of 50-100 packages: web frameworks, DB drivers, parsers, cloud SDKs) mass-produced by the agent fleet and quality-gated by the hardening rig. This converts AI authorship from a bug-risk liability into the moat: verified mass production no incumbent can copy, and ecosystem gravity without waiting years for third-party authors. Sequencing is strict: the hardening rig (docs/proposals/hardening-rig.md) must exist first so "hardened" is a measured fact, not marketing. The doc treats this as a pillar of the takeover sequencing; the implementation epic is minted only after the rig exists.
...
- The hardening-rig design (docs/audits/hardening-rig.md, if it exists by the time this runs) — the triple gate and continuous rig ARE the "public living proof" machinery; cite them as the proof supply chain rather than reinventing them.
