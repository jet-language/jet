...
that, but nothing here ships first.
...
- **OS-I3** Config evaluation is pure (jetpack JP0 rules apply).
...
|----|-------|-------|---------------|
| OS0 | JP0 | option registry + merge engine + J-M01x/02x/03x, driven through builtins (no new syntax yet) | golden: 3 modules merge to canonical JSON; conflict + cycle snapshots; shuffle-order determinism test |
...
| OS2 | JP1–JP5 | recipe generator + activation (symlink, bootloader) + `switch/diff/rollback` | VM test: switch → rollback round-trip; power-cut simulation boots old generation |
...
Agent guardrails: identical to jetpack §7, plus — never add a config escape
hatch "temporarily"; the absence of escape hatches IS the product.

---

## 9. Decisions for the owner (jetos only)
