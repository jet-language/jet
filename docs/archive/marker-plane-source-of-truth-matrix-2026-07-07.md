...
| `effect-capability-directives` | effect row / `@` rule | `--[FS]->`, `--[FS.Read, !FS.Write]->`, `--[via f]->`, `#Caps(Net) { ... }`, `#Caps(caps: FS.Read) { ... }` | D-EFF1, D-EFF2, D-EFFTREE1, D-PROP1, D-SCAP1, D-SHAPE8 | `EFFECT_ARROW_OPEN`, `EFFECT_ARROW_CLOSE`, `KW_CAPS`; parser effect-bound and caps paths; fmt dotted effect paths test | Shipped; former `#(Effects)` / `#(via f)` rows are rejected with E0066 |
...
| `test-claim-directives` | `#` runnable block | `#Test("name") { ... }`, `#Test fn prop(p: T)`, `#Test("name") { .measure { ... } }` | S43, D-TESTPAREN1, D-TEST1, D-CLAIM-BENCH1=A | `KW_TEST`; `test_def`; fmt test and `.measure` member tests | Shipped; former benchmark claim spelling retired |
