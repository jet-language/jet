...
fn run(args: &[&str]) -> Output {
    Command::new(jet()).args(args).output().unwrap()
}
...
fn structural_diff_ignores_format_comments_and_reports_signature_and_move() {
    ...
    let report = String::from_utf8_lossy(&output.stdout);
    assert!(
        report.contains("\"kind\":\"signature_changed\""),
        "{report}"
    );
    ...
    assert_eq!(
        String::from_utf8_lossy(&no_churn.stdout),
...
fn git_driver_install_is_idempotent_and_preserves_config() {
    ...
    assert!(git(&root, &["config", "merge.jetstruct.name", "stale"])
        .status
        .success());
    for _ in 0..2 {
...
fn ambiguous_same_shape_and_cross_delete_rename_edit_never_guess() {
    ...
    let delete_edit = run(&[
        "merge",
        "--structural",
        delete_base.to_str().unwrap(),
        deleted.to_str().unwrap(),
        edited.to_str().unwrap(),
        "--out",
        delete_out.to_str().unwrap(),
        "--json",
    ]);
...
fn git_driver_repairs_exact_keys_and_supports_gitdir_indirection() {
    ...
    assert!(
        git(&repo, &["config", "user.email", "jet-test@example.invalid"])
            .status
            .success()
    );
    assert!(git(&repo, &["config", "user.name", "Jet Test"])
        .status
        .success());
    assert!(git(&repo, &["config", "merge.jetstruct.name", "stale"])
        .status
        .success());
    assert!(git(&repo, &["config", "merge.other.driver", "keep-me"])
        .status
        .success());
    ...
    assert!(
        ...
        "test must exercise real linked-worktree indirection"
    );
}
...
fn structural_commands_have_specific_help() {
    for (command, needles) in [
        ...
        (
            "merge",
            &["--structural", "--out", "install-driver", "--repo"] as &[&str],
        ),
    ] {
        for args in [[command, "--help"], ["help", command]] {
            ...
            for needle in needles {
                assert!(
                    ...
                    "{command} help missing {needle}: {help}"
                );
            }
        }
    }
