...
fn generated(format: &str, path: &str, input: &str, root: Option<&str>, command: &str) -> String {
...
fn run_public_data_bind(
    ...
    let output_path = stdout
        ...
        .unwrap_or_else(|| {
            panic!("public `jet inspect bind` did not report an output path:\n{stdout}")
        });
...
fn assert_generated_project(
    ...
    let dynamic_fields = generated
        ...
        .collect::<Vec<_>>();
    assert_eq!(
        dynamic_fields, allowed_dynamic_fields,
    ...
    let aot = Command::new("rustc")
        ...
        .unwrap();
    assert_success(&aot);
    let aot_run = Command::new(&binary_path)
        .current_dir(&dir)
        .output()
        .unwrap();
...
fn assert_generated_source_parses(source: &str) {
    ...
    assert!(
        ...
        "generated source lexed with diagnostics: {diagnostics:#?}"
    );
    assert!(
...
fn retired_top_level_bind_forms_teach_the_grouped_route() {
    ...
    for args in cases {
        ...
        assert!(
            !output.status.success(),
            "retired bind unexpectedly succeeded: {rendered}"
        );
        assert!(
            rendered.contains("E2101"),
            "retired bind lost its teaching diagnostic: {rendered}"
        );
        assert!(
            rendered.contains("inspect bind"),
            "retired bind lost its canonical fix: {rendered}"
        );
        assert!(
            !rendered.contains("jet bind"),
            "retired bind kept a top-level alias: {rendered}"
        );
        assert!(
            !rendered.contains("E3208"),
            "retired bind reached the binder: {rendered}"
        );
    }
    let _ = fs::remove_dir_all(dir);
}
...
fn xml_bind_resolves_namespaces_omits_declarations_and_keeps_collisions() {
    ...
    assert!(
        !first.contains("xmlns"),
        "namespace declarations are not wire fields"
    );
    ...
    let id_base =
        jet::Syntax::sanitize_generated_name("{urn:one}id", jet::Syntax::NameCase::Snake, "field");
    assert_eq!(
        id_base,
        jet::Syntax::sanitize_generated_name("{urn_one}id", jet::Syntax::NameCase::Snake, "field",)
    );
    ...
    assert!(first.contains(&field_marker("{urn:nested}leaf", &nested_leaf, "String",)));
...
"#,
    ...
    assert!(provenance
        .contains("generated by: jet inspect bind json records\\nname\\u0001.json --type Record"));
...
fn c_bind_sanitizes_hostile_function_and_parameter_names_with_collisions() {
    ...
    assert!(result
        .source
        .contains("#Bindgen module c.hostile_lib.__bindgen__"));
...
fn data_bind_rejects_malformed_inputs() {
    ...
    for (format, filename, input) in malformed {
        ...
        assert_eq!(
            output.status.code(),
            Some(1),
            "{format} unexpectedly succeeded: {stderr}"
        );
        assert!(
            stderr.contains("Error [E3208]:"),
            "{format} missing E3208: {stderr}"
        );
        assert!(
...
fn json_bind_rejects_non_json_unicode_whitespace() {
    ...
    assert!(
        stderr.contains("Error [E3208]:"),
        "strict JSON rejection lost E3208: {stderr}"
    );
    assert!(
        stderr.contains("JSON"),
        "strict JSON rejection lost its format context: {stderr}"
    );
    let _ = fs::remove_dir_all(dir);
}
...
fn data_bind_filesystem_errors_use_e2105() {
    ...
    assert!(
        missing_stderr.contains("Error [E2105]:"),
        "missing input lost E2105: {missing_stderr}"
    );
    assert!(
        !missing_stderr.contains("E3208"),
        "missing input was mislabeled as a schema error: {missing_stderr}"
    );
    ...
    assert!(
        blocked_stderr.contains("Error [E2105]:"),
        "output IO failure lost E2105: {blocked_stderr}"
    );
    assert!(
        !blocked_stderr.contains("E3208"),
        "output IO failure was mislabeled as a schema error: {blocked_stderr}"
    );
    let _ = fs::remove_dir_all(dir);
}
...
fn data_bind_sanitized_filename_collision_does_not_overwrite() {
    ...
    assert!(
        ...
        "filename collision lost E2104: {stderr}"
    );
    assert!(
        !stderr.contains("E3208"),
        "filename collision was mislabeled as a schema error: {stderr}"
    );
    assert_eq!(
        before,
        fs::read_to_string(&output_path).unwrap(),
        "collision overwrote the first binding"
    );
    let _ = fs::remove_dir_all(dir);
}
...
fn data_bind_provenance_is_hashed_and_stable() {
    ...
    assert!(
        first.contains("generated by: jet inspect bind json fixtures/stable.json --type Stable")
    );
