# Task: process-batch-timeout-recovery

Domain: long-running-and-interactive-commands
Case: timeout-cancellation-cleanup
...
The input directory contains exactly this, with small files shown inline:
    ...
    timeout-recovery.tsv  (130 bytes)
      | label	program	arguments	timeout_ms
      | slow	sleep	2	50
      | resist	sh	-c|trap '' TERM; while :; do :; done	50
      | recover	printf	recovered	500
      | 
...
slow|timeout
resist|timeout
recover|exit=0|stdout=recovered
----- END EXPECTED STDOUT -----
