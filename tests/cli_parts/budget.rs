...
fn compiler_probe_rejects_missing_edit_input_and_closed_metric_typos() {
    ...
    assert!(
        missing_text.contains("E2908")
            || String::from_utf8_lossy(&missing.stderr).contains("E2908")
    );
    assert!(
        !dir.join(".jet/perf/reports").exists(),
        "missing edit input must not produce a report"
    );

    let typo_dir = compile_latency_budget_project("compile_probe_closed_metric");
    let source = fs::read_to_string(typo_dir.join("src/run.jet"))
        .unwrap()
        .replace(".CompileTime(.P95)", ".CompileLatency(.P95)");
    fs::write(typo_dir.join("src/run.jet"), source).unwrap();
    let typo = Command::new(jet())
        .args(["budget", "check", "--json"])
        .current_dir(&typo_dir)
        .output()
        .unwrap();
    assert_eq!(typo.status.code(), Some(1));
    assert!(
        String::from_utf8_lossy(&typo.stdout).contains("E2903")
            || String::from_utf8_lossy(&typo.stderr).contains("E2903")
    );
}
...
fn compiler_probe_rejects_unsupported_target_without_a_report() {
    ...
    let source = fs::read_to_string(dir.join("src/run.jet"))
        .unwrap()
        .replace("\"cli\"", "\"unsupported\"");
    ...
    assert!(
        !dir.join(".jet/perf/reports").exists(),
        "unsupported target must not produce a report"
    );
}
...
fn compiler_facts_rejects_compile_time_instead_of_measuring() {
    ...
    assert!(
        text.contains("E2903") || text.contains("CompilerProbe") || text.contains("CompileTime"),
        "{text}"
    );
    assert!(
        !dir.join(".jet/perf/reports").exists(),
        "closed grammar rejection must not produce a report"
    );
}
...
fn allocation_probe_uses_real_bench_boundaries_and_rejects_forged_cache() {
    ...
    for measurement in measurements {
        ...
        assert_eq!(
            provider["kind"],
            CanonicalJson::String("AllocationProbe".into())
        );
        assert_eq!(provider["identity"], CanonicalJson::String("arena".into()));
        assert_eq!(
            provider["isolation"],
            CanonicalJson::String("benchmark-process-counter-reset-per-trial".into())
        );
        assert_eq!(
            provider["version"],
            CanonicalJson::String("jet-arena-events-v1-warmup-auto-trials-20".into())
        );
        ...
        assert!(
            ...
            "reset boundary leaked calibration or a prior trial"
        );
        let CanonicalJson::Object(metric) = &measurement["metric"] else {
            panic!("metric")
        };
        ...
        for sample in samples {
            let CanonicalJson::Object(sample) = sample else {
                panic!("sample")
            };
    ...
    assert!(
        !String::from_utf8_lossy(&second.stdout).contains("ns/iter"),
        "compatible report reran allocation workload"
    );
...
fn budget_stale_history_is_persisted_rendered_and_bootstrap_appends() {
    ...
    let source = fs::read_to_string(dir.join("src/run.jet"))
        .unwrap()
        .replace("enforcement: .Warn", "enforcement: .Fail");
    fs::write(dir.join("src/run.jet"), source).unwrap();
    ...
    let CanonicalJson::Object(check) = CanonicalJson::parse_canonical(&check.stdout).unwrap()
    else {
        panic!("command")
    };
    assert_eq!(check["status"], CanonicalJson::String("stale".into()));
    assert_eq!(
        *canonical_field(&check["budget"], "failure_kind"),
        CanonicalJson::String("evidence".into())
    );
    let CanonicalJson::Array(results) = canonical_field(&check["budget"], "results") else {
        panic!("results")
    };
    ...
    assert_eq!(
        ...
        CanonicalJson::Array(vec![CanonicalJson::String(first_id.clone())])
    );
    let CanonicalJson::String(reason) = &result["reason"] else {
        panic!("reason")
    };
    assert!(reason.contains("compatible history is stale"), "{reason}");
    assert!(
        reason.contains("policy limit is 2592000 seconds"),
        "{reason}"
    );
    let CanonicalJson::Object(report) = canonical_field(&check["budget"], "report") else {
        panic!("report")
    };
    jet_foundation::PerformanceBudget::verify_budget_report(
        &CanonicalJson::Object(report.clone()).bytes(),
    )
    .unwrap();
    let CanonicalJson::Object(content) = &report["content"] else {
        panic!("content")
    ...
    assert_eq!(
        history["state_id"],
        CanonicalJson::String(stale_state_id.clone())
    );
    assert_eq!(
        history["report_ids"],
        CanonicalJson::Array(vec![CanonicalJson::String(first_id.clone())])
    ...
    assert_eq!(
        ...
        CanonicalJson::Null,
        "stale samples must not be pooled"
    );
    let CanonicalJson::Object(decision) = &measurement["decision"] else {
        panic!("decision")
    };
    let CanonicalJson::Object(trend) = &decision["trend"] else {
        panic!("trend")
    };
    assert_eq!(
        trend["report_ids"],
    ...
    let human = Command::new(jet())
        .args(["budget", "check", "--annotations", "none"])
        .current_dir(&dir)
        .output()
        .unwrap();
    ...
    assert!(
        human.contains("Error [E2906]: performance budget parse has no usable evidence"),
        "{human}"
    );
    assert!(human.contains("compatible history is stale"), "{human}");
    assert!(
        human.contains("budgets stale: 1 baseline stale · report "),
        "{human}"
    );
    ...
    let CanonicalJson::Object(bootstrap) =
        CanonicalJson::parse_canonical(&bootstrap.stdout).unwrap()
    else {
        panic!("bootstrap")
    };
    assert_eq!(
        *canonical_field(&bootstrap["budget"], "applied"),
        CanonicalJson::Bool(true)
    );
    assert_eq!(bootstrap["status"], CanonicalJson::String("stale".into()));
    let CanonicalJson::Object(report) = canonical_field(&bootstrap["budget"], "report") else {
        panic!("report")
    };
    let CanonicalJson::String(second_id) = &report["report_id"] else {
        panic!("report id")
    };
    let manifest = CanonicalJson::parse_canonical(
        &fs::read(dir.join(".jet/perf/baselines/names/ci/linux.json")).unwrap(),
    )
    .unwrap();
    let CanonicalJson::Object(wrapper) = manifest else {
        panic!("manifest")
    };
    let CanonicalJson::Object(content) = &wrapper["content"] else {
        panic!("content")
    };
    let CanonicalJson::Array(generations) = &content["generations"] else {
        panic!("generations")
    };
    assert_eq!(generations.len(), 2);
    let CanonicalJson::Object(second) = &generations[1] else {
        panic!("generation")
    };
    let CanonicalJson::Object(audit) = &second["audit"] else {
        panic!("audit")
    };
    assert_eq!(audit["kind"], CanonicalJson::String("bootstrap".into()));
    assert_eq!(
        audit["prior_state_id"],
        CanonicalJson::String(stale_state_id)
    );
    assert_eq!(
        audit["prior_head_report_id"],
        CanonicalJson::String(first_id)
    );
    assert_eq!(
        second["report_id"],
        CanonicalJson::String(second_id.clone())
    );
}
...
"#).unwrap();
    ...
    assert!(
        json.contains("\"point\":{\"den\":1,\"num\":0}"),
        "unused core import must not fabricate an effect: {json}"
    );
}
...
"#).unwrap();
    ...
    assert!(
        stderr.contains("has no exact checked front-end fact; refusing proxy measurement"),
        "{stderr}"
    );
    assert!(
        !dir.join(".jet").exists(),
        "unsupported fact emitted an artifact"
    );
}
...
fn budget_path_tools_cannot_forge_provenance() {
    ...
    let json = String::from_utf8(out.stdout).unwrap();
    assert!(
        !json.contains("fake-forged-triple"),
        "PATH rustc forged target identity"
    );
    assert!(
        !json.contains("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"),
        "PATH digest tool forged compiler identity"
    );
}
...
fn budget_unreadable_compiler_identity_rejects_before_artifact() {
    ...
    let stderr = String::from_utf8(out.stderr).unwrap();
    assert!(
        stderr.contains("cannot hash running compiler executable"),
        "{stderr}"
    );
    assert!(
        !dir.join(".jet").exists(),
...
fn budget_parallel_child_builds_survive_running_compiler_unlink() {
    ...
    for (child, dir) in children.into_iter().zip(&dirs) {
        ...
        let CanonicalJson::Object(toolchain) = &content["toolchain"] else {
            panic!("toolchain object")
        };
        assert_eq!(
            toolchain["compiler_build_id"],
            CanonicalJson::String(expected_compiler.clone())
        );
        assert_eq!(
            toolchain["runner_id"],
            CanonicalJson::String(expected_runner.clone())
        );
        assert_eq!(
            toolchain["stdlib_id"],
            CanonicalJson::String(expected_stdlib.clone())
        );
        ...
        assert_eq!(
            artifact["bytes"],
            CanonicalJson::Integer(fs::metadata(artifact_path).unwrap().len().to_string())
        );
        assert_eq!(
            (&artifact["sha256"], &artifact["bytes"]),
            (&expected_artifact.0, &expected_artifact.1),
            "parallel builds produced different artifact identities"
        );
    }
}
...
fn budget_failure_has_human_github_projection_and_exit_one() {
    ...
    let out = Command::new(jet())
        .args(["budget", "check", "--annotations", "github"])
        .current_dir(&dir)
        .output()
        .unwrap();
    assert_eq!(out.status.code(), Some(1));
    ...
    let stderr = String::from_utf8(out.stderr).unwrap();
    assert!(
        stderr.contains("Error [E2907]: performance budget public-api regressed"),
        "{stderr}"
    );
    assert!(stderr.contains("::error file=src/run.jet"), "{stderr}");
    assert!(
        stderr.contains("performance budget public-api regressed%0AWhy: measured estimator"),
        "{stderr}"
    );
    ...
    assert!(
        stderr.contains("budgets failed: 1 budget failed · report "),
        "{stderr}"
    );
}
...
"#).unwrap();
    ...
    assert!(
        stderr.contains("::error file=src/perf_defs.jet,line=2,col=15,title=Jet E2907::"),
        "{stderr}"
    );
    let report = fs::read_dir(dir.join(".jet/perf/reports"))
        .unwrap()
        .next()
        .unwrap()
        .unwrap()
    ...
    assert!(
        report.contains("\"source\":\"src/perf_defs.jet:2\""),
        "{report}"
    );
}
...
fn budget_update_is_plan_first_and_yes_applies_once() {
    ...
    let CanonicalJson::Object(plan) =
        jet_foundation::PerformanceBudget::CanonicalJson::parse_canonical(&plan.stdout).unwrap()
    else {
        panic!("command")
    };
    assert_eq!(
        *canonical_field(&plan["budget"], "applied"),
        CanonicalJson::Bool(false)
    );
    let CanonicalJson::Object(plan) = canonical_field(&plan["budget"], "plan") else {
        panic!("plan")
    };
    assert_eq!(plan["requires_confirmation"], CanonicalJson::Bool(false));
    let CanonicalJson::Array(rows) = &plan["rows"] else {
        panic!("rows")
    };
    assert_eq!(rows.len(), 2);
    let CanonicalJson::Object(report) = &rows[0] else {
        panic!("report row")
    };
    let CanonicalJson::Object(baseline) = &rows[1] else {
        panic!("baseline row")
    };
    assert_eq!(report["operation"], CanonicalJson::String("create".into()));
    assert_eq!(report["artifact"], CanonicalJson::String("report".into()));
    assert_eq!(
        baseline["operation"],
        CanonicalJson::String("advance".into())
    );
    assert_eq!(
        baseline["artifact"],
        CanonicalJson::String("baseline".into())
    );
    assert!(
        !dir.join(".jet").exists(),
        "JSON plan-only mutated workspace"
    );
    ...
    assert!(dir
        .join(".jet/perf/baselines/names/ci/linux.json")
        .is_file());
}
...
fn build_artifact_receipt_collects_footprint_samples() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "missing statistical baseline must be unavailable"
    );
    assert!(
        output.stderr.is_empty(),
        "JSON mode must keep diagnostics in stdout"
    );

    let CanonicalJson::Object(command) = CanonicalJson::parse_canonical(&output.stdout).unwrap()
    else {
        panic!("command")
    };
    let CanonicalJson::Object(report) = canonical_field(&command["budget"], "report") else {
    ...
    for measurement in measurements {
        ...
        assert_eq!(
            provider["version"],
            CanonicalJson::String("jet-artifact-footprint-v1-first-line-vmhwm-trials-20".into())
        );
        assert_eq!(
            provider["isolation"],
            CanonicalJson::String("fresh-child-process-per-trial".into())
        );
        let CanonicalJson::Object(metric) = &measurement["metric"] else {
            panic!("metric")
        };
        ...
        match &metric["name"] {
            ...
            CanonicalJson::String(name) if name == "StartupTime" || name == "MemoryHighWater" => {
                assert_eq!(samples.len(), 20)
            }
            other => panic!("unexpected metric: {other:?}"),
        }
    ...
    assert!(CanonicalJson::Object(content.clone())
        .bytes()
        .windows(b"Throughput".len())
        .all(|window| window != b"Throughput"));
    jet_foundation::PerformanceBudget::verify_budget_report(
        &CanonicalJson::Object(report.clone()).bytes(),
    )
    .unwrap();
...
fn budget_json_projection_is_exact_and_tool_failure_uses_null_report_fields() {
    ...
    assert_eq!(
        command.keys().map(String::as_str).collect::<Vec<_>>(),
        ["action", "budget", "moment", "ok", "schema", "status"]
    );
    let CanonicalJson::Object(budget) = &command["budget"] else {
        panic!("budget")
    };
    assert_eq!(
    ...
    let CanonicalJson::Array(results) = &budget["results"] else {
        panic!("results")
    };
    let CanonicalJson::Object(result) = &results[0] else {
        panic!("result")
    ...
    let CanonicalJson::Object(source) = &result["source"] else {
        panic!("source")
    };
    assert_eq!(
    ...
    let CanonicalJson::Object(comparison) = &result["comparison"] else {
        panic!("comparison")
    };
    assert_eq!(
        comparison.keys().map(String::as_str).collect::<Vec<_>>(),
        ["direction", "kind", "limit"]
    );
    ...
    let CanonicalJson::Object(invalid) = CanonicalJson::parse_canonical(&invalid.stdout).unwrap()
    else {
        panic!("compiler failure object")
    };
    assert_eq!(
        *canonical_field(&invalid["budget"], "failure_kind"),
        CanonicalJson::String("compiler".into())
    );
    let CanonicalJson::Array(diagnostics) = canonical_field(&invalid["budget"], "diagnostics")
    else {
        panic!("diagnostics")
    };
    let CanonicalJson::Object(diagnostic) = &diagnostics[0] else {
        panic!("diagnostic")
    };
    let CanonicalJson::Object(source) = &diagnostic["source"] else {
        panic!("diagnostic source")
    };
    assert_eq!(
        source.keys().map(String::as_str).collect::<Vec<_>>(),
        ["column", "end_column", "end_line", "line", "path"]
    );
    ...
    let CanonicalJson::Object(failure) = CanonicalJson::parse_canonical(&failed.stdout).unwrap()
    else {
        panic!("failure object")
    };
    assert_eq!(failure["status"], CanonicalJson::String("fail".into()));
    assert_eq!(
        *canonical_field(&failure["budget"], "failure_kind"),
        CanonicalJson::String("tool".into())
    );
    assert_eq!(
        *canonical_field(&failure["budget"], "report"),
        CanonicalJson::Null
    );
    assert_eq!(
        *canonical_field(&failure["budget"], "report_path"),
        CanonicalJson::Null
    );
    assert_eq!(
        *canonical_field(&failure["budget"], "plan"),
        CanonicalJson::Null
    );
}
...
fn budget_non_tty_plan_only_creates_no_artifact_or_baseline() {
    ...
    let stderr = String::from_utf8(out.stderr).unwrap();
    assert!(
        stderr.contains("plan only; pass -y or --yes to apply in a non-interactive shell"),
        "{stderr}"
    );
    assert!(
        !dir.join(".jet").exists(),
        "plan-only update mutated workspace"
    );
}
...
fn budget_tty_confirmation_cancel_and_yes_control_mutation() {
    ...
    assert!(
        transcript.contains("plan cancelled; no baseline changed"),
        "{transcript}"
    );
    assert!(
        !cancelled.join(".jet").exists(),
        "TTY cancel mutated workspace"
    );
    ...
    assert!(transcript.contains("Apply? [y/N]"), "{transcript}");
    assert_eq!(
        transcript.matches("+ report ").count(),
        1,
        "plan/apply duplicated report row: {transcript}"
    );
    assert_eq!(
        transcript.matches("~ baseline ").count(),
        1,
        "plan/apply duplicated baseline row: {transcript}"
    );
    assert!(
        applied
            .join(".jet/perf/baselines/names/ci/linux.json")
            .is_file(),
        "TTY yes did not apply"
    );
    assert!(
        applied
            .join(".jet/perf/reports")
            .read_dir()
            .unwrap()
            .next()
            .is_some(),
        "TTY yes omitted report artifact"
    );
}
...
fn budget_surface_is_generated_into_help_completions_and_man() {
    ...
    let completions = Command::new(jet())
...
fn shape6_groups_inspect_and_registry_while_rejecting_bare_actions() {
    ...
    for (bare, canonical) in [
        ...
        let stderr = String::from_utf8_lossy(&out.stderr);
        assert!(
            stderr.contains("E2101") && stderr.contains(canonical),
            "{stderr}"
        );
    }
}
...
"#,
    ...
    for field_fact in ["--name", "person to greet", "--retries", "--verbose"] {
        assert!(
            help.contains(field_fact),
            "typed help missing {field_fact}: {help}"
        );
    }
    assert_eq!(
        help.lines()
    ...
    for shell in ["bash", "zsh", "fish", "powershell"] {
        ...
        for flag in ["help", "name", "retries", "verbose"] {
            assert!(
                ...
                "{shell} script omitted {flag}: {script}"
            );
        }
        assert!(
            !script.contains("Ada"),
            "completion queried a live value: {script}"
        );
...
"#,
    ...
    for release in [false, true] {
        ...
        assert_eq!(
            String::from_utf8_lossy(&env_fallback.stdout),
            "true\n4100\n"
        );
...
fn colliding_imported_cli_type_resolution_stays_in_codegen_sync() {
    ...
    assert_ne!(
        build.status.code(),
        Some(101),
        "type ambiguity reached rustc: {stderr}"
    );
    if !build.status.success() {
        assert!(
            stderr.contains("E1308"),
            "wrong frontend diagnostic: {stderr}"
        );
    }
    assert!(
...
fn external_completion_rejects_hostile_files_and_names() {
    ...
    for shell in ["bash", "zsh", "fish", "powershell"] {
        ...
        assert!(
            out.stdout.is_empty(),
            "{shell} emitted attacker-controlled script bytes"
        );
        let stderr = String::from_utf8(out.stderr).unwrap();
