...
fn inspect_unsafe_reports_policy_provenance_and_operations() {
    ...
    let output = Command::new(jet())
        .args(["inspect", "unsafe", "main.jet", "--json"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stdout.contains("\"schema\":\"jet.report/v1\"")
            ...
            && stdout.contains("\"mode\":\"Obligations\"")
            && stdout.contains("\"kind\":\"raw_pointer\"")
            && stdout.contains("\"kind\":\"dereference\"")
            && stdout.contains("\"discharged\":true"),
        "{stdout}"
    );
    assert!(
        parse_json(&stdout).is_ok(),
        "inspect unsafe JSON must parse: {stdout}"
    );
    assert!(
        stdout.contains("\"location\":{\"start\":{\"line\":4,\"column\":2}"),
        "{stdout}"
    );
    let repeat = Command::new(jet())
        .args(["inspect", "unsafe", "main.jet", "--json"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
        .output()
...
fn inspect_unsafe_loader_failures_use_standard_diagnostics() {
    ...
    let malformed = Command::new(jet())
        .args(["inspect", "unsafe", "malformed.jet"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stderr.contains("Error [E0003]")
            && stderr.contains("Why:")
            && stderr.contains("Fix:")
            && stderr.contains("malformed.jet:1:9"),
        "{stderr}"
    );

    let colored = Command::new(jet())
        .args(["inspect", "unsafe", "malformed.jet", "--color=always"])
        .current_dir(&dir)
        .output()
        .unwrap();
    ...
    assert!(
        ...
        "--color=always must preserve diagnostic styling"
    );

    let missing = Command::new(jet())
        .args(["inspect", "unsafe", "missing.jet"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stderr.contains("Error [E0603]") && stderr.contains("Why:") && stderr.contains("Fix:"),
        "{stderr}"
    );

    let missing_json = Command::new(jet())
        .args(["inspect", "unsafe", "missing.jet", "--json"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    assert!(
        parse_json(&stdout).is_ok(),
        "loader diagnostic JSON must parse: {stdout}"
    );
    assert!(
        stderr.is_empty(),
        "JSON loader diagnostics should keep stderr quiet: {stderr}"
    );

    fs::write(
        dir.join("missing_reason.jet"),
        "fn run() {\n #Unsafe { print(\"unchecked\") }\n}\n",
    )
    .unwrap();
    let missing_reason = Command::new(jet())
        .args(["inspect", "unsafe", "missing_reason.jet"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stderr.contains("Error [E3112]")
            && stderr.contains("Why:")
            && stderr.contains("Fix:")
            && stderr.contains("missing_reason.jet:2:2"),
        "{stderr}"
    );
}
...
fn inspect_unsafe_reports_sema_diagnostics_with_loaded_module_sources() {
    ...
    let human = Command::new(jet())
        .args(["inspect", "unsafe", main.to_str().unwrap()])
        .current_dir(&runner)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    let json = Command::new(jet())
        .args(["inspect", "unsafe", main.to_str().unwrap(), "--json"])
        .current_dir(&runner)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    assert!(
        stdout.contains("\"code\":\"E3107\"")
            && stdout.contains("helper.jet")
            && stdout.contains("\"line\":4,\"col\":16"),
        "{stdout}"
    );
    ...
    for line in &lines {
        assert!(
            ...
            "each report line must parse: {line}"
        );
    }
    let report = parse_json(lines[0]).unwrap();
    assert_eq!(
        jet_foundation::JSON::json_str(jet_foundation::JSON::json_get(&report, "file").unwrap())
            .unwrap(),
        helper.to_str().unwrap()
    );
    assert!(
        stderr.is_empty(),
        "JSON inspection diagnostics should keep stderr quiet: {stderr}"
    );
    ...
    let loader_human = Command::new(jet())
        .args(["inspect", "unsafe", bad_main.to_str().unwrap()])
        .current_dir(&runner)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    assert_eq!(
        loader_human.status.code(),
        Some(1),
        "{}",
        String::from_utf8_lossy(&loader_human.stderr)
    );
    let loader_stderr = String::from_utf8(loader_human.stderr).unwrap();
    assert!(
        loader_stderr.contains("bad_helper.jet:1:9") && !loader_stderr.contains("bad_main.jet:1:9"),
        "{loader_stderr}"
    );

    let loader_json = Command::new(jet())
        .args(["inspect", "unsafe", bad_main.to_str().unwrap(), "--json"])
        .current_dir(&runner)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    assert_eq!(
        loader_json.status.code(),
        Some(1),
        "{}",
        String::from_utf8_lossy(&loader_json.stderr)
    );
    let loader_stdout = String::from_utf8(loader_json.stdout).unwrap();
    assert!(
        loader_stdout.contains("\"code\":\"E0003\"")
            && loader_stdout.contains("\"line\":1,\"col\":9"),
        "{loader_stdout}"
    );
    ...
    assert!(
        loader_json.stderr.is_empty(),
        "JSON imported loader diagnostics should keep stderr quiet"
    );
}
...
fn epoch3_string_and_set_surface_runs_on_default_tier() {
    ...
    print(Rank.from([1, 2, 3]).union(Rank.from([3, 4])).len())
    print(Rank.from([1, 2, 3]).difference(Rank.from([2, 3, 4])).len())
    print(Set.from(["left", "right"]).intersection(Set.from(["right", "other"])).len())
    print(Rank.from(["left", "right"]).is_subset(Rank.from(["left", "right", "other"])))
...
fn configured_organization_unsafe_policy_fails_closed_and_keeps_path() {
    ...
    assert!(
        stderr.contains("E3109") && stderr.contains(configured.to_str().unwrap()),
        "{stderr}"
    );
    check_snapshot(
        "unsafe_org_policy.txt",
        &stderr.replace(configured.to_str().unwrap(), "ORG_POLICY"),
    );
}
...
fn lua_bind_runs_embedded_vm_and_recovers_after_hostile_calls() {
    ...
    let bind = Command::new(jet())
        .args(["inspect", "bind", "lua", "ops.lua", "--pkg", "ops"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
        .output()
        .unwrap();
    ...
    let cache = dir.join(".jet/bindings/lua");
    assert!(cache.join("libjet_lua_ops.a").is_file());
    let provenance = fs::read_to_string(cache.join("ops.provenance")).unwrap();
    assert!(provenance.contains("state=per-session\ntransport=datatree+table-view\ntable-view=zero-copy\nhook=instructions\n"));
    let generated = fs::read_to_string(cache.join("ops.jet")).unwrap();
    assert!(
    ...
    assert!(
        run.status.success(),
        "embedded Lua binding did not run:\n{}",
    ...
    fs::copy(
        root.join("tests/fixtures/lua_lifecycle.c"),
        dir.join("lifecycle.c"),
    )
    .unwrap();
    let lua_dir = fs::read_to_string(cache.join("ops.lua-path")).unwrap();
    let lua_dir = lua_dir.trim();
    let link_dir = format!("-L{lua_dir}");
    let rpath = format!("-Wl,-rpath,{lua_dir}");
    let cc = Command::new("cc")
        .arg("lifecycle.c")
        .args(["-L.jet/bindings/lua", "-l:libjet_lua_ops.a"])
        .arg(link_dir)
        .arg(rpath)
        .args(["-llua", "-lpthread", "-ldl", "-lm", "-o", "lifecycle"])
        .current_dir(&dir)
        .output()
        .unwrap();
    assert!(
        cc.status.success(),
        "Lua lifecycle probe link failed:\n{}",
...
fn lua_bind_discovers_without_executing_and_launders_parse_errors() {
    ...
    let dir = isolated_cwd("lua_bind_static");
    let script = dir.join("static.lua");
    fs::write(&script,"error('discovery executed source')\n-- function fake(input) end\nlocal function hidden(input) return input end\nfunction visible(input) return input end\n").unwrap();
    let bind = Command::new(jet())
        .args(["inspect", "bind", "lua"])
        .arg(&script)
        .args(["--pkg", "static_ops"])
        .current_dir(&dir)
        .env("NO_COLOR", "1")
    ...
    assert!(
        bind.status.success(),
        "static Lua discovery failed:\n{}",
        String::from_utf8_lossy(&bind.stderr)
    );
    let generated = fs::read_to_string(dir.join(".jet/bindings/lua/static_ops.jet")).unwrap();
    assert!(generated.contains("pub fn visible("));
    assert!(!generated.contains("pub fn fake(") && !generated.contains("pub fn hidden("));
    let invalid = dir.join("invalid.lua");
    fs::write(&invalid, "function broken(input)\n  return {\nend\n").unwrap();
    ...
    assert!(
        !stderr.contains("invalid.lua:") && !stderr.contains("near 'end'"),
        "raw Lua parser detail escaped: {stderr}"
    );
}
...
fn lua_bind_rejects_generated_fixed_abi_names() {
    ...
    let dir = isolated_cwd("lua_bind_reserved_helpers");
    for name in ["take_error", "view_release"] {
        let script = dir.join(format!("{name}.lua"));
        fs::write(
            &script,
            format!("function {name}(input) return input end\n"),
        )
        .unwrap();
        let package = format!("reserved_{name}");
        let failed = Command::new(jet())
            .args(["inspect", "bind", "lua"])
            .arg(&script)
            .args(["--pkg", &package])
        ...
        assert!(
            stderr.contains("Error [E3208]")
                && stderr.contains(&format!("`{name}` cannot be exported")),
            "{stderr}"
        );
        assert!(
            !stderr.contains("E0105"),
            "duplicate generated extern escaped binder validation: {stderr}"
        );
        assert!(!dir
            .join(format!(".jet/bindings/lua/{package}.jet"))
            .exists());
    }
}
...
fn jobs_lists_documented_scheduled_project_jobs_and_matches_run_outside_projects() {
    ...
    assert!(
        unknown_stderr.contains("declared jobs: greet, seed_data, inspect_job"),
        "{unknown_stderr}"
    );
    ...
    assert!(
        String::from_utf8_lossy(&release.stdout).contains(
            "stripped 2 dev job(s): seed_data, inspect_job (mark #Job(.Ship) to include)"
        ),
        "{}",
    ...
    assert!(
        ambiguous_stderr.contains("jet jobs -p <member>") && !ambiguous_stderr.contains("jet run"),
        "{ambiguous_stderr}"
    );
...
fn project_parts_lists_skipped_explicit_and_conflicting_modules() {
    ...
    assert!(
        stdout.contains("skipped") && stdout.contains("_bench"),
        "{stdout}"
    );
    ...
    assert!(
        stdout.contains("\"name\":\"project._bench\"") && stdout.contains("\"state\":\"explicit\""),
        "{stdout}"
    );
...
fn project_parts_lists_plain_source_files() {
    ...
    assert!(
        ...
        "project parts JSON must parse: {json}"
    );
    assert!(
        json.contains("\"name\":\"project.one\"") && json.contains("\"path\":\"five.jet\""),
        "{json}"
    );
}
...
fn project_parts_reports_authority_scan_failures() {
    ...
    for args in [
        ...
        let stderr = String::from_utf8(output.stderr).unwrap();
        assert!(
            stderr.contains("Error [E1334]:") && stderr.contains("result"),
            "{stderr}"
        );
        assert!(
...
fn isolated_cwd_never_reuses_executing_fixture_path() {
    ...
    let copy_result = second.as_ref().map(|dir| {
        fs::copy(
            std::env::current_exe().unwrap(),
            dir.join("cli-test-holder"),
        )
    });
    ...
    assert_ne!(
        first, second,
        "fixture path reused while stale executable was running"
    );
...
fn budget_usage_and_preflight_fail_without_artifacts() {
    ...
    for argv in [
        ...
        assert!(
            !dir.join(".jet").exists(),
            "usage failure created an artifact"
        );
    }
    ...
    assert!(
        !dir.join(".jet").exists(),
        "compiler preflight created an artifact"
    );
}
...
fn budget_check_uses_real_compiler_fact_and_writes_verified_report() {
    ...
    let value =
        jet_foundation::PerformanceBudget::CanonicalJson::parse_canonical(&out.stdout).unwrap();
    ...
    assert!(
        text.contains("\"num\":1"),
        "public API count must be measured: {text}"
    );
    ...
    let command = match &value {
        jet_foundation::PerformanceBudget::CanonicalJson::Object(value) => value,
        _ => panic!("command JSON is not an object"),
    };
    let report = match canonical_field(&command["budget"], "report") {
        jet_foundation::PerformanceBudget::CanonicalJson::Object(value) => value,
        _ => panic!("report is not an object"),
    };
    let content = match &report["content"] {
        jet_foundation::PerformanceBudget::CanonicalJson::Object(value) => value,
        _ => panic!("content is not an object"),
    };
    let tool = match &content["toolchain"] {
        jet_foundation::PerformanceBudget::CanonicalJson::Object(value) => value,
        _ => panic!("toolchain is not an object"),
    };
    for key in ["compiler_build_id", "stdlib_id", "runner_id"] {
        let jet_foundation::PerformanceBudget::CanonicalJson::String(id) = &tool[key] else {
    ...
    let subject = match &content["subject"] {
        jet_foundation::PerformanceBudget::CanonicalJson::Object(value) => value,
        _ => panic!("subject is not an object"),
    };
    let jet_foundation::PerformanceBudget::CanonicalJson::String(triple) =
        &subject["target_triple"]
    else {
        panic!("target triple is not text")
    };
    assert!(
        triple.split('-').count() >= 3,
        "target triple must be canonical: {triple}"
    );
    let jet_foundation::PerformanceBudget::CanonicalJson::String(measured_start) =
        &subject["measured_start"]
    else {
        panic!("measurement start is not text")
    };
    let jet_foundation::PerformanceBudget::CanonicalJson::String(measured_end) =
        &subject["measured_end"]
    else {
        panic!("measurement end is not text")
    };
    assert!(
        measured_start < measured_end,
        "measurement must cover preflight and evidence: {measured_start}..{measured_end}"
    );
    let measurements = match &content["measurements"] {
        jet_foundation::PerformanceBudget::CanonicalJson::Array(value) => value,
        _ => panic!("measurements is not an array"),
    };
    let measurement = match &measurements[0] {
        jet_foundation::PerformanceBudget::CanonicalJson::Object(value) => value,
        _ => panic!("measurement is not an object"),
    };
    let provider = match &measurement["provider"] {
        jet_foundation::PerformanceBudget::CanonicalJson::Object(value) => value,
        _ => panic!("provider is not an object"),
    };
    for key in ["cpu_model", "kernel", "power_governor"] {
        let jet_foundation::PerformanceBudget::CanonicalJson::String(value) = &provider[key] else {
            panic!("{key} is not text")
        };
...
// `budget_check_measures_typed_compile_workloads_and_records_provenance` moved to
// tests/cli_parts/compile_latency.rs (own Cargo target): its pinned 128 child
// compiles spent this binary's whole 900s suite guard and aborted every other
// `cli` case with it.
...
fn budget_build_artifact_measures_real_selected_binary() {
    ...
    assert!(out.stderr.is_empty());
    let CanonicalJson::Object(command) = CanonicalJson::parse_canonical(&out.stdout).unwrap()
    else {
        panic!("command object")
    };
    ...
    let CanonicalJson::Object(content) = &report["content"] else {
        panic!("content object")
    };
    let CanonicalJson::Object(subject) = &content["subject"] else {
        panic!("subject object")
    };
    let CanonicalJson::Object(artifact) = &subject["artifact"] else {
        panic!("artifact identity")
    };
    let CanonicalJson::Integer(bytes) = &artifact["bytes"] else {
        panic!("artifact byte count")
    };
    ...
    assert_eq!(
        digest,
        &jet::SHA256::sha256_file_hex(&artifact_path).unwrap()
    );
    ...
    let CanonicalJson::Object(sample) = &samples[0] else {
        panic!("sample")
    };
    assert_eq!(
        sample["num"],
        CanonicalJson::Integer(metadata.len().to_string())
    );
    ...
    assert_eq!(
        provider["kind"],
        CanonicalJson::String("BuildArtifact".into())
    );
...
fn budget_report_collects_mixed_providers_measurement_locally() {
    ...
    for measurement in measurements {
        let CanonicalJson::Object(measurement) = measurement else {
            panic!("measurement object")
        };
        let CanonicalJson::String(id) = &measurement["budget_id"] else {
            panic!("budget id")
        };
        let CanonicalJson::Object(provider) = &measurement["provider"] else {
            panic!("provider")
        };
        let CanonicalJson::String(kind) = &provider["kind"] else {
            panic!("provider kind")
        };
        let CanonicalJson::Array(samples) = &measurement["samples"] else {
            panic!("samples")
    ...
    assert_eq!(
        providers.get("package:binary").map(String::as_str),
        Some("BuildArtifact")
    );
    assert_eq!(
        providers.get("package:public-api").map(String::as_str),
        Some("CompilerFacts")
    );
    let CanonicalJson::Object(subject) = &content["subject"] else {
        panic!("subject")
    ...
    let CanonicalJson::Object(artifact) = &subject["artifact"] else {
        panic!("shared artifact provenance")
    };
    let CanonicalJson::Integer(bytes) = &artifact["bytes"] else {
...
fn build_enforces_deterministic_fail_budgets_and_reuses_relevant_identity() {
    ...
    assert!(
        !String::from_utf8_lossy(&failed.stdout).contains("built:"),
        "failed budget claimed build success"
    );
    assert!(
        String::from_utf8_lossy(&failed.stderr)
            .contains("Error [E2907]: performance budget public-api regressed"),
        "{}",
        String::from_utf8_lossy(&failed.stderr)
    );
    ...
    assert_eq!(
        fs::read_dir(&report_dir).unwrap().count(),
        2,
        "source/spec change must refresh evidence"
    );

    let reused = Command::new(jet())
        .args(["build", "src/run.jet"])
        .current_dir(&dir)
        .output()
        .unwrap();
    ...
    assert_eq!(
        fs::read_dir(&report_dir).unwrap().count(),
        2,
        "unchanged relevant identity must reuse canonical report"
    );

    let ci = Command::new(jet())
        .args(["build", "src/run.jet", "--profile=ci"])
        .current_dir(&dir)
        .output()
        .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_dir(&report_dir).unwrap().count(),
        3,
        "CI profile identity must refresh evidence"
    );
    let mut profiles = Vec::new();
    for entry in fs::read_dir(&report_dir).unwrap() {
        let value =
            CanonicalJson::parse_canonical(&fs::read(entry.unwrap().path()).unwrap()).unwrap();
        let CanonicalJson::Object(report) = value else {
            panic!("report")
        };
        let CanonicalJson::Object(content) = &report["content"] else {
            panic!("content")
        ...
        let CanonicalJson::String(profile) = &subject["profile"] else {
            panic!("profile")
        };
...
fn budget_bench_measurement_bootstraps_then_consumes_compatible_history() {
    ...
    let CanonicalJson::Object(first) = CanonicalJson::parse_canonical(&bootstrap.stdout).unwrap()
    else {
        panic!("command")
    };
    let CanonicalJson::Object(report) = canonical_field(&first["budget"], "report") else {
        panic!("report")
    };
    let CanonicalJson::Object(content) = &report["content"] else {
        panic!("content")
    ...
    assert!(matches!(
        ...
        CanonicalJson::Object(_)
    ));
    assert!(matches!(measurement["policy"], CanonicalJson::Object(_)));
    assert_eq!(measurement["history"], CanonicalJson::Null);
    assert_eq!(measurement["baseline"], CanonicalJson::Null);
    let CanonicalJson::Object(provider) = &measurement["provider"] else {
        panic!("provider")
    };
    assert_eq!(
        provider["kind"],
        CanonicalJson::String("BenchMeasurement".into())
    );
    assert_eq!(provider["identity"], CanonicalJson::String("parse".into()));
    let first_id = match &report["report_id"] {
        CanonicalJson::String(value) => value.clone(),
        _ => panic!("report id"),
    };
    ...
    let check = Command::new(jet())
        ...
        .unwrap();
    assert!(
        matches!(check.status.code(), Some(0) | Some(1)),
        "stdout: {}\nstderr: {}",
        String::from_utf8_lossy(&check.stdout),
        String::from_utf8_lossy(&check.stderr)
    );
    let CanonicalJson::Object(second) = CanonicalJson::parse_canonical(&check.stdout).unwrap()
    else {
        panic!("command")
    };
    let CanonicalJson::Object(report) = canonical_field(&second["budget"], "report") else {
        panic!("report")
    };
    let CanonicalJson::Object(content) = &report["content"] else {
        panic!("content")
    ...
    let CanonicalJson::Object(history) = &measurement["history"] else {
        panic!("history")
    };
    let CanonicalJson::Array(ids) = &history["report_ids"] else {
        panic!("ids")
    };
    assert_eq!(ids, &vec![CanonicalJson::String(first_id.clone())]);
    let CanonicalJson::Object(baseline) = &measurement["baseline"] else {
        panic!("baseline")
    };
    let CanonicalJson::Array(pooled) = &baseline["pooled_samples"] else {
        panic!("pooled")
    };
    assert_eq!(pooled.len(), 20);
    let CanonicalJson::Object(decision) = &measurement["decision"] else {
        panic!("decision")
    };
    assert_ne!(
        decision["evidence"],
        CanonicalJson::String("unavailable".into())
    );
    let CanonicalJson::Array(results) = canonical_field(&second["budget"], "results") else {
        panic!("results")
    };
    let CanonicalJson::Object(result) = &results[0] else {
        panic!("result")
    };
    assert_eq!(
        result["baseline_report_ids"],
        CanonicalJson::Array(vec![CanonicalJson::String(first_id)])
    );
    assert_eq!(result["metric"], measurement["metric"]);
    assert_eq!(result["lower95"], decision["lower95"]);
    assert_eq!(result["upper95"], decision["upper95"]);
    assert_eq!(result["trend"], decision["trend"]);
    assert_eq!(result["reason"], decision["reason"]);
}
