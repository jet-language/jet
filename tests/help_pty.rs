...
fn final_frame_before_cleanup(transcript: &str) -> String {
    ...
    assert!(
        parts.len() >= 3,
        "missing redraw/cleanup sentinels:\n{transcript}"
    );
...
fn enter_expands_then_guides_without_hooks_instead_of_pasting() {
    ...
    assert!(
        !transcript.contains("Hello, Jet!"),
        "Enter executed selected command:\n{transcript}"
    );
}
...
fn alt_enter_guides_example_without_hooks_instead_of_pasting() {
    ...
    assert!(
        !transcript.contains("Hello, Jet!"),
        "Alt-Enter executed selected command:\n{transcript}"
    );
}
...
fn shell_prefill_mode_keeps_palette_on_tty_while_stdout_is_captured() {
    let jet = env!("CARGO_BIN_EXE_jet");
    let picked =
        std::path::Path::new("/tmp").join(format!("jet-help-picked-{}", std::process::id()));
    let shell_line = format!(
    ...
    assert!(
        transcript.contains("command palette"),
        "captured stdout disabled palette:\n{transcript}"
    );
    assert!(
        transcript.contains("JET_HELP_CAPTURED"),
        "shell did not regain control:\n{transcript}"
    );
    assert_eq!(selected, "jet run\n");
    assert!(
        !transcript.contains("ready to copy"),
        "palette duplicated the shell prefill:\n{transcript}"
    );
    assert!(
        !transcript.contains("Hello, Jet!"),
        "captured selection executed:\n{transcript}"
    );
}
...
fn explicit_color_law_reaches_interactive_renderer() {
    ...
    assert!(
        !plain.contains("\x1b[1;96m"),
        "--color=never leaked styles:\n{plain:?}"
    );
    assert!(
        !plain.contains("\x1b[48;5;24"),
        "--color=never leaked selection bg:\n{plain:?}"
    );
    let no_color = run_pty(b"q", "auto", true);
    assert!(
        !no_color.contains("\x1b[1;96m"),
        "NO_COLOR leaked styles:\n{no_color:?}"
    );
    assert!(
        !no_color.contains("\x1b[48;5;24"),
        "NO_COLOR leaked selection bg:\n{no_color:?}"
    );
}
...
fn f1_uses_alt_screen_and_escape_restores_palette() {
    ...
    assert!(
        ...
        "F1 did not enter reference screen"
    );
    assert!(
        transcript.contains("\x1b[?1049l"),
        "Escape did not restore screen"
    );
    assert!(
        transcript.contains("Usage   jet run"),
        "reference detail pane did not render:\n{transcript}"
    );
    assert!(
        transcript.contains("command palette"),
        "overlay was not preserved:\n{transcript}"
    );
}
...
fn f1_search_opens_verbatim_error_code_page() {
    ...
    assert!(
        ...
        "F1 page diverged from canonical Explain bytes:\n{transcript}"
    );
}
...
fn fuzzy_results_scroll_selected_row_within_terminal_height() {
    ...
    assert!(
        move_count > 18,
        "query must exercise a longer-than-viewport result set"
    );
    ...
    assert!(
        transcript_selected_command(&frame, command(move_count)),
        "selected row was clipped:\n{frame}"
    );
    assert!(
        frame.lines().count() <= 24,
        "final frame exceeded 24 rows:\n{frame}"
    );
    assert!(
...
fn zero_size_pty_bounds_initial_transition_to_effective_height() {
    ...
    assert!(
        frame.lines().count() <= 8,
        "zero-size frame exceeded 8 rows:\n{frame}"
    );
    assert!(
...
fn live_shrink_redraws_categorized_view_within_new_height() {
    ...
    let transcript = run_pty_steps(setup, &[b"", b"", b"", down, b"", b"q"], "never", false);
    let frame = final_frame_before_cleanup(&transcript);
    assert!(
        cursor_up_counts(&transcript)
            .into_iter()
    ...
    assert_eq!(
        frame.lines().count(),
        8,
        "shrunken frame did not use 8 rows:\n{frame}"
    );
    assert!(
        frame.contains("│> ▸ Reference"),
        "later selected category was clipped:\n{frame}"
    );
    assert!(
        frame.lines().all(|line| visible_cols(line) <= 50),
...
fn live_narrow_resize_uses_actual_terminal_size() {
    ...
    assert_eq!(
        frame.lines().count(),
        7,
        "resized frame did not use 7 rows:\n{frame}"
    );
    assert!(
...
fn narrow_fuzzy_results_fit_actual_terminal() {
    ...
    let frame = final_frame_before_cleanup(&transcript);
    assert!(
        frame.contains("jet [run]"),
        "fuzzy result was not rendered:\n{frame}"
    );
    assert_eq!(
        frame.lines().count(),
        7,
        "fuzzy frame exceeded 7 rows:\n{frame}"
    );
    assert!(
        frame.lines().all(|line| visible_cols(line) <= 32),
...
fn short_detail_view_scrolls_without_moving_terminal() {
    ...
    assert!(
        transcript.contains("--target")
            || transcript.contains("--profile")
            || transcript.contains("F1 reference"),
...
fn short_f1_viewport_keeps_later_selection_visible_without_scrolling_terminal() {
    ...
    assert!(
        frame.contains(">   run"),
        "final selected row wrong/clipped:\n{frame}"
    );
    assert_eq!(
        lines.len(),
        8,
        "final frame scrolled or exceeded rows:\n{frame}"
    );
    assert!(
        lines.iter().all(|line| visible_cols(line) <= 60),
        "final frame exceeded 60 cols:\n{frame}"
    );
}
