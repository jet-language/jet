...
// D-AUTH2=A (ratified 2026-07-13): unit-test `jet_auth_verify_jwt_impl` directly.
// Auth.rs is self-contained except for `jet_sha256_raw` which it calls from the
// concatenated prelude. We provide a local definition here.
mod auth_prelude {
    #[allow(dead_code)]
    fn jet_sha256_raw(data: &[u8]) -> [u8; 32] {
        const K: [u32; 64] = [
    ...
    include!("../crates/jet-codegen/src/Prelude/CoreLib/Top/Auth.rs");

    pub fn make_jwt(payload_json: &str, key: &[u8]) -> String {
        let header = r#"{"alg":"HS256","typ":"JWT"}"#;
        let h64 = jet_auth_b64url_encode(header.as_bytes());
        let p64 = jet_auth_b64url_encode(payload_json.as_bytes());
        let signing_input = format!("{}.{}", h64, p64);
        let sig = jet_hmac_sha256(key, signing_input.as_bytes());
        let s64 = jet_auth_b64url_encode(&sig);
        format!("{}.{}.{}", h64, p64, s64)
    }

    pub fn make_jwt_alg(alg: &str, payload_json: &str, key: &[u8]) -> String {
    ...
    pub fn verify(token: &str, key: &[u8]) -> Result<JetAuthClaims, JetAuthError> {
        jet_auth_verify_jwt_impl(&token.to_string(), &key.to_vec())
    }
}

#[test]
fn core_auth_verify_jwt_valid_token_returns_claims() {
    use std::time::{SystemTime, UNIX_EPOCH};
    let key = b"super-secret-key-for-testing";
    let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
    let exp = now + 3600;
    let payload = format!(r#"{{"sub":"alice","aud":"myapp","exp":{},"iat":{}}}"#, exp, now);
    ...
    assert_eq!(claims.audience.as_deref(), Some("myapp"));
    assert_eq!(claims.expires_at, Some(exp as i64));
}

#[test]
fn core_auth_verify_jwt_bad_signature_is_error() {
    use std::time::{SystemTime, UNIX_EPOCH};
...
#[test]
fn core_auth_verify_jwt_expired_token_is_error() {
    let key = b"signing-key";
    let exp = 1_000_000u64; // far in the past
    let payload = format!(r#"{{"sub":"bob","exp":{}}}"#, exp);
    let token = auth_prelude::make_jwt(&payload, key);
    let result = auth_prelude::verify(&token, key);
    assert!(
        matches!(result, Err(auth_prelude::JetAuthError::TokenExpired)),
...
#[test]
fn core_auth_verify_jwt_wrong_alg_is_error() {
    use std::time::{SystemTime, UNIX_EPOCH};
    let key = b"any-key";
    let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
    let exp = now + 3600;
    let payload = format!(r#"{{"sub":"x","exp":{}}}"#, exp);
    let token = auth_prelude::make_jwt_alg("RS256", &payload, key);
    let result = auth_prelude::verify(&token, key);
    assert!(
        matches!(result, Err(auth_prelude::JetAuthError::MalformedToken(_))),
        "expected MalformedToken(alg), got {:?}",
        result
    );
}
