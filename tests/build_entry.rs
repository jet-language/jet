...
#[test]
fn imported_fn_build_never_runs_and_ordinary_build_name_stays_runtime() {
...
"#,
    ...
    .expect("a build-named function without BuildContext is ordinary runtime code");
    assert!(ordinary.build.is_none());
    assert!(ordinary.compile.rust.contains("fn main"));
    assert!(!jet::Driver::selects_build_entry(
...
"#,
    ...
    assert!(
        ...
        "ordinary `fn build` must not receive the build-entry diagnostic: {diags:?}"
    );
}
...
"#;
    ...
    let program = jet::Parser::parse(&tokens).expect("bare BuildPlan entry parses");
    let build = program
        .items
        .iter()
        ...
        .expect("build entry is present");
    assert!(matches!(
        build.return_type.as_ref(),
        Some(jet::AST::Type::Named(name)) if name == "BuildPlan"
    ));
    assert!(jet::Sema::build_entry_signature_is_valid(build));
    assert!(build.failure_contract().is_default());

    let root = project("bare-build-plan");
    let entry = root.join("main.jet");
    write(&entry, source);
    let output = compile_bundle_path_build(entry.to_str().unwrap(), opts())
        .expect("bare BuildPlan entry should run through jet build");
    assert_eq!(
        output
            .build
            .expect("programmable build should produce a result")
            .plan
            .targets()[0]
            .name,
        "app"
...
#[test]
fn criterion9_programmable_build_valid_plan() {
    let root = project("valid-programmable-build");
    let entry = root.join("main.jet");
    write(
        &entry,
        r#"
...
fn duplicate_target_build_source(return_type: &str, declarations: &str) -> String {
    format!(
        r#"
{declarations}
fn build(b: BuildContext) {return_type} -> {{
 first :: b.add_library("app", ["main.jet"], [])
 duplicate :: b.add_library("app", ["main.jet"], [])
    return b.plan()
}}
...
fn assert_duplicate_target_report(errors: &[jet::Diagnostics::Diagnostic]) {
    let diagnostic = errors
        .iter()
        .find(|diagnostic| diagnostic.code == "E3502")
        .unwrap_or_else(|| {
            panic!("duplicate target must use the build report diagnostic: {errors:#?}")
        });
    match &diagnostic.structured {
        Some(jet::Diagnostics::StructuredDiagnostic::BuildError { report }) => {
            assert_eq!(
        ...
        other => panic!("missing structured BuildError report: {other:?}"),
    }
}

#[test]
fn criterion9_programmable_build_implicit_failure_preserves_build_error_identity() {
    let root = project("implicit-build-error");
    let entry = root.join("main.jet");
    write(&entry, &duplicate_target_build_source("BuildPlan", ""));

    let errors = compile_bundle_path_build(entry.to_str().unwrap(), BuildRunOptions::default())
        .expect_err("duplicate target must fail through the implicit Error route");
    assert_duplicate_target_report(&errors);
}

#[test]
fn criterion9_programmable_build_explicit_build_error_preserves_build_error_identity() {
    let root = project("explicit-build-error");
    let entry = root.join("main.jet");
    write(
        &entry,
        &duplicate_target_build_source("BuildPlan !BuildError", ""),
    );

    let errors = compile_bundle_path_build(entry.to_str().unwrap(), BuildRunOptions::default())
        .expect_err("explicit BuildError must fail through the shared carrier");
    assert_duplicate_target_report(&errors);
}

#[test]
fn criterion9_programmable_build_error_union_widens_build_error_without_losing_identity() {
    let root = project("build-error-union");
    let entry = root.join("main.jet");
    write(
        &entry,
        &duplicate_target_build_source(
            "BuildPlan !(BuildError | BuildConfigError)",
            "#Error\nenum BuildConfigError { Invalid }",
        ),
    );

    let errors = compile_bundle_path_build(entry.to_str().unwrap(), BuildRunOptions::default())
        .expect_err("BuildError must widen into the declared error union");
    assert_duplicate_target_report(&errors);
}

#[test]
fn criterion9_programmable_build_ambiguous_error_conversion_is_rejected() {
    let root = project("ambiguous-build-conversion");
    let entry = root.join("main.jet");
    write(
        &entry,
        r#"
#Error
enum BuildLeftError { One }
#Error
enum BuildRightError { One }
#Error
enum BuildTargetError { One }

impl BuildError -> BuildLeftError { return BuildLeftError.One }
impl BuildError -> BuildRightError { return BuildRightError.One }
...
"#,
    ...
    let errors = compile_bundle_path_build(entry.to_str().unwrap(), BuildRunOptions::default())
        .expect_err("ambiguous error conversion must stop programmable-build checking");
    assert!(
        errors.iter().any(|diagnostic| diagnostic.code == "E2419"),
        "expected E2419, got {errors:#?}"
    );
}

#[test]
fn criterion9_programmable_build_cli_human_and_json_reports_match() {
    use jet_foundation::JSON::{json_get, json_str, parse};

    let root = project("build-error-report-parity");
    let entry = root.join("main.jet");
    write(&entry, &duplicate_target_build_source("BuildPlan", ""));
    ...
    let run = |json| {
        ...
        command.output().expect("jet build report command")
    };
    ...
    assert!(
        human_stderr.contains(
            "Error [E3502]: build plan is invalid: target name `app` is registered twice",
        ),
    ...
    let report = parse(json_stderr.trim()).expect("jet build --json emits one report object");
    assert_eq!(
        json_get(&report, "code").and_then(json_str),
        Some("E3502")
    );
    let what = json_get(&report, "what")
        .and_then(json_str)
