...
// The retired counterpart of this test asserted that
// `Codegen::TIR::lower_jit_program` returned a `join` function whose return
// type still carried `FixedList { len: 5 }`. That pipeline was deleted in the
// MIR cutover and had no public replacement entry point, and the fact it
// pinned is covered by observable checks that survive: sema accepts the
// specialized signature above, and
// `aot_jit_and_interpreter_agree_on_additive_fixed_list_length` proves every
// tier prints the specialized length.
