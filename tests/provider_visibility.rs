...
fn raw_provider_backends_are_not_external_api() {
    ...
    let scratch =
        std::env::temp_dir().join(format!("jet-provider-visibility-{}", std::process::id()));
    ...
    assert!(
        ...
        "raw provider API unexpectedly compiled"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    for symbol in ["provider_for", "CoreProvider", "NixProvider", "Provider"] {
        assert!(
            stderr.contains(symbol) && stderr.contains("private"),
            "stderr: {stderr}"
        );
    }
    let _ = fs::remove_dir_all(scratch);
