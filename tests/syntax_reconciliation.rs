        ...
        for path in files(Path::new(root)) {
            ...
            for (line_no, line) in scan_lines(&path, &text) {
                for needle in forbidden_for_path(&path) {
                    if forbidden_occurs(line, needle) {
...
fn lexical_ledger_is_rendered_from_syntax() {
    ...
    let end = decisions
        .find("<!-- END GENERATED LEXICAL LEDGER -->")
        .expect("syntax decisions must close the lexical ledger");
    ...
    let rendered_rows = block
        .lines()
        .filter(|line| line.starts_with("| ") && !line.starts_with("| ---"))
        .count();
    assert_eq!(
        rendered_rows,
        jet::Syntax::LEXICAL_LEDGER.len() + 1,
        "rendered ledger must contain its header plus every Syntax.rs row"
    );
    for row in jet::Syntax::LEXICAL_LEDGER {
        if row.spelling == "`" {
            assert!(
                block.contains("<code>`</code>"),
                "rendered ledger is missing the backtick spelling"
            );
        } else {
            assert!(
                block.contains(row.spelling),
                "rendered ledger is missing spelling `{}`",
                row.spelling
            );
        }
    }
    ...
    for row in jet::Syntax::LEXICAL_LEDGER {
        assert!(
            block.contains(row.meaning),
            "rendered ledger is missing meaning `{}`",
            row.meaning
        );
        if !row.decision.is_empty() {
            assert!(
                block.contains(row.decision),
                "rendered ledger is missing decision `{}`",
                row.decision
            );
        }
    }
