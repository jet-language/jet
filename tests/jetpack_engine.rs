...
#[test]
fn jetpack_import_flake_writes_env_and_lock_from_fixture() {
    let dir = Scratch::new("import-flake");
    fs::write(dir.join("flake.nix"), "{ }").unwrap();
    let fixtures = Scratch::new("import-flake-fx");
    fs::write(
        fixtures.join("flake-devshell.json"),
    ...
    .unwrap();
    fs::write(
        fixtures.join("catalog.json"),
        r#"{"packages": ["fd", "ripgrep"]}"#,
    )
    .unwrap();

    let output = jetpack()
        .args(["import", "flake.nix", "--no-color", "--fixtures"])
        .arg(&fixtures.path)
        .current_dir(&dir.path)
        .env("PATH", "")
        .output()
    ...
    assert_eq!(
        fs::read_to_string(dir.join("env.jet")).unwrap(),
        "// Generated by `jetpack import` from a bounded Nix devShell.\nmodule env.dev {\n    packages: [fd, ripgrep]\n}\n"
    );
    let lock = fs::read_to_string(dir.join(".jet/lock")).unwrap();
    assert!(lock.contains("kind = \"package\""), "lock: {lock}");
    assert!(lock.contains("key = \"fd\""), "lock: {lock}");
    assert!(lock.contains("exact = \"fd@jetpack\""), "lock: {lock}");
    assert!(lock.contains("key = \"ripgrep\""), "lock: {lock}");
}

#[test]
fn jetpack_import_reports_unmappable_and_keeps_mappable_packages() {
    let dir = Scratch::new("import-gap");
    fs::write(dir.join("flake.nix"), "{ }").unwrap();
    let fixtures = Scratch::new("import-gap-fx");
    fs::write(
        fixtures.join("flake-devshell.json"),
        r#"{"buildInputs": ["ripgrep", "ripgrepp"], "shellHook": ""}"#,
    )
    .unwrap();
    fs::write(
        fixtures.join("catalog.json"),
    ...
    assert!(env.contains("packages: [ripgrep]"), "env: {env}");
    assert!(!env.contains("ripgrepp"), "env: {env}");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert_eq!(stderr.matches("JT0101").count(), 1, "stderr: {stderr}");
    assert!(
        stderr.contains(
            "unmappable Nix input `ripgrepp` at `flake.nix`; nearest catalog candidate: `ripgrep`"
        ),
        "stderr: {stderr}"
    );
    let lock = fs::read_to_string(dir.join(".jet/lock")).unwrap();
    assert!(
        lock.contains("key = \"nix-import-unmapped:ripgrepp\""),
        "lock: {lock}"
    );
}

#[test]
fn jetpack_import_accepts_a_direct_shell_nix_expression() {
    let dir = Scratch::new("import-shell");
    fs::write(
        dir.join("shell.nix"),
        "pkgs: mkShell { packages = [ pkgs.fd ]; }",
    )
    .unwrap();
    let output = jetpack()
        .args(["import", "shell.nix", "--no-color"])
        .current_dir(&dir.path)
        .env("PATH", "")
        .output()
        .unwrap();
    ...
    let env = fs::read_to_string(dir.join("env.jet")).unwrap();
    assert!(env.contains("packages: [fd]"), "env: {env}");
    let lock = fs::read_to_string(dir.join(".jet/lock")).unwrap();
    assert!(lock.contains("exact = \"fd@jetpack\""), "lock: {lock}");
}
