...
fn jet_dev_web_exposes_canvas_panel_and_graph() {
    ...
    assert!(js.contains("const execTop = Math.min(72, size.h - 58)"));
    assert!(js.contains("const dataTop = execTop + execRows * 28 + 20"));
    assert!(js.contains("node.kind === \"branch\" || node.kind === \"dispatch\" ? 340 : 314"));
