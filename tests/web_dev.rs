...
fn jet_dev_web_exposes_canvas_panel_and_graph() {
    ...
    assert!(js.contains("create-function-global"));
    assert!(js.contains("new-function-params"));
    ...
    assert!(js.contains("built-ins, functions, source actions"));
    ...
    assert!(js.contains("const hitR = Math.max(18, 23 * view.zoom)"));
    ...
    assert!(js.contains("function pinLabel"));
    assert!(js.contains("function drawPinLabel"));
    assert!(js.contains("function drawSocketRow"));
    assert!(js.contains("const dataTop = 68"));
    assert!(js.contains("node.kind === \"branch\" || node.kind === \"dispatch\" ? 324 : 294"));
