...
3. Add a line to the host repo's AGENTS.md / AGENTS.md pointing agents at
   the **tower** skill (or `Tower/AGENTS.md` for non-Codex agents) so every
   session knows the board is the source of truth.
4. Each working agent starts a listener: `tower agent listen --name <me>`
   (under Codex, inside the Monitor tool) so the owner can reach it
   from the board.
