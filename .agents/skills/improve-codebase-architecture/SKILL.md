...
Use the most specific read-only OMP task agent available under the owner
guidance. If that adapter is unavailable or conflicts with the owner profile,
stop and report the routing failure. Do not invoke a generic Agent tool. Don't
...
- **Want to explore alternative interfaces for the deepened module?** Run the `/codebase-design` skill and use its OMP-backed design-it-twice workflow.
