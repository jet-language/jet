...
Route each request to one **primary skill** and one primary outcome. The primary
may load passive references or use bounded child handoffs only when its contract
allows them. Do not treat a child as a second primary or a metadata field as
runtime proof.

## Routing and composition contract

- **Requested outcome:** One named Jet outcome, selected from the user's request.
- **Supplied inputs:** The request, relevant existing context, and any source or card references named by the primary skill.
- **Allowed child result:** A declared child may return only bounded evidence, a decision answer, a record, a review, or another result named by the primary. It returns to that primary and cannot open an undeclared agenda, expand scope, or turn planning into implementation.
- **Completion owner:** The primary skill owns completion; Tower owns durable board state when the route writes it.
- **Return point:** Every child handoff returns to the primary's open step before the primary records its result.
- **Stopping condition:** Stop when the primary's completion criterion is met. Keep calls acyclic. Passive references are not extra routes.
...
| Kind | Where |
| --- | --- |
| Audit / research reports | `docs/audits/` or `docs/research/` via `tower docs add` |
...
| Skill | Id | Job |
| --- | --- | --- |
| Jet Router | `jet-router` | Route one primary outcome with bounded acyclic support |
| Status | `status` | On-request ELI5 run progress with task and overall estimates |
...
| Surface Frequency Audit | `surface-frequency-audit` | Measure real public-code frequency and Jet friction |
| Mine for Jet | `mine-for-jet` | Mine external resources and cross-check Jet evidence |
| Research | `research` | Answer one source-backed question in one cited result |
| Grilling | `grilling` | Single-question decision interview |
| Batch Grill | `batch-grill-me` | Frontier-round decision interview |
| Grill with Docs | `grill-with-docs` | Interview plus agreed domain record |
| Domain Modeling | `domain-modeling` | Sharpen terms and record agreed glossary/ADR changes |
| Wayfinder | `wayfinder` | Map unresolved decisions to a named destination |
| To Tickets | `to-tickets` | Slice settled implementation work with blocking edges |
| To Spec | `to-spec` | Synthesize and publish the current conversation as a spec |
| Triage | `triage` | Move one issue or PR through its authorized state |
| Codebase Design | `codebase-design` | Design a deep module or deepening candidate |
| Improve Architecture | `improve-codebase-architecture` | Report and discuss deepening opportunities |
...
| --- | --- |
| Unclear which skill / pulse / health check | **Jet Router** (selects the primary outcome) |
| Current run progress / how close to done / named task or milestone status | **Status** (read-only snapshot, then resume authorized work) |
...
| Frequency of public code surfaces or common APIs | **Surface Frequency Audit** |
| Mine a repository, video, paper, docs site, or discussion for Jet lessons | **Mine for Jet** |
| One source-backed question or cited evidence result | **Research** |
| One-question interview or stress test | **Grilling** |
| Frontier-round interview | **Batch Grill** |
| Interview plus an agreed glossary or ADR record | **Grill with Docs** |
| Sharpen a term or record an agreed domain model change | **Domain Modeling** |
| Map unresolved decisions to a named destination | **Wayfinder** |
| Slice settled implementation work into blocked tickets | **To Tickets** |
| Synthesize the current conversation into a spec | **To Spec** |
| Triage one issue or external PR | **Triage** |
| Design a deep module or interface | **Codebase Design** |
| Scan architecture and discuss a selected deepening candidate | **Improve Architecture** |
...
## Skill disposition map

This is the routing index, not a second authority. `AGENTS.md`
...
| F | R | `status` | keep | Status, how close to done, or mid-run progress check | Short ELI5 snapshot; estimated task and work-weighted overall completion; proof before 100%; no unsolicited reports or work interruption. |
