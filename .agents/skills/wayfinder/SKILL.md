...
Two modes. Either way, **never resolve more than one decision ticket per
session**. Research tickets may be batched only when they have disjoint paths,
one close owner, and the owner guide permits the concurrency.
...
concurrency and ownership rules; never assume a fixed parallel wave.
