...
## Refinements learned in Phase 25 (HttpRouter, ambient input, static-new)

- **The HttpRouter producer/parse/dispatch calls are NOT in `core_fixed_sig` — but their
  return types are still FIXED per `(module, method)`** (sema's bespoke `infer_core_call`:
  `router` → HttpRouter, `parse` → HttpRequest, `dispatch` → HttpResponse). So they join
  `core_call_covered` like the Phase-21 `tasks.channel()` producer (a fixed-string `CoreCall`),
  with `core_call_return_ty` carrying the bespoke type total (the `dispatch` → HttpResponse type
  is load-bearing: it lets `resp.status()`/`.body()` read the receiver's total `.ty`). `http.serve`
  stays OUT of this set — it is closure-taking (`CoreClosureCall`, Phase 13). HttpRouter was
  already a covered value type (`net_handle_rust_type` → `JetHttpRouter`) and already forced to
  `let mut` (the Phase-13 handle-binding fix listed it), so the producer binding needed no new work.
- **`router.get(path, handler)` is the FIRST handle method whose arg is rendered at lowering,
  not lowered as a `TExpr`.** The handler (`emit_router_handler`) is a boxed `Fn(HttpRequest)
  ->HttpResponse` closure built from either a bare top-level-fn name (the `move |__req| user_<fn>
  (&__req)` wrapper — DIFFERENT from the Phase-13 `emit_named_fn_value` wrapper, which uses the
  fn's own param names, not `__req`) or a literal lambda (`render_lambda_str`). So the new
  `THandleOp::HttpRouterRegister { verb, handler: String }` carries the PRE-RENDERED handler
  string (resolved at lowering), and emit only assembles `jet_http_router_register(&mut (recv),
  "<VERB>".to_string(), <path>, <handler>)`. Gated as a dedicated shape (h2) BEFORE the generic
  handle/builtin shapes so HttpRouter's `get`/`post` is claimed here (not by the collection
  `get`/`post` builtin arms — the same "a `recv_type == Some(<handle>)` method routes to the
  handle shape first" order `emit_builtin_method` uses).
- **The ambient bare `input()` and the qualified `io.input(prompt)` are DIFFERENT AST nodes
  and only the bare form is covered.** Bare `input()` is an `Expr::Call { name: "input" }` (no
  receiver) — a no-`use` prelude builtin (D-PRELUDE1 = B) sema resolves to `Result<String,
  IOError>`; it lowers to a dedicated `TExprKind::AmbientInput`, byte-for-byte the `emit_call`
  ambient branch. The qualified `io.input(prompt)` is an `Expr::MethodCall` on a core alias with
  a POLYMORPHIC return (Phase-20-deferred) — still on the AST path (`34_parallel_scan`
  `paths_from_prompt`). Covering one says nothing about the other (the recurring "the AST node
  that reaches codegen is what the gate recognises" lesson).
- **The `new`-name over-exclusion was a STATIC-vs-INSTANCE distinction, not a "remove the name"
  fix.** `is_intercepted_method_name("new")` stays WHOLE — it guards the INSTANCE shape (a user
  method `recv.new()` would be mis-dispatched only if `new` were a builtin, but keeping it
  excluded is the conservative choice). The LIFT is narrow: `static_method_call_in_subset` admits
  `new` because a `Type.new(args)` static call (`recv_type == None`, type-name receiver, `(Type,
  "new") ∈ method_sigs`) is the Phase-7 `user_<Type>::user_new(args)` form — provably NOT a
  builtin intercept: `emit_builtin_method` has NO `new` arm, and the only `new` special-case
  (`MEM_ALLOC_NEW`, D-ALLOC1) fires ONLY for a `Field(mem_alias, AllocType)` receiver, never an
  `Ident(Type)` receiver — so the AST falls through to the type-name static dispatch. The guard
  is `method != Syntax::MEM_ALLOC_NEW && is_intercepted_method_name(method)`, lifting exactly
  `new` and nothing else. This is the "a partial sema fact (`recv_type == None`) is the total
  routing signal" rule applied to disambiguate two shapes that share a name.
- **A full routing sweep (the `JET_RS` sentinel) corrected an optimistic inventory.** The
  post-Phase-24 claim "every emittable fn routes EXCEPT latent-bug/unreachable/HttpRouter" was
  INCOMPLETE: 12 live functions still route via the AST path on **uncovered FEATURE surfaces the
  inventory never enumerated** — `require`, `#Caps(Io){}`, `mut`/`take` arg conventions,
  fn-typed-value-stored + struct fn-fields, fan-out result-list destructure, sized-integer
  literal width-elaboration, generic fns + trait-object dispatch, and `io.input`+`?? return`.
  The lesson (again): "every emittable fn routes" claims must be sweep-VERIFIED, never inferred
  from "no *flagged* form remains". The precise residue is enumerated in "What still routes"
  below as classes (A)–(H).
- **Verified byte-identical** via a forced-AST-path diff (a temporary `JET_NO_TIR` bypass on
  `emit_func`/`emit_method`/`emit_trait_method` + the delegation/trait hooks) across the **entire
  example suite** — 90 emittable targets, 0 differ — incl. all three feature targets. A routing
  sentinel confirmed the target fns take the TIR path (`63_named_args` `main`; `65_io_prelude`
  `greet`+`main`; `76_http_routes` `main`+all 3 handlers). All three run end-to-end (`12.0`/`8.0`/
  `2.0`/`100.0`; the prelude prints; `welcome`/`user=42`/`created`/`404 Not Found`). tests/tir.rs
  adds `static_new_constructor`/`ambient_input`/`http_router_dispatch` (build+run); the unit tests
  add `covers_static_new_constructor`/`covers_ambient_input` and extend
  `polymorphic_core_specials_covered` with the three http calls. The instrumentation was removed
...
- **A statement-position builtin can lower as a whole rendered string.** `require`/`require_eq`/
  `panic` produce no value (`()` result) and their entire `{ … }` block emit is a fixed AST
  helper. Rather than model the panic shape as TIR nodes, lower the WHOLE string at lowering
  (`TExprKind::RequireStop(String)`, emitted verbatim) — reusing the existing `render_panic_stop`
  / `render_panic_message` / `render_safe_locals` machinery the `?? panic` path (Phase 15) already
  built. The condition/message/operand sub-exprs are still lowered + emitted through the TIR, so
  the only thing baked at lowering is the position/locals scaffolding (total there, I3).
- **`#Caps` ≡ `Stmt::Region` at codegen.** Effects are a compile-time proof (sema E0741); codegen
  is dumb (I3). The `Stmt::Caps` arm is byte-for-byte `Stmt::Region` — a plain block whose body is
  lowered on the SAME `env` (its `let`s LEAK into the outer scope, exactly like a region). No new
  TIR node; reuse `TStmt::Region`. This made the `Stmt` matches in `stmt_in_subset`/`lower_stmt`
  EXHAUSTIVE — the `_ =>` catch-alls are now forward-safety defaults (kept with
  `#[allow(unreachable_patterns)]`: a future Stmt variant must default to the safe AST path, I2).
- **All three call-arg conventions were already wired in lowering.** `arg_conv_in_subset`'s
  `Mutate` exclusion was the ONLY thing keeping `mut`/`take` free-call args off the TIR.
  `lower_one_call_arg` + `emit_tir_call_args` already reproduce `emit_call_args`' `match conv`
  (`Read`/non-scalar → `&(…)`, `Mutate` → `&mut (…)`, else plain) for every convention — so the
  lift was a one-line gate widening (`arg_conv_in_subset` → `true`), no new emit code.
- **List-destructure element typing must reproduce `expr_jet_ty`'s PARTIALITY, not the true type.**
  `emit_stmt`'s `BindPattern::List` arm types each element from `expr_jet_ty(init)` matching ONLY
  `Some(List(inner))` — so a non-`List` init (e.g. a `[T#N]` fan-out result, whose slot type is a
...
**Phase 25 cleared the HttpRouter handle surface + the ambient `input()` + the `new`-name
over-exclusion** — so `76_http_routes`/`63_named_args`/`65_io_prelude` all route now. But a
full routing sweep (the `JET_RS` sentinel, c109 Phase 25) revealed the post-Phase-24
inventory was OPTIMISTIC: a handful of live functions still route via the AST path on
**uncovered FEATURE surfaces the inventory never listed**. The PRECISE, sweep-verified
AST-path residue after Phase 25 was 12 live functions across (A)–(H); **Phase 26 covered
(A), (B), (C), (E)** (below, struck through). The remaining residue is (D)/(F)/(G)/(H):

**(A) ✅ COVERED (Phase 26) — the `require(cond[, msg])` / `require_eq` / `panic` builtins**
(`14_panic` `main`, `29_embed` `check_nonempty`, `59_debug` `summarize`) —
`emit_require`/`emit_require_eq`/`emit_panic_stop` (Source/Codegen/Statement.rs), bespoke
rich-runtime-report builtin call forms. Now lowered to `TExprKind::RequireStop(rendered)` —
the WHOLE `{ if !(cond) { jet_panic_rich(…); } }` (or the `test_mode` `return Err(…)`) string
is rendered at lowering (`render_require`/`render_require_eq` + the existing `render_panic_stop`),
every source-position/locals fact total there (I3). Gated on the builtin name + arity, excluded
when a user fn / local shadows it.

**(B) ✅ COVERED (Phase 26) — the `#Caps(Io) { … }` capability block** (`effect_caps` `main`) — a
sema-checked effect-gate region (D-EFF1); erased at codegen byte-for-byte `Stmt::Region` (a plain
block, body lowered on the SAME `env` so its `let`s leak). Lowered to the existing `TStmt::Region`.

**(C) ✅ COVERED (Phase 26) — `mut`/`take` argument conventions** (`08_ownership` `main` —
`bump(mut score)`, `archive(take "vault")`) — `arg_conv_in_subset` now admits ALL three
conventions (`Read` → `&(…)`, `Mutate` → `&mut (…)`, `Move`/`take` → plain). `lower_one_call_arg`
already resolved the wrapper from the sig convention; emit reproduces it byte-for-byte.

**(E) ✅ COVERED (Phase 26) — fan-out result-list DESTRUCTURE** (`41_fan_out` `main` —
`[a, b, c] @= doubled`). Lowered to `TStmt::ListDestructure` reproducing `emit_stmt`'s
`BindPattern::List` arm: a `let tmp = &(init);` borrow temp + one `jet_unpack_vec(tmp, want, i,
file, line)` per element. The element-type partiality (`expr_jet_ty`'s `Some(List(inner))`-only
...
| N | Delete the AST `emit_func`/`emit_method`/`emit_trait_method`/delegation function-emit path; TIR is the only seam | ✅ done (c109 Phase N, 2 commits). **Definitive coverage proof was the whole-TEST-SUITE gate-miss check, NOT the examples-only sweep** — a temporary `GATE_MISS=1` route-trace on the four AST-fallthrough branches, run over the ENTIRE suite (`cargo test --nocapture`), reported **0 unique GATE-MISS lines across all 866 tests** (a positive control — forcing `tir_covers` false for `main` — confirmed the instrument fires). The prior examples-only sweep was the trap: examples route 100% but the test suite exercises constructs they don't. **Stage 1 (commit `d287c35`):** each of the four gate hooks raises a clean internal-compiler-error (I2-class, `panic!` → exit 101) on a gate-miss instead of falling through to the AST emit body; the green suite never trips it. **Stage 2 (this commit):** deleted the dead AST function-emit bodies of `emit_func`/`emit_method`/`emit_trait_method` + the whole `emit_delegation_method` (the compiler flagged it `never used`); each hook is now the bare shell `if tir_covers { emit_tir } else { ICE-panic }`. Removed the two now-unused imports (`AccessConvention`, `crate::Syntax`). **Compiler-guided — only what `cargo build` flagged was removed.** KEPT (still have live callers, NOT gated through the TIR): `emit_stmts`/`emit_stmt`/`emit_expr`/`expr_jet_ty`/`operand_is_integer`/`numeric_conversion`/all of `Statement.rs`+`Expression.rs` machinery — they remain live via `emit_error_conv` (D-ERR-CONV `impl From->To` bodies), `#Test`-block body emit (`Codegen/mod.rs`), and lambda/block-expression bodies (`Expression.rs`). Also KEPT: type-definition emit (`emit_struct`/`emit_enum` in Items.rs, `emit_trait_def` in Traits.rs — never gated) and shared helpers (`rust_type`/`field_rust_type`/`mangle`). The task's optimistic "delete `emit_expr`/`emit_stmt`/…" list did NOT apply: those helpers are NOT dead because the three non-gated callers above still drive them. Build clean (back to the 14 pre-existing warnings; the pre-existing `EnumLitArg` warning lives in `Parser/mod.rs`, not deleted codegen, so it stays); full suite + golden + ui = 866/0 with NO AST function-emit path (golden proves byte-identical output); `jet run 01_hello` → `hello, world`. The remaining gate exclusions are all (1) sema-unreachable constructs (generic-type methods, trait-object dispatch sema gaps, bare `?? return`) and (2) latent-bug-gated constructs — **none reachable in a valid Jet program, so none needs a codegen path**; if one ever became reachable it now surfaces as a clean ICE, never a miscompile (I2). |
