...
| U22 | `D-JPK-GC1` | B | Hangar disk contract: auto-GC ages out unreferenced objects (14d default, opportunistic, no daemon) + manual `jet gc` + honest `jet hangar du`; lockfile/generation-reachable never collected; zero-/tmp guarantee golden-tested; build scratch hangar-scoped and crash-cleaned. |
