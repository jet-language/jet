...
|-----|-------|---------------------|-----|--------------------------|--------|
| `file-target-directives` | `#` directive | `#PubFile`, `#Target(Web)`, `#Target(Os.Linux)`, `#Html("path.html")`, `#Js`, `#Wasm`, `#WasmExport` | S18, D-VISDEFAULT2, D-WEBDEFAULT1, D-HTMLPAIR1, D-OSTARGET1, D-WASM1 | `MARKER_PUB_FILE`, `ATTR_TARGET`, `ATTR_HTML`, `ATTR_JS`, `ATTR_WASM`, `ATTR_WASM_EXPORT`; parser fixed-position/file and item markers; fmt web/OS marker tests | Shipped |
...
| `maturity-markers` | `#` metadata directive | `#Experimental`, `#Tested`, `#Hardened` | D-MATURITY1 | `ATTR_EXPERIMENTAL`, `ATTR_TESTED`, `ATTR_HARDENED`; marker constants only | Reserved/tracked; no behavior without follow-up |
