...
_One card open for the owner: **D-ASSOC-NOW** — sequence the associated-types
completion (the freshly-discovered gate under ratified layer 2) against D-PARSE-1.
Card below. The `Rollback` trait shape (**D-ROLLBACK-TRAIT**) was already decided **A**
and ratified into [`syntax-decisions.md`](syntax-decisions.md); the earlier batch is there too._

---

### D-ASSOC-NOW — complete associated types now, or ship D-PARSE-1 first?

**Gist:** Ratified layer-2 `Rollback` needs an associated `type Snapshot`, but associated
types are parse-only (never resolved in sema/codegen). Do we finish associated types next
and ship layer 2, or land the D-PARSE-1 parser-correctness work first and park layer 2?

**Story.** Grace ships a Jet library and tags `1.4.0+build.7` in her manifest. Jet's SemVer
*subset* silently drops the `+build.7`, so a downstream lock that pinned that build resolves
to the wrong artifact — no error, no diagnostic. Meanwhile Dana (from D-ROLLBACK-TRAIT) still
can't get her cheap diff-snapshot: layer 2 needs an associated `type Snapshot` the compiler
parses, formats, and then ignores. Both are "unblocked on paper"; only one can lead.

**In the wild:**
```jet
// D-PARSE-1 today: `+build` metadata is silently dropped.
dep "acme/widgets" = "1.4.0+build.7"   // resolves as if it were 1.4.0 — wrong artifact, no diagnostic

// Layer 2 today: parses, then fails to typecheck.
impl Ledger: Rollback {
    type Snapshot = [Int]              // parsed + formatted… and ignored by sema
    fn snapshot(self) -> [Int] { … }   // E0907: `snapshot` doesn't match `Rollback`
    fn restore(self: ~Ledger, snap: [Int]) { … }
}
```

**Other languages:** Rust treats associated types as foundational (`Iterator::Item`), so its
trait library leans on them everywhere; the cost of *not* having them resolved is that any
trait whose method type varies per impl can't be expressed. D-LIB2 already ratified them
(2026-06-17) — this card is purely *when to finish the build*, not whether to have them.

**Tradeoffs:**

| Option | Ships first | Fixes silent mis-parse | Unblocks layer 2 | Unblocks future generics |
|--------|-------------|------------------------|------------------|--------------------------|
| A — associated types first | type-system foundation | no (parsers wait) | yes | yes |
| B — D-PARSE-1 first | parser correctness | **yes** | no (waits behind a new card) | no |
| C — both streams in parallel | both | yes | yes | yes |

**Worked example of every option:**

- **Option A — finish associated types, then layer 2.** Sema substitutes the impl's concrete
  type for the trait's associated type during signature matching + return/param typing; codegen
  emits the Rust assoc binding. Layer 2 then lands as ratified.
```jet
impl Ledger: Rollback {
    type Snapshot = [Int]              // now resolved: `snapshot`'s return checks against [Int]
    fn snapshot(self) -> [Int] { return self.dirty_indices() }
    fn restore(self: ~Ledger, snap: [Int]) { self.revert_rows(snap) }
}
// #Transact dispatches ledger.snapshot()/ledger.restore(snap) instead of a deep clone.
```

- **Option B — ship D-PARSE-1 first; park layer 2 (recommended).** Land the full native
  JSON/TOML/SemVer parsers (incl. `+build` metadata) so silent mis-parses become correct parses
  or loud rejections; layer 2 waits behind a dedicated associated-types card.
```shell
$ jet add "acme/widgets@1.4.0+build.7"
# resolves the exact build, or errors loudly if the registry can't honor it — never silently 1.4.0
```

- **Option C — run both streams concurrently.** Associated-types (D-LIB2) and D-PARSE-1 are
  independent; a separate agent-stream drives each. Layer 2 lands as soon as associated types do.
```text
stream 1: assoc-types → D-TXN-ROLLBACK layer 2
stream 2: D-PARSE-1 (SemVer → JSON → TOML), each format shipped complete
```

**Recommendation:** B — D-PARSE-1 first. A silent wrong-parse in package resolution is a
correctness/safety defect (ranks above an expert-tier rollback *optimization*, since layer 1
already makes rollback correct); associated types + layer 2 follow as the next foundation push.
Pick A if finishing the generics foundation outranks the parser fix; pick C if you want both
agent-streams funded now.

**Owner Q 1:** Associated types are already ratified (D-LIB2) — completing them needs no new
decision, only a priority slot. Confirm you want them sequenced as a normal build card (A/C), or
should they keep waiting until a generics-heavy feature *forces* them (B)?
